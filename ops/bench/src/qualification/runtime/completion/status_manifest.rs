...
use super::super::artifact::{DirectQualificationArtifactPath, RepositoryBinding};
...
use super::{
    ...
    CompletionReportArgs, CompletionReportValidation, MAX_COMPLETION_REPORT_BYTES,
    MemoryScalingStatus, RELEASE_SCOPE_ID, STIM_COMMIT, STIM_TAG, parse_canonical,
    read_completion_artifact, run_report_with_repository, scope, validate_manifest,
};
...
pub(in crate::qualification::runtime) fn checkpoint_manifest_with_repository(
    ...
    let CompletionReportValidation::Replayed(input) = input else {
        return Err(CompletionError::Boundary);
    };
    let input = DirectQualificationArtifactPath::try_new(&input)?;
