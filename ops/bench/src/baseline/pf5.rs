...
const FLOW_GENERATOR_MEASUREMENT_CASES: usize = 11;
const FLOW_GENERATOR_MEASUREMENT_FLOWS: usize = 22;
...
pub(super) fn compare_note(row_id: &str) -> Option<&'static str> {
    match row_id {
        "pf5-flow-generators-measurement-rich" => Some(
            "report-only: Stab measures the Rust circuit_flow_generators single-instruction measurement/reset/MPAD subset without a faithful pinned Stim CLI timing ratio",
        ),
        _ => None,
    }
}
