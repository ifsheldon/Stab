...
enum Command {
    ...
    /// Regenerate the deterministic performance qualification ledger.
...
fn run(cli: Cli) -> Result<(), BenchError> {
    ...
    match command {
        ...
        Command::QualificationRegenerate => {
            let manifest = checked_manifest(&root)?;
            qualification::regenerate(&root, &manifest)?;
        }
        ...
        Command::QualificationParity(args) => {
