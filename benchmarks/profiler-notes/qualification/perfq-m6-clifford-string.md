...
Next owner action: commit the refreshed correctness and performance inventories, then reproduce both private workers and publish same-revision focused CQ evidence before rerunning both adapter probes. The controlled host must have no swap-counter movement before the twelve pre-migration full and soak scale reports can become promotable. The most recent non-identity full-small diagnostic passed at median ratio `0.701250` with bootstrap upper bound `0.701789`, but it is retained as non-promotable because `pswpin` changed during the run.
