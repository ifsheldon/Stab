...
The A2 parser closure changes the measured Stab entry point from `Circuit::from_stim_str` to the public `Circuit::from_stim_bytes` path used at byte-oriented boundaries. Valid UTF-8 fixtures borrow their source without a model-sized conversion; opaque comment and tag bytes use a bounded sanitization and exact-byte restoration path that is covered separately and is not exercised by this workload. Capacity estimation inspects only the admitted prefix, so a rejected hostile suffix cannot inflate parser allocation before the resource error. Source-current timing must bind this entry point and the current profiler-note digest.
...
Next owner action: whenever source-current parser evidence is refreshed, require exact current-inventory correctness preflight, private-worker reproducibility, one full and one soak report at all three scales, regression checks, and architecture-scoped rollups from one clean commit that binds this note. Preserve every failed or noisy outcome instead of promoting an earlier passing result. Completion status belongs in the PQ2 progress report because this file remains the immutable diagnostic input to each bound run. Native x86-64 evidence remains independent of AArch64 evidence.
