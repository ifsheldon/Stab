...
`stim-qualification-suite.json` and `qualification-threshold-migrations.json` are exceptions: they are deterministic, checked source-owned contracts whose semantic content is validated by `ops/bench`, not machine-specific timing results. The migration ledger currently binds the reviewed Clifford retirement to the exact pre-migration completion report and preflight that authorized retirement, the authorization and migration revisions and inventories, the exact replacement group, measurement, and scale, and any retained memory baseline.
