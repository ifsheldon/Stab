...
Private Stab build-receipt schema version 2 hashes the ordered framed collection of `worker.rs`, `worker/bits.rs`, `worker/not_zero.rs`, `worker/sparse_xor.rs`, `worker/transpose.rs`, and `worker/error.rs` from the materialized commit.
Adapter receipt schema version 8, contract-preflight schema version 8, and qualification report schema version 26 retain 90 accepted or rejected worker receipts and exact worker identities for offline replay.
...
The BitMatrix transpose completion receipts at clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` authorized retirement of only the heterogeneous `m5-simd-bit-table` M12 timing threshold, and clean post-migration revision `1264d885087761b19b37beded47811cc0c117e4d` completed the replacement chain. Independent review then strengthened the exact edge oracle and the pre-setup semantic-work overflow contract, making both worker identities historical and requiring a new source-current chain. The exact allocating and in-place groups retain both pinned Stim transpose symbols as provenance, while the unrelated legacy row-XOR proxy is not paired to either group. The current inventory preserves `m5-simd-bit-table` in `m12-primary-memory-baseline.json` until PQ6 provides equal or stronger memory evidence.
