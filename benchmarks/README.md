...
PQ1 and later executable qualification groups are separately owned by `qualification-runtime-groups.json`. Schema version 2 binds group claim class, baseline eligibility, workload and measurement IDs, immutable named scales and work counts, and exact correctness cases to the frozen performance inventory. `qualification-run` selects `--group` and `--scale`; callers cannot replace source-owned work counts. `qualification-baseline.json` must have one exact entry per runtime group: report-only groups have no measurement thresholds, and threshold-eligible groups must have neither missing nor stale measurement rules. `just bench::qualification-check` validates both files together.

`PERFQ-M4-CIRCUIT-PARSE` is the first PQ2 product group. Its `small`, `medium`, and `large` scales contain 64, 4,096, and 65,536 deterministic instructions. Both workers construct the same fixture outside the timer, measure parsing only, and compare an untimed digest of the parsed canonical circuit after normalizing only the known terminal-newline representation difference. Product runs require the exact CQ2 parser and canonical-round-trip cases plus controller-approved correctness request and completion digests. Correctness evidence may use any producer-valid normal run directory below `target/qualification/`; the performance consumer applies the same path boundary before reopening the artifacts.
