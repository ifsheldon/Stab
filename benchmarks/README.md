...
The two DEM groups retain a complete historical controlled-AArch64 machine chain from clean revision `9497df0350cb33dcd249ea12fda802b5a68efe00`: exact correctness preflight, reproducible workers, both probes, twelve first-attempt passing full and soak reports, twelve regressions, four replayed rollups, and two independently replayed completion receipts. Their medians range from `1.090387x` through `1.145890x` for parse and `0.569331x` through `0.589777x` for print, with worst confidence-interval upper bound `1.150114x`. Independent review rejected that chain as current evidence after finding missing pinned-Stim DEM edge contracts, Stim-only post-return assignment in the C++ timed interval, and mismatched peak-RSS phase boundaries. The source-current inventories bind the repaired parser, zero-capable DEM repeat type, output-returning adapter timing, and pre-digest RSS capture; fresh correctness, timing, memory, rollup, completion, audit, and review evidence is required.
