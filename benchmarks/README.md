...
Performance qualification inventory schema version 2 records the timing batch policy and normalized statistic for every planned and executable group. PQ1 and later executable qualification groups are separately owned by `qualification-runtime-groups.json`. Runtime-group schema version 5 binds group claim class, baseline eligibility, source-owned timing batch policy, workload and measurement IDs, immutable named scales, semantic work counts, exact fixture byte counts and digests, exact correctness cases, an owner, any source-owned profiler note, and any exact comparator source paths and SHA-256 digests to the frozen performance inventory. `qualification-run` selects `--group` and `--scale`; callers cannot replace source-owned work counts, input identities, or timing policy, and every formal run producer rejects an existing output directory. `qualification-baseline.json` must have one exact entry per runtime group: report-only groups have no measurement thresholds, and threshold-eligible groups must have neither missing nor stale measurement rules. `just bench::qualification-check` validates both files together, requires exact set equality between promotable runtime groups and implemented `primary-1.25` inventory groups, and rejects a missing or stale timing policy, profiler-note, or comparator-source digest.
...
The circuit parser and printer scales contain 64, 4,096, and 65,536 deterministic instructions; gate lookup contains 82, 5,248, and 335,872 hashes; popcount and dense XOR contain 4,096, 262,144, and 16,777,216 deterministic bits; each `not_zero` group contains 10,000, 640,000, and 40,960,000 logical bits; sparse row XOR contains 1,997, 127,808, and 8,179,712 row operations; sparse item toggle contains 7, 448, and 28,672 item operations; each transpose group contains 65,536, 4,194,304, and 268,435,456 logical matrix bits at dimensions 256, 2,048, and 16,384; Pauli multiplication contains 10,000, 100,000, and 1,000,000 logical qubits; range iteration uses widths 5, 11, and 22 with 232, 21,604, and 972,972 yielded strings; singleton iteration uses widths 1,000, 32,000, and 1,000,000 with 3,000, 96,000, and 3,000,000 yielded strings; and each DEM group contains 64, 4,096, and 65,536 top-level items from the frozen heterogeneous eight-item cycle.
Each executable scale binds exact semantic work and generated input identity.
The two DEM groups retain a complete historical controlled-AArch64 machine chain from clean revision `9497df0350cb33dcd249ea12fda802b5a68efe00`, but independent review rejected it as current evidence after finding missing pinned-Stim DEM edge contracts, Stim-only post-return assignment in the C++ timed interval, and mismatched peak-RSS phase boundaries. Clean repaired-source revision `80fb5405fb077c694a8a8a18e64a3a5831e20a5e` binds the repaired parser, zero-capable DEM repeat type, output-returning adapter timing, and pre-digest RSS capture. Its fresh exact correctness prerequisite, reproducible workers, both probes, twelve first-attempt passing full and soak reports and regressions, four replayed rollups, and two independently replayed completions all pass under the current inventories. Parse medians range from `1.009355x` through `1.051397x`, print medians range from `0.571684x` through `0.591557x`, and the worst confidence-interval upper bound is `1.073689x`. Final slice acceptance remains pending milestone audit and five new independent review lanes; accepted-maximum RSS remains report-only for PQ6.
...
Adapter receipt schema version 12, worker-protocol schema version 4, contract-preflight schema version 14, and qualification report schema version 33 retain 228 accepted or rejected worker receipts and exact worker identities for offline replay. Every semantic preflight request and receipt uses the distinct `contract` evidence mode: a finite zero elapsed duration is valid because it is not ratio evidence, while `timing` and `memory` receipts still require finite positive elapsed duration.
