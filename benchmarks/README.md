...
The Pauli right-multiplication completion receipt at clean pre-migration revision `3a0fcd814f8d1a9441420ab85edf3d757572ba93` authorized retirement of only the identity-only `m6-pauli-string` M12 timing threshold, its three explicit timing pairs, and their temporary scale mappings. Clean post-migration revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb` regenerated and replayed the strengthened exact correctness preflight, all six first-attempt full and soak reports, all regressions, both rollups, and the completion receipt at source-current inventory `e79edf2e1eaa49a801606245d4a845d47a1d000ed527c9669d95e091c4480237`. The legacy memory baseline remains guarded for PQ6; exact results and hashes are in `docs/plans/pq2-pauli-string-multiplication-qualification-progress-report.md`.

The split Pauli iterator completion receipts at clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized retirement of only the bundled `m6-pauli-iter` M12 timing threshold and its two exact small-scale mappings. Migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` made that focused change, and clean evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632` regenerated and replayed the source-current post-migration exact correctness prerequisite, all 12 first-attempt full and soak reports, all regressions, four workload-specific rollups, and both completion receipts on controlled Linux AArch64. The legacy memory baseline remains guarded for PQ6; exact results and hashes are in `docs/plans/pq2-pauli-string-iterator-qualification-progress-report.md`.
