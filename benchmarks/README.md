...
The two DEM groups have a complete source-current controlled-AArch64 machine chain from clean revision `9497df0350cb33dcd249ea12fda802b5a68efe00`: exact correctness preflight, reproducible workers, both probes, twelve first-attempt passing full and soak reports, twelve regressions, four replayed rollups, and two independently replayed completion receipts. Their medians range from `1.090387x` through `1.145890x` for parse and `0.569331x` through `0.589777x` for print, with worst confidence-interval upper bound `1.150114x`. Milestone audit is closed, including report-only accepted-maximum RSS observations; independent review remains pending before final qualification acceptance.
