...
Private Stab build-receipt schema version 3 hashes the ordered framed collection of `worker.rs`, `worker/bits.rs`, `worker/not_zero.rs`, `worker/pauli.rs`, `worker/pauli_iter.rs`, `worker/sparse_xor.rs`, `worker/transpose.rs`, `worker/workload.rs`, and `worker/error.rs` from the materialized commit.
Adapter receipt schema version 10, contract-preflight schema version 10, and qualification report schema version 28 retain 140 accepted or rejected worker receipts and exact worker identities for offline replay.
