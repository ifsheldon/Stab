...
A reviewed non-check regeneration validates the complete generated ledger against the current CQ0 API map before replacing stale checked bytes; check mode remains fail-closed on any drift.
The generated qualification dashboard distinguishes a malformed completion checkpoint from a well-formed historical checkpoint whose inventory identities predate the current source.
PQ1 is complete at the clean schema-version-13 evidence recorded in [../docs/plans/pq1-performance-harness-progress-report.md](../docs/plans/pq1-performance-harness-progress-report.md). It implements symmetric bounded worker evidence, a faithful pinned-Stim adapter seam, paired confidence intervals, process-memory execution, private committed-source builds, reconstructable build receipts, checked-inventory and current-toolchain replay, and exact report/preflight publication binding. Its protocol-smoke group remains diagnostic. PQ2 extends that harness with source-owned group and scale selection and currently has nineteen correctness-bound release groups. Four additional A2 product diagnostics measure Stab-only planning operations without entering paired Stim evidence. The manifest, M12 primary matrix, threshold files, waiver files, and commands remain authoritative; completed legacy timing pairs are retired only after exact replacement completion evidence, while PQ6 owns memory and scaling graduation.
...
`stim-qualification-suite.json` and `qualification-threshold-migrations.json` are exceptions: they are deterministic, checked source-owned contracts whose semantic content is validated by `ops/bench`, not machine-specific timing results. Migration-ledger schema version 2 distinguishes a retired `timing-threshold` from a retired `no-ratio-waiver`, binds each reviewed retirement to the exact pre-migration completion report and preflight that authorized it, binds the authorization and migration revisions and inventories, names the exact executable replacement group and measurement, and preserves the applicable legacy memory baseline. A timing-threshold migration must name both legacy measurements, while a no-ratio-waiver migration must leave them null instead of inventing a comparison that never existed. A null replacement scale authorizes the complete checked scale family; a named scale authorizes only that exact scale. Validation fails closed for an extra or altered migration, a reopened legacy row, a stale threshold or waiver reference, a missing replacement family or runtime measurement, or a missing retained memory baseline.
...
The heterogeneous report-only rows `m6-stabilizers-to-tableau`, `m6-tableau`, `m6-tableau-iter`, `m7-cli-dispatch`, `m8-frame-simulator`, and `m8-sample-analysis-1shot` intentionally have no row-level threshold because their current Stab and Stim measurements do not perform comparable work. The in-process `m8-sample-throughput-1024` and `m8-sample-throughput-1000000` rows are also report-only and excluded from the threshold file because their pinned Stim references include process startup, parsing, and CLI output.
`m8-sample-analysis-1shot` is the source-owned A4 phase diagnostic: compilation and session construction create fresh values per sample, measurements ending in `_continuous_session` deliberately advance one preconstructed session, typed-batch consumption uses a prebuilt batch with no simulation, and encoding uses that same prebuilt batch with no simulation. These phase measurements are not promoted into the curated release matrix. The four generated-circuit CLI sampling rows execute the built Stab and Stim binaries through the same bounded process supervisor with identical stdin, arguments, iteration policy, and discarded stdout. Each Stab runner invocation owns one timed process launch, so the outer three-run compare population matches the three-launch Stim CLI baseline instead of nesting two repetition schedules. Before timing Stab, an untimed preflight must match a source-owned output-length and digest witness captured from clean pre-A4 revision `18099bf3`.
Legacy M8 measurements do not control CPU affinity, so heterogeneous shared-host results can be bimodal and remain diagnostic. Controlled qualification runners own affinity and host validation for release evidence.
...
The A3 `m8-record-writer-contract`, `m8-record-batch-transpose-contract`, and `m8-record-dets-layout-contract` rows call canonical `stab-records` APIs directly. Typed packed writing, PTB64 bit-plane writing, and both public layout conversions report bits per second. Typed D/L DETS parsing reports records per second because namespace width does not describe the number of parsed tokens or bytes. These rows are contract-only component diagnostics because pinned Stim exposes no faithful public API process boundary for these in-memory Rust operations; representative M7 and M8 CLI and reader rows remain the Stim-comparable end-to-end evidence.
...
Performance qualification inventory schema version 3 treats the complete API and checklist inventory as a coverage map, not a benchmark obligation. Unselected plausible workloads use `future-candidate`; selected APIs otherwise map to `covered-by-parent` or `not-performance-relevant`. The active matrix is capped at 40 release groups and 60 diagnostic groups. PQ1 and later executable groups are owned by `qualification-runtime-groups.json`. Runtime-group schema version 9 binds the explicit `raw-work-v2` timing boundary, group claim class, parity eligibility, source-owned timing batch policy, the enforced 600-second product-diagnostic measurement-suite deadline, workload and measurement IDs, immutable scales with `family_id` and `size_class`, semantic work, exact input identities, correctness cases, ownership, profiler notes, and comparator-source digests. `qualification-run` selects promotable groups for paired Stim evidence; `qualification-diagnostic` accepts only Stab-only product diagnostics and reports raw samples plus normalized per-work-item medians without parity, self-regression, rollup, completion, or legacy-row claims. Its sampler diagnostic measures the complete compile-and-release lifecycle and validates one complete compiled plan outside timing. Callers cannot replace source-owned work, input identity, family, size class, or timing policy, and every producer rejects an existing output directory.

`qualification-parity-policy.json` owns Stim-relative policy for paired-worker groups. Its infrastructure diagnostic has no measurement thresholds, and every parity-eligible product group has an exact complete rule set. Stab-only product diagnostics are intentionally absent because the paired runner rejects them. No Stim parity rule may exceed `1.25x`. `qualification-regression-policy.json` separately owns Stab self-regression tolerance for promotable groups, while `qualification-regression-baselines.json` stores reviewed architecture-specific accepted baselines. The default self-regression tolerance is exactly 15 percent; source-owned exceptions must be committed before measurement, justified, and no greater than 25 percent. A missing or identity-mismatched baseline is `unseeded`, never passing. `just bench::qualification-check` validates the inventory, runtime groups, policy scope, baselines, caps, executable contracts, correctness prerequisites, profiler notes, comparator sources, and timing-boundary identity together.

Checklist evidence prose, source lines, anchors, and `Done` versus `Reopened` presentation do not change performance workload ownership. Qualification checking validates the live checklist structure but preserves those frozen presentation fields when comparing the checked performance inventory. A change among `Partial`, `Deferred`, and whole selected scope still changes child ownership and must regenerate the performance inventory; the generated dashboard reads current checklist status directly.

Nineteen PQ2 product groups are executable: circuit parsing, circuit canonical printing, gate-name hashing, SIMD-word popcount, dense XOR, separate early-hit, all-zero, and late-hit `not_zero` scans, sparse row XOR, sparse item toggle, allocating BitMatrix transpose, square in-place BitMatrix transpose, non-identity Pauli-string right multiplication, X/Z Pauli-string range iteration, X/Y/Z Pauli-string singleton iteration, identity Clifford-string multiplication, complete-cycle non-identity Clifford-string multiplication, direct DEM parsing, and direct DEM canonical printing.
The circuit parser and printer scales contain 64, 4,096, and 65,536 deterministic instructions; gate lookup contains 82, 5,248, and 335,872 hashes; popcount and dense XOR contain 4,096, 262,144, and 16,777,216 deterministic bits; each `not_zero` group contains 10,000, 640,000, and 40,960,000 logical bits; sparse row XOR contains 1,997, 127,808, and 8,179,712 row operations; sparse item toggle contains 7, 448, and 28,672 item operations; each transpose group contains 65,536, 4,194,304, and 268,435,456 logical matrix bits at dimensions 256, 2,048, and 16,384; Pauli multiplication contains 10,000, 100,000, and 1,000,000 logical qubits; range iteration uses widths 5, 11, and 22 with 232, 21,604, and 972,972 yielded strings; and singleton iteration uses widths 1,000, 32,000, and 1,000,000 with 3,000, 96,000, and 3,000,000 yielded strings. Each DEM group has nine scales: small, medium, and large variants with 64, 4,096, and 65,536 compact work items for `flat-errors`, `coordinate-sparse`, and `folded-repeats`.
Each executable scale binds exact semantic work and generated input identity.
The two DEM groups retain complete historical controlled-AArch64 machine chains from clean revisions `9497df0350cb33dcd249ea12fda802b5a68efe00` and `80fb5405fb077c694a8a8a18e64a3a5831e20a5e`. Independent review rejected both as current evidence. Clean `raw-work-v2` revision `68d107a42f655254f31628f0cbedc55479f6c0f3` later passed all 36 full and soak reports across the three representative parse and print families, both accepted-maximum memory probes, four rollups, and the `dem-r6` completion replay. The completion records Stim parity as passed and self-regression as unseeded because the first 18 AArch64 baselines were reviewed and committed afterward. Exact identities and diagnostic caveats are recorded in [../docs/plans/qualification-economy-regression-progress-report.md](../docs/plans/qualification-economy-regression-progress-report.md).
The three `not_zero` groups deliberately remain independent because early termination and full scans execute different work, the two sparse-XOR groups remain independent because row symmetric difference and sorted item insertion or removal are different algorithms, and allocating and in-place transpose remain independent because their allocation and result-lifetime contracts differ. Pauli multiplication uses dense non-identity operands because the inherited identity-only callbacks exercise Stab's intentional identity-right fast path and cannot qualify full-width mutation. The range and singleton iterator groups remain independent because combinatorial weight-range advancement and wide singleton position advancement execute different work and require separate timing, allocation, rollup, and completion outcomes.
...
Private Stab build-receipt schema version 6 hashes the ordered framed collection of every executable worker source from the materialized commit, including the prepared-workload lifecycle and dedicated Clifford-string and DEM-model workers.
Adapter receipt schema version 12, worker-protocol schema version 5, contract-preflight schema version 16, and qualification report schema version 35 bind the `raw-work-v2` timing boundary and exact worker identities for offline replay. The global preflight derives one accepted small receipt per active runtime group and implementation plus three shared rejection receipts per implementation, currently 46 ordered receipts with a hard cap of 128. Group-specific edge cases remain in unit tests and adapter probes. Every semantic preflight request and receipt uses the distinct `contract` evidence mode: a finite zero elapsed duration is valid because it is not ratio evidence, while `timing` and `memory` receipts still require finite positive elapsed duration.
...
The exact Clifford identity group instead uses source-owned `independent-throughput` timing because Stim's width-proportional public operation and Stab's O(1) identity-right public fast path have no bounded stable common iteration count. Both implementations retain independent 350-millisecond through 2-second batches, a smaller-count common batch proves exact semantics, alternating samples bind each exact work total and output digest, and the reported ratio is reconstructed from normalized seconds per declared work item. Every other group remains common-iteration.
...
Probe Pauli iteration with `just bench::qualification-probe --group pq2-pauli-iter-range-adapter-smoke` and `just bench::qualification-probe --group pq2-pauli-iter-singleton-adapter-smoke`; each probe validates exact complete traversal, the workload-specific accepted maximum, malformed shape, the first unsupported request, wrong measurement, semantic-work overflow, and width-checksum overflow for both workers, with rejected requests failing before fixture allocation or start-barrier consumption.
Probe direct DEM parsing and canonical printing with `just bench::qualification-probe --group pq2-dem-parse-adapter-smoke` and `just bench::qualification-probe --group pq2-dem-canonical-print-adapter-smoke`; each probe validates exact cross-worker input and normalized canonical-output identity for all three families and scale classes, odd and even requests, family-specific accepted maxima, malformed descriptors, wrong measurements, semantic-work overflow, nested-repeat preservation, and the first over-cap rejection for both workers before start-barrier consumption. Accepted maxima run in memory mode so setup and peak RSS remain separate from timing. On the clean formal revision, publish the two immutable receipts with `--evidence-mode memory --out target/benchmarks/qualification/<unique-name>`; each receipt contains flat-error, coordinate-sparse, and folded-repeat observations.
...
The BitMatrix transpose completion receipts at clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` authorized retirement of only the heterogeneous `m5-simd-bit-table` M12 timing threshold, and clean post-migration revision `1264d885087761b19b37beded47811cc0c117e4d` completed the first replacement chain. Independent review then strengthened the exact edge oracle and pre-setup semantic-work overflow contract. Clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` regenerated and replayed the reviewed two-case correctness preflight, all 12 first-attempt full and soak reports, all regressions, four method-specific rollups, and both method-specific completion receipts under its recorded inventory. The exact allocating and in-place groups retain both pinned Stim transpose symbols as provenance, while the unrelated legacy row-XOR proxy is not paired to either group. The current inventory preserves `m5-simd-bit-table` in `m12-primary-memory-baseline.json` until PQ6 provides equal or stronger memory evidence; exact historical results and hashes are in `docs/plans/pq2-bit-matrix-transpose-qualification-progress-report.md`.

The Pauli right-multiplication completion receipt at clean pre-migration revision `3a0fcd814f8d1a9441420ab85edf3d757572ba93` authorized retirement of only the identity-only `m6-pauli-string` M12 timing threshold, its three explicit timing pairs, and their temporary scale mappings. Clean post-migration revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb` regenerated and replayed the strengthened exact correctness preflight, all six first-attempt full and soak reports, all regressions, both rollups, and the completion receipt under historical inventory `e79edf2e1eaa49a801606245d4a845d47a1d000ed527c9669d95e091c4480237`. The legacy memory baseline remains guarded for PQ6; exact historical results and hashes are in `docs/plans/pq2-pauli-string-multiplication-qualification-progress-report.md`.

The split Pauli iterator completion receipts at clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized retirement of only the bundled `m6-pauli-iter` M12 timing threshold and its two exact small-scale mappings. Migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` made that focused change, and clean evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632` regenerated and replayed the accepted historical post-migration exact correctness prerequisite, all 12 first-attempt full and soak reports, all regressions, four workload-specific rollups, and both completion receipts on controlled Linux AArch64. The legacy memory baseline remains guarded for PQ6; exact historical results and hashes are in `docs/plans/pq2-pauli-string-iterator-qualification-progress-report.md`.
