...
The two DEM groups retain complete historical controlled-AArch64 machine chains from clean revisions `9497df0350cb33dcd249ea12fda802b5a68efe00` and `80fb5405fb077c694a8a8a18e64a3a5831e20a5e`. Independent review rejected both as current evidence. The later chain repaired the parser and several adapter contracts, but it still used an implicit `raw-work-v1` boundary whose Rust timer sampled after moving the raw output into its protocol return tuple. Its twelve reports, four rollups, two completions, and ratios remain immutable historical evidence under their recorded schemas and inventories. No current promotable DEM timing evidence exists until the explicit `raw-work-v2` workers are committed and the complete chain is regenerated from that clean revision; accepted-maximum RSS also remains report-only for PQ6.
