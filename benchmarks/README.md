...
Pass `--strict` to fail when any selected row is still pending, missing from the selected baseline report, contract-only without a Stab-side measurement, or backed by baseline metadata that does not match pinned Stim v1.16.0.
