...
`qualification-parity-policy.json` owns Stim-relative policy. Every report-only group has no measurement thresholds, and every parity-eligible group has an exact complete rule set. `qualification-regression-policy.json` separately owns Stab self-regression tolerance, while `qualification-regression-baselines.json` stores reviewed architecture-specific accepted baselines. The default self-regression tolerance is 15 percent; source-owned exceptions must be committed before measurement, justified, and no greater than 25 percent. A missing or identity-mismatched baseline is `unseeded`, never passing. `just bench::qualification-check` validates the inventory, runtime groups, parity policy, regression policy, baselines, caps, executable contracts, correctness prerequisites, profiler notes, comparator sources, and timing-boundary identity together.
