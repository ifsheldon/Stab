...
`PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, and `PERFQ-M5-SIMD-WORD` are the first four PQ2 product groups. The parser and printer share `small`, `medium`, and `large` scales containing 64, 4,096, and 65,536 deterministic instructions; the gate lookup scales contain 82, 5,248, and 335,872 hashes; and the popcount scales contain 4,096, 262,144, and 16,777,216 deterministic bits. Each scale binds exact semantic work and generated input identity. The parse group measures repeated parsing and compares an untimed digest of the final parsed circuit. The canonical-print group parses once before the start barrier, measures only repeated serialization, consumes every output string, and compares the final canonical bytes outside timing. The gate group prepares runtime-owned name tables outside timing, measures only complete `gate_name_to_hash` or `Gate::stim_name_hash` sweeps, and compares the final checksum plus an order-sensitive table fingerprint. The popcount group prepares identical little-endian SplitMix64 words, then measures one compiler fence, toggling bit 300, and complete-vector popcount. Stim accumulates the exact architecture-dependent `ptr_simd[k].popcount()` loop under CMake's resolved `libstim` compile flags, Stab accumulates `BitVec::popcount()`, and both encode checksum, iterations, bit width, all four input-fingerprint lanes, and final toggle state as eight little-endian `u64` fields after timing. The generated inventory and runtime group bind both the adapter call site and isolated comparator implementation by path and digest, and the materialized adapter receipt must match them before invocation or report replay. The parser and printer normalize only the known terminal-newline representation difference. Product runs require the exact source-owned CQ2 cases plus controller-approved correctness request and completion digests. Correctness evidence may use any producer-valid normal run directory below `target/qualification/`; the performance consumer applies the same path boundary before reopening the artifacts. The private Stab build hashes the worker source in the materialized commit, uses runtime repository paths instead of embedded temporary source paths, and omits nondeterministic symbol and linker build-id sections, so identical receipts produce byte-identical sealed workers that can be required across a scale family. Every worker preparation requires a report-bound canonical preflight covering the shared frozen protocol vector, one- and two-iteration popcount vectors, actual maximum-width execution, the first unsupported shared circuit scale, an 83-item partial gate-table sweep, and below-minimum, unaligned, and over-cap popcount widths. Performance report schema version 18 retains all 18 actual accepted and rejected receipts, includes both workers' exact source, build-fingerprint, and binary identities in the preflight digest material, and cross-checks those identities against every report worker receipt during offline replay. A correctly refingerprinted preflight transplanted from another worker pair is rejected. After committing changes to worker sources, build inputs, or receipt policy, run `just bench::qualification-worker-reproducibility` from the clean unchanged commit to repeat that contract across two isolated builds and require exact source, build, binary, and preflight identities. The command fails before the private builds when the checkout is dirty. Reports retain setup and peak RSS separately. Gate-hash and popcount memory observations are report-only until PQ6 defines explicit cross-scale growth slack. Any failed or noisy promotable group must retain its source-owned owner and profiler-note contract during offline replay.
