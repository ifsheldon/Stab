...
Adapter receipt schema version 8, contract-preflight schema version 7, and qualification report schema version 25 retain 86 accepted or rejected worker receipts and exact worker identities for offline replay.
...
The BitMatrix transpose completion receipts at clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` authorize retirement of only the heterogeneous `m5-simd-bit-table` M12 timing threshold. The exact allocating and in-place groups retain both pinned Stim transpose symbols as provenance, while the unrelated legacy row-XOR proxy is not paired to either group. The current inventory preserves `m5-simd-bit-table` in `m12-primary-memory-baseline.json` until PQ6 provides equal or stronger memory evidence, and source-current timing claims require the complete post-migration evidence chain.
