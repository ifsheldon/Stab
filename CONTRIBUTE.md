...
Use `just release::publish-order` to validate and print the coordinated product-crate order. From the final clean reviewed release revision, use `just release::check --out target/releases/<unique-preflight-name>` to assemble all ten crates, preserve immutable reviewed copies, and record source, toolchain, and bounded archive identities through an isolated Cargo environment. After review, provide `CARGO_REGISTRY_TOKEN` only in the publication environment; `just release::publish-reviewed --preflight <path> --confirm-version 0.2.0` exposes it only to each post-verification `cargo publish --no-verify` child, performs a byte-identical rebuild, and verifies the registry checksum. The release workflow uses `just release::build-binary` and `just release::verify-assets` through the same Rust ops binary before creating a draft; complex release logic must not move back into workflow shell steps. The complete irreversible publication, partial-failure recovery, tag, draft, and GitHub verification procedure is in [docs/RELEASING.md](docs/RELEASING.md).
