...
`stab capabilities`, `stab inspect`, and `stab plan sample` are Stab-native agent extensions. `capabilities` renders source-owned command, gate-syntax, codec, compiler, and parse-limit descriptors; `inspect` parses and fingerprints a `.stim` circuit or `.dem` detector error model without compilation or execution; `plan sample` compiles the sole scalar plan for validation and reports request identity, plan identity, run configuration, and resource estimates without sampling. The current successful contracts are [capabilities schema version 4](docs/architecture/agent-cli-capabilities-schema-v4.md), [plan schema version 3](docs/architecture/agent-cli-plan-schema-v3.md), and inspect in [agent CLI schema version 2](docs/architecture/agent-cli-schema-v2.md).
