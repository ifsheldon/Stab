...
Record report-only accepted-maximum DEM memory on the clean formal revision with one parse and one print probe: add `--evidence-mode memory --out target/benchmarks/qualification/<unique-name>` to each direct DEM probe. Memory-receipt schema version 2 contains setup and peak RSS for Stim and Stab across all three DEM families and binds the verified host, clean repository, timing boundary, and exact worker identities later required by completion. An existing output path, dirty repository, changed revision, host-policy violation, non-DEM group, or non-memory mode is rejected.
