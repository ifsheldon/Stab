...
`stab capabilities`, `stab inspect`, and `stab plan sample` are Stab-native agent extensions. `capabilities` renders source-owned command, gate-syntax, codec, compiler, parse-limit, and backend descriptors; `inspect` parses and fingerprints a `.stim` circuit or `.dem` detector error model without compilation or execution; `plan sample` accepts `--backend=auto|scalar|portable-simd`, compiles for validation, and reports backend-neutral request identity, backend-bearing plan identity, selected backend, run configuration, and resource estimates without sampling. `auto` and `scalar` currently select the registered scalar plan, while an explicit unavailable backend fails instead of silently falling back. The current successful `capabilities --format=json` contract is [capabilities schema version 3](docs/architecture/agent-cli-capabilities-schema-v3.md), while `inspect` and `plan sample` retain [agent CLI schema version 2](docs/architecture/agent-cli-schema-v2.md); [schema version 1](docs/architecture/agent-cli-schema-v1.md) remains historical.
`--error-format=human|json` is a global Stab extension. Human output remains the default and preserves existing stderr text, warning order, exit status, and help/version routing. JSON mode emits schema-version-1 JSON Lines with stable code, severity, message, optional byte span, labels, optional help, and typed context; each warning precedes the eventual error as a separate object. The pre-parser selects JSON for Clap failures only when exactly one valid `--error-format=json` request appears before `--`, while Clap remains authoritative for malformed or duplicate options.
...
The active product migration is the [agent-native modular QEC architecture plan](docs/plans/agent-native-modular-qec-architecture-plan.md), with execution rules in [GOAL.md](docs/plans/GOAL.md).
Correctness and performance evidence continue to follow the [comprehensive correctness qualification](docs/plans/comprehensive-correctness-qualification-plan.md) and [comprehensive Stim performance qualification](docs/plans/comprehensive-stim-performance-qualification-plan.md) contracts.
CQ0 and PQ0 provide case-level correctness and feature-level performance disposition ledgers, and CQ1 provides independently selectable correctness execution, PR/full/soak tiers, manifest-bound reports, and machine-readable preflight evidence. Clean CQ1 acceptance results are recorded in [the CQ1 progress report](docs/plans/cq1-correctness-harness-progress-report.md), while the active CQ2 inventory and current-digest refresh are recorded in [the CQ2 progress report](docs/plans/cq2-deterministic-qualification-progress-report.md).
PQ1 is complete at the clean evidence in [pq1-performance-harness-progress-report.md](docs/plans/pq1-performance-harness-progress-report.md), with a current-digest refresh summarized in the CQ2 report. It provides the bounded symmetric process runner, pinned-Stim adapter, Stab worker, calibrated paired statistics, controlled-host and current-toolchain checks, process-memory evidence, atomic reports, and regression dispatch. Its synthetic protocol-smoke group validates infrastructure only and can never become a product performance claim; product workload qualification, scaling, and threshold graduation remain PQ2 through PQ7 work.
...
Historical DEM reconstruction uses `just bench::qualification-completion --scope dem-r6 --rollup <parse-full> --rollup <parse-soak> --rollup <print-full> --rollup <print-soak> --out <new-directory>`. Stab 0.2 release qualification uses `--scope a9-release` with one full and one soak rollup for every promotable runtime group, currently 38 rollups and 138 source reports across 19 groups. The schema-version-3 producer derives this matrix from the checked runtime contracts, rejects diagnostic groups, binds each group to its exact correctness preflight, and binds source reports, memory observations, parity outcomes, and passed or unseeded self-regression outcomes into one revision manifest. Replay a current scope with `just bench::qualification-completion-report --input <completion-directory>`. Historical schema-version-1 receipts and schema-version-2 manifests remain readable but have no active producer.
Record report-only accepted-maximum DEM memory on the clean formal revision with one parse and one print probe: add `--evidence-mode memory --out target/benchmarks/qualification/<unique-name>` to each direct DEM probe. Each immutable receipt contains setup and peak RSS for Stim and Stab across all three DEM families. An existing output path, dirty repository, changed revision, non-DEM group, or non-memory mode is rejected.
...
Use `just release::publish-order` to validate and print the coordinated product-crate order. From the final clean reviewed release revision, use `just release::check --out target/releases/<unique-preflight-name>` to assemble all ten crates, preserve immutable reviewed archives and canonical crates.io metadata, and record source, toolchain, and bounded payload identities through an isolated Cargo environment. After review, provide `CARGO_REGISTRY_TOKEN` only in the publication environment; `just release::publish-reviewed --preflight <path> --confirm-version 0.2.0` first rebuilds `stab-release` with every publication-token variable removed, executes that current operator without another Cargo process while stripping every unrelated recognized release credential, requires the checked A9 completion, performs a byte-identical package rebuild in its private token-free Cargo sandbox, uploads the exact retained metadata and archive through the crates.io protocol, and verifies the visible registry checksum. Direct operator invocations reject unrelated release credentials without exposing their values. The release workflow uses `just release::build-binary`, builds `stab-release` without explicit publication-token variables, and then gives a step-local `GITHUB_TOKEN` binding only to the exact prebuilt `create-draft` command that retains all six validated assets, including their target-aware pinned-toolchain identities, through private draft creation and remote digest verification; `just release::verify-assets` remains the non-publishing local asset check. Architecture policy rejects those explicit token variables when inherited by a workflow or job, placed on Cargo or an action step, combined with another publication token, or attached to any other shell command. Reviewed full-SHA actions and their declared GitHub job permissions remain a separate trusted workflow boundary. Complex release logic must not move back into workflow shell steps. The complete irreversible publication, partial-failure recovery, tag, draft, and GitHub verification procedure is in [docs/RELEASING.md](docs/RELEASING.md).
