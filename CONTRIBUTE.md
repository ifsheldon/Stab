...
`stab capabilities`, `stab inspect`, and `stab plan sample` are Stab-native agent extensions. `capabilities` renders source-owned command, gate-syntax, codec, compiler, parse-limit, and backend descriptors; `inspect` parses and fingerprints a `.stim` circuit or `.dem` detector error model without compilation or execution; `plan sample` compiles for validation and reports backend-neutral request identity, run configuration, and resource estimates without sampling. Their successful `--format=json` contract is documented in [docs/architecture/agent-cli-schema-v1.md](docs/architecture/agent-cli-schema-v1.md).
