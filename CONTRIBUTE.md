...
`stab capabilities`, `stab inspect`, and `stab plan sample` are Stab-native agent extensions. `capabilities` renders source-owned command, gate-syntax, codec, compiler, parse-limit, and backend descriptors; `inspect` parses and fingerprints a `.stim` circuit or `.dem` detector error model without compilation or execution; `plan sample` compiles for validation and reports backend-neutral request identity, backend-bearing plan identity, selected backend, run configuration, and resource estimates without sampling. Their current successful `--format=json` contract is documented in [docs/architecture/agent-cli-schema-v2.md](docs/architecture/agent-cli-schema-v2.md); [schema version 1](docs/architecture/agent-cli-schema-v1.md) remains historical.
...
`qualification-diagnostic` accepts only runtime groups with claim class `product-diagnostic`. It uses isolated clean-source builds, bounded workers, host capture, calibration, immutable output paths, `raw-work-v2`, and a source-owned 600-second monotonic measurement-suite deadline that caps every child by the remaining time, but it deliberately produces no Stim ratio, parity outcome, self-regression outcome, release rollup, or formal completion evidence. The sampler diagnostic measures compile-and-release as one honest lifecycle operation; complete plan equality is checked by untimed recompilation.
...
# Or, only when the fixed capability probe is denied by kernel policy:
...
Publication requires a clean unchanged revision and request schema version 2. `benchmarks/a6-predecessors.json` owns every non-seed phase through a reviewed direct-child instrumentation backport, canonical tree-delta digest, and deterministic preservation tag; requests select predecessor reports but cannot assign phases. Reproduced focused rows select typed receipts rather than free-form profiler claims. The publisher derives the baseline, registry, report, and nested profile bindings, exact crossings, row-native internal timing counts, ratios, and outcomes, then atomically creates one append-only `benchmarks/a6-focused-evidence-<source-revision>-<sha256>.json` candidate. Commit that exact object before normal validation; untracked candidates are not source-owned evidence. Validation always reads tracked object bytes, reopens every immutable artifact, reconstructs medians, paired ratios, aggregate ratios, memory maxima, and pass/fail state from raw measurements, replays current threshold, waiver, and profiler-note policy, and rederives every bound timing value. The checked A6 measurement contract requires exact executable semantic preflights for every threshold-owned row plus the selected equal-width `m6-clifford-string` row; remaining report-only rows prove workload continuity only and make no correctness claim. The A6 validator independently retains the exact `m5-simd-bits` and equal-width `m6-clifford-string` `1.25x` pairs even when a completed qualification migration retires their legacy matrix threshold. The hidden legacy `--verify-artifacts` flag no longer changes validation strength.
...
The active DEM completion scope uses `just bench::qualification-completion --scope dem-r6 --rollup <parse-full> --rollup <parse-soak> --rollup <print-full> --rollup <print-soak> --out <new-directory>`. It binds four rollups, 36 source reports, memory observations, parity outcomes, and passed or unseeded self-regression outcomes into one revision manifest. Replay it with `just bench::qualification-completion-report --input <completion-directory>`. Historical schema-version-1 completion receipts remain readable but have no active producer.
