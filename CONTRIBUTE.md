...
Use `just release::publish-order` to validate and print the coordinated product-crate order. From the final clean reviewed release revision, use `just release::check --out target/releases/<unique-preflight-name>` to assemble all ten crates, preserve immutable reviewed archives and canonical crates.io metadata, and record source, toolchain, and bounded payload identities through an isolated Cargo environment. After review, provide `CARGO_REGISTRY_TOKEN` only in the publication environment; `just release::publish-reviewed --preflight <path> --confirm-version 0.2.0` first requires the checked A9 completion, performs a byte-identical rebuild, uploads the exact retained metadata and archive through the crates.io protocol, and verifies the visible registry checksum. The release workflow uses `just release::build-binary`, builds `stab-release` without credentials, and then gives a step-local `GITHUB_TOKEN` only to the exact prebuilt `create-draft` command that retains all six validated assets through private draft creation and remote digest verification; `just release::verify-assets` remains the non-publishing local asset check. Architecture policy rejects release credentials inherited by a workflow or job, exposed to Cargo or an action, combined with another release credential, or attached to any other shell command. Complex release logic must not move back into workflow shell steps. The complete irreversible publication, partial-failure recovery, tag, draft, and GitHub verification procedure is in [docs/RELEASING.md](docs/RELEASING.md).
