...
M10 DEM compatibility has started with the `.dem` parser/printer, DEM core types, in-process oracle parse-print rows, and a staged `stab analyze_errors` command for deterministic detectors plus simple single-qubit Pauli-error analysis; graphlike decomposition, loop folding, gauge-detector analysis, and large analyzer internals remain pending M10 work.
