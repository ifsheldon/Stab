...
M11 DEM sampling compatibility includes a staged `stab sample_dem` command with deterministic and statistical oracle rows, detector and observable streams, sampled-error output and replay, `ptb64` and `r8` stream coverage, Stim-compatible CRLF text replay handling, zero-shot declared-path validation, a 64 MiB DEM input cap, bounded materialized output and replay buffers, and report-only DEM sampling benchmark rows; true streaming and strict performance gates remain M12 work.
