...
M8 sampling compatibility has started with deterministic `01`, `hits`, and `dets` output for the parser-backed Z-basis subset plus seeded `X_ERROR` sampling for the first statistical oracle case; bit-packed result formats, broader noise channels, and reference-sample behavior still belong to later M8 work.
