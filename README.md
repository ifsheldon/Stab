...
Use `--require-memory-gate --memory-baseline <compare.json>` with `just bench::compare-allocations` once the first complete allocation report exists to fail rows whose peak live allocation bytes exceed the M12 25 percent regression budget.
