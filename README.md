...
Strict runs reject host-policy violations. `--allow-unverified-host` preserves local diagnostic evidence with `host_verified=false`, but that evidence cannot be promoted. Offline report refresh, preflight, and regression commands reload the current source-owned host policy, require the same host CPU identity and affinity set, and reconstruct the exact violations and verification outcome from the recorded probes. The PQ1 protocol-smoke group is report-only in `benchmarks/qualification-baseline.json`, does not accept CQ evidence, and must not be cited as Stab-versus-Stim product speed evidence.
