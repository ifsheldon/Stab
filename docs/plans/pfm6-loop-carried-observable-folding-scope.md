...
- The period-8 and period-127 loop-folding examples from the same upstream test are not implemented by this slice.
- Nested loop-folding cases from `ErrorAnalyzer.loop_folding_nested_loop` are not implemented by this slice.
- Generated-code true folded analyzer output remains under-specified outside the selected detector-chain and loop-carried observable shapes.
