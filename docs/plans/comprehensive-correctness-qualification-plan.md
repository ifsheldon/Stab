...
Correctness artifact identity is content-addressed and intentionally independent of the allowed report directory name: a byte-identical report tree may be relocated beneath `target/qualification/` and replayed there, while repository-anchored publication protects the destination during writes without making that path part of provenance.
Execution-receipt schema version 4 and report and preflight schema version 7 store retained artifact locators as validated normal-component paths relative to the report root, require each locator to remain beneath its exact `cases/<case-id>/` directory, and replay content through the relocated report root.
...
- PR, full, and soak runs complete their selected executable cases without an unexpected skip, and a generated preflight validates exact manifest, commit, selector, semantic-output, and result bindings.

## Milestone CQ2: Deterministic Models, Formats, Generation, And Algebra

Status: Implementation and exact ownership are complete as of 2026-07-23 with 271 parents across the eight selected domains. The source-current correctness inventory is `e5635ca4a2c217ee79768d35336c50611a9f12f86edded8a7f01fce1ca72add4`; its only change from the preceding inventory is the same 108 DEM rustdoc `source_line` values moved again by the second parser optimization, with no selected parent, owner, selector, disposition, case ID, or count change. Clean revision `859bf202bdd4bdfbca07e9b1d647afb1b0542846` published and replayed the exact three-case focused Clifford report under preceding correctness digest `648e7ea5a66997a810498dc871257bd654c7f9af9304651d43a88103eded0289`, and the same revision consumed it before dependent Clifford timing. Clean revision `3f2f382627c8421de0a668819d467a9f252de20f` passed the PR tier's 145 selected parents and all 271 parents in both full and soak under preceding digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`; focused revisions `ac20ffca`, `91f62d0a78659da2e8e264a6968b3c6cd32456de`, and `da7c787d1e9f49110d7054868b146b5fb7d7bda4` passed their exact Clifford prerequisites under recorded preceding digests, while `29a29d5f68767e4ab131b051c88f6b77417e0338` and `0b86f07881198c57df1237b23a7d7c0084f2a272` remain historical and review-rejected. Every recorded accepted report replay and exact controller-bound preflight passed with `local_modifications=false`, but each artifact remains valid only for its exact producer and inventory contract. The source-current inventory has no all-domain execution claim yet. Exact replay from a newer checkout remains intentionally invalid, and every dependent timing revision must generate its focused prerequisites at the same clean revision as its performance workers. Current and historical execution, audit, and full-code-review evidence is recorded in [cq2-deterministic-qualification-progress-report.md](cq2-deterministic-qualification-progress-report.md) and [pq2-clifford-string-qualification-progress-report.md](pq2-clifford-string-qualification-progress-report.md).
...
- Compute the complete explicit, expanded-family, public-API, and oracle-fixture owner count with checked arithmetic and enforce the 2,048-owner admission limit before allocating the expanded owner vector. Reuse that bounded expansion during assignment instead of expanding it again.
