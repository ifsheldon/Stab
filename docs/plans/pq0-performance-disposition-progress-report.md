...
The completed selected CQ2 `.stim`, `.dem`, result-format, gate-contract, bit-kernel, circuit-API, Generation, and Algebra slices changed correctness owner ids, exact classifications, checklist rows, and public API shape, so PQ0 was regenerated without relaxing any performance gate. The current inventory has 541 qualification groups and exactly three implemented runtime contracts: `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, and `PERFQ-M4-GATE-LOOKUP`. All retain exact `1.25x` rules. Clean revision `ba70a52025fdd4122ac97cec263725b2ec56e431` supplies historical full and soak AArch64 timing, memory, regression, and rollup evidence for the first two groups under the preceding performance inventory. The gate group's clean evidence is pending. The legacy canonical-print row is superseded, retains only non-primary diagnostic history, and no longer owns an M12 beta, timing-regression, or memory waiver.
