...
Current correctness dependency after the third DEM parser source refresh: schema-version-3 CQ0 semantic digest `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`.

Current schema-version-2 performance inventory digest after the contract-evidence protocol correction is `a3be3a7404d6dbc4bcffae0e3dea52e6b93797102dc5b08a776323044efdabcf`. Runtime-group schema version 5 binds the DEM fixture identities, exact correctness parent, comparator sources, fixed scales, and independent `1.25x` gates while preserving the reviewed Clifford scalable oracles, normalized statistic, source-owned independent-throughput timing only for the identity group, and every checked legacy migration. Clean revision `d9e2405d18cfff05d9b5d908525394476b0edcbc` completed the full pre-migration DEM machine chain under performance digest `a98f57cf194f3a021d321266656cf688c9f7780fb39fa337475e8132411eb88a`, and migration commit `1cfecd64cde4a5effdf07fdaabdbe51017e25a4a` retired only the exact parse timing threshold and print no-ratio waiver while preserving both memory baselines. Clean post-migration revision `7c3e55301b3f098497613d7dad2d624dc08a4dda` passed the scale matrix but is review-rejected because completion replay exposed that semantic preflight probes were mislabeled as timing evidence. Worker protocol version 4, contract-preflight schema version 14, and report schema version 33 fix that distinction. Clean post-fix revision `9497df0350cb33dcd249ea12fda802b5a68efe00` supplies the complete replayed source-current machine chain with twelve first-attempt passing reports, four rollups, and both completions; its worst confidence-interval upper bound is `1.150114x`. Milestone audit closed the accepted-maximum RSS evidence omission and two documentation drifts without finding a product or evidence-integrity defect. The DEM contracts inherit no older ratio into the current inventory and remain pending independent review before final qualification acceptance.
