...
Current schema-version-2 performance inventory digest after independent DEM and shared-adapter review is `30e9df3e8004b59e43716dbb9e7aa847f472811e0adba74c43ef6bc7b243d498`. Runtime-group schema version 5 binds the strengthened exact DEM parent, corrected shared comparator source, fixed scales, and independent `1.25x` gates while preserving every checked legacy migration and retained memory baseline. Clean revision `9497df0350cb33dcd249ea12fda802b5a68efe00` retains a complete historical twelve-report chain with worst confidence-interval upper bound `1.150114x`, but independent review rejected it as current evidence because the exact CQ case missed five pinned-Stim edge contracts, C++ timed a post-return assignment that Rust excluded, and C++ peak RSS included post-timing digest work. Fresh correctness, timing, memory, rollup, completion, audit, and review evidence is required from the corrected source state.
