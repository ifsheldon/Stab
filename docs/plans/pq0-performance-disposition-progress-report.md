...
The completed selected CQ2 `.stim`, `.dem`, result-format, gate-contract, bit-kernel, circuit-API, Generation, and Algebra slices changed correctness owner ids, exact classifications, checklist rows, and public API shape, so PQ0 was regenerated without relaxing any performance gate. The current inventory has 541 qualification groups and exactly four implemented runtime contracts: `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, and `PERFQ-M5-SIMD-WORD`. All retain exact `1.25x` rules. Clean revisions `ba70a52025fdd4122ac97cec263725b2ec56e431` and `c76b7071fc4d7ac358ef3a2fffc053ea675bd05f` supply historical passing AArch64 evidence for the first three groups under preceding performance inventories. Clean revision `56dfe7569c6da48ffe76bde18f21ff43095f029b` supplies current passing full and soak AArch64 evidence for the exact popcount group. The legacy canonical-print row is superseded and retains only non-primary diagnostic history; the inherited `m5-simd-word` row is reworked while its M12 threshold remains active pending explicit replacement migration.
