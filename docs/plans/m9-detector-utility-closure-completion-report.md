...
- At the time of the M9 closure, `circuit_detecting_regions` accepted unrestricted feedback-controlled and sweep-controlled `CX` shapes; fixed then with explicit target validation and negative tests. Later PFM5 target-shape slices intentionally re-promoted selected sweep/qubit `CX`, `CY`, `CZ`, `XCZ`, and `YCZ` groups plus selected `CZ` classical-only bit-bit groups as unsigned sign-only no-ops while keeping non-`CZ` sweep/sweep, non-`CZ` record/sweep, and non-`CZ` record/record groups rejected.
