...
- At the time of the M9 closure, `circuit_detecting_regions` accepted unrestricted feedback-controlled and sweep-controlled `CX` shapes; fixed then with explicit target validation and negative tests. A later PFM5 target-shape slice intentionally re-promoted only selected sweep/qubit `CX`, `CY`, `CZ`, `XCZ`, and `YCZ` groups as unsigned sign-only no-ops while keeping sweep/sweep and record/sweep groups rejected.
