...
- Added bucketed statistical oracle rows for `PAULI_CHANNEL_2`, correlated errors, independent X/Y/Z errors, depolarizing basis variants, and multi-target `X_ERROR`.
...
- `just bench::compare --milestone M8 --strict` validates pinned Stim v1.16.0 baseline metadata, rejects selected pending rows, rejects missing selected baseline rows, rejects empty contract-only placeholders, and reports measured Stab-side runners for every M8 benchmark manifest row.
...
| M8 strict benchmark compare has no pending, missing-baseline, metadata-mismatch, or empty contract-only rows | Satisfied | `just bench::compare --milestone M8 --strict` |
