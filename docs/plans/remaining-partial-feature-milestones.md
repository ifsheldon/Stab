...
| Circuit transforms | PFM-B1 closure pending | PFM-B1 | The selected transform APIs are implemented, including the nineteen-case general reverse-flow closure, and GPT-5.6/max review is closed; the implementation commit, clean allocation evidence, and final evidence audit remain before the selected Rust scope is marked done. |
...
| Flows | PFM-B1 closure pending | PFM-B1 | The selected Rust flow engine is complete under PFM-B4 and all nineteen reverse-flow transform cases are implemented with distinct selectors; the implementation commit, clean allocation evidence, and final evidence audit remain. |
...
- Implement full or explicitly scoped `decomposed` behavior for compound gates, pair measurements, MPP, SPP, target grouping, and base-gate lowering. Rust `Circuit::decomposed` now covers public ISWAP, MPP, SPP, pair-measurement, tag-preservation, noise-preservation, annotation-preservation, constant-MPP, anti-Hermitian rejection, and selected MPP or pair-measurement flow-generator preservation cases; broader flow-dependent semantic checks remain open when new RPF5 flow families are promoted.
- Extend `circuit_with_inlined_feedback` into the selected public feedback-inlining surface, including repeat-block behavior or precise rejection. The scoped Rust `Circuit::with_inlined_feedback` API is implemented for the current top-level Pauli and MPP feedback subset, selected bounded repeat-loop refolding, selected nested bounded-repeat feedback detector-parity preservation, excessive repeat-work rejection, and unsupported classical controlled gates; broader repeat-contained feedback parity beyond selected cases is under-specified in `docs/plans/milestone-spec-gaps.md` until exact repeat structures, comparator behavior, resource behavior, oracle metadata, and benchmark policy are selected.
...
- Finish folded traversal for coordinate maps where current APIs still require caps or do not prove non-flat ambiguous overlapping selected-coordinate lookup through very large repeats. The current all-detector coordinate-map cap, folded non-overlapping selected-query lookup, flat sparse-overlap selected-query fast path, bounded nested sparse-overlap selected-query fast path, valid flat sparse-hole behavior, selected large non-flat sparse-hole declared-bound pruning, and many-selected flat-overlap scan are implemented with tests, oracle metadata, benchmark metadata, and progress evidence in `docs/plans/rpf4-dem-coordinate-progress-report.md`; final shifts, final detector shifts, counts, recursive `without_tags`, and selected coordinates through shifted repeats have PF4 query evidence in `docs/plans/rpf4-dem-transform-progress-report.md`.
- Add folded or capped traversal behavior for graphlike, hypergraph, SAT, matcher-adjacent, shifted or otherwise non-selected repeated stochastic direct DEM sampling, and analyzer-adjacent DEM consumers where owned by RPF4. The current capped graphlike, hypergraph, unweighted SAT, analyzer, ErrorMatcher circuit traversal, graphlike and hypergraph zero-probability repeat-skip, selected flat detector-touching and detectorless logical-only zero-shift graphlike and hypergraph repeat folding, selected flat no-target zero-shift graphlike and hypergraph repeat skipping, selected flat zero-detector-shift `shift_detectors 0` graphlike and hypergraph repeat folding, selected flat annotation-bearing graphlike and hypergraph repeat folding, selected flat and nested annotation-only zero-shift graphlike, hypergraph, unweighted SAT, and weighted WCNF repeat skipping including high sparse annotation ids, selected nested zero-shift graphlike and hypergraph repeat folding, selected flat and nested detector-touching plus detectorless logical-observable-only and annotation-bearing ErrorMatcher filter DEM repeat folding, weighted SAT/WCNF zero-probability variable elision, repeated-body skip, selected flat and nested zero-shift SAT repeat folding including unweighted zero-probability structural mechanisms, dense-target cap, folded DEM sampler repeat subsets including selected nested zero-shift stochastic direct sampling, and flat sampled-error output or replay caps are implemented with tests, oracle metadata, progress evidence, and benchmark metadata for throughput-relevant folded sampler/search behavior in `docs/plans/rpf4-dem-search-sat-progress-report.md`, `docs/plans/rpf4-dem-sampler-progress-report.md`, `docs/plans/pfm4-dem-sampler-nested-stochastic-repeat-progress-report.md`, and `docs/plans/pfm4-dem-sampler-error-bit-cap-evidence-lock.md`; unweighted SAT keeps shifted, non-flat, high-index dense-target error-target, and other non-selected structural repeats capped.
...
- Implement `pf4-dem-flatten-repeat`, `pf4-dem-rounded`, `pf4-dem-coordinate-map`, `pf4-dem-folded-traversal`, `pf4-dem-folded-graphlike-traversal`, `pf4-dem-hypergraph-logical-repeat`, `pf4-dem-hypergraph-no-target-repeat`, `pf4-dem-search-zero-shift-repeat`, `pf4-dem-search-annotation-repeat`, `pf4-dem-search-mixed-zero-probability-repeat`, `pf4-dem-search-nested-repeat`, `pf4-dem-sat-flat-repeat-fold`, `pf4-error-matcher-filter-flat-repeat`, `pf4-error-matcher-filter-nested-repeat`, `pf4-error-matcher-filter-logical-repeat`, `pf4-error-matcher-filter-annotation-repeat`, and `pf4-dem-sampler-folded-repeat`. These rows now have report-only runner coverage for current materialized transforms, coordinate resource behavior, capped search/analyzer traversal behavior, graphlike and hypergraph zero-probability repeat skipping, selected flat detector-touching and detectorless logical-only zero-shift graphlike and hypergraph repeat folding, selected flat no-target zero-shift graphlike and hypergraph repeat skipping, selected flat zero-detector-shift `shift_detectors 0` graphlike and hypergraph repeat folding, selected flat annotation-bearing graphlike and hypergraph repeat folding, selected mixed zero-probability plus active graphlike and hypergraph repeat folding, selected nested zero-shift graphlike and hypergraph repeat folding, selected flat and nested detector-touching plus detectorless logical-observable-only and annotation-bearing ErrorMatcher filter DEM repeat folding, weighted SAT/WCNF zero-probability variable elision and repeated-body skipping, selected flat and nested zero-shift SAT repeat folding, folded sampler compile, stochastic direct-sampling, sampler zero-probability repeat-skip behavior, and sampled-error caps. Selected annotation-only search/SAT/WCNF repeat skipping has no separate benchmark because it is admission/resource evidence for avoiding repeat-count-scaled work rather than a throughput path. Full folded traversal remains active beyond these report-only rows.
...
Supersession note, 2026-07-10: PFM-B4 in `docs/plans/non-deferred-partial-feature-milestones.md` is the authoritative finite closure for detecting regions, missing detectors, and the Rust flow engine. The historical tasks below must not reopen behavior outside the ledger; focused selectors, GF(2) solver evidence, and the matrix benchmark are recorded in `docs/plans/pfm-b4-detector-flow-progress-report.md`.
...
- Extend `circuit_detecting_regions` for selected repeat traversal, Clifford gates, target shapes, tick windows, detector filtering, multi-detector regions, anticommutation behavior, and gauge behavior. The bounded repeat-tick traversal, detector/logical-observable target-filter, generated repetition-code all-target/all-tick selection with selected exact D0, D6, and L0 regions, selected generated rotated surface-code all-target/all-tick helper counts plus exact D0, D4, and L0 regions, promoted single-qubit and fixed two-qubit Clifford propagation, inverted measurement-target subset, selected measurement-record feedback placements, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups including selected `CZ` classical-only bit-bit groups, `MPAD`, heralded record-producing noise, `MPP` Pauli-product target subset, `SPP`/`SPP_DAG` unitary Pauli-product target subset, ignored-anticommutation subset, selected measurement-gauge ignored-mode behavior, and product-measurement gauge-cancellation behavior are implemented with tests, oracle metadata, benchmark metadata where performance-sensitive, and progress evidence in `docs/plans/rpf5-detecting-regions-progress-report.md`.
- Extend `missing_detectors` for selected multi-record row reduction, repeated MPP stabilizer products, observable interaction, Clifford propagation, repeat traversal, honeycomb suffix, and toric suffix cases. The Gaussian row-reduction, repeated MPP and pair-measurement stabilizer-product, record-only observable-row, ignored Pauli observable-row, tableau-backed Clifford propagation, bounded repeat traversal with explicit expansion caps, selected observable-neutral final-repeat traversal, and pinned honeycomb and toric generated-code suffix subset is implemented with tests, oracle metadata, and progress evidence in `docs/plans/rpf5-missing-detectors-progress-report.md`; benchmark metadata exists for the promoted MPP or observable and generated-code workloads, while the Clifford and repeat traversal slices are structural and not separately benchmarked.
- Implement measurement-rich flow solving for `Flow`, `has_flow`, `has_all_flows`, `flow_generators`, and future solver or generator diagnostics. The unsigned `has_flow`, unsigned `has_all_flows`, and unsigned diagnostic Rust helper subset with measurement-record and observable dependencies, the exact `circuit_flow_generators` subset for measurement, reset, pair-measurement, Python multi-target measurement and `MPP`, nonconstant and constant `MPP`, feedback, `MPAD`, and selected single- and multi-target heralded-noise MPP generator cases, and the selected measurement-rich `time_reversed_for_flows` bridge including the selected `dont_turn_measurements_into_resets` single-measurement option, selected `MZZ` unitary-suffix packet, and exact `flow_flip` packet are implemented and tracked in [rpf5-flow-progress-report.md](rpf5-flow-progress-report.md).
- Integrate flow semantics with broader `time_reversed_for_flows`, feedback inlining, and gate metadata where selected.
...
- Supplement `pf5-detecting-regions-extended`, `pf5-missing-detectors-extended`, and `pf5-measurement-rich-flows`. The detecting-regions subset is supplemented by `pf5-detecting-regions-repeat-rust`, `pf5-detecting-regions-targets-rust`, `pf5-detecting-regions-target-shapes-rust`, `pf5-detecting-regions-clifford-rust`, `pf5-detecting-regions-anticommutation-rust`, `pf5-detecting-regions-gauge-rust`, `pf5-detecting-regions-generated-repetition-rust`, and `pf5-detecting-regions-generated-surface-rust`; the promoted missing-detectors subset is supplemented by `pf5-missing-detectors-row-reduction-rust`, `pf5-missing-detectors-mpp-observable-rust`, `pf5-missing-detectors-generated-honeycomb-rust`, `pf5-missing-detectors-generated-toric-rust`, `pf5-missing-detectors-clifford-rust`, and `pf5-missing-detectors-repeat-rust`; the unsigned has-flow record and observable subset is supplemented by `pf5-has-flow-record-observable-rust`; the unsigned has-all helper subset is supplemented by `pf5-has-all-flows-rust`; the unsigned diagnostic checker subset is supplemented by `pf5-has-flow-diagnostics-rust`; the promoted measurement-rich generator subset is supplemented by `pf5-flow-generators-measurement-rust` and `pf5-flow-generators-measurement-python-rust`; the promoted solve-for-measurements examples are supplemented by `pf5-flow-solve-measurement-rust`, `pf5-flow-solve-measurement-python-rust`, and `pfm-b4-flow-solver-matrix-rust`; the selected single-instruction measurement-rich plus measurement-ordering, plain reset-to-measurement over one or more unique qubit targets, measurement-to-reset, and measure-reset time-reversal bridge over one or more unique qubit targets with inverted result-target support is supplemented by `pf2-time-reverse-flow-measurement-rust`; and the selected `MZZ` unitary-suffix time-reversal packet is supplemented by `pf2-time-reverse-flow-mzz-unitary-suffix-rust`.
- Use structural comparators when exact text is unstable or when result ordering is intentionally set-like.

Benchmarks:

- Implement `pf5-detecting-regions-repeat`, `pf5-detecting-regions-targets`, `pf5-detecting-regions-clifford`, `pf5-detecting-regions-generated-repetition`, `pf5-detecting-regions-generated-surface`, `pf5-missing-detectors-mpp`, `pf5-missing-detectors-generated-code`, `pf5-flow-solve-measurement-rich`, `pf5-flow-solve-measurement-python`, `pf5-has-all-flows-batch`, `pf5-flow-generators-measurement-rich`, and `pf5-flow-generators-measurement-python`. The detecting-regions repeat, target-filter, Clifford, generated repetition-code, and selected generated rotated surface-code rows now have report-only runner coverage for the promoted performance-sensitive subsets; the selected target-shape, ignored-anticommutation, and selected gauge modes are not separately benchmarked because they change only sparse-tracker validation, feedback, sweep-control no-op, product-measurement, error-policy, or gauge-cancellation branches. `pf5-missing-detectors-mpp` has report-only runner coverage for the promoted MPP and observable-row subset, `pf5-missing-detectors-generated-code` has report-only runner coverage for the promoted honeycomb and toric generated-code suffix subset, `pf5-has-all-flows-batch` has report-only runner coverage for the promoted unsigned has-all-flow helper over record and observable batches, `pf5-flow-generators-measurement-rich` plus `pf5-flow-generators-measurement-python` have report-only runner coverage for the promoted C++ and Python measurement-rich generator examples, and `pf5-flow-solve-measurement-rich` plus `pf5-flow-solve-measurement-python` have report-only runner coverage for the promoted C++ and Python solve-for-measurements examples.
...
- Harden matched-error value objects only when required by active analyzer/search outputs. The selected `ExplainedError` and `CircuitErrorLocation` canonicalization slice is implemented by `pf6-matched-error-canonicalize-rust`; broader hardening remains active only when new analyzer or search outputs require it.
