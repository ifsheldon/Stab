...
- Extend `missing_detectors` for selected multi-record row reduction, repeated MPP stabilizer products, observable interaction, Clifford propagation, honeycomb suffix, and toric suffix cases. The Gaussian row-reduction, repeated MPP and pair-measurement stabilizer-product, record-only observable-row, ignored Pauli observable-row, tableau-backed Clifford propagation, and pinned honeycomb and toric generated-code suffix subset is implemented with tests, oracle metadata, and progress evidence in `docs/plans/rpf5-missing-detectors-progress-report.md`; benchmark metadata exists for the promoted MPP or observable and generated-code workloads, while the Clifford slice is structural and not separately benchmarked.
