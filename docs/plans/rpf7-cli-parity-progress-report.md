...
Stab now has source-owned report-only benchmark coverage for the PF7 public CLI `m2d --sweep` packed `b8` path, the public CLI `m2d --ran_without_feedback` path, the public CLI `analyze_errors --decompose_errors` path, and accepted legacy `--gen` dispatch.
