...
It is not an RPF4 completion report because folded traversal across every DEM consumer, full coordinate-map resource policy, diagram APIs, and Python binding shape remain outside this slice or still active.
...
- `pf4_dem_flattened_matches_pinned_stim_cases`
- `pf4_dem_flattened_rejects_excessive_materialized_repeat`
- `pf4_dem_rounded_matches_pinned_stim_probability_cases`
- `pf4_dem_rounded_keeps_zero_probability_errors`
...
```sh
cargo test -p stab-core --test dem_api pf4_dem_ --quiet
...
- Finish folded coordinate-map and final-shift resource policy where current APIs still require caps or do not prove large nested repeat behavior.
