...
Stable Rust 1.97.1 model tests, warnings-denied Clippy, rustdoc, the complete core suite, exact selector validation, and correctness regeneration pass for this source slice. Clean-revision moved-path timing evidence follows the focused source commit; A6 now proceeds to the Stable `stab-analysis` extraction.
