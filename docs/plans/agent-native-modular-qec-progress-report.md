...
The focused implementation commit is `b82c5e04`. Clean-revision generation and MBQC benchmark evidence is produced after this qualification and documentation checkpoint so every report records `local_modifications=false`; until then, this slice makes no fresh timing claim.
