...
All baseline, comparison, beta-gate, phase, and memory artifacts produced through `1936f9d3` predate the final benchmark and admission-contract repairs and are retained as historical diagnostics only. Failed witness-discovery paths under `target/benchmarks/a5-stim-witness-probe`, `target/benchmarks/a5-phase-witness-probe`, and the later `a5-sequence-probe-*` paths are likewise immutable development history. A clean documentation commit, independent re-audit and review, unique source-current evidence paths, and final workspace verification remain before A5 can be marked complete.
