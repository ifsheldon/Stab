...
`stab-core` now has an empty default feature set and forwards its additive `portable-simd` feature only to `stab-bits` and `stab-algebra`. `stab-cli`, `stab-oracle`, and `stab-bench` select that feature through their own declared defaults while disabling dependency defaults, so their Nightly intent is visible in each manifest and the benchmark package can still construct a scalar worker with `--no-default-features`.
...
The private Stab build receipt advances to schema version 7 and records one explicit build variant. Current Stim-relative qualification and Stab-only product diagnostics construct the portable worker with `--no-default-features --features portable-simd`; the new A6 comparison constructs its scalar peer with `--no-default-features`. Historical receipts that predate this field remain parseable as `legacy-default`, but that state cannot validate as source-current evidence.
...
This report is an A6 architecture diagnostic. It does not create a Stim parity result, a Stab self-regression result, a release rollup, a sampling-backend claim, or a new runtime group. Source-current evidence waits for the harness commit because the producer rejects a dirty checkout and an existing output path.
