...
| PF7-M2D-CLI | `stim m2d`; Measurement-to-detection conversion | PF7 | `stab-cli` m2d | `src/stim/cmd/command_m2d.test.cc`; `src/stim/simulators/measurements_to_detection_events.test.cc`; `doc/usage_command_line.md` | Exact and structural CLI oracle rows for formats, sweep, feedback inlining, skip-reference, append observables, side outputs, invalid paths, invalid formats, writer failure, and resource boundaries. | `pf7-m2d-cli-parity` manifest-only row plus existing M9 rows. | `pf7-cli-m2d-sweep-b8` has a non-primary report-only public CLI runner with measurement work and compare notes; see [rpf7-cli-parity-progress-report.md](rpf7-cli-parity-progress-report.md). `pf7-cli-m2d-feedback-inline` remains a placeholder. | `--detector_hypergraph` remains excluded. |
