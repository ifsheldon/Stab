...
| PF5-MEASUREMENT-RICH-FLOWS | Flows; Circuit transforms | PF5 | `stab-core` flow API | `src/stim/stabilizers/flow.test.cc`; `src/stim/stabilizers/flow_pybind_test.py`; `src/stim/util_top/circuit_flow_generators.test.cc`; `src/stim/util_top/has_flow.test.cc` | Structural tests now cover unsigned `has_flow` measurement-record dependencies, pair-measurement records, observable dependencies, observable Pauli targets, sign-insensitive matching, exact `circuit_flow_generators` output for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, `MPAD`, and scoped measurement-record feedback examples, generated-flow checker satisfaction for the supported checker subset, and explicit rejection of unpromoted measurement-rich generator shapes. Broader tests for composed measurement-rich `flow_generators`, `solve_for_flow_measurements`, failure explanations, transform integration, and gate metadata decisions remain active. | `pf5-has-flow-record-observable-rust` and `pf5-flow-generators-measurement-rust` supplement the broad `pf5-measurement-rich-flows` manifest-only row. | `pf5-has-all-flows-batch` and `pf5-flow-generators-measurement-rich` have non-primary report-only Rust runners with measurement work and compare notes; `pf5-flow-solve-measurement-rich` remains a placeholder. | Python flow binding ergonomics remain deferred. |
