...
- Missing-detector generated-code suffix cases for honeycomb and toric circuits, plus precise errors for any unpromoted family. Gaussian row reduction, repeated MPP and pair-measurement stabilizer products, and observable-row interactions are implemented for the promoted Rust subset.
...
| PF5-MISSING-DETECTORS | Detector-analysis utility APIs | PF5 | `stab-core` detector utility API | `src/stim/util_top/missing_detectors.test.cc` | Structural tests now cover Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer products, record-only observable rows, ignored Pauli observable rows, and unknown-input behavior. Honeycomb suffixes, toric suffixes, generated-code workloads, and broader measurement-rich public flow semantics remain active. | `pf5-missing-detectors-row-reduction-rust` and `pf5-missing-detectors-mpp-observable-rust` supplement the broad `pf5-missing-detectors-extended` manifest-only row plus historical M9 future rows. | `pf5-missing-detectors-mpp` has a non-primary report-only Rust runner with measurement work and compare notes; `pf5-missing-detectors-generated-code` remains a placeholder. | Any generated-code or flow-dependent missing-detector family not implemented by RPF5 must remain explicitly deferred or fail closed. |
