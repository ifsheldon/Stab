...
| PF7-M2D-CLI | `stim m2d`; Measurement-to-detection conversion | PF7 | `stab-cli` m2d | `src/stim/cmd/command_m2d.test.cc`; `src/stim/simulators/measurements_to_detection_events.test.cc`; `doc/usage_command_line.md` | Exact and structural CLI oracle rows for formats, sweep, feedback inlining, skip-reference, append observables, side outputs, invalid paths, invalid formats, writer failure, and resource boundaries. The path-IO subset is implemented for `--circuit`, `--in`, `--out`, `--sweep`, and `--obs_out` success, path-error precedence before converter setup, stdout behavior, stderr class, and exit status. The selected command-contract subset is implemented for default detector-only `dets` output, `--append_observables`, `--skip_reference_sample`, observable side-output widths for sparse observable ids, Pauli-target observable annotations, selected format and width failures, writer failure propagation, stdout behavior, stderr class, and exit status. | `pf7-m2d-path-io-rust` and `pf7-m2d-command-contract-rust` supplement the broad `pf7-m2d-cli-parity` manifest-only row plus existing M9 rows. | `pf7-cli-m2d-sweep-b8` and `pf7-cli-m2d-feedback-inline` have non-primary report-only public CLI runners with measurement work and compare notes; see [rpf7-cli-parity-progress-report.md](rpf7-cli-parity-progress-report.md). | `--detector_hypergraph` remains excluded. |
