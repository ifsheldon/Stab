...
| PF7-LEGACY-DISPATCH | Legacy top-level command flags | PF7 | `stab-cli` dispatch | `src/stim/main_namespaced.test.cc`; `src/stim/main_namespaced.perf.cc`; `doc/usage_command_line.md` | CLI tests cover accepted aliases through existing command rows, selected conflicts between multiple legacy modes, help normalization, and explicit rejection or absence of `--detector_hypergraph`. Unknown legacy flag coverage remains active under the broad row. | `pf7-legacy-dispatch-conflicts-rust` and `pf7-detector-hypergraph-excluded-rust` supplement the broad `pf7-legacy-dispatch-parity` manifest-only row. | `pf7-cli-legacy-dispatch-startup` has a non-primary report-only public CLI runner with measurement work and compare notes. | Deprecated aliases not selected for Stab remain excluded, including `--detector_hypergraph`. |
