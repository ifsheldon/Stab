...
| PF7-LEGACY-DISPATCH | Legacy top-level command flags | PF7 | `stab-cli` dispatch | `src/stim/main_namespaced.test.cc`; `src/stim/main_namespaced.perf.cc`; `doc/usage_command_line.md` | CLI tests cover accepted aliases through the selected `--gen`, `--convert`, `--sample`, `--detect`, `--m2d`, and `--analyze_errors` legacy-dispatch row, selected conflicts between multiple legacy modes, help normalization, explicit rejection or absence of `--detector_hypergraph`, and fail-closed behavior for unselected legacy-style `--diagram`, `--explain_errors`, `--repl`, and `--sample_dem` flags. Broader deprecated spelling coverage remains active only where the CLI plan selects exact flags. | `pf7-legacy-dispatch-accepted-rust`, `pf7-legacy-dispatch-conflicts-rust`, `pf7-detector-hypergraph-excluded-rust`, and `pf7-legacy-unselected-modes-rust` supplement the broad `pf7-legacy-dispatch-parity` manifest-only row. | `pf7-cli-legacy-dispatch-startup` has a non-primary report-only public CLI runner with measurement work and compare notes. | Deprecated aliases not selected for Stab remain excluded, including `--detector_hypergraph`. |
