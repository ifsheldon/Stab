...
| PF6-SPARSE-REV-TRACKER | Sparse reverse detector-frame tracking; Error explanation value objects | PF6 | `stab-core` sparse reverse tracker and analyzer support | `src/stim/simulators/sparse_rev_frame_tracker.test.cc`; `src/stim/simulators/error_matcher.test.cc`; `src/stim/simulators/matched_error.test.cc` | Structural tests now cover supported-Clifford unitary-repeat folding including `CY`, mixed-loop equivalence to naive traversal, huge periodic loop folding, fallback for non-unitary or unsupported repeat bodies, and public unsigned-flow consumption through a measurement-dependent repeat. Broader all-unitary fuzz cases, analyzer/search-specific consumption, and matched-error value-object hardening needed by future analyzer paths remain active. | `pf6-sparse-rev-unitary-repeat-rust` supplements the broad `pf6-sparse-rev-tracker` manifest-only row. | `pf6-sparse-rev-frame-loop` has a non-primary report-only Rust runner with measurement work and compare notes. | Heralded matching, repeat-contained noise stack frames, and `stim explain_errors` CLI remain deferred. |
