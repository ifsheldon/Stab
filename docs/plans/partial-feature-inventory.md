...
| PF1 gate metadata rows | RPF1 | RPF1 owns remaining gate decomposition metadata, unsupported metadata accessors, and gate execution contract documentation. |
...
- Gate decomposition metadata accessor behavior for the H/S/CX/M/R decomposition table, including representative exact text, the full supported gate set, parseability as `.stim` text, and fail-closed behavior for gates without decomposition metadata.
...
- A canonical gate support table separating validation support, tableau metadata, unitary metadata, flow metadata, decomposition metadata, sampler execution, detector conversion, analyzer propagation, and explicit rejection.
...
- `flattened` and flattened-operation traversal for repeat blocks, tags, annotations, detectors, observables, measurement references, coordinate shifts, and large-repeat resource behavior.
- `without_noise` for noise removal while preserving deterministic operations, coordinates, ticks, detectors, observables, and measurement-record semantics.
- `decomposed` for compound gates, MPP, SPP, pair measurements, grouped targets, base-gate lowering, tableau or flow semantic checks, and unsupported decomposition target errors.
- Feedback inlining for selected single-control Pauli feedback and MPP feedback cases, including repeat-block support or explicit repeat-block rejection.
...
| PF2-FLATTEN-WITHOUT-NOISE | Repeat handling; Circuit transforms; Full circuit transform API parity | PF2 | `stab-core` circuit transforms | `src/stim/circuit/circuit_pybind_test.py`; `src/stim/circuit/circuit.test.cc` | Exact canonical-output and structural tests for `flattened`, `flattened_operations`, `without_noise`, tags, annotations, detectors, observables, measurement references, coordinate shifts, and large repeats. | `pf2-circuit-flatten-without-noise` manifest-only row. | `pf2-circuit-flatten-repeat` and `pf2-circuit-without-noise` non-primary placeholders. | QASM, Quirk, Crumble, and diagrams remain deferred. |
