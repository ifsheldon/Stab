...
| PF4-DEM-FOLDED-TRAVERSAL | DEM flattening and large repeat traversal; DEM analysis and shortest graphlike error | PF4 | `stab-core` DEM traversal consumers | `src/stim/simulators/dem_sampler.test.cc`; `src/stim/search/graphlike/algo.test.cc`; `src/stim/search/hyper/algo.test.cc`; `src/stim/search/sat/wcnf.test.cc` | Resource-boundary tests now cover the current DEM sampler capped-repeat behavior for allowed shifted repeats, excessive repeat counts, and nested repeat expansion. Broader resource-boundary tests for search, SAT, matcher-adjacent, and analyzer-adjacent traversal remain active and must prove folded traversal or a documented cap. | `pf4-dem-sampler-repeat-resource-rust` supplements the broad `pf4-dem-folded-traversal` manifest-only row. | `pf4-dem-sampler-folded-repeat` has non-primary report-only Rust runner coverage; `pf4-dem-folded-traversal` and `pf4-dem-folded-graphlike-traversal` remain placeholders. | Match-graph rendering remains deferred. |
