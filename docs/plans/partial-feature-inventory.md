...
- Missing-detector generated-code suffix cases for honeycomb and broader toric circuits, plus precise errors for any unpromoted family. Gaussian row reduction, repeated MPP and pair-measurement stabilizer products, observable-row interactions, and the toric global-stabilizer suffix case are implemented for the promoted Rust subset.
...
| PF5-MISSING-DETECTORS | Detector-analysis utility APIs | PF5 | `stab-core` detector utility API | `src/stim/util_top/missing_detectors.test.cc` | Structural tests now cover Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer products, record-only observable rows, ignored Pauli observable rows, unknown-input behavior, and the pinned Stim toric global-stabilizer generated-code suffix case. Honeycomb suffixes, broader generated-code workloads, and broader measurement-rich public flow semantics remain active. | `pf5-missing-detectors-row-reduction-rust`, `pf5-missing-detectors-mpp-observable-rust`, and `pf5-missing-detectors-generated-toric-rust` supplement the broad `pf5-missing-detectors-extended` manifest-only row plus historical M9 future rows. | `pf5-missing-detectors-mpp` and `pf5-missing-detectors-generated-code` have non-primary report-only Rust runners with measurement work and compare notes. | Any generated-code or flow-dependent missing-detector family not implemented by RPF5 must remain explicitly deferred or fail closed. |
