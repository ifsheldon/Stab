...
| PF6-ANALYZER-GENERATED | Circuit-to-DEM analysis; `analyze_errors --decompose_errors` and related flags; Sparse reverse detector-frame tracking | PF6 | `stab-core` analyzer | `src/stim/simulators/error_analyzer.test.cc`; `src/stim/util_top/circuit_to_dem.test.cc`; `src/stim/cmd/command_analyze_errors.test.cc` | Generated-QEC semantic DEM tests now cover noisy generated repetition-code and rotated-surface-code circuits against pinned Stim v1.16.0 expectations; broader exact `.dem` and structural tests for generated looping, loop folding, gauge detectors, approximate disjoint errors, decomposition options, remnant-edge blocking, and ignored failures remain active. | `pf6-analyzer-generated-qec-rust` supplements the broad `pf6-analyzer-generated-looping` manifest-only row plus existing M10 analyzer rows. | `pf6-analyze-errors-generated-surface` has a non-primary report-only Rust core runner with measurement work and compare notes; `pf6-error-decomp-loop-folded` remains a placeholder. | Full ErrorMatcher provenance remains deferred unless needed for analyzer correctness. |
