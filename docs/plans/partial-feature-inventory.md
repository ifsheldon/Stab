...
| PF5-DETECTING-REGIONS | Detector-analysis utility APIs | PF5 | `stab-core` detector utility API | `src/stim/util_top/circuit_to_detecting_regions.test.cc`; `src/stim/util_top/circuit_to_detecting_regions_test.py` | Structural tests now cover the M9 simple H/CX/MXX case and the PF5 bounded repeat-tick traversal subset, including excessive repeat-expansion rejection. Broader Clifford gates, target shapes, tick windows, detector filtering, multi-detector regions, anticommutation behavior, and gauge behavior remain active. | `pf5-detecting-regions-repeat-rust` supplements the broad `pf5-detecting-regions-extended` manifest-only row plus M9 simple implemented row. | `pf5-detecting-regions-repeat` has a non-primary report-only Rust runner with measurement work and compare notes. | Unpromoted gauge or anticommutation behavior must fail closed and stay partial. |
