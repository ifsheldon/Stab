...
- `decomposed` for compound gates, MPP, SPP, pair measurements, grouped targets, base-gate lowering, tableau or flow semantic checks, and unsupported decomposition target errors. A scoped Rust `Circuit::decomposed` API now exposes the existing H/S/CX simplification subset and preserves unowned MPP, SPP, and pair-phasing families; full Stim decomposition parity remains open.
...
| PF2-DECOMPOSED | Circuit transforms; Gate semantic execution | PF2 | `stab-core` decomposition and algebra integration | `src/stim/circuit/gate_decomposition.test.cc`; `src/stim/util_top/simplified_circuit.test.cc`; `src/stim/util_top/circuit_vs_tableau.test.cc` | `cargo test -p stab-core --test circuit_transforms decomposed` covers the scoped Rust `Circuit::decomposed` API aliasing the current simplification subset and preserving unowned MPP, SPP, and pair-phasing families. Full exact and semantic tests for MPP, SPP, pair measurements, grouped targets, base-gate lowering, tableau equivalence, and unsupported decomposition targets remain open. | `pf2-circuit-decomposed-scoped-rust` is implemented and supplements the broad `pf2-circuit-decomposed` manifest-only row. | `pf2-circuit-decompose-mpp-spp` remains a non-primary placeholder until the full decomposition runner exists. | Any decomposition requiring deferred simulator products must fail closed. |
