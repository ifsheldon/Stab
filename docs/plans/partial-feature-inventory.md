...
| PF5-MEASUREMENT-RICH-FLOWS | Flows; Circuit transforms | PF5 | `stab-core` flow API | `src/stim/stabilizers/flow.test.cc`; `src/stim/stabilizers/flow_pybind_test.py`; `src/stim/util_top/circuit_flow_generators.test.cc`; `src/stim/util_top/has_flow.test.cc` | Structural tests now cover unsigned `has_flow` measurement-record dependencies, pair-measurement records, observable dependencies, observable Pauli targets, sign-insensitive matching, exact `circuit_flow_generators` output for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, nonconstant and constant single-instruction `MPP`, `MPAD`, scoped measurement-record feedback examples, the pinned heralded-noise MPP fixture, generated-flow checker satisfaction for the supported checker subset, explicit rejection of unpromoted measurement-rich generator shapes, anti-Hermitian `MPP` products, and pinned Stim `solve_for_flow_measurements` empty, `MX`, idle-extra-qubit, and repetition-code examples with Stab-specific fallback resource hardening. Broader tests for composed measurement-rich `flow_generators`, broader heralded-noise generator synthesis, full generator-table measurement solving, failure explanations, and transform integration remain active. | `pf5-has-flow-record-observable-rust`, `pf5-flow-generators-measurement-rust`, and `pf5-flow-solve-measurement-rust` supplement the broad `pf5-measurement-rich-flows` manifest-only row. | `pf5-has-all-flows-batch`, `pf5-flow-generators-measurement-rich`, and `pf5-flow-solve-measurement-rich` have non-primary report-only Rust runners with measurement work and compare notes. | Python flow binding ergonomics remain deferred. |
