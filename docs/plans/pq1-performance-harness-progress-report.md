...
> Current-contract note, 2026-07-20: the product harness has seventeen executable exact-case groups plus its diagnostic protocol group, private Stab build-receipt schema version 5, adapter receipt schema version 11, contract-preflight schema version 12 with 212 probes, runtime-group schema version 5, and qualification report schema version 31. Formal run, rollup, and completion producers publish only to previously absent direct-child output directories; offline replay alone may compare-and-swap an existing artifact. Output admission and repository binding occur before lock, artifact, Git, source, build, or subprocess work; every nested qualification action consumes the retained Linux descriptor-root view; and publication binds the exact staged, target, source, correctness, and displaced artifact trees by descriptor identity, length, digest, directory identity, and child set while repeatedly proving the admitted path still names the retained live root. The harness rejects in-place mutation and unexpected or substituted entries before and after exchange, propagates replay cleanup failure, and fsyncs and revalidates the complete output hierarchy before success. Clean revision `da7c787d1e9f49110d7054868b146b5fb7d7bda4` completed and replayed a schema-version-31 Clifford chain under correctness digest `4dbbb4b2cda3117bdd3d3ddfcd30b55f09e6f401352e3e86130222189d47791f` and performance digest `c238dc4e2500192f310ef3d2378ecaafc9744662b5127784dd4eeb6c60726176`; that chain is historical after the artifact-lifecycle, Clifford short-right-operand, and inventory-count fixes. The source state that replaces those contracts must receive new checked inventory digests and clean evidence before promotion. The schema-version-30 chain from revision `91f62d0a78659da2e8e264a6968b3c6cd32456de` and clean iterator and earlier product evidence also remain historical under their recorded inventories. The schema-version-13 PQ1 reports remain historical diagnostic infrastructure evidence, not product performance.
