...
> Current-contract note, 2026-07-19: correctness inventory digest `4dbbb4b2cda3117bdd3d3ddfcd30b55f09e6f401352e3e86130222189d47791f` and schema-version-2 performance inventory digest `c238dc4e2500192f310ef3d2378ecaafc9744662b5127784dd4eeb6c60726176` are source-current. The product harness has seventeen executable exact-case groups plus its diagnostic protocol group, private Stab build-receipt schema version 5, adapter receipt schema version 11, contract-preflight schema version 12 with 212 probes, runtime-group schema version 5, and qualification report schema version 31. Formal run, rollup, and completion producers now publish only to absent output directories, while offline replay alone performs compare-and-swap refresh of an existing artifact. The checked threshold-migration ledger binds the exact pre-migration authorization receipts and source inventories to each retired legacy timing contract. The clean schema-version-30 Clifford chain from revision `91f62d0a78659da2e8e264a6968b3c6cd32456de` is accepted historical evidence under its recorded inventory; shared worker-source, hostile-request, publication, and migration-ledger changes require a fresh clean schema-version-31 chain before either Clifford result is source-current again. Clean iterator and earlier product evidence remains accepted historical evidence under each recorded inventory. The schema-version-13 PQ1 reports remain historical diagnostic infrastructure evidence, not product performance.
...
| Commands and atomic publication | Satisfied | `qualification-list`, `qualification-check`, `qualification-probe`, `qualification-run`, `qualification-report`, and `qualification-regression`; producers reject an existing output directory, while replay binds exact report/preflight bytes and uses compare-and-swap refresh |
