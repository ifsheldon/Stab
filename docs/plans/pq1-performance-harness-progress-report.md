...
> Current-contract note, 2026-07-23: the product harness has nineteen executable exact-case groups plus its diagnostic protocol group, private Stab build-receipt schema version 6, adapter receipt schema version 12, worker-protocol schema version 5, contract-preflight schema version 15 with 228 probes, runtime-group schema version 6, and qualification report schema version 34. The current worker and runtime contracts bind `raw-work-v2`; semantic preflight probes use distinct non-statistical `contract` evidence, while timing and memory receipts retain strict positive-duration validation. The two newest groups independently qualify direct DEM parsing and canonical printing but have no promotable post-fix performance evidence yet. Formal run, rollup, and completion producers publish only to previously absent direct-child output directories; offline replay alone may compare-and-swap an existing artifact. Output admission and repository binding occur before lock, artifact, Git, source, build, or subprocess work; every nested qualification action consumes the retained Linux descriptor-root view; and publication binds the exact staged, target, source, correctness, and displaced artifact trees by descriptor identity, length, digest, directory identity, and child set while repeatedly proving the admitted path still names the retained live root. The harness rejects in-place mutation and unexpected or substituted entries before and after exchange, propagates replay cleanup failure, and fsyncs and revalidates the complete output hierarchy before success. Every earlier product chain remains historical under its exact inventory, and the schema-version-13 PQ1 reports remain diagnostic infrastructure evidence rather than product performance.
...
| Symmetric bounded process execution | Satisfied | `ops/bench/src/process.rs`; shared baseline and qualification supervisor with success, exit, signal, timeout, output-limit, process-group cleanup, start-barrier, affinity, file-limit, environment, discard, invalid UTF-8, cancellation, and descendant-held-pipe tests |
...
| Commands and atomic publication | Satisfied | `qualification-list`, `qualification-check`, `qualification-probe`, `qualification-run`, `qualification-report`, and `qualification-regression`; producers reject an existing or nondirect output before mutation, while publication binds the exact staged, source, and target file sets by descriptor identity, length, and digest, durably syncs the output hierarchy, and verifies both directory entries before rollback or cleanup |
...
Seventeen exact-case PQ2 product contracts are executable; the remaining work must continue adding equivalent-work deterministic model, format, gate, kernel, and algebra groups instead of promoting the PQ1 synthetic ratio.
