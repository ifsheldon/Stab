...
After the M7 convert benchmark expansion and the current M7/M10 fixes, the current dirty primary beta diagnostic at `target/benchmarks/m12-primary-beta/compare.json` measures 85 primary rows, passes beta with 80 comparable rows and 5 checked `waived-not-comparable` no-ratio rows, reports `m7-convert-b8-to-b8-wide` at `0.0018439704138050598x`, and reports `m10-error-decomp` at `1.1764705882352942x`.
