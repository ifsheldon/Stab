...
Post-A9 review correction, 2026-08-24: the historical `m10-error-decomp` p10 threshold and four-pair direct-match claim were invalid. Failed pinned Stim calls retain a final Newton iterate, while Stab's public API returns `None`. The active benchmark row is `partial-match`; only successful exact and independent conversions form Stim-relative pairs, failed Stab calls are report-only, and schema-version-2 timing-regression policy records the faithful nanosecond pairs as unstable until clean evidence provides threshold headroom.
...
- `m10-error-decomp` historical disposition, now review-rejected: schema-version-2 thresholding treated `disjoint_to_independent_xyz_errors_approx_p10` as a direct pair at `1.25`; the 2026-08-24 correction above removes that threshold because the failed-call witnesses differ. The exact closed-form and independent-to-disjoint paths remain faithful nanosecond-scale pairs, while p10 and p100 remain report-only.
...
- Source-current waiver policy, updated 2026-08-24: `m7-convert-stim-canonical`, `m7-convert-01-to-ptb64`, and `m8-measure-reader-ptb64-contract` are the three explicit no-ratio rows in both waiver ledgers. `m10-error-decomp` has a separate timing-regression stability waiver bound to its exact and independent successful pairs; it is not a no-comparator waiver.
...
- `m10-error-decomp`: successful exact and independent-to-disjoint calls remain under the active `1.25x` beta gate, but neither currently owns a strict timing threshold because the pairs are nanosecond-scale and lack stable headroom. Failed p10 and p100 calls remain report-only because Stim retains the final Newton iterate while Stab's public API returns `None`.
