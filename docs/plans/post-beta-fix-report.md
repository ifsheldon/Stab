...
After the M7 convert benchmark expansion and the current M7/M10/M4 fixes, the current dirty primary beta diagnostic at `target/benchmarks/m12-primary-beta/compare.json` measures 85 primary rows, passes beta with 80 comparable rows and 5 checked `waived-not-comparable` no-ratio rows, reports `m7-convert-b8-to-b8-wide` well below the gate, reports `m4-circuit-parse` at `1.110794520547945x`, reports `m8-sample-primary-unrotated-surface-contract` as a narrow watch-row pass at `1.2458306026893222x`, and reports `m10-error-decomp` at `1.25x`.
The current dirty timing-regression diagnostic at `target/benchmarks/m10-error-decomp-primary-regression/compare.json` passes with 80 configured threshold rows and 5 checked `waived-not-thresholdable` rows, including `m7-convert-b8-to-b8-wide` at `0.0015078042257655317x`.
The current dirty memory-regression diagnostic at `target/benchmarks/m12-primary-memory-regression/compare.json` passes all 85 rows using the schema-version-2 resident-delta memory baseline.
Those expanded reports are not final archival evidence until regenerated from committed code with `local_modifications=false`.
