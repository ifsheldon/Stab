...
- Gate metadata: Rust-accessible metadata for aliases, inverse, category, target requirements, validation flags, flow, unitary/tableau, and decomposition where Stim semantics depend on it. The implemented PF1 slices cover aliases, argument rules, target rules, target grouping, fusing, noisy/reset/measurement/unitary/single-qubit/two-qubit/target-capability/symmetry flags, unitary inverse, generalized inverse, local Clifford tableau metadata, and tableau-backed unitary flow metadata; measurement-rich or variable-target flow data, arbitrary unitary matrices, and decomposition metadata remain active work in PF1/PF5.
