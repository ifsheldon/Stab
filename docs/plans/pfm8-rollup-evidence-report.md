...
| --- | --- | --- |
| `Rust core library equivalent for core Stim semantics` | Rollup over active Rust APIs, transforms, DEMs, utilities, flows, analyzer, search, and sparse-tracker rows. | Keep `Partial`; PFM-B1, PFM-B3, PFM-B4, and PFM-B5 are complete, while the PFM-B2 generated semantic matrix plus PFM-B6 rollup remain. |
...
- PFM-B5 is complete for all 52 selected analyzer, graphlike, hypergraph, shared traversal, WCNF, sparse-tracker, and matched-error cases with zero shared selectors, no fixture-specific recurrence periods, bounded analyzer, graph-construction, and search work, pre-reservation graph admission, early trivial SAT handling, exact diagnostic and source-membership assertions, content-bound direct oracle fixtures, fresh allocation-tracked evidence, milestone-audit closure, and final full-code-review sign-off.
...
PFM-B5 replaces period-specific report-only rows with a generic analyzer and finite search matrix; its clean final-audit d25/r25 and d11/r1000 graphlike rows remain report-only at 4.647x and 4.279x Stim and therefore do not enter the 1.25x threshold file.
