...
- PFM3 broader analyzer sweep-shape behavior and legal non-tableau execution beyond the selected sweep matrix, fixed-tableau execution, supported Hermitian `SPP` or `SPP_DAG`, deterministic `MPP`, deterministic `MPAD`, selected stochastic `MPAD(p)` sampler or detection-sampling, and selected noisy `MPAD(p)` analyzer boundary.
