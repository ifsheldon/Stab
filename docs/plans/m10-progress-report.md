...
- Added the first graphlike search internal types for observable masks, edges, nodes, graphs, canonical search states, and shortest graphlike undetectable logical error search over direct DEM graph structures, including DEM flattening into graphlike edges and DEM error-instruction emission for search-state transitions.
