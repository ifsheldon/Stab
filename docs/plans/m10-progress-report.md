...
- Added a backward detector-sensitivity pass for the first `--allow_gauge_detectors` case, including default rejection of non-deterministic detectors and 50 percent gauge-error emission for detector-only gauge sets through `H` and `CX`.
...
| `stim analyze_errors --allow_gauge_detectors` | Partial | `m10-analyze-errors-allow-gauge-detectors`, `cargo test -p stab-core gauge`, `cargo test -p stab-cli gauge`, `just oracle::run --milestone M10 --exact`, `just oracle::record --check-clean` |
