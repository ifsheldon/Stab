...
This slice implements the `.dem` format core and staged `analyze_errors` paths needed by the existing deterministic oracle rows, correlated-error rows, the first `--decompose_errors` fallback row, the high-repeat loop-folding row, and the first approximate disjoint-error threshold rows.
...
- Added the first `--decompose_errors` target-level fallback path that decomposes a non-graphlike detector error through a known graphlike component when the remnant is also graphlike, and rejects simple undecomposable detector triples.
...
| `stim analyze_errors --decompose_errors` graphlike fallback decomposition | Partial | `m10-analyze-errors-decompose-fallback`, `cargo test -p stab-core decompose`, `cargo test -p stab-cli decompose_fallback`, `just oracle::run --milestone M10 --exact` |
