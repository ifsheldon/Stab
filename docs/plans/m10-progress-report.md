...
- `cargo test -p stab-core graphlike` covers the M10-owned graphlike edge, node, and search-state construction, canonicalization, equality, ordering, hashing, display, and DEM transition subset ported from `src/stim/search/graphlike/edge.test.cc`, `src/stim/search/graphlike/node.test.cc`, and `src/stim/search/graphlike/search_state.test.cc`.
...
- Added the first graphlike search internal types for observable masks, edges, nodes, and canonical search states, including DEM error-instruction emission for search-state transitions.
...
| Graphlike search edge, node, and search state internals | Partial | `coverage-search-graphlike-edge`, `coverage-search-graphlike-node`, `coverage-search-graphlike-search-state`, `cargo test -p stab-core graphlike`, `just oracle::run --milestone M10 --structural` |
