...
- `cargo test -p stab-bench m10_dem_benchmark_rows_have_stab_compare_runners` covers Stab-side M10 graphlike search, generated surface-code analyzer, `analyze_errors --decompose_errors`, `.dem` parse, `.dem` print, and loop-folding analyzer benchmark runners.
...
- Added Stab-side benchmark compare runners for the M10 graphlike search, generated surface-code analyzer, `analyze_errors --decompose_errors`, `.dem` parse, `.dem` print, and loop-folding analyzer rows.
...
| M10 benchmark reporting | Partial | `just bench::compare --milestone M10` measures graphlike search, generated surface-code analyzer, `analyze_errors --decompose_errors`, `.dem` parse, `.dem` print, and loop-folding rows; error-decomposition utility remains pending or missing baseline |
