...
- Added scoped Clifford propagation for pending Pauli errors through `H` and `CX`, including cancellation of duplicate Pauli effects after propagation and an explicit error instead of silent misanalysis when pending single-qubit Pauli channels would need to cross `CX`.
...
| `stim analyze_errors` Clifford propagation | Partial | `m10-analyze-errors-h-propagates-pauli-error`, `m10-analyze-errors-cnot-propagates-pauli-error`, `cargo test -p stab-core propagates_pauli`, `cargo test -p stab-cli propagates_pauli`, `just oracle::run --milestone M10 --exact`, `just oracle::record --check-clean` |
