...
The current program has a curated release matrix, separate Stim parity and Stab self-regression policies, `raw-work-v2`, representative DEM families, and a schema-version-2 revision completion manifest. Clean revision `68d107a42f655254f31628f0cbedc55479f6c0f3` completed the first repaired-contract AArch64 DEM evidence chain, and the subsequent reviewed baseline seeds 18 exact self-regression identities. Current generated counts and evidence state live in [../qualification-status.md](../qualification-status.md).
