...
Status: Active as of 2026-07-16. All 271 selected CQ2 parents have complete exact ownership. Four exact runtime groups are implemented at performance inventory `9be7dea23a221e9d7fe6ba2e9bdfaf65ad2b4a7675b557386b3de6ea58306557`. Parser, canonical printing, and gate-name hashing have historical passing controlled-host evidence under preceding inventories. The new `PERFQ-M5-SIMD-WORD` toggle-plus-popcount group requires clean exact CQ, full, and soak execution plus replayed scale-family rollups before it owns a performance claim. The remaining model, format, kernel, and algebra groups are still planned.
...
1. Graduate `PERFQ-M5-SIMD-WORD` as one execute-phase `toggle-popcount` measurement derived from pinned Stim's `simd_compat_popcnt` workload. Do not combine it with XOR, nonzero scans, masked operations, copies, or sparse-vector work.
...
3. Generate identical little-endian `u64` fixture words in both workers with the source-owned `splitmix64-word-v1` function. Construct Stim `simd_bits<MAX_BITWORD_WIDTH>` and Stab `BitVec` values before the start barrier and release temporary fixture storage before setup RSS is sampled.
...
5. Time only one compiler fence, toggling bit 300, and popcounting the complete vector per iteration. Count every visited bit as semantic work and keep allocation, fixture generation, and input hashing outside timing.
6. Accumulate every popcount into a wrapping checksum and bind the checksum, iteration count, bit width, fixture fingerprint, and final toggle state into the semantic output digest. The parent must reject any Stim and Stab work, input, or output disagreement before producing a ratio.
7. Reject widths below 512 bits, widths not divisible by 256 bits, and widths above 268,435,456 bits before allocation and before reading the start barrier. Worker reproducibility must execute both sealed workers at the first over-cap aligned width and at one in-range unaligned width with no barrier input and require the exact source-owned errors.
...
The fourth runtime contract is implemented in source at performance inventory `9be7dea23a221e9d7fe6ba2e9bdfaf65ad2b4a7675b557386b3de6ea58306557`, but clean committed execution is pending. The earlier gate-name-hash timing reports remain valid historical evidence under performance inventory `1cc0be5c8c0a37c98bd4fb56f331dd6964e6f53e56b328b9564be507cbf88a42`; they are not relabeled as current after the shared workers and global performance inventory changed.
...
- Verify the popcount fixture's three exact input byte counts and digests, checksum accumulation, final toggle state, accepted maximum, first over-cap rejection, and unaligned-width rejection.
- Verify the popcount adapter probe rejects caller widths outside the source-owned range or alignment and accepts the exact medium scale.
