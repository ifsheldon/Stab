...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), [pq2-circuit-parse-qualification-progress-report.md](pq2-circuit-parse-qualification-progress-report.md), [pq2-circuit-canonical-print-qualification-progress-report.md](pq2-circuit-canonical-print-qualification-progress-report.md), [pq2-gate-name-hash-qualification-progress-report.md](pq2-gate-name-hash-qualification-progress-report.md), [pq2-simd-word-popcount-qualification-progress-report.md](pq2-simd-word-popcount-qualification-progress-report.md), [pq2-simd-bits-xor-qualification-progress-report.md](pq2-simd-bits-xor-qualification-progress-report.md), and `benchmarks/stim-qualification-suite.json` at current performance digest `315c1d985af62f08068cd273a0e06399aaa7dd85ff3009309e284bc46aaaaf3d`, bound to correctness digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`. Eight exact runtime contracts are implemented with exact `1.25x` targets: the first five historical groups plus independent early-hit, all-zero, and late-hit `not_zero` groups. The `not_zero` contract now binds optimizer-opaque immutable input references on both workers so each timed iteration executes the scan. The frozen timing policy also binds the 2-second independent and 10-second wide-ratio common ceilings. The first five groups' passing AArch64 reports remain historical after later inventory changes. Dense-XOR evidence from clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` passed all full and soak scales at its original inventory, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. The three `not_zero` contracts have no promotable timing evidence yet. The legacy canonical-print diagnostic is superseded and no longer owns beta, timing-regression, or memory waivers. The legacy XOR and `not_zero` threshold pairs now have exact executable replacement mappings, but their M12 guards remain active until the corresponding migration evidence lands; `m5-simd-word` also remains active pending its own migration.
...
Status: Active as of 2026-07-16. All 271 selected CQ2 parents have complete exact ownership. Eight exact runtime groups are implemented at performance inventory `315c1d985af62f08068cd273a0e06399aaa7dd85ff3009309e284bc46aaaaf3d`. Parser, canonical printing, gate-name hashing, SIMD-word popcount, and complete-vector XOR have historical passing controlled-host evidence under preceding inventories. The three independent `not_zero` groups are implementation-complete, including matching anti-elision boundaries and bounded wide-ratio common calibration, but still require clean correctness, full, soak, regression, rollup, completion-receipt, audit, and review evidence. The remaining model, format, kernel, and algebra groups are still planned.
...
8. Add `benchmarks/stim_adapter/simd_bits_not_zero_contract.h` to the ordered adapter comparator receipt and bind its digest in every new group. Adapter receipt schema version 6 must reject omissions, reordering, substitutions, or altered bytes. Keep private Stab build-receipt schema version 2 but expand its framed source collection to `worker.rs`, `worker/bits.rs`, and `worker/not_zero.rs`. Contract-preflight schema version 5 must bind 42 actual receipts, and qualification report schema version 22 must preserve and replay that exact preflight plus the derived common-batch mode.
9. Keep one prepared vector live during timing and record setup and peak process RSS separately at all scales. Under `count-allocations`, prove zero timed allocations for every pattern at every runtime scale. This slice makes no accepted cross-scale RSS ratio or Stim allocation claim; PQ6 remains the owner of resource-growth acceptance.
...
The source-current performance inventory is `315c1d985af62f08068cd273a0e06399aaa7dd85ff3009309e284bc46aaaaf3d`. It contains all three implemented `not_zero` runtime contracts, matching optimizer-opaque immutable input references, the bounded wide-ratio common-batch policy, and their independent `1.25x` rules, plus an exact legacy replacement mapping to the early group. This is contract and implementation state, not performance acceptance. No sixth-slice ratio, rollup, or completion receipt is yet promotable, and every first-five timing report remains historical after the global inventory and shared worker identities changed.
...
- Verify standard common-batch classification at both boundaries; valid wide-ratio classification with either implementation as the slower side; hard rejection below 250 milliseconds, above 10 seconds, when the common-iteration owner exceeds 2 seconds, when equal iteration decisions claim wide-ratio mode, or when both implementations exceed 2 seconds; and offline rejection when a report mutates the derived mode.
