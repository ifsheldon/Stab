...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), [pq2-circuit-parse-qualification-progress-report.md](pq2-circuit-parse-qualification-progress-report.md), and `benchmarks/stim-qualification-suite.json` at current PQ2 first-slice performance digest `9687d45fa57b97388e7a8c7b1676f2545619ff3185c19aa714583b25d1680924`, bound to correctness digest `4d9faa21e318eeebc4614c7bf62491bb2db73b5db57ae3dab7d0f19f3fda7cad`. The refresh graduates exactly `PERFQ-M4-CIRCUIT-PARSE` from planned inventory metadata into an implemented exact runtime contract and reworks the inherited `m4-circuit-parse` disposition without relaxing its `1.25x` target. Clean revision `d2df9766c5e3543c8df016db31f48f552354d79f` produced current-schema correctness-bound full and soak reports plus replayed AArch64 full and soak scale-family rollups. All six promotable timing measurements are valid, non-noisy passes with medians from `0.859679x` to `0.917843x` and worst upper confidence bound `0.926499x`; no threshold or comparator rule is waived. The dependent schema-version-13 PQ1 PR, full, and soak execution from revision `d0ecafd62794daad0ab5eb63d54c481a5e32a30b` remains historical diagnostic infrastructure evidence rather than product-performance qualification.
...
The first slice is infrastructure plus one proving workload. It graduates exactly `PERFQ-M4-CIRCUIT-PARSE` into the checked performance inventory and reclassifies the inherited `m4-circuit-parse` row from retained to reworked because the exact replacement contract now exists. Clean current-schema AArch64 full and soak evidence and both scale-family rollups are complete at revision `d2df9766c5e3543c8df016db31f48f552354d79f`. All six promotable measurements pass the unchanged `1.25x` target, with full and soak medians of `0.883627x` and `0.881622x` at small scale, `0.860613x` and `0.859679x` at medium scale, and `0.917797x` and `0.917843x` at large scale. The parser optimization blocker is closed, and the next owner action is the next source-owned PQ2 runtime group. This slice does not complete `PERF-CIRCUIT-MODEL`, supersede the inherited row, satisfy PQ2's remaining 218 planned groups, or provide native x86-64 evidence.
