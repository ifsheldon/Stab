...
12. Bump the private Stab build-receipt schema to version 6, adapter receipt schema to version 12, contract-preflight schema to version 13 with 228 ordered receipts, and qualification report schema to version 32. Add every new Rust and C++ source to the ordered worker or adapter source receipt. Reproducibility and offline replay must reject an omitted, extra, reordered, renamed, modified, stale, or cross-worker-transplanted DEM source or receipt, an incorrect receipt count, and any fixture or digest drift.
