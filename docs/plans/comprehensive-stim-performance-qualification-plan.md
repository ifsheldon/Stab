...
Status: Active as of 2026-07-23. All 271 selected CQ2 parents have complete exact ownership. The source-current correctness inventory is `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`, and the source-current post-migration performance inventory is `3f51801b592b0cb8dc3b340cced3dc3b7644b913168073c7d4106188e444d83d`. Nineteen exact product runtime groups are implemented; the twelfth slice adds independent direct DEM parse and canonical-print groups with fixed scales, exact input and output identities, common-iteration timing, accepted-maximum and hostile-boundary contracts, strict `1.25x` rules, and a source-owned profiler note. Their private Stab build-receipt schema is version 6, adapter receipt schema is version 12, contract-preflight schema is version 13 with 228 ordered receipts, and qualification report schema is version 32. Earlier stable parse failures remain historical under their exact inventories. Clean pre-migration revision `d9e2405d18cfff05d9b5d908525394476b0edcbc` passed all twelve first-attempt DEM reports, all replays and regressions, four rollups, and both completion receipts; focused migration commit `1cfecd64cde4a5effdf07fdaabdbe51017e25a4a` then retired only the exact legacy timing guards and preserved both memory baselines. The checked migration ledger binds the distinct parse threshold and print waiver authorizations, but the complete post-migration chain has not run and the passing pre-migration ratios are not source-current evidence. Every earlier Clifford, iterator, Pauli, transpose, kernel, and circuit result remains accepted or rejected only under its recorded inventory. Native x86-64, PQ6 memory growth, and all later runtime groups remain unclaimed.
