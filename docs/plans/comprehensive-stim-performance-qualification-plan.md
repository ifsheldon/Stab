...
Historical performance contract, revised 2026-07-23 by [qualification-economy-regression-plan.md](qualification-economy-regression-plan.md). Its paired timing, output validation, host control, and memory principles remain durable inputs to [the active core parity plan](stim-core-parity-and-lean-evidence-plan.md), but its inventories and publication lifecycle no longer define current work.
...
The historical program has a curated release matrix, separate Stim parity and Stab self-regression policies, `raw-work-v2`, representative DEM families, and a schema-version-3 release completion manifest with exact correctness evidence per group. Clean revision `68d107a42f655254f31628f0cbedc55479f6c0f3` completed the first repaired-contract AArch64 DEM evidence chain under historical schema 2, and the subsequent reviewed baseline seeded 18 exact self-regression identities. Its generated counts and evidence state remain in the [transitional qualification dashboard](../qualification-status.md).
...
The adapter protocol must be schema-versioned JSON Lines containing the evidence mode, workload id, measurement id, iteration count, elapsed seconds, semantic work count, output digest, peak memory when available, Stim commit, and build fingerprint. Semantic acceptance and rejection preflight requests must use `contract`, formal runtime evidence must use `timing`, and separately instrumented resident-memory evidence must use `memory`. Contract receipts may report a finite zero elapsed duration because elapsed time is not consumed as evidence; timing and memory receipts must report a finite positive duration, and all report statistics must reject contract receipts.
...
- The active producer publishes one schema-version-4 architecture/revision completion manifest. Completion schemas 1 through 3 remain readable historical evidence and are not relabeled or reconstructed under the current contract.
- Historical DEM R6 reconstruction retains four rollups and its exact parse and print schema-version-2 accepted-maximum memory receipts. Those receipts remain historical and nonpromotable; coordinated release qualification uses `--scope a9-release`, one full and one soak rollup for every promotable runtime group, and fresh schema-version-3 parse and print memory receipts from the release revision.
- Invoke `just bench::qualification-completion --scope <scope> --rollup <rollup> ... --memory-receipt <parse-memory> --memory-receipt <print-memory> --out <completion-directory>` from the same clean committed revision recorded by all source evidence. Release-scope creation, offline replay, and checkpoint replay must each run through `prlimit --nofile=1024:1024 --` and authenticate that exact soft limit.
- For coordinated release qualification, invoke the same command with `--scope a9-release` and one full and one soak rollup for every promotable runtime group. Derive this scope from the checked runtime contracts instead of copying a group list into the completion implementation; reject product diagnostics, missing release groups, and extra rollups.
- Replay every source rollup and report, validate every Stim parity result, and require exact repository, Stim, toolchain, verified-host, CPU, private-worker source/build/binary, inventory, policy, timing-boundary, correctness, memory, and workload identity. A policy-consistent diagnostic host record with `verified=false` is never promotable.
...
- Invoke `just bench::qualification-completion-report --input <completion-directory>` to reconstruct the manifest deterministically while retaining its exact reports, rollups, correctness trees, memory receipts, completion directory, and repository binding through final validation. Human milestone audit and full code review remain separately recorded human evidence.
...
- Preserve the successful exact and independent `m10-error-decomp` pairs, keep failed p10 and p100 public-`None` calls report-only unless Stab exposes the same final-iterate witness as pinned Stim, and add sizes and error shapes only when they preserve observable-work equivalence.
...
Performance inventory schema version 4 binds every source waiver's file, typed kind, reason, follow-up, and exact measurement-pair set into its semantic digest.

Historical checkpoint, recorded as active on 2026-08-04. The inventories and counts were generated in [../qualification-status.md](../qualification-status.md). Nineteen promotable product runtime groups and nine Stab-only product-diagnostic groups were implemented. DEM parse and canonical print each own three representative families at small, medium, and large scales, exact input and output identities, common-iteration timing, family-specific accepted maxima, strict `1.25x` parity, and source-owned profiler notes. Historical DEM chains at revisions `80fb5405fb077c694a8a8a18e64a3a5831e20a5e` and `9497df0350cb33dcd249ea12fda802b5a68efe00` remain immutable `raw-work-v1` evidence and are review-rejected as current evidence. Source-current runtime-group schema version 10, worker protocol version 5, contract-preflight version 16, report version 35, rollup version 5, completion version 4, and accepted-maximum memory-receipt version 3 bind `raw-work-v2`; completion schemas 1 through 3 and memory-receipt schema 2 remain historical readers. Completion version 4 records each release group's exact correctness preflight, both DEM memory receipts, and soft `RLIMIT_NOFILE`; release creation, replay, checkpointing, and checked status require exactly 1024. Memory receipts require verified controlled-host evidence and use the same sealed private Stab worker source, build, and binary identity as formal timing. A current public checkpoint must be generated from a successful replay, retain the exact canonical manifest bytes in checked source, and pass digest, scope, policy, inventory, Git-ancestry, clean-worktree, and one-descendant validation. Product diagnostics reuse the timing boundary and bounded worker contracts, enforce one source-owned 600-second measurement-suite deadline, bind every scale to an independently owned correctness witness and exact output digest, and never enter Stim parity, Stab self-regression, rollups, completion, or legacy benchmark rows. Calibrated stateless groups validate one complete operation before timing; stateful groups use one witnessed complete pass per sample. Their sampler row measures compile-and-release because excluding destruction while retaining calibrated output would make memory scale with iteration count. Clean revision `68d107a42f655254f31628f0cbedc55479f6c0f3` passed all 36 representative DEM reports, four rollups, two accepted-maximum memory probes, and one replayed historical schema-version-2 completion manifest; its completion records Stim parity passed and self-regression unseeded because the first AArch64 baseline was seeded afterward. The complete `d424175c6620549086fb4ee358077632bdab97d9` release matrix also remains historical and review-rejected because it predates the authenticated completion and release-boundary repairs.
...
8. Bind every report to the exact runtime and checked-inventory contract and retain setup and peak RSS separately. Preserve a first failed or noisy product result even when no profiler note existed before measurement: publish it immutably with the source-owned group owner, a null profiler-note binding, `promotable=false`, and failure analysis pending. Offline replay under that exact source contract must reproduce it, while parity, rollup, and completion reject it. Binding the required note is a source change and requires fresh evidence instead of relabeling the retained report.
9. Derive report promotion from the evidence: product PR, dirty, unverified-host, or failure-analysis-pending reports may remain valid diagnostics with exact CQ preflight, but only clean verified full or soak reports whose failed or noisy attempts already carry a source-owned profiler note are promotable and eligible for parity or regression dispatch.
...
10. Run milestone audit and GPT-5.6/max full code review before recording completion evidence. Keep any failed or noisy ratio visible: the first report may record pending analysis, but fresh promotable evidence requires a committed profiler note and owner action.
...
10. Apply independent `1.25x` median and bootstrap confidence-interval-upper thresholds to `not-zero` at every scale for all three groups. No aggregate, waiver, favorable-pattern substitution, or row median may hide a failure. A first failed or noisy report without a pre-existing note remains retained and unqualified with pending analysis. The group then gains a source-owned profiler note with the observed cost and next owner action, and any promotable replacement evidence starts from that new clean source identity.
...
3. Bind both groups to the exact source owners `cq2-algebra-pauli-iterator-order-contract`, `cq2-algebra-pauli-iterator-api-contract`, and `cq2-algebra-resource-pauli-materialization` after regenerating their current hashed ids. The strengthened order owner lives in the dedicated Stable target `crates/stab-algebra/tests/cq2_algebra_pauli_iterator.rs`; do not move it back through the facade or broaden its selector.
...
Historical schema-version-31 eleventh-slice AArch64 checkpoint, 2026-07-20: clean review-fix revision `da7c787d1e9f49110d7054868b146b5fb7d7bda4` completed the exact three-case CQ prerequisite, worker reproducibility, both adapter probes, all 12 first-attempt full and soak scale reports, every report and regression replay, four rollups, and two 16-step completion receipts under correctness inventory `4dbbb4b2cda3117bdd3d3ddfcd30b55f09e6f401352e3e86130222189d47791f` and performance inventory `c238dc4e2500192f310ef3d2378ecaafc9744662b5127784dd4eeb6c60726176`. Identity medians ranged from `0.000146x` to `0.014535x`, non-identity medians ranged from `0.743053x` to `0.765340x`, and the worst bootstrap confidence-interval upper bound was `0.765806x`. Follow-up review found that short right operands retained whole-left-tail metadata work; path admission occurred after some mutating work; source, target, Markdown, correctness, and repository state were not all bound through publication; source child sets and staged cleanup were not descriptor-exact; the real completion dispatcher lacked a production-path regression; and checklist counts drifted from generated ownership. Those issues are fixed in the source-current contract, making the `da7c787` chain historical and requiring a fresh clean replacement before source-current closure. The checked timing migration remains narrow, the M12 memory baseline remains guarded for PQ6, and exact historical hashes and replacement status are recorded in `docs/plans/pq2-clifford-string-qualification-progress-report.md`.

Accepted eleventh-slice AArch64 machine checkpoint, 2026-07-22: clean revision `859bf202bdd4bdfbca07e9b1d647afb1b0542846` completed and replayed the exact three-case focused CQ prerequisite, reproduced both private workers, passed both diagnostic adapter probes, published all 12 first-attempt full and soak scale reports, replayed and regression-checked every report, published and replayed four architecture-scoped rollups, and published and independently replayed two 16-step completion receipts under correctness inventory `648e7ea5a66997a810498dc871257bd654c7f9af9304651d43a88103eded0289` and then-current performance inventory `a47866ba5eab70392dd2754391d3d7d8588567a7cbfc1f81a569be813804ce51`. Identity medians range from `0.000179x` to `0.017874x`, non-identity medians range from `0.906236x` to `0.927767x`, and the worst bootstrap confidence-interval upper bound is `0.928804x`. Every scale passed its independent `1.25x` rule without a rerun, waiver, noisy outcome, or threshold relaxation. Revisions `29a29d5f68767e4ab131b051c88f6b77417e0338` and `0b86f07881198c57df1237b23a7d7c0084f2a272` remain historical and review-rejected after exact-case admission, early session binding, all-write staged abort, retained Stim-inventory, and dual session-error defects were found and fixed. Milestone audit and independent review found no unresolved P0 through P3 issue or newly revealed under-specification, so the slice is accepted on controlled Linux AArch64; exact hashes, resource observations, failed no-artifact attempts, and review status are recorded in `docs/plans/pq2-clifford-string-qualification-progress-report.md`. The current DEM inventory does not relabel this chain.
...
13. Bump the private-worker, adapter, contract-preflight, and report schemas when their source or receipt shapes change. Extend the canonical preflight and worker reproducibility matrices with exactly the 72 ordered Clifford invocation receipts defined above; replay must reject an incorrect count and any omitted, reordered, renamed, modified, or cross-worker-transplanted Clifford receipt. The accepted eleventh-slice chain uses private Stab build-receipt schema version 5, adapter receipt schema version 11, contract-preflight schema version 12, and qualification report schema version 31; the twelfth slice supersedes those shared-harness schemas without relabeling the Clifford evidence.
...
5. Use 64, 4,096, and 65,536 compact work items for small, medium, and large. Accept at most 524,288 work items for flat and coordinate families and 131,072 for folded repeats so the six-line folded cycle remains below Stab's public one-million-line DEM parser boundary. Validate monotonic work only within one family.
...
12. Use private Stab build-receipt schema version 7, adapter receipt schema version 12, worker protocol version 5, contract-preflight version 16, runtime-group version 10, report version 35, rollup version 5, completion version 4, and accepted-maximum memory-receipt version 3. Every current runtime receipt binds `raw-work-v2`; current Stab workers also bind the explicit `scalar` build variant instead of inheriting Cargo defaults. Variant-comparison diagnostics bind scalar and `portable-simd` independently.
...
14. Report setup and peak process RSS for all nine scales and the accepted-maximum probes. On the clean formal revision, publish one immutable parse receipt and one immutable print receipt through the existing DEM probe in memory mode; each receipt must contain verified controlled-host evidence, the exact sealed private Stab worker source/build/binary identity, the exact flat-errors, coordinate-sparse, and folded-repeats accepted maximum, input and output identities, and Stim and Stab setup and peak RSS. Preserve inherited M12 memory guards until equal or stronger reviewed qualification evidence replaces them.
...
- The historical DEM R6 scope retains one replayed schema-version-2 architecture/revision completion manifest binding its 36 source reports, four rollups, worker identity, correctness evidence, parity, self-regression, environment, and memory evidence. Current release completion uses schema version 4 with fresh schema-version-3 memory receipts; all earlier completion and memory schemas remain historical.
...
4. Preserve exact `m10-error-decomp` submeasurement identities without aggregating unlike modes; threshold only semantically faithful successful pairs with stable headroom, and keep failed public-`None` calls outside Stim-relative thresholds.
