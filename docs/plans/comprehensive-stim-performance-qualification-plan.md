...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), [pq2-circuit-parse-qualification-progress-report.md](pq2-circuit-parse-qualification-progress-report.md), [pq2-circuit-canonical-print-qualification-progress-report.md](pq2-circuit-canonical-print-qualification-progress-report.md), [pq2-gate-name-hash-qualification-progress-report.md](pq2-gate-name-hash-qualification-progress-report.md), and `benchmarks/stim-qualification-suite.json` at current performance digest `877df12bf1b3d63da92289e22f117097cedbc20860d165c90b41554aa110263b`, bound to correctness digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`. `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, and `PERFQ-M5-SIMD-WORD` are implemented exact runtime contracts with exact `1.25x` targets. Clean revisions `ba70a52025fdd4122ac97cec263725b2ec56e431` and `c76b7071fc4d7ac358ef3a2fffc053ea675bd05f` produced passing AArch64 evidence for the first three groups under preceding inventories; those reports remain historical after the popcount group changed the global inventory and shared workers. Clean popcount evidence is pending. The legacy canonical-print diagnostic is superseded and no longer owns beta, timing-regression, or memory waivers, while the legacy `m5-simd-word` M12 threshold remains active until explicit replacement migration.
...
Status: Active as of 2026-07-16. All 271 selected CQ2 parents have complete exact ownership. Four exact runtime groups are implemented at performance inventory `877df12bf1b3d63da92289e22f117097cedbc20860d165c90b41554aa110263b`. Parser, canonical printing, and gate-name hashing have historical passing controlled-host evidence under preceding inventories. The new `PERFQ-M5-SIMD-WORD` toggle-plus-popcount group requires clean exact CQ, full, and soak execution plus replayed scale-family rollups before it owns a performance claim. The remaining model, format, kernel, and algebra groups are still planned.
...
The fourth runtime contract remains under acceptance correction at performance inventory `877df12bf1b3d63da92289e22f117097cedbc20860d165c90b41554aa110263b`. Initial full-tier reports from clean revision `38a2d5eab2fec3211eb9466899c6afd0ba91c4ca` under superseded performance inventory `9be7dea23a221e9d7fe6ba2e9bdfaf65ad2b4a7675b557386b3de6ea58306557` are diagnostic history only because milestone audit found that the Stim worker called the broader `simd_bits::popcnt()` helper, the output identity omitted three fixture-fingerprint lanes, and final-state work remained inside timing. The corrected source now binds the exact comparator implementation through two source digests and binds every worker preparation through 18 actual probe receipts, but it still requires source review closure, a clean commit, and the complete CQ, reproducibility, probe, full, soak, regression, rollup, audit, and review chain before any popcount ratio is current evidence. The earlier gate-name-hash timing reports remain valid historical evidence under performance inventory `1cc0be5c8c0a37c98bd4fb56f331dd6964e6f53e56b328b9564be507cbf88a42`; they are not relabeled as current after the shared workers and global performance inventory changed.
...
- Verify every normal qualification run and `just bench::qualification-worker-reproducibility` require the canonical worker preflight. The preflight must make both sealed workers confirm source and build identity through the protocol, execute the frozen protocol and fixed odd and even popcount vectors, invoke the first unsupported circuit-parse scale and an 83-item partial gate-table sweep with the start barrier enabled and no input, invoke the below-minimum, unaligned, and over-cap popcount widths with the start barrier enabled and no input, and execute the accepted maximum. The report must retain all 18 actual receipts with accepted row fields or rejected process digests, and the worker identity must bind their recomputed digest. Offline replay must reject omitted, reordered, altered, refingerprinted, or stale receipts. The standalone reproducibility command must additionally require two isolated private builds to produce identical source, build-fingerprint, binary-digest, and preflight identities; a dirty checkout must fail before either private build.
