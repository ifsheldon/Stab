...
Status: Active as of 2026-07-16. All 271 selected CQ2 parents have complete exact ownership. Eight exact runtime groups are implemented at performance inventory `1edd56083186a64b2a1772f01fdd14b835563dd2db4e002673b724b0aec3e4de`. Parser, canonical printing, gate-name hashing, SIMD-word popcount, and complete-vector XOR have historical passing controlled-host evidence under preceding inventories. The three independent `not_zero` groups are implementation-complete, including matching anti-elision boundaries, but still require clean correctness, full, soak, regression, rollup, completion-receipt, audit, and review evidence. The remaining model, format, kernel, and algebra groups are still planned.
...
8. Add `benchmarks/stim_adapter/simd_bits_not_zero_contract.h` to the ordered adapter comparator receipt and bind its digest in every new group. Adapter receipt schema version 6 must reject omissions, reordering, substitutions, or altered bytes. Keep private Stab build-receipt schema version 2 but expand its framed source collection to `worker.rs`, `worker/bits.rs`, and `worker/not_zero.rs`. Contract-preflight schema version 5 must bind 42 actual receipts, and qualification report schema version 21 must preserve and replay that exact preflight.
...
The source-current performance inventory is `1edd56083186a64b2a1772f01fdd14b835563dd2db4e002673b724b0aec3e4de`. It contains all three implemented `not_zero` runtime contracts, matching optimizer-opaque immutable input references, and their independent `1.25x` rules, plus an exact legacy replacement mapping to the early group. This is contract and implementation state, not performance acceptance. No sixth-slice ratio, rollup, or completion receipt is yet promotable, and every first-five timing report remains historical after the global inventory and shared worker identities changed.
