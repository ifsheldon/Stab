...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), the completed PQ2 slice reports, `benchmarks/stim-qualification-suite.json` at current schema-version-2 performance digest `a3be3a7404d6dbc4bcffae0e3dea52e6b93797102dc5b08a776323044efdabcf`, and the checked schema-version-2 `benchmarks/qualification-threshold-migrations.json` authorization ledger, all bound to correctness digest `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`. Nineteen exact product runtime contracts are implemented with one exact `1.25x` measurement rule over each three-scale family, for 57 scale outcomes. Clean revision `d9e2405d18cfff05d9b5d908525394476b0edcbc` completed and replayed the full pre-migration DEM parse and print machine chain; all twelve first attempts passed, parse medians range from `1.102449x` through `1.159075x`, print medians range from `0.559351x` through `0.580445x`, and the worst confidence-interval upper bound is `1.167830x`. Focused migration commit `1cfecd64cde4a5effdf07fdaabdbe51017e25a4a` retired only the exact legacy parse timing threshold and print no-ratio waiver and preserved both memory baselines. Clean post-migration revision `7c3e55301b3f098497613d7dad2d624dc08a4dda` passed all twelve first-attempt reports but remains review-rejected because independent completion replay exposed the preflight evidence-mode defect. Worker protocol version 4, contract-preflight schema version 14, and report schema version 33 bind the correction. Clean post-fix revision `9497df0350cb33dcd249ea12fda802b5a68efe00` completed and replayed the exact focused correctness prerequisite, both probes, worker reproducibility, all twelve first-attempt formal reports and regressions, four rollups, and both completion receipts. Parse medians range from `1.090387x` through `1.145890x`, print medians range from `0.569331x` through `0.589777x`, and the worst confidence-interval upper bound is `1.150114x`. Milestone audit closed one missing accepted-maximum RSS observation and two documentation drifts without changing measured source or finding a product, comparator, threshold, migration, artifact, or specification defect; independent review remains pending. The inventory preserves separate identity and non-identity Clifford contracts, normalized statistics, independent-throughput timing only for Clifford identity, six explicitly planned heterogeneous legacy rows, every checked migration, and all retained memory baselines. Clean Clifford, iterator, Pauli, transpose, and kernel evidence remains accepted only under its recorded inventory and is not relabeled under the DEM inventory.
...
Status: Active as of 2026-07-23. All 271 selected CQ2 parents have complete exact ownership. The source-current correctness inventory is `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`, and the source-current post-migration performance inventory is `a3be3a7404d6dbc4bcffae0e3dea52e6b93797102dc5b08a776323044efdabcf`. Nineteen exact product runtime groups are implemented; the twelfth slice adds independent direct DEM parse and canonical-print groups with fixed scales, exact input and output identities, common-iteration timing, accepted-maximum and hostile-boundary contracts, strict `1.25x` rules, and a source-owned profiler note. Their private Stab build-receipt schema is version 6, adapter receipt schema is version 12, worker-protocol schema is version 4, contract-preflight schema is version 14 with 228 ordered receipts, and qualification report schema is version 33. Earlier stable parse failures remain historical under their exact inventories. Clean pre-migration revision `d9e2405d18cfff05d9b5d908525394476b0edcbc` passed all twelve first-attempt DEM reports, all replays and regressions, four rollups, and both completion receipts; focused migration commit `1cfecd64cde4a5effdf07fdaabdbe51017e25a4a` then retired only the exact legacy timing guards and preserved both memory baselines. Clean post-migration revision `7c3e55301b3f098497613d7dad2d624dc08a4dda` passed both probes, worker reproducibility, focused correctness, all twelve first-attempt scale reports, regressions, and four rollups, but independent parse-completion replay exposed that semantic preflight probes were mislabeled as timing evidence and could reject a legitimate zero clock reading. That chain is retained but review-rejected. Worker protocol version 4 and preflight schema version 14 separate non-statistical contract evidence from formal timing and memory evidence. Clean post-fix revision `9497df0350cb33dcd249ea12fda802b5a68efe00` now supplies the complete source-current AArch64 machine chain: one replayed exact CQ prerequisite, both probes, reproducible workers, twelve first-attempt passing scale reports and regressions, four replayed rollups, and two independently replayed completion receipts. The worst confidence-interval upper bound is `1.150114x`. Milestone audit closed the missing accepted-maximum RSS observations and two documentation drifts without finding a product or evidence-integrity defect; independent review remains pending before slice acceptance. Every earlier Clifford, iterator, Pauli, transpose, kernel, and circuit result remains accepted or rejected only under its recorded inventory. Native x86-64, PQ6 memory growth, and all later runtime groups remain unclaimed.
...
12. Use private Stab build-receipt schema version 6, adapter receipt schema version 12, worker-protocol schema version 4, contract-preflight schema version 14 with 228 ordered receipts, and qualification report schema version 33. Every accepted and rejected contract-preflight receipt must bind `contract`, finite zero elapsed time is valid only in that non-statistical mode, and `timing` and `memory` must remain finite and strictly positive. Add every new Rust and C++ source to the ordered worker or adapter source receipt. Reproducibility and offline replay must reject an omitted, extra, reordered, renamed, modified, stale, or cross-worker-transplanted DEM source or receipt, an incorrect receipt count, any fixture or digest drift, a stale schema, or evidence-mode drift.
