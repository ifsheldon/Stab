...
Status: Active as of 2026-07-23. All 271 selected CQ2 parents have complete exact ownership. The source-current correctness inventory is `648e7ea5a66997a810498dc871257bd654c7f9af9304651d43a88103eded0289`, and the source-current performance inventory is `8094995692c48c98723467cf6e90c3c685797fb4a02bf0efe15c273d844fbfef`. Nineteen exact product runtime groups are implemented; the twelfth slice adds independent direct DEM parse and canonical-print groups with fixed scales, exact input and output identities, common-iteration timing, accepted-maximum and hostile-boundary contracts, and strict `1.25x` rules. Their private Stab build-receipt schema is version 6, adapter receipt schema is version 12, contract-preflight schema is version 13 with 228 ordered receipts, and qualification report schema is version 32. The DEM pair has no clean performance evidence or migration authorization yet. Every earlier Clifford, iterator, Pauli, transpose, kernel, and circuit result remains accepted or rejected only under its recorded inventory; the new shared-worker and inventory state does not relabel it as current evidence. Native x86-64, PQ6 memory growth, and all later runtime groups remain unclaimed.
