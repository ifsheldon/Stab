...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), the completed PQ2 slice reports, `benchmarks/stim-qualification-suite.json` at current schema-version-2 performance digest `d27b71d205ec938a77c9981fb524e70c9b17eeafeadf3ade6367632aeb3b1693`, and the checked `benchmarks/qualification-threshold-migrations.json` authorization ledger, all bound to correctness digest `e5635ca4a2c217ee79768d35336c50611a9f12f86edded8a7f01fce1ca72add4`. Nineteen exact product runtime contracts are implemented with one exact `1.25x` measurement rule over each three-scale family, for 57 scale outcomes. The two newest contracts independently bind direct DEM parsing and canonical printing to one frozen fixture family, exact output identities, an exact CQ2 parent, separately hashed comparator sources, and a source-owned profiler note; they have no passing second-optimization timing evidence or migration authorization yet. The inventory preserves separate identity and non-identity Clifford contracts, normalized statistics, independent-throughput timing only for Clifford identity, six explicitly planned heterogeneous legacy rows, every checked migration, and all retained memory baselines. Clean Clifford, iterator, Pauli, transpose, and kernel evidence remains accepted only under its recorded inventory and is not relabeled under the DEM inventory.
...
Status: Active as of 2026-07-23. All 271 selected CQ2 parents have complete exact ownership. The source-current correctness inventory is `e5635ca4a2c217ee79768d35336c50611a9f12f86edded8a7f01fce1ca72add4`, and the source-current performance inventory is `d27b71d205ec938a77c9981fb524e70c9b17eeafeadf3ade6367632aeb3b1693`. Nineteen exact product runtime groups are implemented; the twelfth slice adds independent direct DEM parse and canonical-print groups with fixed scales, exact input and output identities, common-iteration timing, accepted-maximum and hostile-boundary contracts, strict `1.25x` rules, and a source-owned profiler note. Their private Stab build-receipt schema is version 6, adapter receipt schema is version 12, contract-preflight schema is version 13 with 228 ordered receipts, and qualification report schema is version 32. The first faithful pre-optimization parse/small/full report is retained as a stable `1.773450x` failure, and the first optimized parser report is retained under its preceding inventory as a stable `1.372971x` failure. Clean implementation commit `e9b94d524167b2ca7bb953e525abc26e77641629` contains the second optimization, but it has no formal timing evidence or migration authorization yet. Every earlier Clifford, iterator, Pauli, transpose, kernel, and circuit result remains accepted or rejected only under its recorded inventory; the new shared-worker and inventory state does not relabel it as current evidence. Native x86-64, PQ6 memory growth, and all later runtime groups remain unclaimed.
