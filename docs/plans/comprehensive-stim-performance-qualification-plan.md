...
- Use the larger independently selected iteration count as the identical-work common batch. Standard mode requires both common-validation durations between 250 milliseconds and 2 seconds. When a genuine ratio has no standard overlap, wide-ratio mode may permit only the implementation that selected fewer independent iterations to exceed 2 seconds; the implementation that owns the common iteration count must remain at or below 2 seconds, both sides must remain at least 250 milliseconds, and neither may exceed the hard 20-second common ceiling under the 30-second invocation timeout. The 20-second bound covers the approximately 40x ratio implied by a 6-percent early-hit short circuit combined with the observed full-scan implementation advantage, while retaining 10 seconds of per-invocation timeout headroom. Derive and offline-replay the mode from raw calibration decisions and common receipts instead of accepting a caller or report classification.
...
- Publish completion `report.json`, `preflight.json`, and derived `report.md` atomically only after compare-and-swap checks prove that every bound source `report.json`, `preflight.json`, and `report.md` remains unchanged. Completion receipt schema version 1 and preflight schema version 1 bind the complete typed step sequence and all source directory artifact digests.
...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), the completed PQ2 slice reports, and `benchmarks/stim-qualification-suite.json` at current performance digest `868eb831a034042b43573fed612af14db225421a2733bbf10e4a5eb2b515ec90`, bound to correctness digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`. Fifteen exact product runtime contracts are implemented with 21 independent exact `1.25x` targets. The refresh from post-migration digest `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` changes only the synchronized CQ2 checklist anchor and leaves every runtime contract unchanged, but its timing artifacts remain historical until exact groups rerun under the current digest. Clean reviewed iterator evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632` remains accepted historical AArch64 evidence with 12 first-attempt passing reports and regressions, four replayed rollups, and two replayed completion receipts after exact accepted-maximum strengthening and clean final review. The memory baseline remains guarded for PQ6. Earlier completed groups remain historical under their recorded inventories.
...
- Unit-test calibration lower and upper bounds, zero-duration handling, overflow, timeouts, maximum iterations, standard common batches, valid wide-ratio batches in both implementation directions, and floor, cap, equal-iteration, wrong-owner, and both-over-standard rejection.
...
Status: Active as of 2026-07-18. All 271 selected CQ2 parents have complete exact ownership. Clean correctness revision `6351fe0a529462efd9f18a88bd59c08cfec9f81b` provides the latest complete historical PR, full, and soak execution, while controller hardening at `763b04055f969668a4152e019ed62a3555cd0476` requires a fresh source-current chain before new promotable timing. Fifteen exact product runtime groups are implemented at performance inventory `868eb831a034042b43573fed612af14db225421a2733bbf10e4a5eb2b515ec90`, including exact public in-place Pauli-string multiplication and the two split Pauli-string iterator workload shapes. The synchronized CQ2 checklist-status refresh changes no executable contract or threshold, but it ages every report under preceding inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf`. The ninth and tenth slices therefore remain accepted historical Linux AArch64 evidence at their recorded revisions and inventories. Native x86-64, PQ6 memory growth, current-inventory timing evidence, and all later runtime groups remain unclaimed.
...
4. Freeze the identity fixture as equal-width left and right identity strings. Freeze the non-identity fixture from the canonical 24-gate order: position `i` uses left gate `all[i % 24]` and right gate `all[1 + ((i / 24) % 23)]`, so each complete 552-position cycle covers every left Clifford against every non-identity right Clifford exactly once. Repeat the cycle without RNG state, and require the first 552 positions to remain a complete cross-product in every timing scale.
...
6. Encode each canonical input descriptor as eight little-endian `u64` fields: width, workload marker, fixture schema, canonical gate count, right-cycle count, complete cross-product span, public Clifford-qubit cap, and reserved zero. Give identity and non-identity distinct workload markers and right-cycle counts, freeze all six descriptor byte lengths and digests, and reject any nonzero reserved field.
7. Construct both equal-width operands from the canonical descriptor before the start barrier. Each timed callback must call public Rust `CliffordString::right_multiply_in_place` or pinned C++ `CliffordString<MAX_BITWORD_WIDTH>::operator*=`, consume the mutated left state through an optimizer barrier, and leave the right operand unchanged. Do not reconstruct operands inside timing, substitute allocating multiplication, call a private kernel directly, or replace the logical product with a checksum-only loop.
8. Derive the semantic output outside timing from exactly fourteen little-endian `u64` fields: iteration count, checked work, width, workload marker, observed left and right non-identity counts, four canonical final-left gate-sequence digest lanes, and four unchanged-right gate-sequence digest lanes. Require exact Stim/Stab equality and freeze odd and even iteration vectors for both workloads because repeated Clifford composition can cycle back to identity.
9. Validate the final gate-sequence digest with an untimed scalar reference that composes the canonical 24-by-24 table independently of the production string implementation. For the identity workload, verify every position and both non-identity counts remain zero. For the non-identity workload, verify the complete first 552-position cross-product, the scale tail, the final left state, and right immutability.
10. Reject zero width, width above 1,048,576, unknown workload marker, wrong measurement id, malformed cycle metadata, nonzero reserved input, checked semantic-work overflow, and output-field overflow before allocation and before consuming an enabled start barrier. Freeze accepted odd, accepted even, all six scales, both accepted maxima, first-over-cap, wrong-measurement, malformed-cycle, work-overflow, and reserved-field receipts for both workers.
...
13. Bump the private-worker, adapter, contract-preflight, and report schemas when their source or receipt shapes change. Extend the canonical preflight and worker reproducibility matrices with both workload vectors, accepted maxima, first rejection, malformed input, wrong measurement, and overflow classes; replay must reject omitted, reordered, renamed, modified, or cross-worker-transplanted Clifford receipts.
...
15. Apply independent `1.25x` median and bootstrap-upper-bound thresholds at every scale with no waiver path. Preserve the first faithful non-identity diagnostic even if it fails badly. Profile that exact public lifecycle before optimization; if scalar per-gate composition is the bottleneck, prefer a private six-bit-plane Clifford composition kernel with scalar reference tests and isolated `std::simd` acceleration over architecture-specific intrinsics or an identity-only shortcut. Any storage redesign must preserve every public behavior and restart affected evidence from a clean revision.
...
- Verify completion publication rejects dirty or changed repositories, missing or duplicate scales, colliding or nondirect paths, mixed report workers, stale probe identity, nonpromotable reports, report-only or incomplete regression results, failed or mixed-identity rollups, non-idempotent report or rollup replay, altered artifact byte identities, nonzero step status, malformed step order, stale preflight, noncanonical JSON, and byte-identical source-directory or replay-target inode replacement during publication. Verify the real completion publication path makes replacement durable before treating bounded cleanup of the old tree as best effort.
