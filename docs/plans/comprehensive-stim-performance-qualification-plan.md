...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), [pq2-circuit-parse-qualification-progress-report.md](pq2-circuit-parse-qualification-progress-report.md), [pq2-circuit-canonical-print-qualification-progress-report.md](pq2-circuit-canonical-print-qualification-progress-report.md), [pq2-gate-name-hash-qualification-progress-report.md](pq2-gate-name-hash-qualification-progress-report.md), [pq2-simd-word-popcount-qualification-progress-report.md](pq2-simd-word-popcount-qualification-progress-report.md), [pq2-simd-bits-xor-qualification-progress-report.md](pq2-simd-bits-xor-qualification-progress-report.md), [pq2-simd-bits-not-zero-qualification-progress-report.md](pq2-simd-bits-not-zero-qualification-progress-report.md), and `benchmarks/stim-qualification-suite.json` at current performance digest `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0`, bound to correctness digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`. Eight exact runtime contracts are implemented with exact `1.25x` targets: the first five historical groups plus independent early-hit, all-zero, and late-hit `not_zero` groups. The `not_zero` contract binds optimizer-opaque immutable input references on both workers, the 2-second independent ceiling, and the 20-second wide-ratio common ceiling. Clean revision `817d0fe870fd1b02c8e30f18e534e35df705a1ee` passed all 18 full and soak measurements at historical evidence inventory `c2362b6fda35626e5f571399512067ab406f92a22692c162ae70002ef7a651f8`, with median ratios from `0.030337x` to `0.663540x`, six replayed rollups, and three replayed completion receipts. Independent review requires the same chain to be regenerated at the source-current post-migration inventory before closure. The duplicate legacy M12 `not_zero` pair is retired in source, while the XOR pair and `m5-simd-word` guard remain active pending their own migration evidence. The legacy canonical-print diagnostic remains superseded and no longer owns beta, timing-regression, or memory waivers.
...
Status: Active as of 2026-07-16. All 271 selected CQ2 parents have complete exact ownership. Eight exact runtime groups are implemented at performance inventory `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0`. Parser, canonical printing, gate-name hashing, SIMD-word popcount, and complete-vector XOR have historical passing controlled-host evidence under preceding inventories. The three independent `not_zero` groups have a complete historical chain at the immediately preceding inventory. Independent GPT-5.6/max review confirmed the implementation but requires accepted-maximum allocation coverage and a complete post-migration evidence rerun; the allocation test is fixed and the current-inventory rerun remains the sixth-slice blocker. The remaining model, format, kernel, and algebra groups are still planned.
...
11. Add an exact replacement mapping from legacy pair `simd_bits_not_zero_100K` / `stab_simd_bits_not_zero_10K` only to the early group. Keep the legacy M12 submeasurement threshold active until clean completion evidence and migration review explicitly retire it. The clean early-hit completion receipt at revision `817d0fe870fd1b02c8e30f18e534e35df705a1ee` satisfied that migration gate, so the source-current M12 file retires only the `not_zero` pair and removes its stale replacement marker. The all-zero and late groups remain additional regression protection, not alternative targets for the legacy pair.
...
The historical sixth-slice evidence inventory is `c2362b6fda35626e5f571399512067ab406f92a22692c162ae70002ef7a651f8`. Clean revision `817d0fe870fd1b02c8e30f18e534e35df705a1ee` binds both exact CQ prerequisites, reproducible sealed workers, 42 actual contract-preflight receipts per report, 18 passing first-attempt full and soak measurements, 18 successful report and regression replays, six replayed rollups, and three replayed completion receipts on controlled Linux AArch64. Median ratios range from `0.030337x` to `0.663540x`; the worst confidence-interval upper bounds are `0.071537x` for early-hit, `0.663810x` for all-zero, and `0.663639x` for late-hit. The source-current performance inventory is `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0` after removal of the duplicate legacy M12 `not_zero` pair. Because report and completion replay reject inventory drift, rerun the entire sixth-slice chain from one clean post-migration commit before promoting current closure. Native Linux x86-64, PQ6 memory growth, and every remaining PQ2 group remain unclaimed.
