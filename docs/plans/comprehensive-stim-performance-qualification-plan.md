...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), the completed PQ2 slice reports, `benchmarks/stim-qualification-suite.json` at current schema-version-2 performance digest `a98f57cf194f3a021d321266656cf688c9f7780fb39fa337475e8132411eb88a`, and the checked `benchmarks/qualification-threshold-migrations.json` authorization ledger, all bound to correctness digest `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`. Nineteen exact product runtime contracts are implemented with one exact `1.25x` measurement rule over each three-scale family, for 57 scale outcomes. The two newest contracts independently bind direct DEM parsing and canonical printing to one frozen fixture family, exact output identities, an exact CQ2 parent, separately hashed pinned-Stim comparator sources, and a source-owned profiler note. The clean Git revision, private Cargo build, sealed Stab binary digest, and worker identity bind the private parser implementation. Parse has passing second-optimization full evidence at small and medium scale but a retained stable large-scale failure, while print has no formal timing evidence; neither contract has migration authorization. The inventory preserves separate identity and non-identity Clifford contracts, normalized statistics, independent-throughput timing only for Clifford identity, six explicitly planned heterogeneous legacy rows, every checked migration, and all retained memory baselines. Clean Clifford, iterator, Pauli, transpose, and kernel evidence remains accepted only under its recorded inventory and is not relabeled under the DEM inventory.
...
Status: Active as of 2026-07-23. All 271 selected CQ2 parents have complete exact ownership. The source-current correctness inventory is `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`, and the source-current performance inventory is `a98f57cf194f3a021d321266656cf688c9f7780fb39fa337475e8132411eb88a`. Nineteen exact product runtime groups are implemented; the twelfth slice adds independent direct DEM parse and canonical-print groups with fixed scales, exact input and output identities, common-iteration timing, accepted-maximum and hostile-boundary contracts, strict `1.25x` rules, and a source-owned profiler note. Their private Stab build-receipt schema is version 6, adapter receipt schema is version 12, contract-preflight schema is version 13 with 228 ordered receipts, and qualification report schema is version 32. Earlier parse/small/full reports retain stable `1.773450x` and `1.372971x` failures under their exact inventories. Revision `ca9fd68d3856e9cac9cc6da16433947d056e8848` passed small and medium but retained a stable large-scale failure at median `1.248221x` and confidence-interval upper bound `1.263920x`. Clean implementation commit `430428ea93a40af25a352746acc2bc517e7ad1fd` contains the third optimization but has no formal timing evidence or migration authorization yet. Every earlier Clifford, iterator, Pauli, transpose, kernel, and circuit result remains accepted or rejected only under its recorded inventory; the new shared-worker and inventory state does not relabel it as current evidence. Native x86-64, PQ6 memory growth, and all later runtime groups remain unclaimed.
