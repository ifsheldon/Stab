...
11. Add exact replacement mappings from `SparseXorTable_SmallRowXor_1000` / `stab_sparse_table_row_xor_1000` to `PERFQ-M5-SPARSE-XOR/row-xor` and from `SparseXorVec_XorItem` / `stab_sparse_xor_item_7` to `PERFQ-M5-SPARSE-XOR-ITEM/xor-item`. Keep both legacy M12 submeasurement thresholds active through the first clean completion receipts and migration review. After both receipts pass, retire exactly those legacy timing pairs and their replacement markers in one focused migration commit, then regenerate and replay the complete seventh-slice chain at the post-migration inventory before claiming source-current closure. Keep the M12 memory baseline until PQ6 supplies equal or stronger memory evidence.
