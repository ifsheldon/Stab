...
Evidence: [pq0-performance-disposition-progress-report.md](pq0-performance-disposition-progress-report.md), [pq2-circuit-parse-qualification-progress-report.md](pq2-circuit-parse-qualification-progress-report.md), [pq2-circuit-canonical-print-qualification-progress-report.md](pq2-circuit-canonical-print-qualification-progress-report.md), [pq2-gate-name-hash-qualification-progress-report.md](pq2-gate-name-hash-qualification-progress-report.md), [pq2-simd-word-popcount-qualification-progress-report.md](pq2-simd-word-popcount-qualification-progress-report.md), [pq2-simd-bits-xor-qualification-progress-report.md](pq2-simd-bits-xor-qualification-progress-report.md), [pq2-simd-bits-not-zero-qualification-progress-report.md](pq2-simd-bits-not-zero-qualification-progress-report.md), [pq2-sparse-xor-qualification-progress-report.md](pq2-sparse-xor-qualification-progress-report.md), [pq2-bit-matrix-transpose-qualification-progress-report.md](pq2-bit-matrix-transpose-qualification-progress-report.md), and `benchmarks/stim-qualification-suite.json` at current performance digest `1d38c155acbaf78234f9b92857cfef8c25ffa059a4a9e9756b272a72272dfd0d`, bound to correctness digest `5d795e831bc20b3f2780ca72c1eaea7c75387388d38f8e37f4539254a41e821b`. Twelve exact runtime contracts are implemented with independent exact `1.25x` targets, including allocating and in-place BitMatrix transpose. Clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` passed the complete first-stage transpose chain and authorized superseding only the heterogeneous `m5-simd-bit-table` timing threshold; the M12 memory baseline remains. Clean post-migration revision `1264d885087761b19b37beded47811cc0c117e4d` also passed the complete chain, but independent review then strengthened the exact edge oracle and pre-setup semantic-work overflow contract. Clean reviewed revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` regenerated and replayed the exact two-case correctness preflight, worker reproducibility, both probes, all 12 first-attempt reports and regressions, four method-specific rollups, and both method-specific completion receipts at the source-current inventory. Clean revisions `60b732c77f1828058fbd65ec6c5c4ad582467fd1` and `7b43b46d1c08f669264d009b8d3872ce86838f0e` passed their `not_zero` and sparse-XOR evidence chains under the recorded historical inventories. Completed evidence retired the duplicate legacy M12 `not_zero` and sparse-XOR timing pairs while retaining the sparse-XOR memory baseline, the XOR timing pair, and the `m5-simd-word` guard pending their own migration evidence. The legacy canonical-print diagnostic remains superseded and no longer owns beta, timing-regression, or memory waivers.
...
Status: Active as of 2026-07-17. All 271 selected CQ2 parents have complete exact ownership. Twelve exact runtime groups are implemented at performance inventory `1d38c155acbaf78234f9b92857cfef8c25ffa059a4a9e9756b272a72272dfd0d`, including independent allocating and in-place BitMatrix transpose groups. Parser, canonical printing, gate-name hashing, SIMD-word popcount, complete-vector XOR, the three independent `not_zero` groups, and the independent sparse row-XOR and item-toggle groups have historical passing controlled-host evidence under their recorded inventories. The reviewed allocating and in-place transpose groups have complete passing source-current Linux AArch64 evidence at clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0`; native x86-64 and PQ6 memory growth remain unclaimed. The active ninth slice is exact public in-place Pauli-string multiplication with deterministic non-identity operands. Remaining model, format, kernel, and algebra groups are still planned.
...
5. Define one deterministic dense non-identity fixture named `pauli-right-multiply-splitmix64-v1`. At logical qubit `q`, derive the left basis from the low two bits of SplitMix64 applied to `0x243f6a8885a308d3 + q * 0x9e3779b97f4a7c15` and derive the right basis from the low two bits of SplitMix64 applied to `0x13198a2e03707344 + q * 0xbf58476d1ce4e5b9`, with wrapping `u64` arithmetic. Use a positive left sign and negative right sign. Encode four little-endian `u64` header fields for width, generator marker, left sign, and right sign, followed by complete logical `left_x`, `left_z`, `right_x`, and `right_z` word planes with canonical tail masking. The exact input byte counts are 5,056, 50,048, and 500,032 for the runtime scales and 524,320 at the accepted maximum. Freeze independently recomputed input and odd/even output digests before inventory graduation.
...
14. Keep the legacy `m6-pauli-string` row-level `1.25x` timing threshold active through the first clean completion receipt. The old identity-only callback and Stab early return are not the new non-identity comparison, so do not cite the legacy ratio as authorization. Once the pre-migration receipt passes and replays with all three scale-aware source mappings, retire only the legacy timing threshold and mapping in one focused migration commit. Preserve `benchmarks/m12-primary-memory-baseline.json` until PQ6 supplies equal or stronger memory evidence.
