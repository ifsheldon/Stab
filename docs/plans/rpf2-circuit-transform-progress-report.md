...
- `just bench::compare --only pf2-time-reverse-flow-measurement --baseline target/benchmarks/pf2-time-reverse-measure-reset-probe/baseline.json --report target/benchmarks/pf2-time-reverse-measure-reset-compare` measured `stab_circuit_time_reversed_for_flows_measurement` at `0.000010198s`, about `1.275e6 flows/s`.
...
- `just bench::baseline --only pf2-time-reverse-flow-measurement --out target/benchmarks/pf2-time-reverse-measure-reset-probe`
- `just bench::compare --only pf2-time-reverse-flow-measurement --baseline target/benchmarks/pf2-time-reverse-measure-reset-probe/baseline.json --report target/benchmarks/pf2-time-reverse-measure-reset-compare`
...
The current selected measure-reset slice promotes one noiseless plain single-target `MR`, `MRX`, or `MRY` instruction in the same API, adds `time_reversed_for_flows_measurement_rich_subset_reverses_measure_resets`, refreshes the report-only benchmark corpus, and keeps reset-only operations, inverted or multi-target measure-reset groups, detectors, feedback, noise, repeats, multi-instruction circuits, and larger QEC inverse behavior open.
