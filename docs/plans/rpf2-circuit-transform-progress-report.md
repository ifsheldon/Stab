...
Milestone-audit for the selected noisy measurement-only inverse-QEC slice found the promoted scope complete against `pfm2-inverse-qec-noisy-measurement-scope.md`: the selector covers pinned `noisy_m` top-level `M`/`MX`/`MY` measurement-only reversal, duplicate target reversal, probability args, tags, inverted measurement targets, and empty measurement omission, while noisy measure-reset, pair measurements, MPP, detectors, observables, feedback, repeats, and broader detector-flow rewrites remain fail-closed or active follow-up work.
