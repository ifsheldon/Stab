...
- Keep broader Stim `circuit_inverse_qec` cases such as `two_to_one`, `pass_through`, broader noisy measure-reset detector-flow beyond the exact `noisy_mr_det` packet, `m_det`, `mzz`, `mpp`, and `obs_include_pauli` active for later PFM2 or PFM5 slices.
