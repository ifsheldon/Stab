...
PFM-B2 implementation and executable oracle evidence are complete at core commit `f60ea17` and oracle commit `e7a67a0`; clean committed-HEAD benchmark evidence, milestone audit, GPT-5.6/max full-code-review, and final documentation rollup remain in progress.
...
This report must not be treated as the final completion report until the remaining process gates close.
...
| `pfm3-contract-mpp-deterministic` | Hermitian single-term, multi-term, repeated-qubit, inverted, and empty MPP groups | Exact records, conversion, detector events, analyzer DEM, and flows | Exact |
...
Ledger validation must calculate the discrete two-sided binomial rejection probability at the resulting integer boundaries and reject a plan whose union bound exceeds the declared familywise budget.
...
- Statistical tests materialize at most their declared 100,000 small records and use fixed-size bucket counters.
...
The existing `pf3-gate-semantic-wide` representative row remains report-only for fixed-tableau, measurement, MPP, MPAD, SPP, noise, annotation, classical-control, and repeat cases.
`pf3-analyze-errors-sweep` remains the allocation and maximum-sweep-ID evidence row.
PFM-B2 fixed production defects in controlled-Pauli dispatch, heralded detection-reference handling, empty-target analyzer no-ops, and anti-Hermitian analyzer validation, but no production path consults the semantic contract table.
...
- [ ] The low and maximum sweep-ID allocation evidence remains clean.
- [ ] Documentation agrees on active and deferred scope.
- [ ] Milestone-audit findings are fixed or genuine under-specification is logged.
