...
- Publish one schema-version-2 accepted-maximum memory receipt for DEM parse and one for DEM print from the clean measured revision. Each receipt must pass the controlled-host policy, bind `raw-work-v2`, the repository revision, both worker source and build identities, and all three family maxima, and be retained and revalidated by creation, replay, checkpoint publication, and checked status.
...
- The remote annotated `v0.2.0` tag resolves to the release commit; the private draft contains exactly the two validated binaries, checksum sidecars, and provenance manifests with matching GitHub-recorded digests; a human publishes that reviewed draft.
