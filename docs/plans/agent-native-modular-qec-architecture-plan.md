...
- Treat every scratch-release rehearsal as immutable historical evidence. The temporary repository, workflow, binary, ruleset target, and commands are retired; no new scratch rehearsal is required. The production operator remains non-redirectable, retains its token-free hidden Cargo-sandbox re-exec boundary, and requires authenticated A9 completion before reading release credentials.
- Require exact-head `Rust` and `Qualification Contracts` CI after the entropy program and before formal evidence, then bind every correctness, performance, package, and production-release identity to the final clean revision or its single permitted status descendant.
