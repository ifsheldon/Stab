...
- Compare each report-only source-current median with the latest clean report for the same row, complete ordered measurement set, workload observations, implementation and executable digest, private host fingerprint and CPU identity, build profile, scalar feature selection, measurement-contract digest, timing boundary, and normalized row-native iteration count. Historical schema-version-1 through schema-version-3 reports lack enough identity to serve as predecessors. Regenerate each required predecessor on the same host before the current matrix from a clean instrumentation backport revision that contains the historical product source plus only the reviewed schema-version-4 evidence harness; each predecessor is warmed, strict, create-new, and records exactly one outer run. Preserve each backport commit through a source-owned branch or tag so ledger validation can resolve it as a Git commit, preserve its report as historical evidence, and do not merge its product tree into current source. A deterioration greater than 15% requires a focused diagnostic before acceptance. A focused diagnostic isolates the exact source-owned row, completes any exact executable preflight required by the checked contract, performs a warmup, and records exactly one outer comparison run. The row retains its source-owned release-profile internal timing count, which must be at least eight and must be recorded in the evidence ledger; changing that count solely to obtain a preferred result is prohibited. Repeating outer runs or aggregating independent outer runs is not a substitute for profiling. If the focused result remains greater than `1.15x` its clean predecessor and host policy permits hardware profiling, capture a hardware or CPU profile and fix or record the unavailable restriction. The machine outcome is derived from measurements and profile availability as `resolved-within-boundary`, `reproduced-profiled`, or `reproduced-profile-unavailable`; prose is only an owner action and cannot relabel the measured outcome.
- Publish one A6 focused-evidence ledger after the final clean run through `just bench::a6-focused-evidence --publish-from <request>`. The request may select only matrix, predecessor, focused-report, and optional profile artifact paths, predecessor phase assignments, profile availability details, and owner actions; the publisher derives every digest, median, ratio, crossing, timing count, initial seed, and typed outcome and validates the checked executable-preflight contract. It validates all bound artifacts, rechecks the unchanged clean revision, and atomically creates `benchmarks/a6-focused-evidence.json` without replacement. Each ledger entry binds the source revision, fresh baseline and matrix, selected row and measurements, measurement-contract identity, focused and predecessor reports, exact internal timing count, normalized ratios, profile availability, typed outcome, and owner action. Normal validation always reopens and hashes every artifact; the former optional verification mode is not an acceptance path. Structural validation must reject missing crossings, extra or duplicate entries, reused mutable artifacts, stale source revisions, policy drift, invalid digests, timing counts below eight, and ratios inconsistent with the bound reports.
