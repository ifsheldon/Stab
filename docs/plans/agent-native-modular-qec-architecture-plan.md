...
The current architecture-migration checkpoint is recorded in [the append-only progress report](agent-native-modular-qec-progress-report.md) so this normative contract does not acquire a stale source hash after every focused extraction commit.
...
- The external review correctly warned against turning every API or artifact into a separate qualification product. A6 therefore keeps exact executable preflights for its selected M5 and M6 comparisons and direct owner-package evidence for extracted behavior, while A9 retains the broader release qualification matrix.
- An implementation audit found that the first A6 closure procedure nevertheless grew into a retrospective attestation program: it required a complete unrelated 166-row matrix, historical instrumentation backports, mandatory profiles, and a focused-evidence publication object. That procedure also could not prove that an externally supplied `perf.data` came from the row it certified. The revised A6 contract removes that ceremony, keeps profiles optional and diagnostic, and requires source-current evidence only for workloads whose execution or SIMD selection A6 actually changes.
...
- `cargo test -p stab-model --test model_parse_diagnostics opaque_comments_do_not_shift_later_parser_diagnostics -- --exact` owns the exact post-comment circuit and DEM error spans, and `cargo test -p stab-model --test model_parse_diagnostics opaque_unterminated_tags_report_the_exact_original_eof -- --exact` owns controlled EOF spans for opaque circuit and DEM tags.
...
```text
cargo test -p stab-analysis --test circuit_flatten_limits policy_preserves_defaults_and_rejects_before_output_allocation -- --exact
```
...
- Canonical-package ownership checks requiring every implemented API parent and owned oracle fixture whose behavior belongs wholly to `stab-bits`, `stab-model`, `stab-records`, `stab-algebra`, `stab-analysis`, or `stab-engine` to execute its Stable owner package directly. A source-owned exception ledger may retain only genuine cross-component, circuit, CLI, or facade-integration cases under `stab-core`; each exception must name the integration behavior that prevents direct owner-package execution.
...
- Re-run diagnostic Stim-relative continuity for the selected default scalar implementation after the SIMD diagnostics without changing the `1.25x` numeric boundary. Use the current paired qualification contracts `PERFQ-M5-SIMD-BITS` / `xor-complete-vector` and `PERFQ-M6-CLIFFORD-STRING` / `right-multiply-identity`, which own paired medians and deterministic confidence intervals. Run the `small` scale for each as the focused A6 continuity sample. A successful diagnostic requires exact semantic preflight, a non-noisy authoritative sample, median ratio at most `1.25`, and confidence-interval upper bound at most `1.25`. Host-unverified evidence remains diagnostic and cannot be promoted; A9 owns controlled-host full and soak parity evidence across the complete scale families.
- Keep the legacy `m5-simd-bits` and `m6-clifford-string` rows available only for historical or optional row-native continuity. The completed M6 migration is not reopened, and the legacy compare harness is not cited for a confidence-bound conclusion. The M6 short-right-operand measurements remain report-only Stab diagnostics because Stim has no equivalent workload; the separate scalar-versus-SIMD report owns non-identity Clifford kernel selection.
...
- On the final reviewed clean A6 source revision, produce one full-tier diagnostic qualification report for the `small` scale of each selected M5 and M6 group. Use create-new output paths, current correctness prerequisites, the scalar feature set, `raw-work-v2`, and unchanged parity policy. Record host validity separately from the numeric outcome. A report from this host may close A6's diagnostic continuity criterion when its semantic, noise, median, and upper-bound checks pass, but it is not promotable release evidence.
- Run benchmark smoke and exactly the A2, A4, and A5 phase diagnostics frozen in [the A6 extraction map](../architecture/a6-component-extraction-map.md#extraction-continuity). These reports are diagnostic continuity only unless an existing source-owned parity or self-regression policy already applies to the exact measurement. Changing that finite set requires a recorded milestone-specification amendment before timing.
- A profile may be captured to investigate an unexpected result, but profiling is optional diagnostic evidence and cannot relabel a failed timing gate. A6 does not accept an externally supplied profile as proof of workload provenance.
- Preserve the complete historical A6 matrices, focused reports, predecessor design, and failed publication attempts as historical diagnostics. They are not current closure requirements and are not deleted or rewritten.
...
- The selected M5 and M6 scalar Stim diagnostics are source-current, use the exact current qualification groups and measurements above, and satisfy the unchanged `1.25x` median and confidence-upper-bound numeric criteria. The closure record states whether host policy was verified and does not promote host-unverified results. Legacy row-native comparisons, M6 short-right-operand rows, and the scalar-versus-SIMD non-identity kernel report remain distinct diagnostics and are not substituted for the paired qualification measurements.
- Every A2, A4, or A5 diagnostic whose executed path changed passes its existing semantic witness and remains classified according to its source-owned comparator policy; unrelated manifest rows do not become A6 blockers.
- Canonical compatibility and resource suites execute `stab-bits`, `stab-model`, `stab-records`, `stab-algebra`, `stab-analysis`, and `stab-engine` directly; `stab-core` owns only checked deliberate cross-component and facade integration.
...
- Before formal correctness or timing evidence, execute the complete WS4 draft-stage rehearsal from the exact clean source revision in fixed public scratch repository `ifsheldon/Stab-release-rehearsal` (numeric ID `1342241032`) under active no-bypass tag ruleset `21169813`. Mirror the exact source commit without rewriting scratch history, create and push the commit-derived annotated `v0.2.0-rehearsal-<commit>` tag, dispatch only `.github/workflows/release-rehearsal.yml` from that tag, require one exact-SHA workflow run, download and verify all six assets, and run the read-only live-draft verifier. Record the run, tag, identities, and digests before timing begins; a source fix revealed by rehearsal restarts this gate before any new evidence.
- Permit one rehearsal-record-only descendant after a successful live run so checked documentation can name its external identities without a dirty formal-evidence tree. The complete committed diff from the exercised revision may touch only `docs/plans/agent-native-modular-qec-progress-report.md`, `docs/plans/post-review-remediation-progress-report.md`, and `docs/plans/GOAL.md`; it must contain no product, test, workflow, release-tool, qualification-contract, fixture, policy, or normative procedure change. That descendant must pass exact-revision CI before formal evidence. Any broader or substantive change invalidates the rehearsal and requires a new source-derived tag and run.
- Keep production and rehearsal as separate fixed-destination binaries and workflows. The rehearsal binary may build and verify assets, create one private scratch draft, and verify that draft; it must expose no registry publication, production-tag, public-release, or draft-to-publication operation. Both binaries must implement the token-free hidden Cargo-sandbox re-exec boundary used by artifact construction and authorization, with a process-level test that reaches that boundary. Rehearsal authorization runs architecture and ordinary checked-status validation before reading `GITHUB_TOKEN` but deliberately does not require a release-completion checkpoint, so release-path defects are found before the expensive formal evidence program. The production operator remains non-redirectable and retains its authenticated A9 completion requirement.
- Regenerate from one clean commit.
- Before formal timing, reproduce both sealed workers and run the complete `pq2-dem-parse-adapter-smoke` and `pq2-dem-canonical-print-adapter-smoke` probes from that exact clean revision. Dirty-tree or earlier-revision probes remain diagnostic.
- Record every attempted formal output path, failure class, materialization state, and any replacement path before continuing. This ledger includes host and controller rejection before artifact publication, and an attempted path is never reusable merely because no directory materialized.
- Bind every correctness and performance artifact to one clean measured source commit. After evidence and post-evidence review pass without a source fix, replay the schema-version-4 completion and publish its byte-identical canonical source copy with `just qualification::completion-checkpoint --input <completion-directory>`. Creation, offline replay, and checkpoint replay of the release scope must each run with and authenticate soft `RLIMIT_NOFILE=1024`; the replay object must retain every report, rollup, correctness, memory-receipt, completion, and repository binding through final validation and atomic checked-status publication. Permit exactly one status-only descendant whose complete committed diff from the measured source contains only `benchmarks/qualification-completion-checkpoint.json`, `benchmarks/qualification-completion-manifest.json`, `docs/qualification-status.md`, `docs/plans/agent-native-modular-qec-progress-report.md`, `docs/plans/agent-native-modular-qec-architecture-plan.md`, `docs/plans/GOAL.md`, and `docs/plans/milestone-spec-gaps.md`. The generated dashboard must open, hash, and semantically validate the checked manifest, prove measured-commit ancestry, reject a second descendant, require a clean worktree in check mode, and reject every other committed path. The descendant records the original evidence identity and never relabels artifacts as having been produced from itself.
- Publish all product crates and `stab-core` together as `0.2.0`.
- Treat every pre-status package probe as diagnostic. After the single status descendant passes exact-revision CI, generate the final immutable schema-version-4 package preflight from that exact descendant, then validate the coordinated package set, exact internal versions, topological publication order, shared package README, bounded archive expansion, archive VCS identities and digests, canonical crates.io metadata, isolated Cargo configuration and credential scope, complete Cargo and rustc identities from one pinned target toolchain root, annotated release tag, source-bound executable version and architecture, and binary checksums through the Rust-backed `release::` workflow. Both irreversible producers must mechanically require the authenticated A9 completion before reading credentials. Crates.io publication must submit the exact retained canonical metadata and reviewed archive bytes without upload-time repackaging, verify the visible checksum before advancing, and remain safely resumable across matching versions. Local `just` publication must use typed quoted options, build each operator in a fresh owner-only target with every explicit publication-token variable removed, and bind only the needed variable while invoking that exact operator; run it from an isolated user session because repository controls do not claim to protect environment credentials from a malicious same-UID host process. The release workflow must dispatch from `refs/tags/v0.2.0`, check out the immutable event SHA, prove tag, ref, and `HEAD` agreement, and build the operator in a SHA-scoped runner target before giving the exact command a step-local token. Architecture policy must reject inherited, action-bound, Cargo-bound, unrelated, or shell-composed explicit publication-token variables in workflows. Reviewed full-SHA actions and declared job permissions remain a separate trusted boundary. GitHub draft creation must retain the complete six-file asset set from local verification through upload, require the remote annotated tag to resolve to the reviewed commit both before mutation and immediately before success, create no public release, and verify the final remote names, sizes, states, and SHA-256 digests. Publish crates and create the tag without a second source commit; record post-publication identities in crates.io, the annotated tag, and the GitHub release assets instead of requiring another documentation descendant.
- Freeze the complete release workflow execution context in architecture policy, including exact jobs, runners, permissions, timeouts, environment bindings, action inputs, shell-bearing steps, and commands; mutable runner labels or semantically different workflow steps are contract changes even when action SHAs remain pinned.
- Require active no-bypass tag ruleset `20419793` to protect only `refs/tags/v0.2.0` against update and deletion. Check that ruleset and the exact annotated remote tag before and after draft creation, immediately before manual publication, and immediately after publication; require exact retained asset names, sizes, upload states, and SHA-256 digests at both handoff checks, and require the final public release state and publication timestamp.
- Freeze the complete rehearsal workflow in the same architecture policy as production, require both fixed workflow paths to exist, and bind the exact scratch repository name and numeric identity, native AArch64 runners, action commits, permissions, timeouts, immutable checkout, source-derived tag, credential-free operator build, and one final explicit step-local GitHub token binding. Reviewed full-SHA actions and the automatic job token available under declared job permissions remain an explicit trusted boundary; checkout must not persist that credential. The scratch ruleset must require update and deletion protection with an authenticated empty bypass list and `current_user_can_bypass=never`; because GitHub's public projection omits those two fields, pin its GitHub-owned node, creation, and update fingerprint to that authenticated inspection and reject any partial bypass response or fingerprint change before and after each remote operation.
- After exact-tag dispatch, poll the workflow-scoped API under a fixed bound for exactly one `workflow_dispatch` run whose head SHA is the reviewed commit, because `gh workflow run` returns no run identity. Reject zero or multiple matches and latest-run discovery, then require the captured run's workflow, event, tag branch, head SHA, URL, completion status, and conclusion to match the release contract.
...
- Retain one descriptor-safe correctness tree per exact prerequisite identity during completion reconstruction instead of one tree per source report. Identical case sets must use identical evidence, and the current release scope admits exactly the source-derived 11 prerequisite artifacts under direct `target/qualification/<name>` paths. Creation, offline replay, and checkpoint replay of the complete 138-report release scope must each pass with and record a soft `RLIMIT_NOFILE` of 1024; the production-shape resource test must use real correctness trees and final descriptor-backed Git auditing. Descriptor exhaustion is a resource failure and must never be reported as artifact mutation.
- Audit final tracked, staged, and untracked Git state through two complete descriptor-rooted scans, reject any between-scan change as unstable state, and preserve typed Git failures instead of collapsing them into artifact-mutation diagnostics.
...
- The fixed WS4 scratch rehearsal completes before replacement formal evidence: exactly one source-bound workflow run succeeds, its private draft retains the exact six digest-matched assets, and the read-only verifier passes without exposing an irreversible command in the rehearsal binary.
- All ten crates.io versions expose the exact reviewed checksums, and no upload child repackages or substitutes their bytes.
- The protected remote annotated `v0.2.0` tag resolves to the release commit; the private draft contains exactly the two validated binaries, checksum sidecars, and provenance manifests with matching GitHub-recorded digests; a human publishes that immediately reverified draft; and the post-publication verifier proves the exact public release state, assets, tag, and active ruleset.
