...
- On the final reviewed A6 source revision, run one complete 166-row ownership-continuity matrix covering M4 through M11 and PF1 through PF7. The matrix contains 165 executable timing rows plus the explicit `m7-perf-harness` `baseline-metadata` provenance anchor; strict comparison must require measurements from every non-metadata contract and must not count the metadata anchor as timing evidence. Use one fresh schema-version-2 pinned-Stim baseline and one schema-version-3 warmed three-run Stab comparison, both created with `--new-output`, and pass `--measurement-contract benchmarks/a6-measurement-contract.json` to the comparison. The comparison must bind the exact baseline SHA-256, scalar Cargo feature set, `source-owned-row-native-v1` timing boundary, and selected measurement-contract path and SHA-256. The checked measurement contract freezes all 166 ordered rows, 309 ordered Stab measurement identities, and one exact Rust semantic-witness source for every executable row. Read profiler notes from every owning milestone directory through repeated source-root options, replay current thresholds, waivers, and profiler notes during validation, and reject duplicate selected identities regardless of the current ratio. Threshold-eligible rows must retain their existing Stim policies.
- Compare each report-only source-current median with the latest clean report for the same row, complete ordered measurement set, workload observations, implementation, host and CPU identity, build profile, scalar feature selection, measurement-contract digest, timing boundary, and normalized row-native iteration count. Historical schema-version-1 or schema-version-2 reports lack enough identity to serve as predecessors. Regenerate each required predecessor on the same host before the current matrix from a clean instrumentation backport revision that contains the historical product source plus only the reviewed schema-version-3 evidence harness; preserve that committed revision and report as historical evidence and do not merge its product tree into current source. A deterioration greater than 15% requires a focused diagnostic before acceptance. A focused diagnostic isolates the exact source-owned row and contract-owned semantic witness, performs a warmup, and records exactly one outer comparison run. The row retains its source-owned release-profile internal timing count, which must be at least eight and must be recorded in the evidence ledger; changing that count solely to obtain a preferred result is prohibited. Repeating outer runs or aggregating independent outer runs is not a substitute for profiling. If the focused result remains greater than `1.15x` its clean predecessor and host policy permits hardware profiling, capture a hardware or CPU profile and fix or record the unavailable restriction. The machine outcome is derived from measurements and profile availability as `resolved-within-boundary`, `reproduced-profiled`, or `reproduced-profile-unavailable`; prose is only an owner action and cannot relabel the measured outcome.
- Publish one A6 focused-evidence ledger after the final clean run through `just bench::a6-focused-evidence --publish-from <request>`. The request may select only evidence paths, predecessor phase assignments, profile availability, and owner actions; the publisher derives every digest, median, ratio, crossing, timing count, initial seed, and typed outcome, validates all bound artifacts, rechecks the unchanged clean revision, and atomically creates `benchmarks/a6-focused-evidence.json` without replacement. Each ledger entry binds the source revision, fresh baseline and matrix, selected row and measurements, contract-owned semantic-witness identity, focused and predecessor reports, exact internal timing count, normalized ratios, profile availability, typed outcome, and owner action. Normal validation always reopens and hashes every artifact; the former optional verification mode is not an acceptance path. Structural validation must reject missing crossings, extra or duplicate entries, reused mutable artifacts, stale source revisions, policy drift, invalid digests, timing counts below eight, and ratios inconsistent with the bound reports.
