...
- Treat every pre-status package probe as diagnostic. After the single status descendant passes exact-revision CI, generate the final immutable package preflight from that exact descendant, then validate the coordinated package set, exact internal versions, topological publication order, shared package README, bounded archive expansion, archive VCS identities and digests, isolated Cargo configuration and credential scope, Cargo and Rust toolchain identities, annotated release tag, source-bound executable version and architecture, and binary checksums through the Rust-backed `release::` workflow. Publish crates and create the tag without a second source commit; record post-publication identities in crates.io, the annotated tag, and the GitHub release assets instead of requiring another documentation descendant.
