...
- Compare each report-only source-current median with the latest clean report for the same row, measurement, workload, implementation, build profile, feature selection, and timing boundary. A deterioration greater than 15% requires a focused diagnostic before acceptance. A focused diagnostic isolates the exact source-owned row and semantic witness, performs a warmup, and records one outer comparison run; the legacy runner's 128 internal release-profile timings are the retained samples, so repeating outer runs is not a substitute for profiling. If the deterioration reproduces and host policy permits hardware profiling, capture a hardware or CPU profile and fix or explain the hot path. If hardware profiling is unavailable, record the host restriction without changing it; a focused non-reproduction plus an adjacent clean complete-matrix repeat may support a host-variance explanation.
