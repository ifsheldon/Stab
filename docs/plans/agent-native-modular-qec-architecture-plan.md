...
- Capture the workflow run URL returned by exact-tag dispatch and derive the numeric run identity from that URL. Reject latest-run discovery, and require the captured run's workflow, event, tag branch, head SHA, URL, completion status, and conclusion to match the release contract.
