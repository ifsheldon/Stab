...
- The external review correctly warned against turning every API or artifact into a separate qualification product. A6 therefore requires exact executable preflights only for the 65 policy-gated rows plus the selected equal-width M6 row; the other report-only rows prove workload continuity without acquiring an implicit Stim-correctness claim.
- A6 provenance uses one checked predecessor registry, one typed profile-receipt format, and one append-only content-addressed evidence object per source revision. It deliberately does not add a mutable latest pointer, an index, sidecar digests, per-phase completion trees, or a second replay hierarchy.
...
- Re-run diagnostic Stim-relative continuity for the selected default scalar implementation after the SIMD diagnostics without changing the `1.25x` boundary. The complete matrix owns the M5 pair `m5-simd-bits` / `stab_simd_bits_xor_10K` versus `simd_bits_xor_10K` and the M6 pair `m6-clifford-string` / `stab_clifford_string_multiplication_10K` versus `CliffordString_multiplication_10K`. The M6 short-right-operand measurements remain report-only Stab diagnostics because Stim has no equivalent workload; the separate scalar-versus-SIMD report owns non-identity Clifford kernel selection. A9 owns promotable controlled-host parity evidence.
...
- On the final reviewed A6 source revision, run one complete 166-row ownership-continuity matrix covering M4 through M11 and PF1 through PF7. The matrix contains 165 executable timing rows plus the explicit `m7-perf-harness` `baseline-metadata` provenance anchor; strict comparison must require measurements from every non-metadata contract and must not count the metadata anchor as timing evidence. Use one fresh schema-version-3 pinned-Stim baseline and one schema-version-4 warmed three-run Stab comparison, both created with `--new-output`, and pass `--measurement-contract benchmarks/a6-measurement-contract.json` to the comparison. The comparison must bind the exact baseline SHA-256, scalar Cargo feature set, `source-owned-row-native-v1` timing boundary, private host fingerprint, exact executing-binary digest, and selected measurement-contract path and SHA-256. The checked measurement contract freezes all 166 ordered rows and 309 ordered Stab measurement identities. It derives an exact 66-row executable-preflight set from the 65 threshold-owned rows plus `m6-clifford-string`; remaining report-only rows establish workload continuity only and make no correctness claim. Read profiler notes from every owning milestone directory through repeated source-root options, reconstruct all derived timing and memory fields from raw measurements, replay tracked thresholds, waivers, and profiler notes during validation, and reject duplicate selected identities regardless of the current ratio. The validator independently keeps the selected equal-width M5 and M6 `1.25x` pair gates even if legacy threshold migration retires their old matrix entries.
- Compare each report-only source-current median with the latest clean report for the same row, complete ordered measurement set, workload observations, implementation and executable digest, private host fingerprint and CPU identity, build profile, scalar feature selection, measurement-contract digest, timing boundary, and normalized row-native iteration count. Historical schema-version-1 through schema-version-3 reports lack enough identity to serve as predecessors. Regenerate each required predecessor on the same host before the current matrix from a clean instrumentation backport revision that contains the historical product source plus only the reviewed schema-version-4 evidence harness; each predecessor is warmed, strict, create-new, and records exactly one outer run. Record every accepted backport in `benchmarks/a6-predecessors.json` with its historical product commit, direct-child instrumentation commit, canonical raw-tree-delta SHA-256, and exact owned phases. Preserve the instrumentation commit at `refs/tags/a6-predecessors/<instrumentation-commit>`, require exactly one parent equal to the registered product commit, and prohibit the backport from entering current source ancestry. Publication derives phase ownership from this checked registry and accepts only predecessor report paths, so a request cannot hand-assign a favorable phase to an unrelated report. A deterioration greater than 15% requires a focused diagnostic before acceptance. A focused diagnostic isolates the exact source-owned row, completes any exact executable preflight required by the checked contract, performs a warmup, and records exactly one outer comparison run. The row retains its source-owned release-profile internal timing count, which must be at least eight and must be recorded in the evidence ledger; changing that count solely to obtain a preferred result is prohibited. Repeating outer runs or aggregating independent outer runs is not a substitute for profiling. If the focused result remains greater than `1.15x` its clean predecessor, capture a Linux `perf record` profile or run the fixed capability probe through `just bench::a6-profile-receipt`; an unavailable result is accepted only when bounded diagnostics and `perf_event_paranoid` identify a kernel-policy denial. The project does not lower host policy for evidence. The machine outcome is derived from measurements and the typed receipt as `resolved-within-boundary`, `reproduced-profiled`, or `reproduced-profile-unavailable`; prose is only an owner action and cannot relabel the measured outcome.
- Publish one A6 focused-evidence object after the final clean run through `just bench::a6-focused-evidence --publish-from <request>`. Request schema version 2 may select only the matrix report, predecessor reports, focused reports, optional typed profile receipts, and bounded owner actions; the publisher derives every registry mapping, digest, median, ratio, crossing, timing count, initial seed, and typed outcome and validates the checked executable-preflight contract. It validates all bound artifacts, rechecks the unchanged clean revision, and atomically creates `benchmarks/a6-focused-evidence-<source-revision>-<exact-bytes-sha256>.json` without replacement. No mutable latest pointer or index is maintained, and `benchmarks/a6-focused-evidence.json` remains readable only as an explicit historical legacy path. Each schema-version-4 object binds the source revision, predecessor-registry digest, fresh baseline and matrix, selected row and measurements, measurement-contract identity, focused and predecessor reports, exact internal timing count, normalized ratios, typed profile receipt, machine-derived outcome, and owner action. After the object is committed, only that exact object plus `GOAL.md` and the append-only progress report may differ from its source revision; another evidence object or any product, policy, inventory, fixture, or workflow change makes it historical. Normal validation reads tracked Git bytes, reopens and hashes every artifact and nested profile capture, and never treats an untracked candidate as source-owned evidence. Structural validation must reject missing crossings, extra or duplicate entries, reused mutable artifacts, stale source revisions, policy drift, invalid digests, timing counts below eight, unbounded prose, and ratios inconsistent with the bound reports.
- Treat the following 19 phases as initial clean seeds because no semantically identical clean extraction report exists: `pf3-m2d-sweep-b8` / `stab_pf3_m2d_sweep_b8`; `pf3-m2d-sweep-ptb64-input` / `stab_pf3_m2d_sweep_ptb64`; both `pf3-detect-sweep-sampling` phases; all six `pfm-b5-analyzer-cycle-folding` phases; both `pfm-b5-analyzer-generated-qec` phases; `pfm-b5-graphlike-search-direct-dem` / `stab_pfm_b5_graphlike_direct_dem`; `pfm-b5-hypergraph-search-direct-dem` / `stab_pfm_b5_hypergraph_direct_dem`; and the five PF7 phases from `pf7-cli-m2d-sweep-b8`, `pf7-cli-m2d-feedback-inline`, `pf7-cli-analyze-errors-generated`, `pf7-cli-analyze-errors-decompose`, and `pf7-cli-legacy-dispatch-startup`. Require their semantic witnesses and current clean measurements to pass, but make no retrospective self-regression claim.
- Require focused diagnostics for every threshold crossing not already covered by a valid focused artifact. The review-identified missing set is `pf5-flow-generators-measurement-rich`, `pf5-flow-solve-measurement-python`, `pf5-flow-solve-measurement-rich`, `pf5-has-all-flows-batch`, `pf5-missing-detectors-mpp`, `pf6-sparse-rev-frame-loop`, `pfm-b5-wcnf-direct-dem`, and `pfm-b5-wcnf-generated-qec`.
...
- The final 166-row ownership-continuity report is source-current, binds the fresh baseline and checked 309-measurement contract, contains all 165 executable rows plus the one validated metadata anchor, survives source-owned threshold, waiver, and profiler-note replay, has no missing executable runner or failed semantic witness, labels all 19 initial-seed phases without a retrospective claim, and records every report-only phase that crossed the 15% review threshold together with its focused diagnostic and typed outcome.
- The selected M5 and M6 scalar Stim comparisons use the exact named pairs above. M6 short-right-operand rows and the scalar-versus-SIMD non-identity kernel report remain distinct diagnostics and are not described as Stim-relative evidence.
