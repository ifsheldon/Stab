...
- On the final reviewed clean A6 source revision, produce one fresh pinned-Stim baseline and one warmed three-run comparison for `m5-simd-bits`, and repeat that sequence for `m6-clifford-string`. Use create-new output paths, exact source-owned semantic preflights, unchanged tracked threshold and profiler-note policy, the scalar feature set, and the existing row-native timing boundary. Both named Stim pairs must pass the median and confidence-upper-bound `1.25x` gate.
- Run benchmark smoke and the affected A2, A4, and A5 phase diagnostics whose source package, feature selection, or executed call path changed. These reports are diagnostic continuity only unless an existing source-owned parity or self-regression policy already applies to the exact measurement.
...
- The selected M5 and M6 scalar Stim comparisons are source-current, use the exact named pairs above, and pass the unchanged `1.25x` median and confidence-upper-bound gate. M6 short-right-operand rows and the scalar-versus-SIMD non-identity kernel report remain distinct diagnostics and are not described as Stim-relative evidence.
...
- The A6 closure revision has green local verification and exact-revision CI. Historical evidence remains bound to its original source and cannot become current merely because the closure procedure was simplified.
