...
- On the final reviewed A6 source revision, run one complete 166-row ownership-continuity matrix covering M4 through M11 and PF1 through PF7. The matrix contains 165 executable timing rows plus the explicit `m7-perf-harness` `baseline-metadata` provenance anchor; strict comparison must require measurements from every non-metadata contract and must not count the metadata anchor as timing evidence. Use one fresh pinned-Stim baseline, one warmed three-run Stab comparison, unchanged comparator classes, and exact output witnesses. Read profiler notes from every owning milestone directory through repeated source-root options and require note validation to reject duplicate selected identities regardless of the current ratio. Threshold-eligible rows must retain their existing Stim policies.
- Compare each report-only source-current median with the latest clean report for the same row, measurement, workload, implementation, build profile, feature selection, and timing boundary. A deterioration greater than 15% requires a focused diagnostic before acceptance. A focused diagnostic isolates the exact source-owned row and semantic witness, performs a warmup, and records exactly one outer comparison run. The row retains its source-owned release-profile internal timing count, which must be at least eight and must be recorded in the evidence ledger; changing that count solely to obtain a preferred result is prohibited. Repeating outer runs or aggregating independent outer runs is not a substitute for profiling. If the focused result remains greater than `1.15x` its clean predecessor and host policy permits hardware profiling, capture a hardware or CPU profile and fix or explicitly disposition the hot path. If hardware profiling is unavailable, record the host restriction without changing it and give an explicit source-owned disposition; do not call the deterioration a non-reproduction. A focused result at or below `1.15x` plus an adjacent clean complete-matrix repeat may support a host-variance explanation.
- Check in one A6 focused-evidence ledger after the final clean run. Each entry must bind the source revision, selected row and measurements, semantic-witness identity, focused report path and SHA-256, extraction report path and SHA-256, exact internal timing count, focused and predecessor medians, normalized ratio, host-profile availability, and final disposition. Structural validation must reject missing crossings, extra or duplicate entries, reused mutable artifact paths, stale source revisions, invalid digests, timing counts below eight, and ratios inconsistent with the bound reports.
...
- The final 166-row ownership-continuity report is source-current, contains all 165 executable measurements plus the one validated metadata anchor, has no missing executable runner or failed semantic witness, labels all 19 initial-seed phases without a retrospective claim, and records every report-only phase that crossed the 15% review threshold together with its focused diagnostic and disposition.
