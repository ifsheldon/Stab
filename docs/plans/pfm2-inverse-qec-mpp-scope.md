...
- Keep broader `mzz` beyond the selected exact packet, noisy measure-reset detector-flow beyond the exact `noisy_mr_det` packet, broader observable-aware QEC inverse behavior beyond the selected exact `obs_include_pauli` packet, and multi-instruction QEC inverse behavior active for later PFM2 or PFM5 slices.
