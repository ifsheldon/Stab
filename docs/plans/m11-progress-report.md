...
This slice implements the first deterministic `sample_dem` path, the first one-bit noisy statistical `sample_dem` row, and the M11-owned structural subset of `src/stim/simulators/dem_sampler.test.cc`, while broad benchmark reporting remains pending.
