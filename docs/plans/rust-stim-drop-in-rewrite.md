...
PFM-B5 supersedes the staged RPF6 evidence with a generic reverse-state recurrence engine and a finite source-owned closure matrix. The analyzer now discovers transient, period-8, period-127, nested, coordinate, gauge, repetition-code, and generated-surface behavior without fixture or period dispatch; seven graphlike, eight hypergraph, and ten exact WCNF cases close the selected search surface; sparse detector and observable IDs use touched-slot compression; and traversal-mechanism limits are distinct from search-state limits. Current benchmark ownership is `pfm-b5-analyzer-cycle-folding`, `pfm-b5-analyzer-generated-qec`, the direct and generated graphlike or hypergraph rows, the direct and generated WCNF rows, `pf6-error-decomp-loop-folded`, and `pf6-sparse-rev-frame-loop`. The faithful d25/r25 and d11/r1000 graphlike rows remain report-only at 5.938x and 5.247x Stim and are not promoted into the 1.25x gate. Full ErrorMatcher provenance remains deferred. See `docs/plans/pfm-b5-analyzer-search-progress-report.md`.
