...
`coverage-util-top-has-flow` owns deterministic unsigned unitary-flow checks, and RPF5 promotes the unsigned checker subset for measurement-record and observable dependencies plus scoped `solve_for_flow_measurements` examples; sampled checks, full generator-table measurement solving, composed flow-generator synthesis, and transform integration stay active or deferred according to the RPF plan.
