...
M9 now implements scoped feedback-removal conversion for `m2d --ran_without_feedback` by applying `circuit_with_inlined_feedback` before compiling the detection converter. This is not full Python `Circuit.with_inlined_feedback` parity; the PF2 feedback slices add source-owned evidence for selected `XCZ`/`YCZ` measurement-record feedback equivalent to `CX`/`CY`, selected pinned loop-refolding, and nested bounded-repeat detector-parity cases with bounded repeat work, while broader repeat-contained feedback and broader transform APIs remain future transform work unless a later milestone promotes them explicitly.
