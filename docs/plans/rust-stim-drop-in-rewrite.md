...
`coverage-util-top-circuit-inverse-qec` owns the unitary QEC inverse case, and PF2 now promotes the selected direct `circuit_inverse_qec` reset-measure-detector target-list packet for one detector, including Stim's pinned `r_m_det` shape, multi-target record remapping, sparse detector subsets, duplicate detector-record parity, and empty-detector behavior, while broader measurement-rich inverse rewrites remain active follow-up work. PF2 and PF5 now own the selected single-instruction measurement-rich `time_reversed_for_flows` bridge, including selected measurement-ordering, selected plain unique-target reset-to-measurement conversion, selected single-target measurement-to-reset conversion, and selected unique-target measure-reset flow reversal with inverted result-target support, while duplicate reset-only and duplicate measure-reset behaviors stay fail-closed pending PFM spec-gap decisions, and broader detector-flow rewrites with interleaved operations, noise, feedback, repeat, and detecting-region rewrites stay deferred or active under the PFM plan.
