...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases and explicit measurement rejection; measurement-rich generator synthesis, reset, pair-measurement, and noise-derived generated flows stay deferred until RPF5 promotes them.
`coverage-util-top-has-flow` owns deterministic unsigned unitary-flow checks, and RPF5 promotes the unsigned checker subset for measurement-record and observable dependencies; sampled checks, solve-for-measurements, flow-generator synthesis, and transform integration stay active or deferred according to the RPF plan.
...
The M9 detector utility closure promotes simple detecting regions, basic single-record missing-detector suggestions, and MPP feedback inlining into explicit Rust APIs and executable oracle rows. PF5 extends the missing-detectors utility with Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer-product cases, record-only observable rows, and ignored Pauli observable rows, and extends unsigned has-flow checking with measurement-record and observable dependencies. Broader detecting-region gate and target-shape support, gauge handling, generated-code missing-detector suffix analysis for honeycomb and toric cases, exact feedback-loop refolding, measurement-rich flow generator solving, and full transform API parity remain future detector-analysis or transform work.
