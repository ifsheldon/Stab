...
`coverage-util-top-has-flow` owns deterministic unsigned unitary-flow checks, and RPF5 promotes the unsigned checker subset for measurement-record and observable dependencies; sampled checks, solve-for-measurements, composed flow-generator synthesis, and transform integration stay active or deferred according to the RPF plan.
...
Current RPF5 flow progress promotes unsigned has-flow checking for measurement-record and observable dependencies with explicit oracle row `pf5-has-flow-record-observable-rust` and report-only benchmark runner `pf5-has-all-flows-batch`, plus exact measurement/reset/pair-measurement/feedback/MPAD flow generators with explicit oracle row `pf5-flow-generators-measurement-rust` and report-only benchmark runner `pf5-flow-generators-measurement-rich`; broader composed measurement-rich flow generators, solve-for-measurements, transform integration, and variable-target gate flow metadata remain governed by the PFM plan.
