...
M9 implements sweep input data for public `m2d` detection conversion through `--sweep` and `--sweep_format`, using all-false sweep bits when `--sweep` is omitted. PF3 now permits non-frame `detect` sampling to use omitted all-false sweep bits for the current sweep execution subset, while frame-path Pauli-observable sweep sampling remains explicitly rejected until the frame executor owns typed sweep semantics.
