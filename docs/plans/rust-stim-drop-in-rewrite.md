...
Current RPF6 progress promotes the noisy generated-QEC analyzer semantic subset with explicit oracle row `pf6-analyzer-generated-qec-rust` and report-only benchmark runner `pf6-analyze-errors-generated-surface`, the selected prefix/repeat/tail detector-chain true-folded analyzer subset with explicit oracle row `pf6-analyzer-prefix-repeat-tail-folding-rust`, the selected loop-folded error-decomposition subset with explicit oracle row `pf6-error-decomp-loop-folded-rust` and report-only benchmark runner `pf6-error-decomp-loop-folded`, the bounded mixed-top-level `fold_loops=true` fallback with explicit oracle rows `pf6-analyzer-mixed-top-level-fallback-rust` and `pf6-analyzer-generated-fold-loop-fallback-rust`, the generated-QEC graphlike and hypergraph search subset with explicit oracle row `pf6-search-generated-qec-rust` and report-only benchmark runners `pf6-graphlike-search-generated` and `pf6-hypergraph-search-generated`, the selected generated-QEC SAT/WCNF structural subset with explicit oracle row `pf6-search-generated-sat-wcnf-rust` and report-only benchmark runner `pf6-generated-sat-wcnf`, the supported-Clifford sparse reverse tracker unitary-repeat folding subset including the full single-qubit Clifford gate set, fixed two-qubit tableau-backed Clifford gates with plain qubit-pair targets, deterministic generated repeat tests, and nested grouped-target repeat tests, the shifted-copy sparse reverse tracker measurement/detector repeat subset with explicit oracle row `pf6-sparse-rev-shifted-repeat-rust`, the unsigned sparse-tracker `SPP`/`SPP_DAG` product propagation subset with explicit oracle row `pf6-sparse-rev-spp-rust`, and the selected matched-error canonicalization hardening with explicit oracle row `pf6-matched-error-canonicalize-rust`, with report-only benchmark runner `pf6-sparse-rev-frame-loop` remaining scoped to loop folding; true folded generated-loop analyzer behavior beyond the selected detector-chain slice, broader loop-folded decomposition families, broader generated search, broader generated SAT or WCNF families, broader sparse tracker consumption, and broader matched-error hardening are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
