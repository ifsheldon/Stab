...
- Post-beta beta-hardening status: `docs/plans/beta-125-performance-plan.md` owns the stricter `1.25x` beta target; the last clean beta-125 evidence passed the 76-row primary gate with 72 comparable rows and 4 checked no-ratio waivers, and the current dirty-worktree diagnostics for the expanded 85-row matrix pass beta with 80 comparable rows and 5 checked no-ratio waivers, pass timing regression with 80 configured threshold rows and 5 checked no-ratio waivers, and pass memory regression for all 85 rows after the byte-aligned `b8 -> b8` convert fast path, exact error-decomposition branch split, streaming parser and exact common plain-instruction fast paths, and schema-version-2 resident-delta memory baseline. `m8-sample-primary-unrotated-surface-contract` remains a narrow beta watch row and needs a dedicated sampler plan if committed-code evidence repeatedly crosses 1.25x. Final archival acceptance still requires regenerating the expanded reports from committed code with `local_modifications=false`.
