...
`coverage-util-top-has-flow` owns deterministic unsigned unitary-flow checks, and RPF5 promotes the unsigned checker and `has_all_flows` Rust helper subset for measurement-record and observable dependencies plus scoped `solve_for_flow_measurements` examples covering empty input, `MX`, measured-idle, multi-target measurement and `MPP`, fewer-measurements heuristic, and repetition-code cases; sampled signed checks, full generator-table measurement solving, composed flow-generator synthesis, and transform integration stay active or deferred according to the RPF plan.
