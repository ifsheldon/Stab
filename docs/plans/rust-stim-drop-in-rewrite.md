...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact single-instruction generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, and `MPAD`; composed measurement-rich generator synthesis, MPP, feedback, heralded-noise, and all-operation generated flows stay active or deferred according to the RPF plan.
...
Current RPF5 flow progress promotes unsigned has-flow checking for measurement-record and observable dependencies with explicit oracle row `pf5-has-flow-record-observable-rust` and report-only benchmark runner `pf5-has-all-flows-batch`, plus exact single-instruction measurement/reset/pair-measurement/MPAD flow generators with explicit oracle row `pf5-flow-generators-measurement-rust` and report-only benchmark runner `pf5-flow-generators-measurement-rich`; composed measurement-rich flow generators, solve-for-measurements, transform integration, and variable-target gate flow metadata remain governed by the PFM plan.
