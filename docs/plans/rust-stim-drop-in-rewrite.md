...
Current RPF2 progress implements Rust `Circuit::flattened`, `Circuit::flattened_operations`, `Circuit::without_noise`, scoped `Circuit::decomposed`, and scoped `Circuit::with_inlined_feedback` with source-owned tests, oracle metadata, and report-only benchmark runners for the performance-sensitive flatten, noise-removal, and feedback-inlining transforms; full Stim `decomposed`, exact feedback loop refolding, and flow-time-reversal remain governed by the RPF plan.
