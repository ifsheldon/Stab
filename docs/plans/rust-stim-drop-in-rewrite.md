...
Current RPF2 progress implements Rust `Circuit::flattened`, `Circuit::flattened_operations`, `Circuit::without_noise`, `Circuit::decomposed`, scoped unitary and selected single-instruction measurement-rich `Circuit::time_reversed_for_flows`, and scoped `Circuit::with_inlined_feedback` with source-owned tests, oracle metadata, and report-only benchmark runners for the performance-sensitive flatten, noise-removal, decomposition, unitary and selected measurement-rich flow-time-reversal, and feedback-inlining transforms; exact feedback loop refolding and broader measurement-rich flow-time-reversal remain governed by the PFM plan.
