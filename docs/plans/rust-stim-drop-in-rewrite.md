...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact single-instruction generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, and `MPAD`; composed measurement-rich generator synthesis, pair-measurement, MPP, feedback, heralded-noise, and all-operation generated flows stay active or deferred according to the RPF plan.
