...
M9 now implements scoped feedback-removal conversion for `m2d --ran_without_feedback` by applying `circuit_with_inlined_feedback` before compiling the detection converter. This is not full Python `Circuit.with_inlined_feedback` parity; the PF2 feedback slices add source-owned evidence for selected `XCZ`/`YCZ` measurement-record feedback equivalent to `CX`/`CY`, selected pinned loop-refolding, and nested bounded-repeat detector-parity cases with bounded repeat work, while broader repeat-contained feedback is under-specified in `docs/plans/milestone-spec-gaps.md` until exact repeat structures, comparator behavior, resource behavior, oracle metadata, and benchmark policy are selected.
...
- `cargo test -p stab-core detecting_regions`, `cargo test -p stab-core missing_detectors`, and `cargo test -p stab-core circuit_with_inlined_feedback` cover the M9 and PF5 detector utility closure for simple detecting regions, bounded repeat traversal, detector and logical-observable target filters, basic and extended missing-detector suggestions, and MPP feedback inlining. `cargo test -p stab-core --test circuit_transforms feedback` covers the PF2 scoped transform evidence including selected `XCZ`/`YCZ` measurement-record feedback inlining.
