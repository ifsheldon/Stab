...
M9 now implements scoped feedback-removal conversion for `m2d --ran_without_feedback` by applying `circuit_with_inlined_feedback` before compiling the detection converter. This is not full Python `Circuit.with_inlined_feedback` parity; the selected pinned loop-refolding case is implemented with bounded repeat work, while broader repeat-contained feedback and broader transform APIs remain future transform work unless a later milestone promotes them explicitly.
