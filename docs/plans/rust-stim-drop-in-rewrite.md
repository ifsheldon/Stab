...
Current RPF2 progress implements Rust `Circuit::flattened`, `Circuit::flattened_operations`, `Circuit::without_noise`, `Circuit::decomposed`, and scoped `Circuit::with_inlined_feedback` with source-owned tests, oracle metadata, and report-only benchmark runners for the performance-sensitive flatten, noise-removal, decomposition, and feedback-inlining transforms; exact feedback loop refolding and flow-time-reversal remain governed by the RPF plan.
Current RPF5 flow progress promotes unsigned has-flow checking for measurement-record and observable dependencies with explicit oracle row `pf5-has-flow-record-observable-rust` and report-only benchmark runner `pf5-has-all-flows-batch`; measurement-rich flow generators, solve-for-measurements, transform integration, and variable-target gate flow metadata remain governed by the RPF plan.
Current RPF6 progress promotes the noisy generated-QEC analyzer semantic subset with explicit oracle row `pf6-analyzer-generated-qec-rust` and report-only benchmark runner `pf6-analyze-errors-generated-surface`; broad generated-loop analyzer behavior, loop-folded decomposition, generated search, and sparse reverse tracker optimization remain governed by the RPF plan.
