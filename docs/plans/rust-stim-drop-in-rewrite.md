...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, nonconstant and constant single-instruction `MPP`, `MPAD`, scoped measurement-record feedback, and the pinned heralded-noise MPP fixture; broader composed measurement-rich generator synthesis, unsupported feedback shapes, broader heralded-noise synthesis, and all-operation generated flows stay active or deferred according to the RPF plan.
