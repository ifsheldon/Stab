...
PF4 later added selected DEM search, ErrorMatcher filter, and SAT/WCNF folding for large flat zero-shift DEM repeat bodies: graphlike and hypergraph search represent selected detector-touching repeated body errors once in the search graph, ErrorMatcher filter DEM traversal represents selected detector-touching repeated filter keys once, unweighted shortest-error SAT represents each repeated all-nonzero body error once while preserving probability-insensitive structural semantics, and weighted SAT/WCNF folds repeated body errors by concrete MAP parity cost instead of sampling marginal parity probability. Shifted, nested, non-flat, detectorless logical-only search repeats, ErrorMatcher circuit-repeat provenance, and unweighted zero-probability structural repeat bodies retain explicit caps until separately promoted.
