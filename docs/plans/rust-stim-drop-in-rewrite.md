...
M9 implements sweep input data for public `m2d` detection conversion through `--sweep` and `--sweep_format`, using all-false sweep bits when `--sweep` is omitted. PF3 now permits non-frame and selected frame-path Pauli-observable `detect` sampling to use omitted all-false sweep bits for the current sweep execution subset, including sweep-controlled `CX` and `CY` qubit targets, sweep-controlled `CZ`, and `CZ` bit/bit no-op groups in the frame path, and covers the selected analyzer sweep-control no-op matrix with invalid controlled-Pauli target-position rejection. Pinned Stim v1.16.0 does not expose `stim detect --sweep`; broader sweep target placements, broader analyzer sweep-shape parity, and Python detector-sampler sweep APIs remain explicitly open until those executors own typed sweep semantics.
