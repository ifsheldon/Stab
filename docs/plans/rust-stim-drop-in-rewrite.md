...
PFM-B5 supersedes the staged RPF6 evidence with a generic reverse-state recurrence engine and a finite source-owned closure matrix. The analyzer now discovers transient, period-8, period-127, nested, coordinate, ordinary and nested gauge, repetition-code, and generated-surface behavior without fixture or period dispatch; saturating diagnostics cannot alter analysis success; folded noisy `MPAD` preserves its error mechanism; nested recurrence probing has one cumulative work budget; and the local decomposition boundary accepts sixteen detector symptoms and rejects seventeen. Nine graphlike, ten hypergraph, and twelve selected exact WCNF cases close the finite search surface, including zero-probability diagnostics, variable-state payload bounds, early trivial UNSAT, and low-quantization headers. Sparse detector and observable IDs use touched-slot compression, hypergraph adjacency interns each edge once, and traversal mechanisms, per-error target occurrences, touched nodes, unique states, attempted transitions, per-state terms, aggregate stored terms, edge degree, edge incidences, SAT materialization, and conservative output bytes have independent limits. Selected finite WCNF text remains exact, while sparse-ID and large folded-repeat compression are intentional semantic hardening rather than universal byte-exact claims. Current benchmark ownership is `pfm-b5-analyzer-cycle-folding`, `pfm-b5-analyzer-generated-qec`, the direct and generated graphlike or hypergraph rows, the direct and generated WCNF rows, `pf6-error-decomp-loop-folded`, and `pf6-sparse-rev-frame-loop`. Prior clean d25/r25 and d11/r1000 graphlike evidence was report-only at 5.969x and 5.500x Stim; fresh committed-HEAD evidence, milestone-audit closure, and final review remain after second-review remediation. Full ErrorMatcher provenance remains deferred. See `docs/plans/pfm-b5-analyzer-search-progress-report.md`.
