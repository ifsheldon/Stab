...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, `MPAD`, and scoped measurement-record feedback; broader composed measurement-rich generator synthesis, MPP, unsupported feedback shapes, heralded-noise, and all-operation generated flows stay active or deferred according to the RPF plan.
...
The M9 detector utility closure promotes simple detecting regions, basic single-record missing-detector suggestions, and MPP feedback inlining into explicit Rust APIs and executable oracle rows. PF5 extends the missing-detectors utility with Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer-product cases, record-only observable rows, ignored Pauli observable rows, and the toric global-stabilizer generated-code suffix case, extends unsigned has-flow checking with measurement-record and observable dependencies, promotes exact measurement/reset/pair-measurement/feedback/MPAD flow generators, and promotes pinned Stim `solve_for_flow_measurements` examples for empty input, `MX`, idle extra qubits, and repetition-code measurement solving. Broader detecting-region gate and target-shape support, gauge handling, honeycomb and broader toric generated-code missing-detector suffix analysis, exact feedback-loop refolding, composed measurement-rich flow generator solving, full generator-table measurement solving, and full transform API parity remain future detector-analysis or transform work.
...
- Full detector-analysis parity: after the PF plan closes selected non-deferred Rust and CLI subcases, promote any remaining broader detecting-region gauge handling, MPP and observable missing-detector analysis, honeycomb suffix analysis, broader toric suffix analysis beyond the promoted global-stabilizer case, generated surface-code repeat matching, heralded matching, repeat-contained noise stack frames, and full sparse reverse tracker consumption into explicit detector-analysis acceptance rows before claiming full analyzer utility parity.
