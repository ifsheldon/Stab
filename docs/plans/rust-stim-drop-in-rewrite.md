...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, Python multi-target `M`/`MX`/`MYY`/`MPP`, nonconstant and constant single-instruction `MPP`, pinned variable-target `SPP` and `SPP_DAG` unitary examples, selected unitary-mixed composed measurement-rich instruction sequences, selected all-operation annotation and ordinary-noise no-op traversal, selected composed `SPP` and `SPP_DAG` unitary decomposition, bounded repeat-contained measurement-rich instruction sequences, `MPAD`, scoped measurement-record feedback, and the pinned heralded-noise MPP fixture; broader all-operation composed measurement-rich generator synthesis beyond the promoted no-op, tableau, feedback, heralded-record, and `SPP` or `SPP_DAG` decomposition subcases, unsupported feedback shapes, broader heralded-noise synthesis, folded repeat traversal beyond current caps, and all-operation generated flows stay active or deferred according to the RPF plan.
