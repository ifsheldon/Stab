...
PF4 later replaced the DEM sampler's flat repeat-operation compilation with a folded operation tree for compile and direct detector sampling, skips detector-only zero-probability repeat bodies, folds detector-only deterministic zero-shift repeats by parity, and folds selected detector-only single-stochastic zero-shift repeats by odd-parity probability, while sampled-error output and replay still preserve flat repeated error-bit order and keep materialized width caps.
...
The remaining post-PF4 sampler work is optimized broader repeated stochastic nonzero-probability body traversal that can tighten or remove the current sampled-error application work cap without changing flat sampled-error semantics.
