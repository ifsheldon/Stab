...
M9 implements sweep input data for public `m2d` detection conversion through `--sweep` and `--sweep_format`, using all-false sweep bits when `--sweep` is omitted. PF3 now permits non-frame `detect` sampling to use omitted all-false sweep bits for the current sweep execution subset and covers analyzer sweep-control no-op behavior, while frame-path Pauli-observable sweep sampling and broader analyzer sweep-shape parity remain explicitly open until those executors own typed sweep semantics.
...
- `cargo test -p stab-core detection` covers coordinate shifts, repeats, measurement-record observables, Pauli-target observable flips, empty-detector circuits, invalid measurement references, bounded record-shape validation, sweep-conditioned conversion with all-false defaults and per-shot sweep records, non-frame sweep-conditioned detection sampling with omitted all-false sweep bits, frame-path sweep rejection, and unsupported sweep-shape rejection.
