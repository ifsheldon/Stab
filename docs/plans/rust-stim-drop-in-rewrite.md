...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY`, `MXX`/`MYY`/`MZZ`, nonconstant and constant single-instruction `MPP`, `MPAD`, and scoped measurement-record feedback; broader composed measurement-rich generator synthesis, unsupported feedback shapes, heralded-noise, and all-operation generated flows stay active or deferred according to the RPF plan.
