...
- Add exact resource tests for accepted representatives, first rejection, collection and RNG side effects, projected growth, overflow, circuit-derived admission, and precedence over later algorithmic work.
...
CQ2 hardening makes caller-size-driven Algebra construction fallible and exposes the source-owned caps through `StabilizerResource`: owned Pauli strings, Clifford strings, and reusable Pauli-iterator state accept at most 1,048,576 qubits or entries; flow values accept at most 65,536 aggregate measurement plus observable terms; dense Tableau construction, circuit-to-Tableau conversion, and stabilizer solving accept at most 512 qubits; compact circuit-to-Tableau repeat composition accepts at most 16,777,216 aggregate width-squared work units; random-Tableau construction accepts at most 64 qubits; and unitary conversion accepts matrices through dimension 64.
These are deliberate Stab resource contracts, not claims that pinned Stim rejects the same sizes. Explicit-size constructors reject before allocation, iterable constructors stop at the first excess item without growing beyond the cap or returning partial output, random constructors reject before RNG use, projected Clifford growth checks overflow and final size, Flow multiplication performs bounded symmetric-difference cancellation before checking the resulting aggregate term count, and repeating an empty Clifford string takes constant work for every repetition count.
`Circuit::to_tableau` converts compact repeat blocks without materializing their repetitions: it converts the repeat body once, raises that Tableau by binary exponentiation, stops when the squared power becomes identity, and charges every remaining hidden composition to the aggregate work budget. The same conversion supports `SPP` and `SPP_DAG` by applying the source-owned Hermitian Pauli-product decomposition, including repeated-qubit phase reduction, and rejects anti-Hermitian products.
Annotation-only circuit flow generation separately limits aggregate identity-flow storage to 134,217,728 Pauli bits, preserving the established 2,049-qubit high-index regression while rejecting larger quadratic output before materialization.
...
`coverage-util-top-circuit-flow-generators` owns unitary circuit flow-generator cases, and RPF5 promotes exact generators for `M`/`MX`/`MY`, `R`/`RX`/`RY`, `MR`/`MRX`/`MRY` including inverted result targets, `MXX`/`MYY`/`MZZ`, Python multi-target `M`/`MX`/`MYY`/`MPP`, nonconstant and constant single-instruction `MPP`, pinned variable-target `SPP` and `SPP_DAG` unitary examples, selected unitary-mixed composed measurement-rich instruction sequences, selected all-operation annotation and ordinary-noise no-op traversal, selected composed `SPP` and `SPP_DAG` unitary decomposition, bounded repeat-contained measurement-rich instruction sequences, `MPAD` including asymmetric record association, duplicate reset and measure-reset generators with final-record parity and inversion semantics, scoped measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, and selected single- and multi-target heralded-noise MPP generator cases, plus exact signed-flow-set and checker-satisfaction coverage for the pinned generated all-operations fixture; broader all-operation composed measurement-rich generator synthesis beyond the promoted pinned generated all-operations fixture, no-op, tableau, measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, heralded-record, and `SPP` or `SPP_DAG` decomposition subcases, malformed controlled-Pauli target groups, broader heralded-noise synthesis beyond the selected MPP cases, folded repeat traversal beyond current caps, and broader generated all-operation flows are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
`coverage-util-top-has-flow` owns deterministic unsigned unitary-flow checks, and RPF5 promotes the unsigned checker and `has_all_flows` Rust helper subset for measurement-record and observable dependencies, scoped signed sampled Rust flow checks for sign-sensitive unitary, measurement-record, record-backed observable, Pauli-backed observable, inverted Pauli-backed observable, and inverted record-backed observable cases, plus scoped `solve_for_flow_measurements` examples covering empty input, `MX`, measured-idle, multi-target measurement and `MPP`, fewer-measurements heuristic, and repetition-code cases. PFM-B4 replaces bounded subset enumeration with general GF(2) elimination over the supported generator table, independently evidence-closes the selected C++ and Python solver cases, defines Stim-compatible Pauli-projection solving that ignores pre-existing query measurement and observable terms, and keeps unsupported generator shapes on one typed error path regardless of measurement count. Composed generator behavior outside the finite ledger and transform integration remain separately scoped; Python-style signed sampled binding parity remains deferred.
`coverage-util-top-circuit-inverse-qec` owns the selected direct `Circuit::inverse_qec` packets for reset-measure-detector, two-to-one, measure-reset pass-through, MPP, MPAD record tails, noisy measurement and measure-reset, `m_det`, noisy MZZ, observable Pauli includes, the pinned `r_m_det_keep_m` option, selected duplicate-record parity, tags, and coordinates. PFM-B1 supersedes the packet-era `time_reversed_for_flows` bridge with one tracker-driven gate-family engine for the finite nineteen-case Rust scope: selected Clifford, collapse, pair-measurement, MPP, MPAD, detector, observable, coordinate, ordinary-noise, generated-surface, and `dont_turn_measurements_into_resets` semantics share typed reverse transitions and sparse state. Pure unitary repeats stay folded, measurement-rich traversal has a checked one-million-instruction cap, empty nested repeats skip count-proportional work, returned flows validate in one batch, tableau validation has an explicit qubit budget, high maximum qubit ids stay sparse, and absolute/relative record aliases reject like pinned Stim. Feedback, heralded records, and duplicate collapse targets remain fail-closed under the selected scope; behavior outside the finite ledger requires a new exact plan instead of reopening packet-specific dispatch.
...
- Preflight materializing generator entry points before quadratic allocation. Keep the typed Stim-compatible distance domain, but reject a generated family when its exact projected physical-qubit count exceeds 131,072; this admits repetition through distance 2047, rotated surface through 256, unrotated surface through 181, and valid odd color code through 341.
...
- Generator resource tests prove the last accepted and first rejected materialization boundary for each family, rejection happens before output creation, projected-count arithmetic is checked, and maximum folded round counts do not force unrolling. Documentation identifies the surface/color distance difference from pinned Stim as deliberate bounded materialization instead of exact CLI acceptance parity.
...
M9 implements sweep input data for public `m2d` detection conversion through `--sweep` and `--sweep_format`, using all-false sweep bits when `--sweep` is omitted and covering accepted sweep-record formats `01`, `b8`, `r8`, `hits`, `dets`, and input-only `ptb64`. PF3 now permits non-frame and selected frame-path Pauli-observable `detect` sampling to use omitted all-false sweep bits for the current sweep execution subset, including sweep-controlled `CX` and `CY` qubit targets, sweep-controlled `CZ`, `CZ` bit/bit no-op groups, and qubit/sweep `XCZ` or `YCZ` no-op groups in the frame path; it also accepts measurement-record `XCZ` or `YCZ` feedback as X or Y feedback equivalent to `CX` or `CY` feedback. PF3 also source-owns sampler-backed target-order rejection for Stim-parsed but execution-invalid `CX q sweep[k]` and `CY q sweep[k]` circuits while preserving sweep-first `CX` or `CY` and both-order `CZ` sweep/qubit execution. PF3 covers the selected analyzer sweep-control and `CZ` classical-only no-op matrix, including `CZ` sweep/sweep, record/sweep, sweep/record, and record/record groups, with invalid controlled-Pauli target-position rejection, and locks the selected fixed-tableau plus supported Hermitian `SPP` or `SPP_DAG` plus selected deterministic `MPP`, selected stochastic `MPP(p)` sampler and detection-sampling, selected deterministic `MPAD`, selected stochastic `MPAD(p)` sampler and detection-sampling, and selected noisy `MPAD(p)` analyzer gate semantic execution boundary in `docs/plans/pfm3-gate-semantic-boundary-scope.md`. Pinned Stim v1.16.0 does not expose `stim detect --sweep`; analyzer sweep behavior is evidence-closed at the selected matrix, while legal-operation execution is complete for the selected Rust and CLI scope through 37 independently selectable schema-v2 `pfm3-contract-*` cases covering all nineteen semantic families and all eight contract surfaces in `docs/plans/blocker-closure-ledger.json`. Broader sweep target placements and Python detector-sampler sweep APIs remain outside the selected scope and require an explicit new plan before implementation.
M9 now implements scoped feedback-removal conversion for `m2d --ran_without_feedback` by applying `circuit_with_inlined_feedback` before compiling the detection converter. This is not full Python `Circuit.with_inlined_feedback` parity; the PF2 feedback slices add source-owned public-method evidence for pinned demolition feedback, pinned interleaved-ordering, selected `XCZ`/`YCZ` measurement-record feedback equivalent to `CX`/`CY`, selected pinned loop-refolding, and nested bounded-repeat detector-parity cases with bounded repeat work, while broader repeat-contained feedback is under-specified in `docs/plans/milestone-spec-gaps.md` until exact repeat structures, comparator behavior, resource behavior, oracle metadata, and benchmark policy are selected.
...
The M9 detector utility closure promotes simple detecting regions, basic single-record missing-detector suggestions, and MPP feedback inlining into explicit Rust APIs and executable oracle rows. PF5 extends detecting regions with bounded repeat traversal, detector and logical-observable `DemTarget` filters, dense-capped default-like all-target and all-tick helpers, pinned `MX` and `MZZ` examples, generated repetition-code all-target and all-tick selection with selected exact detector and observable regions, selected generated memory-Z and memory-X surface-code all-target/all-tick helper counts plus exact D0, D4, and L0 regions, the full single-qubit Clifford gate set with plain qubit targets, fixed two-qubit tableau-backed Clifford gates with plain qubit-pair targets, selected measurement-record feedback target placements for `CX`, `CY`, `CZ`, `XCZ`, and `YCZ`, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups for `CX`, `CY`, `CZ`, `XCZ`, and `YCZ` plus selected `CZ` classical-only bit-bit groups, explicit fail-closed validation for non-`CZ` sweep/sweep, record/sweep, and record/record groups, inverted targets for promoted measurement and reset-measurement families, `MPAD` measurement pads, heralded record-producing noise record dropping, `MPP` Pauli-product measurement targets, `SPP` and `SPP_DAG` unitary Pauli-product target shapes, ignored anticommutation mode for the current supported subset, selected measurement-gauge ignored-mode behavior, and product-measurement gauge-cancellation behavior; extends the missing-detectors utility with Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer-product cases, record-only observable rows, ignored Pauli observable rows, `MPAD` measurement-pad records, tableau-backed single-qubit and fixed two-qubit Clifford propagation with plain qubit target groups, `SPP` and `SPP_DAG` unitary Pauli-product analysis through decomposition, bounded repeat traversal with explicit expansion caps, selected folded final-repeat traversal for covered deterministic measurement loops with flat or bounded nested local bodies, selected observable-neutral final repeats where top-level record-only observable rows are redundant under independent detector evidence, and the pinned honeycomb and toric generated-code suffix cases; extends unsigned has-flow checking and unsigned has-all-flow helpers with measurement-record and observable dependencies; adds scoped signed sampled Rust flow checking for sign-sensitive unitary, measurement-record, record-backed observable, Pauli-backed observable, inverted Pauli-backed observable, and inverted record-backed observable cases; promotes exact measurement, reset, inverted measure-reset, pair-measurement, Python multi-target measurement and `MPP`, nonconstant and constant `MPP`, pinned variable-target `SPP` and `SPP_DAG` unitary examples, selected unitary-mixed composed measurement-rich instruction sequences, selected all-operation annotation and ordinary-noise no-op traversal, selected composed `SPP` and `SPP_DAG` unitary decomposition, bounded repeat-contained measurement-rich instruction sequences, measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, `MPAD`, and selected single- and multi-target heralded-noise MPP flow generators, plus exact signed-flow-set and checker-satisfaction coverage for the pinned generated all-operations fixture; and promotes pinned Stim `solve_for_flow_measurements` examples for empty input, `MX`, measured-idle identity and collapse, multi-target measurement and `MPP`, fewer-measurements heuristic, and repetition-code measurement solving. PFM-B4 independently locks the named detecting-region, missing-detector, generator, and solver subcases, removes exhaustive measurement-subset solving in favor of generator-table GF(2) elimination, and shares typed reverse-transition classification between generator construction and sparse checking. The PF2 feedback slice promotes selected `XCZ`/`YCZ` measurement-record feedback, selected bounded feedback loop refolding, and selected nested bounded-repeat detector-parity preservation. Broader detecting-region target shapes outside the promoted positive set and source-owned fail-closed set, broader generated-code regions beyond the promoted repetition-code and selected memory-Z and memory-X surface-code cases, broader gauge handling, broader `missing_detectors` utility families outside the promoted evidence including generated-code suffix analysis governed by `docs/plans/pfm5-missing-detectors-generated-boundary-scope.md`, broader folded large-repeat traversal beyond the selected final covered deterministic measurement-loop cases with flat or bounded nested local bodies and selected observable-neutral final repeats, broader repeat-contained feedback beyond selected cases, broader all-operation composed measurement-rich flow generator solving beyond the promoted pinned generated all-operations fixture, no-op, tableau, measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, heralded-record, and `SPP` or `SPP_DAG` decomposition subcases, broader heralded-noise flow-generator synthesis beyond the selected MPP cases, and full transform API parity require explicit future scope; Python-style signed sampled flow binding parity remains deferred.
...
- `cargo test -p stab-core detection` covers coordinate shifts, repeats, measurement-record observables, Pauli-target observable flips, empty-detector circuits, invalid measurement references, bounded record-shape validation, sweep-conditioned conversion with all-false defaults and per-shot sweep records, non-frame and selected frame-path sweep-conditioned detection sampling with omitted all-false sweep bits, and unsupported sweep-shape rejection.
- `cargo test -p stab-core detecting_regions`, `cargo test -p stab-core missing_detectors`, and `cargo test -p stab-core circuit_with_inlined_feedback` cover the M9 and PF5 detector utility closure for simple detecting regions, bounded repeat traversal, detector and logical-observable target filters, basic and extended missing-detector suggestions, and helper-level feedback inlining. `cargo test -p stab-core --test circuit_transforms feedback` covers the PF2 scoped transform evidence including selected `XCZ`/`YCZ` measurement-record feedback inlining, and `cargo test -p stab-core --test circuit_transforms with_inlined_feedback_matches_pinned` covers pinned demolition and interleaved-ordering public-method exact output.
...
Generated surface-code repeat matching, heralded matching, repeat-contained noise stack frames, and full sparse reverse tracker consumption remain deferred or under-specified until promoted into explicit acceptance rows with exact tests, comparators, resource behavior, oracle metadata, and benchmark policy.
...
PF4 later added selected DEM search, ErrorMatcher filter, and SAT/WCNF folding for large zero-shift DEM repeat bodies: graphlike and hypergraph search represent selected detector-touching and detectorless logical-only repeated body errors once in the search graph, skip selected no-target repeated body errors, fold selected zero-detector-shift `shift_detectors 0` repeated bodies, fold selected annotation-bearing repeated bodies containing `detector` or standalone `logical_observable` declarations, skip selected flat and nested annotation-only zero-shift repeated bodies containing `detector`, standalone `logical_observable`, and zero-detector-shift `shift_detectors` declarations including high sparse annotation ids that do not appear on error targets, fold selected nested zero-shift repeated bodies, and fold selected mixed zero-probability plus active repeated body errors while ignoring zero-probability search targets; ErrorMatcher filter DEM traversal represents selected detector-touching, detectorless logical-observable-only, and annotation-bearing repeated filter keys once and skips empty or annotation-only neutral repeats; unweighted shortest-error SAT represents each repeated body error once including zero-probability mechanisms while preserving probability-insensitive structural semantics and skips selected annotation-only zero-shift repeated bodies including high sparse annotation ids that do not appear on error targets; and weighted SAT/WCNF folds repeated body errors by concrete MAP parity cost instead of sampling marginal parity probability while skipping selected annotation-only zero-shift repeated bodies including high sparse annotation ids that do not appear on error targets. Nonzero-shift, shifted nested, non-flat, broader non-annotation mixed-instruction search repeats, shifted active or broader active mixed-instruction ErrorMatcher filter repeats, ErrorMatcher circuit-repeat provenance, and high-index dense-target structural SAT error-target repeat bodies retain explicit caps until separately promoted. Raw numeric `error` targets and separator-only `error` target lists are rejected at DEM validation and are not active search-traversal gaps.
PFM-B3 later consolidated the selected bounded-result consumers onto one checked compact traversal tree and visitor. Counts, shifts, selected coordinates, DEM sampler compilation, graphlike and hypergraph collection, SAT/WCNF collection, and ErrorMatcher filter-key collection share scalar summaries and repeat-selection machinery; coordinate vectors are opt-in and resource-capped. Compact `rounded` and `without_tags` transforms intentionally recurse directly to avoid auxiliary-tree allocation. Materialized flattening, the full detector-coordinate map, sampled-error and replay records, shifted active search state, emitted clauses, and full ErrorMatcher provenance retain tested caps or deferral because their result or algorithmic state is inherently expanded; see `docs/plans/pfm-b3-folded-dem-traversal-progress-report.md`.
...
PF4 later replaced the DEM sampler's flat repeat-operation compilation with a folded operation tree for compile and direct detection-event sampling, skips zero-probability repeat bodies, folds deterministic zero-shift repeats by parity, folds selected single-stochastic zero-shift repeats by odd-parity probability, folds selected flat stochastic zero-shift repeats by per-error odd-parity probability, and folds selected nested zero-shift stochastic repeats by recursive effective-error parity, while sampled-error output and replay intentionally preserve flat repeated error-bit order and keep source-owned materialized width caps.
...
The remaining post-PF4 sampler work is optimized shifted or otherwise non-selected repeated stochastic nonzero-probability direct-sampling traversal that can tighten or remove the current per-occurrence work cap for selected dense-output shapes without changing flat sampled-error semantics.
...
- Qualification-era M12 status: four M9 sweep and feedback conversion rows expanded the inherited matrix to 89 rows. Fresh clean compatibility reports record 78 beta passes, 5 checked no-ratio waivers, and 6 non-comparable beta blockers; 74 timing-threshold passes, 5 timing waivers, 4 unconfigured rows, and 6 non-comparable threshold blockers; and 83 memory passes, 4 missing baselines, and 2 M10 allocation failures. The comprehensive CQ2 through PQ6 program owns these product-row gaps, while PQ1 preserves the commands and gates without treating inherited failures as synthetic-harness defects.
...
The selected non-deferred Rust and CLI blocker program after M12 is complete under [non-deferred-partial-feature-milestones.md](non-deferred-partial-feature-milestones.md), [partial-feature-inventory.md](partial-feature-inventory.md), and [pfm8-rollup-evidence-report.md](pfm8-rollup-evidence-report.md). [partial-feature-closure-plan.md](partial-feature-closure-plan.md) and [remaining-partial-feature-milestones.md](remaining-partial-feature-milestones.md) are retained as historical PF and RPF planning context. The active follow-up is comprehensive qualification of the implemented selected surface under [comprehensive-correctness-qualification-plan.md](comprehensive-correctness-qualification-plan.md), [comprehensive-stim-performance-qualification-plan.md](comprehensive-stim-performance-qualification-plan.md), and [GOAL.md](GOAL.md). The future list below covers intentionally deferred products and full-parity work outside that selected qualification boundary.
Current RPF2 progress implements Rust `Circuit::flattened`, `Circuit::flattened_operations`, `Circuit::without_noise`, `Circuit::decomposed`, the finite selected `Circuit::inverse_qec` packet set, tracker-driven `Circuit::time_reversed_for_flows`, and the selected `Circuit::with_inlined_feedback` surface. PFM-B1 replaces packet-specific flow reversal with one `ReverseFlowTransition` and sparse-map `SparseReverseFrameTracker` engine for the nineteen selected C++ and Python cases, including multi-instruction detector and observable remapping, measurement, reset, measure-reset, pair-measurement, MPP, MPAD, ordinary-noise and coordinate handling, Stim-compatible pair-aware raw target reversal, generated rotated-surface-code reversal, sparse high-qubit IDs, pure-unitary repeat folding, checked measurement-rich repeat expansion, recursively empty-repeat skipping, batched flow validation, and record-alias rejection. Nineteen exact Cargo selectors, thirteen ledger pinned-golden rows, one additional pinned alias-rejection row, focused structural/error rows, a pinned C++ golden recorder with path and digest bindings, and report-only historical-corpus, repeat-free generated-size, MPAD scaling, repeat-scaling, and nonempty-flow sparse-index benchmarks are recorded in `docs/plans/pfm-b1-reverse-flow-progress-report.md`; PFM-B1 is complete with clean committed-HEAD allocation reports from `4f193f19cebf132f7baf0a3aa1cc799a153a71ed`, final audit, and GPT-5.6/max review closure. Feedback, heralded record reversal, duplicate collapse targets, and unselected `inverse_qec`, QASM, Quirk, and Crumble surfaces require a new exact plan instead of reopening the finite PFM-B1 ledger.
Current RPF5 flow progress promotes unsigned has-flow checking and unsigned has-all-flow helpers for measurement-record and observable dependencies with explicit oracle rows `pf5-has-flow-record-observable-rust` and `pf5-has-all-flows-rust` plus report-only benchmark runner `pf5-has-all-flows-batch`, additive unsigned diagnostic checking for output mismatches, input mismatches, out-of-range measurement records, unsupported-circuit reasons, and bool-checker equivalence with explicit oracle row `pf5-has-flow-diagnostics-rust`, scoped signed sampled Rust flow checking with explicit oracle row `pf5-signed-sampled-flows-rust`, exact measurement, reset, inverted measure-reset, pair-measurement, Python multi-target measurement and `MPP`, nonconstant and constant `MPP`, pinned variable-target `SPP` and `SPP_DAG` unitary examples, selected unitary-mixed composed measurement-rich instruction sequences, selected all-operation annotation and ordinary-noise no-op traversal, selected composed `SPP` and `SPP_DAG` unitary decomposition, bounded repeat-contained measurement-rich instruction sequences, measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, `MPAD`, and selected single- and multi-target heralded-noise MPP flow generators with source-specific oracle rows `pf5-flow-generators-measurement-rust` and `pf5-flow-generators-measurement-python-rust` plus report-only benchmark runners `pf5-flow-generators-measurement-rich` and `pf5-flow-generators-measurement-python`, exact signed-flow-set and checker-satisfaction coverage for the pinned generated all-operations fixture, and scoped `solve_for_flow_measurements` examples covering empty input, `MX`, measured-idle, multi-target measurement and `MPP`, fewer-measurements heuristic, and repetition-code solving with source-specific oracle rows `pf5-flow-solve-measurement-rust`, `pf5-flow-solve-measurement-python-rust`, and `pfm-b4-flow-solver-matrix-rust` plus report-only runners `pf5-flow-solve-measurement-rich`, `pf5-flow-solve-measurement-python`, and `pfm-b4-flow-solve-matrix-sizes`. PFM-B4 generalizes the supported solver to GF(2) elimination over generator rows, removes the bounded exhaustive fallback, and locks the selected generator and solver subcases with independent tests. PFM-B1 completes the selected `time_reversed_for_flows` transform integration through one general tracker-driven engine with nineteen distinct selectors, generated surface-code and MPAD matrix evidence, bounded measurement-rich repeats, folded unitary repeats, and report-only allocation coverage; PFM1 exposes Stim v1.16.0 `GateData.flows` metadata for tableau-backed unitary gates plus representative measurement-rich and variable-target gates. Broader composed generator families outside the finite ledger, folded generator traversal beyond current caps, and broader diagnostic wording require an explicit future plan; Python-style signed sampled flow binding parity remains deferred.
PFM-B4 correction evidence additionally gives pure ignored annotation or ordinary-noise circuits a folded identity path, ignores noise during mixed unitary tableau propagation, routes measurement-free mixed classical instructions through typed reverse transitions, excludes `MPAD` pad values from internal simulated-qubit width while preserving public qubit statistics, matches asymmetric `MPAD` record association, supports duplicate reset and measure-reset generator semantics, preserves sweep-only feedback groups only for `CZ`, restores Stim-compatible flow ordering, and makes supported unitary-repeat transform construction proportional to body-touched qubits instead of idle tracker width.
PFM-B5 supersedes the staged RPF6 evidence with a shared shifted-recurrence engine and a finite source-owned closure matrix. The analyzer now discovers transient, period-8, period-127, nested, coordinate, ordinary and nested gauge, repetition-code, and generated-surface behavior without fixture or period dispatch; saturating diagnostics cannot alter analysis success; folded noisy `MPAD` preserves its error mechanism; nested recurrence probing has one cumulative work budget even through supported-unitary nested repeats; and the local decomposition boundary accepts sixteen detector symptoms and rejects seventeen. Ten graphlike, eleven hypergraph, one shared traversal-resource, and twelve selected exact WCNF cases close the finite search surface, including exact zero-probability diagnostics, generated source-mechanism membership, variable-state payload bounds, indexed graph construction, early trivial UNSAT, and low-quantization headers. Sparse detector and observable IDs use touched-slot compression, hypergraph adjacency interns each edge once, two-phase graph admission rejects unique-edge or payload violations before persistent capacity reservation, and expanded mechanisms, per-error and aggregate target occurrences, touched nodes, unique graph edges, persistent graph terms, unique states, attempted transitions, per-state terms, aggregate stored state terms, edge degree, edge incidences, and SAT materialization have independent limits. The conservative SAT output-byte guard remains redundant defense in depth rather than an independently reachable boundary. Selected finite WCNF text remains exact, while sparse-ID and large folded-repeat compression are intentional semantic hardening rather than universal byte-exact claims. Current benchmark ownership is `pfm-b5-analyzer-cycle-folding`, `pfm-b5-analyzer-generated-qec`, the direct and generated graphlike or hypergraph rows, the direct and generated WCNF rows, `pf6-error-decomp-loop-folded`, and `pf6-sparse-rev-frame-loop`. Clean final-audit benchmark evidence identifies `HEAD=4c5901e2eaf03ddf0c8043b5655d943b70b92a70`; milestone-audit and final full-code-review closure are complete. Full ErrorMatcher provenance remains deferred. See `docs/plans/pfm-b5-analyzer-search-progress-report.md`.
...
- Full detector-analysis parity: after the PF plan closes selected non-deferred Rust and CLI subcases, promote any remaining broader detecting-region gauge handling, `missing_detectors` utility families beyond the promoted evidence, generated surface-code repeat matching, heralded matching, repeat-contained noise stack frames, and full sparse reverse tracker consumption into explicit detector-analysis acceptance rows only after exact circuits or generated families, comparators, resource behavior, oracle metadata, and benchmark policy are selected.
...
The original milestone testing rules below remain valid, while [comprehensive-correctness-qualification-plan.md](comprehensive-correctness-qualification-plan.md) now owns the case-level follow-up for every implemented selected Rust and CLI contract.
That plan replaces file-level completion inference with a validated disposition ledger, independently selectable evidence, explicit statistical budgets, hostile-input and resource contracts, and PR, full, and soak tiers.
...
The comprehensive follow-up in [comprehensive-stim-performance-qualification-plan.md](comprehensive-stim-performance-qualification-plan.md) requalifies the existing manifest by implemented feature, requires correctness preflights and equivalent semantic work, replaces asymmetric public CLI claims with process-versus-process evidence, adds a pinned-Stim adapter where existing runners cannot expose a faithful phase, and separates 1.25x timing parity from suite completeness, memory, and scaling conclusions.
