...
The remaining post-PF4 sampler work is optimized repeated nonzero-probability body traversal that can tighten or remove the current sampled-error application work cap without changing flat sampled-error semantics; detector-only zero-probability repeat bodies are skipped by the folded sampler.
