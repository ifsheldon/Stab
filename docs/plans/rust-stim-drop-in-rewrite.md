...
The M9 detector utility closure promotes simple detecting regions, basic single-record missing-detector suggestions, and MPP feedback inlining into explicit Rust APIs and executable oracle rows. PF5 extends the missing-detectors utility with Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer-product cases, record-only observable rows, and ignored Pauli observable rows, extends unsigned has-flow checking with measurement-record and observable dependencies, promotes exact measurement/reset/pair-measurement/feedback/MPAD flow generators, and promotes pinned Stim `solve_for_flow_measurements` examples for empty input, `MX`, idle extra qubits, and repetition-code measurement solving. Broader detecting-region gate and target-shape support, gauge handling, generated-code missing-detector suffix analysis for honeycomb and toric cases, exact feedback-loop refolding, composed measurement-rich flow generator solving, full generator-table measurement solving, and full transform API parity remain future detector-analysis or transform work.
