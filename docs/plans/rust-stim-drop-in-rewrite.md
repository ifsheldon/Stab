...
`coverage-util-top-circuit-inverse-qec` owns the unitary QEC inverse case and explicit direct `circuit_inverse_qec` measurement-rewrite rejection. PF2 and PF5 now own the selected single-instruction measurement-rich `time_reversed_for_flows` bridge, including selected measurement-ordering, selected single-target reset-to-measurement conversion, selected single-target measurement-to-reset conversion, and selected plain unique-target measure-reset flow reversal, while broader reset-only, duplicate or inverted measure-reset, detector, noise, feedback, repeat, and detecting-region rewrites stay deferred or active under the PFM plan.
