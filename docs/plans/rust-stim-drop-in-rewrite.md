...
`coverage-util-top-circuit-inverse-qec` owns the unitary QEC inverse case, and PF2 now promotes the selected direct `circuit_inverse_qec` no-flow reset-measure-detector target-list packet, exact two-to-one `R`/`CX`/`M` detector-flow packet, exact `m_det` two-detector packet, exact noiseless `MPP` plus one-detector all-record identity-parity packet, selected `MPAD` record-tail packet, exact noisy `MZZ` detector-flow packet, exact observable Pauli include packet, selected noisy `M`/`MX`/`MY` measurement-only reversal, selected noisy `MR`/`MRX`/`MRY` measure-reset-only reversal, selected exact noisy measure-reset detector-flow packet, and measure-reset pass-through target-list packet for their selected detector counts, including Stim's pinned `r_m_det`, the exact one-qubit pinned `r_m_det_keep_m` options branch through `InverseQecOptions { keep_measurements: true }`, `two_to_one`, `m_det`, `mpp`, `mpad` empty-flow and Pauli-only flow probes, `mzz`, `obs_include_pauli`, `noisy_m`, `noisy_mr`, `noisy_mr_det`, and `pass_through` shapes, multi-target record remapping where selected, sparse detector subsets for the no-flow packet, duplicate detector-record parity where selected, selected duplicate MPAD record parity, empty-detector behavior where selected, and tag or coordinate preservation where applicable, while broader measurement-rich inverse rewrites are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected. PF2 and PF5 now own the selected measurement-rich `time_reversed_for_flows` bridge, including selected measurement-ordering, selected empty-flow and Pauli-only MPAD record-tail reversal, selected plain reset-to-measurement conversion over one or more unique qubit targets, selected single-target measurement-to-reset conversion including the selected `dont_turn_measurements_into_resets` single-measurement option, selected measure-reset flow reversal over one or more unique qubit targets with inverted result-target support, the selected `MZZ` plus plain-qubit unitary suffix packet matching pinned `flow_through_mzz_h_cx_s`, and the exact pinned `flow_flip` packet, while duplicate reset-only and duplicate measure-reset behaviors stay fail-closed under the scope locked in `docs/plans/pfm2-time-reverse-duplicate-target-boundary-scope.md`, and broader MPAD measurement-record or observable flow terms, MPAD Pauli-observable tails, duplicate MPAD observable-id merging, detector-flow rewrites with interleaved operations beyond the exact selected packets, broader observable-aware QEC inverse rewrites beyond the selected observable Pauli include packet, prior-measurement detector refs, feedback, repeat, and detecting-region rewrites are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
