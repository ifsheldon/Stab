...
Current RPF6 progress promotes the noisy generated-QEC analyzer semantic subset with explicit oracle row `pf6-analyzer-generated-qec-rust` and report-only benchmark runner `pf6-analyze-errors-generated-surface`, the selected loop-folded error-decomposition subset with explicit oracle row `pf6-error-decomp-loop-folded-rust` and report-only benchmark runner `pf6-error-decomp-loop-folded`, the generated-QEC graphlike and hypergraph search subset with explicit oracle row `pf6-search-generated-qec-rust` and report-only benchmark runners `pf6-graphlike-search-generated` and `pf6-hypergraph-search-generated`, and the supported-Clifford sparse reverse tracker unitary-repeat folding subset including the full single-qubit Clifford gate set, fixed two-qubit tableau-backed Clifford gates with plain qubit-pair targets, deterministic generated repeat tests, and nested grouped-target repeat tests, with explicit oracle row `pf6-sparse-rev-unitary-repeat-rust` and report-only benchmark runner `pf6-sparse-rev-frame-loop`; broad generated-loop analyzer behavior, broader loop-folded decomposition families, broader generated search, generated SAT or WCNF evidence, and broader sparse tracker consumption remain governed by the PFM plan.
