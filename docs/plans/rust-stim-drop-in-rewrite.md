...
PF4 later replaced the DEM sampler's flat repeat-operation compilation with a folded operation tree for compile and direct detection-event sampling, skips zero-probability repeat bodies, folds deterministic zero-shift repeats by parity, folds selected single-stochastic zero-shift repeats by odd-parity probability, and folds selected flat stochastic zero-shift repeats by per-error odd-parity probability, while sampled-error output and replay intentionally preserve flat repeated error-bit order and keep source-owned materialized width caps.
...
The remaining post-PF4 sampler work is optimized nested or shifted repeated stochastic nonzero-probability direct-sampling traversal that can tighten or remove the current per-occurrence work cap for selected dense-output shapes without changing flat sampled-error semantics.
