...
Current RPF6 progress promotes the noisy generated-QEC analyzer semantic subset with explicit oracle row `pf6-analyzer-generated-qec-rust` and report-only benchmark runner `pf6-analyze-errors-generated-surface`, the selected loop-folded error-decomposition subset with explicit oracle row `pf6-error-decomp-loop-folded-rust` and report-only benchmark runner `pf6-error-decomp-loop-folded`, the generated-QEC graphlike and hypergraph search subset with explicit oracle row `pf6-search-generated-qec-rust` and report-only benchmark runners `pf6-graphlike-search-generated` and `pf6-hypergraph-search-generated`, the selected generated-QEC SAT/WCNF structural subset with explicit oracle row `pf6-search-generated-sat-wcnf-rust` and report-only benchmark runner `pf6-generated-sat-wcnf`, the supported-Clifford sparse reverse tracker unitary-repeat folding subset including the full single-qubit Clifford gate set, fixed two-qubit tableau-backed Clifford gates with plain qubit-pair targets, deterministic generated repeat tests, and nested grouped-target repeat tests, and the unsigned sparse-tracker `SPP`/`SPP_DAG` product propagation subset with explicit oracle row `pf6-sparse-rev-spp-rust`, with report-only benchmark runner `pf6-sparse-rev-frame-loop` remaining scoped to loop folding; broad generated-loop analyzer behavior, broader loop-folded decomposition families, broader generated search, broader generated SAT or WCNF families, and broader sparse tracker consumption remain governed by the PFM plan.
