...
`coverage-util-top-circuit-inverse-qec` owns the unitary QEC inverse case, and PF2 now promotes the selected direct `circuit_inverse_qec` no-flow reset-measure-detector target-list packet, exact two-to-one `R`/`CX`/`M` detector-flow packet, exact `m_det` two-detector packet, exact noiseless `MPP` plus one-detector all-record identity-parity packet, selected direct `MPAD` record-tail packet, exact noisy `MZZ` detector-flow packet, exact observable Pauli include packet, selected noisy `M`/`MX`/`MY` measurement-only reversal, selected noisy `MR`/`MRX`/`MRY` measure-reset-only reversal, selected exact noisy measure-reset detector-flow packet, and measure-reset pass-through target-list packet for their selected detector counts, including Stim's pinned `r_m_det`, the exact one-qubit pinned `r_m_det_keep_m` options branch through `InverseQecOptions { keep_measurements: true }`, `two_to_one`, `m_det`, `mpp`, direct `mpad` inverse behavior, `mzz`, `obs_include_pauli`, `noisy_m`, `noisy_mr`, `noisy_mr_det`, and `pass_through` shapes, multi-target record remapping where selected, sparse detector subsets for the no-flow packet, duplicate detector-record parity where selected, selected duplicate MPAD record parity, selected duplicate MPAD observable-id record parity, empty-detector behavior where selected, and tag or coordinate preservation where applicable, while broader measurement-rich inverse rewrites are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected. PF2 and PF5 now own the selected measurement-rich `time_reversed_for_flows` bridge, including selected measurement-ordering, selected empty-flow plus Pauli-only, measurement-record, and observable MPAD record-tail reversal, selected duplicate MPAD observable-id record parity, selected plain reset-to-measurement conversion over one or more unique qubit targets, selected single-target measurement-to-reset conversion including the selected `dont_turn_measurements_into_resets` single-measurement option, selected measure-reset flow reversal over one or more unique qubit targets with inverted result-target support, the selected `MZZ` plus plain-qubit unitary suffix packet matching pinned `flow_through_mzz_h_cx_s`, and the exact pinned `flow_flip` packet, while duplicate reset-only and duplicate measure-reset behaviors stay fail-closed under the scope locked in `docs/plans/pfm2-time-reverse-duplicate-target-boundary-scope.md`, and broader MPAD observable flow terms outside selected record-only tails, MPAD Pauli-observable tails, duplicate MPAD observable-id merging with non-record targets, detector-flow rewrites with interleaved operations beyond the exact selected packets, broader observable-aware QEC inverse rewrites beyond the selected observable Pauli include packet, prior-measurement detector refs, feedback, repeat, and detecting-region rewrites are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
...
Non-deferred partial Rust and CLI surfaces after M12 are governed by [non-deferred-partial-feature-milestones.md](non-deferred-partial-feature-milestones.md), [partial-feature-inventory.md](partial-feature-inventory.md), and [GOAL.md](GOAL.md). [partial-feature-closure-plan.md](partial-feature-closure-plan.md) and [remaining-partial-feature-milestones.md](remaining-partial-feature-milestones.md) are retained as historical PF and RPF planning context. The future list below covers intentionally deferred surfaces and any full-parity work that remains outside those PFM milestones.
Current RPF2 progress implements Rust `Circuit::flattened`, `Circuit::flattened_operations`, `Circuit::without_noise`, `Circuit::decomposed`, scoped `Circuit::inverse_qec` packets including selected no-flow reset-measure-detector, exact two-to-one detector-flow, exact `m_det` two-detector packet, exact noiseless MPP all-record identity-parity detector-flow, selected MPAD record-tail reversal, exact noisy MZZ detector-flow, exact observable Pauli include, selected noisy `M`/`MX`/`MY` measurement-only reversal, selected noisy `MR`/`MRX`/`MRY` measure-reset-only reversal, selected exact noisy measure-reset detector-flow, and measure-reset pass-through behavior, scoped unitary and selected measurement-rich `Circuit::time_reversed_for_flows`, including selected measurement-ordering, selected empty-flow plus Pauli-only, measurement-record, and observable MPAD record-tail reversal, selected duplicate MPAD observable-id record parity, selected plain `R`/`RX`/`RY` reset-to-measurement conversion over one or more unique qubit targets, selected single-target measurement-to-reset conversion including the selected `TimeReversedForFlowsOptions { dont_turn_measurements_into_resets: true }` single-measurement option, selected `MR`/`MRX`/`MRY` measure-reset flow reversal over one or more unique qubit targets with inverted result-target support, the selected `MZZ` plus plain-qubit unitary suffix packet, and the exact pinned `flow_flip` packet, and scoped `Circuit::with_inlined_feedback` with source-owned tests, oracle metadata, and report-only benchmark runners for the performance-sensitive flatten, noise-removal, decomposition, unitary and selected measurement-rich flow-time-reversal, and feedback-inlining transforms, including selected MPP or pair-measurement decomposition flow-generator preservation, pinned demolition feedback and interleaved-ordering public-method exact output, selected `XCZ`/`YCZ` measurement-record feedback, selected bounded feedback loop refolding, and selected nested bounded-repeat feedback detector-parity preservation. Broader repeat-contained feedback, decomposition flow semantics, detector-flow QEC inverse rewrites, MPAD observable flow terms outside selected record-only tails, MPAD Pauli-observable or non-record duplicate-observable-id rewrites, observable-aware QEC inverse rewrites beyond the selected packet, and measurement-rich flow-time-reversal are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
Current RPF5 flow progress promotes unsigned has-flow checking and unsigned has-all-flow helpers for measurement-record and observable dependencies with explicit oracle rows `pf5-has-flow-record-observable-rust` and `pf5-has-all-flows-rust` plus report-only benchmark runner `pf5-has-all-flows-batch`, additive unsigned diagnostic checking for output mismatches, input mismatches, out-of-range measurement records, unsupported-circuit reasons, and bool-checker equivalence with explicit oracle row `pf5-has-flow-diagnostics-rust`, scoped signed sampled Rust flow checking with explicit oracle row `pf5-signed-sampled-flows-rust`, exact measurement, reset, inverted measure-reset, pair-measurement, Python multi-target measurement and `MPP`, nonconstant and constant `MPP`, pinned variable-target `SPP` and `SPP_DAG` unitary examples, selected unitary-mixed composed measurement-rich instruction sequences, selected all-operation annotation and ordinary-noise no-op traversal, selected composed `SPP` and `SPP_DAG` unitary decomposition, bounded repeat-contained measurement-rich instruction sequences, measurement-record feedback, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, `MPAD`, and selected single- and multi-target heralded-noise MPP flow generators with source-specific oracle rows `pf5-flow-generators-measurement-rust` and `pf5-flow-generators-measurement-python-rust` plus report-only benchmark runners `pf5-flow-generators-measurement-rich` and `pf5-flow-generators-measurement-python`, exact signed-flow-set and checker-satisfaction coverage for the pinned generated all-operations fixture, and scoped `solve_for_flow_measurements` examples covering empty input, `MX`, measured-idle, multi-target measurement and `MPP`, fewer-measurements heuristic, and repetition-code solving with source-specific oracle rows `pf5-flow-solve-measurement-rust`, `pf5-flow-solve-measurement-python-rust`, and `pfm-b4-flow-solver-matrix-rust` plus report-only runners `pf5-flow-solve-measurement-rich`, `pf5-flow-solve-measurement-python`, and `pfm-b4-flow-solve-matrix-sizes`. PFM-B4 generalizes the supported solver to GF(2) elimination over generator rows, removes the bounded exhaustive fallback, and locks the selected generator and solver subcases with independent tests. Selected measurement-rich `time_reversed_for_flows` examples include selected measurement ordering, selected empty-flow plus Pauli-only, measurement-record, and observable MPAD record-tail reversal with selected duplicate MPAD observable-id record parity, selected plain reset-to-measurement conversion over one or more unique qubit targets, selected single-target measurement-to-reset conversion including the selected `dont_turn_measurements_into_resets` single-measurement option, selected measure-reset reversal over one or more unique qubit targets with inverted result-target support, the selected `MZZ` plus plain-qubit unitary suffix packet, and the exact pinned `flow_flip` packet with explicit oracle rows `pf2-inverse-qec-mpad-rust`, `pf2-time-reverse-flow-measurement-rust`, and `pf2-time-reverse-flow-mzz-unitary-suffix-rust` plus report-only benchmark runner `pf2-time-reverse-flow-measurement`; PFM1 exposes Stim v1.16.0 `GateData.flows` metadata for tableau-backed unitary gates plus representative measurement-rich and variable-target gates. Broader composed generator families outside the finite ledger, folded repeat traversal beyond current caps, broader diagnostic wording, and broader transform integration require an explicit future plan; Python-style signed sampled flow binding parity remains deferred.
...
Current RPF6 progress promotes the noisy generated-QEC analyzer semantic subset with explicit oracle row `pf6-analyzer-generated-qec-rust` and report-only benchmark runner `pf6-analyze-errors-generated-surface`, the selected prefix/repeat/tail detector-chain true-folded analyzer subset with explicit oracle row `pf6-analyzer-prefix-repeat-tail-folding-rust`, the selected loop-carried observable true-folded analyzer subset with explicit oracle rows `pf6-analyzer-loop-carried-observable-rust` and `pf6-analyze-errors-loop-carried-observable-cli` plus report-only benchmark runner `pf6-analyzer-loop-observable-folded`, the selected period-8 observable true-folded analyzer subset with explicit oracle rows `pf6-analyzer-period8-observable-rust` and `pf6-analyze-errors-period8-observable-cli` plus report-only benchmark runner `pf6-analyzer-period8-observable-folded`, the selected period-127 observable true-folded analyzer subset with explicit oracle rows `pf6-analyzer-period127-observable-rust` and `pf6-analyze-errors-period127-observable-cli` plus report-only benchmark runner `pf6-analyzer-period127-observable-folded`, the selected loop-folded error-decomposition subset with explicit oracle row `pf6-error-decomp-loop-folded-rust` and report-only benchmark runner `pf6-error-decomp-loop-folded`, the bounded mixed-top-level `fold_loops=true` fallback with explicit oracle rows `pf6-analyzer-mixed-top-level-fallback-rust` and `pf6-analyzer-generated-fold-loop-fallback-rust`, the generated-QEC graphlike and hypergraph search subset with explicit oracle row `pf6-search-generated-qec-rust` and report-only benchmark runners `pf6-graphlike-search-generated` and `pf6-hypergraph-search-generated`, the selected generated-QEC SAT/WCNF structural subset with explicit oracle row `pf6-search-generated-sat-wcnf-rust` and report-only benchmark runner `pf6-generated-sat-wcnf`, the supported-Clifford sparse reverse tracker unitary-repeat folding subset including the full single-qubit Clifford gate set, fixed two-qubit tableau-backed Clifford gates with plain qubit-pair targets, deterministic generated repeat tests, nested grouped-target repeat tests, and touched-qubit transform construction with 65,536-wide idle and sparse high-active suffix evidence, the shifted-copy sparse reverse tracker measurement/detector repeat subset with explicit oracle row `pf6-sparse-rev-shifted-repeat-rust`, the unsigned sparse-tracker `SPP`/`SPP_DAG` product propagation subset with explicit oracle row `pf6-sparse-rev-spp-rust`, and the selected matched-error canonicalization hardening with explicit oracle row `pf6-matched-error-canonicalize-rust`, with report-only benchmark runner `pf6-sparse-rev-frame-loop` remaining scoped to loop folding and now including high-idle-width allocation evidence; true folded generated-loop analyzer behavior beyond the selected detector-chain, loop-carried observable, period-8 observable, and period-127 observable slices, broader loop-folded decomposition families, broader generated search, broader generated SAT or WCNF families, broader sparse tracker consumption, and broader matched-error hardening are under-specified in `docs/plans/milestone-spec-gaps.md` until exact subcases are selected.
