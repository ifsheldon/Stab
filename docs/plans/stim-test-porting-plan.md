...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 2,065 default-feature public Rust API items and 1,757 evidence parents in `oracle/qualification-manifest.json`.
Current generated parent counts and inventory identities live in [../qualification-status.md](../qualification-status.md) instead of this historical porting ledger. Source ownership across the eight CQ2 deterministic domains remains exact and independently selectable. Clean revision `68d107a42f655254f31628f0cbedc55479f6c0f3` passed 406 PR executions and 612 full and soak executions after the strict result-format grammar, typed DETS, CLI propagation, and file-role repairs, so the corrected result-format slice is no longer reopened. Qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. Algebra selects portable Pauli, Clifford, Flow, Tableau, iterator, stabilizer-solving, scoped unitary-to-Tableau, and typed resource-admission semantics while separating Python products and interactive simulator conversions. This is semantic mapping, not file-level coverage promotion.
The Python inventory expands 113 static parameter subcases, keeps 727 unparameterized cases, and records 23 dynamic parameter families as non-executable content-addressed families; no dynamic family contributes passing evidence.
The upstream record count remains case-based even when one exact command or Python semantic case is relevant to more than one CQ domain; 651 current records carry explicit multi-domain relevance instead of being forced into a file-level bucket, while deferred records carry no executable ownership.
The resource inventory contains two implemented qualification cases and thirteen independent planned boundary families covering admission, buffering, failures, traversal, allocation, paths, and output lifecycle; later CQ milestones must implement those owners instead of treating resource non-emptiness as completion.
...
Milestone PQ0 reconciles all 23 upstream perf files, all 74 discovered `BENCHMARK(...)` symbols, every existing `benchmarks/manifest.csv` row, all 127 checklist rows, and all 2,065 CQ0 public API items in `benchmarks/stim-qualification-suite.json`.
Its source-current 9 retained, 133 reworked, 4 diagnostic, 13 superseded, and 2 removed row decisions are inventory evidence only. PQ1 has completed the symmetric paired harness, canonical CQ preflight consumer, process-memory evidence, host and toolchain replay, and report pipeline; PQ2 through PQ7 must apply that infrastructure to product workloads and close the recorded correctness-preflight, output-digest, comparator, scale, memory, and statistical gaps before a product benchmark group counts as comprehensive Stim evidence. The shared process supervisor and `raw-work-v2` worker contract are source-complete in the active repair, but formal reproducibility and product evidence wait for a clean committed revision.
...
- Port each complete pinned family case as one exact Rust contract, including noisy circuit text, layout text, and generator hint, instead of accepting a nearby broad fixture as ownership.
- Port the upstream no-noise distance, round, family, and task matrix with exact detector-count formulas, one observable, absence of noise instructions, and deterministic error-free analyzer output for every valid cell. Retain representative sampled execution for every task, the pinned d5/r10 detector-shape case, and a portable multiword batch instead of using repeated random shots as the proof of a deterministic invariant.
- Cover the typed parameter surface independently: distance and round boundaries, all task enums, all three parameter objects, immutable noise builders, generated-circuit value semantics and accessors, invalid color-family combinations, and maximum folded round counts.
- Cover materialization admission at the public generator and CLI boundaries. Prove the last accepted and first valid rejected distance for each family, checked projected-count arithmetic, accepted projection-to-materialization cardinality, constant allocation bounds on rejected calls, rejection before output creation, and the deliberate 131,072 projected-physical-qubit cap.
- Keep `circuit_pybind_test.py::test_circuit_generation_errors` deferred as a complete Python product symbol because it enters through Python `Circuit.generated` string dispatch; do not use that deferral to avoid independently owning portable typed Rust validation.
...
- Current PFM-B5 closure covers ten graphlike, eleven hypergraph, one shared traversal-resource, and twelve selected exact shortest or weighted WDIMACS ledger cases through `crates/stab-core/tests/dem_search_pfm_b5.rs` and focused graphlike or hypergraph unit regressions. Generated tie-sensitive results use minimum-distance and canonical target-signature invariants plus source-mechanism membership; deterministic direct results, exact zero-probability diagnostics, and finite WCNF text use exact assertions, including Stim's stored-clause header for a low-quantization clause whose output line is omitted. Resource tests independently cross mechanism, per-error and aggregate target work, touched-node, unique graph-edge, persistent graph-payload, search-state, transition, per-state term, aggregate stored-state term, hyperedge-degree, edge-incidence, SAT total-target, clause, and literal admission boundaries; the SAT output-byte guard remains helper-tested defense in depth, and sparse-ID or large folded-repeat WCNF compression intentionally proves semantic equivalence instead of universal byte identity.
...
- PFM-B2 closes the selected gate-execution extraction through 37 independently selectable cases sourced from exact anchors in `frame_simulator.test.cc`, `tableau_simulator.test.cc`, and `error_analyzer.test.cc`. The cases cover all nineteen canonical semantic families across parser, measurement sampler, reference sampler, detection converter, detector frame, detection sampler, error analyzer, and flow generator using exact, structural, rejection, state-equivalence, semantic-invariant, or source-owned statistical comparators; broader public simulator products remain deferred.
...
- The historical M12 graph/vector cross-check was removed in P2 because it hand-rolled test-only state-vector logic and duplicated the owned circuit-to-tableau and unitary suites without implementing either deferred simulator product. Public graph/vector simulator APIs remain deferred until Stab exposes equivalent surfaces.
...
- P2 Future: amplitude and state-vector checks are deferred unless a later matrix/state-vector parity plan selects exact product behavior. Existing algebra and circuit-to-tableau tests must not be relabeled as public simulator evidence.
