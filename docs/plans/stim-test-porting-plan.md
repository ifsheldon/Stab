...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,974 default-feature public Rust API items and 1,744 evidence parents in `oracle/qualification-manifest.json`.
...
Milestone PQ0 reconciles all 23 upstream perf files, all 74 discovered `BENCHMARK(...)` symbols, every existing `benchmarks/manifest.csv` row, all 127 checklist rows, and all 1,974 CQ0 public API items in `benchmarks/stim-qualification-suite.json`.
