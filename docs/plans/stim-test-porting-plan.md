...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 1,877 directly extracted C++ cases, 844 directly extracted Python semantic records, explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, and 165 exact blocker-ledger subcases, plus 1,922 default-feature public Rust API items and 4,099 evidence owners in `oracle/qualification-manifest.json`.
Of those upstream records, 165 are bound to ported Rust evidence, 2,069 remain semantic-mining work, 640 belong to twelve named deferred products, and 12 are not applicable; 604 evidence owners reference current implemented or evidence-close primary evidence and 3,495 remain planned for CQ2 through CQ5.
The Python inventory expands 94 directly extracted static parameter cases, keeps 727 unparameterized direct cases, and records 23 dynamic parameter families as non-executable content-addressed families; no dynamic family contributes passing evidence.
