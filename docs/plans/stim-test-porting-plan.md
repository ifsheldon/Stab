...
The selected MPAD record-tail inverse and Pauli-only time-reversal evidence is tracked by `pf2-inverse-qec-mpad-rust` and is derived from pinned `time_reversed_for_flows` probes plus upstream `CircuitFlowReverser::do_measuring_instruction` source inspection, with the exact probe transcript recorded in `docs/plans/pfm2-inverse-qec-mpad-scope.md`.
