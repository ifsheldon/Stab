...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,968 default-feature public Rust API items and 2,145 evidence owners in `oracle/qualification-manifest.json`.
Of those upstream records, 465 are bound to ported Rust evidence, 1,198 remain semantic-mining inputs, 926 belong to named deferred products, and 297 are not applicable; 524 evidence owners are implemented, 17 are evidence-close, and 1,604 remain planned for CQ2 through CQ6. The completed selected CQ2 `.stim`, `.dem`, result-format, gate-contract, bit-kernel, circuit-API, and Generation slices have 29, 28, 39, 59, 12, 24, and 25 implemented evidence parents respectively and zero planned owners. Their qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. The Algebra reconciliation selects portable Pauli, Clifford, Flow-value, Tableau, iterator, stabilizer-solving, scoped unitary-to-Tableau, and typed resource-admission semantics while separating Python products, interactive simulator conversions, and unexposed C++ helpers. This is semantic mapping, not file-level coverage promotion.
...
Milestone PQ0 reconciles all 23 upstream perf files, all 74 discovered `BENCHMARK(...)` symbols, every existing `benchmarks/manifest.csv` row, all 127 checklist rows, and all 1,968 CQ0 public API items in `benchmarks/stim-qualification-suite.json`.
Its 15 retained, 135 reworked, 4 diagnostic, 5 superseded, and 2 removed row decisions are inventory evidence only. PQ1 has completed the symmetric paired harness, canonical CQ preflight consumer, process-memory evidence, host and toolchain replay, and report pipeline; PQ2 through PQ7 must apply that infrastructure to product workloads and close the recorded correctness-preflight, output-digest, comparator, scale, memory, and statistical gaps before a product benchmark group counts as comprehensive Stim evidence.
