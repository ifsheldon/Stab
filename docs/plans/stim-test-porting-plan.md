...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,972 default-feature public Rust API items and 1,744 evidence parents in `oracle/qualification-manifest.json`.
Of those upstream records, 477 are bound to ported Rust evidence, 1,178 remain semantic-mining inputs, 925 belong to named deferred products, and 306 are not applicable; 580 evidence parents are implemented, 17 are evidence-close, and 1,147 remain planned for CQ3 through CQ6. Completed CQ2 has 271 implemented exact parents and zero planned parents across `.stim` 29, `.dem` 28, result formats 39, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. Their qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. Algebra selects portable Pauli, Clifford, Flow, Tableau, iterator, stabilizer-solving, scoped unitary-to-Tableau, and typed resource-admission semantics while separating Python products and interactive simulator conversions. This is semantic mapping, not file-level coverage promotion.
...
Milestone PQ0 reconciles all 23 upstream perf files, all 74 discovered `BENCHMARK(...)` symbols, every existing `benchmarks/manifest.csv` row, all 127 checklist rows, and all 1,972 CQ0 public API items in `benchmarks/stim-qualification-suite.json`.
Its 12 retained, 135 reworked, 4 diagnostic, 8 superseded, and 2 removed row decisions are inventory evidence only after the exact transpose groups superseded the heterogeneous legacy BitMatrix timing row. PQ1 has completed the symmetric paired harness, canonical CQ preflight consumer, process-memory evidence, host and toolchain replay, and report pipeline; PQ2 through PQ7 must apply that infrastructure to product workloads and close the recorded correctness-preflight, output-digest, comparator, scale, memory, and statistical gaps before a product benchmark group counts as comprehensive Stim evidence.
