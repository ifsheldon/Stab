...
Of those upstream records, 465 are bound to ported Rust evidence, 1,398 remain semantic-mining inputs, 820 belong to named deferred products, and 203 are not applicable; 524 evidence owners are implemented, 17 are evidence-close, and 1,800 remain planned for CQ2 through CQ6. The completed selected CQ2 `.stim`, `.dem`, result-format, gate-contract, bit-kernel, circuit-API, and Generation slices have 29, 28, 39, 59, 12, 24, and 25 implemented evidence parents respectively and zero planned owners. Their qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. This is semantic mapping, not file-level coverage promotion.
