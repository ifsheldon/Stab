...
Of those upstream records, 477 are bound to ported Rust evidence, 1,178 remain semantic-mining inputs, 925 belong to named deferred products, and 306 are not applicable; 593 evidence parents are implemented, 17 are evidence-close, and 1,147 remain planned for CQ3 through CQ6. Source ownership across the eight CQ2 deterministic domains now has 275 implemented exact parents and zero planned parents: `.stim` 29, `.dem` 28, result formats 43, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. The prior clean all-domain execution covered 271 parents under an older inventory; the result-format slice is reopened until its corrected strict grammar, typed DETS, CLI propagation, and file-role cases run from a clean committed revision. Qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. Algebra selects portable Pauli, Clifford, Flow, Tableau, iterator, stabilizer-solving, scoped unitary-to-Tableau, and typed resource-admission semantics while separating Python products and interactive simulator conversions. This is semantic mapping, not file-level coverage promotion.
