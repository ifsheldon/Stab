...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,922 default-feature public Rust API items and 4,009 evidence owners in `oracle/qualification-manifest.json`.
Of those upstream records, 165 are bound to ported Rust evidence, 2,071 remain semantic-mining work, 641 belong to twelve named deferred products, and nine are not applicable; 424 evidence owners are implemented, 17 are evidence-close, and 3,568 remain planned for CQ2 through CQ5. The completed selected CQ2 `.stim` slice has 32 implemented evidence parents and zero planned owners: 24 exact qualification parents map 44 upstream owners and nine exported-API owners, while eight independently selected oracle-fixture parents remain direct evidence. This is semantic mapping, not file-level coverage promotion.
...
The upstream record count remains case-based even when one exact command or Python semantic case is relevant to more than one CQ domain; 657 current records carry explicit multi-domain relevance instead of being forced into a file-level bucket, while deferred records carry no executable ownership.
