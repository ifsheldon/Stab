...
Of those upstream records, 165 are bound to ported Rust evidence, 2,051 remain semantic-mining work, 659 belong to twelve named deferred products, and 11 are not applicable; 440 evidence owners are implemented, 17 are evidence-close, and 3,434 remain planned for CQ2 through CQ5. The completed selected CQ2 `.stim` and `.dem` slices have 32 and 28 implemented evidence parents respectively and zero planned owners. Their qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. This is semantic mapping, not file-level coverage promotion.
