...
Current `src/stim/util_top/circuit_inverse_qec.test.cc` ownership is split across structural rows for the unitary QEC inverse subset, selected reset-measure-detector target-list rewrites, the exact two-to-one detector-flow packet, the exact noiseless MPP all-record identity-parity detector-flow packet, selected noisy `M`/`MX`/`MY` measurement-only reversal, selected noisy `MR`/`MRX`/`MRY` measure-reset-only reversal, selected measure-reset pass-through rewrites, selected measurement-rich `time_reversed_for_flows`, and explicit fail-closed nearby measurement-rewrite shapes.
