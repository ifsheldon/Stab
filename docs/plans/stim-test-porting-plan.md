...
Its 14 retained, 135 reworked, 4 diagnostic, 6 superseded, and 2 removed row decisions are inventory evidence only. PQ1 has completed the symmetric paired harness, canonical CQ preflight consumer, process-memory evidence, host and toolchain replay, and report pipeline; PQ2 through PQ7 must apply that infrastructure to product workloads and close the recorded correctness-preflight, output-digest, comparator, scale, memory, and statistical gaps before a product benchmark group counts as comprehensive Stim evidence.
