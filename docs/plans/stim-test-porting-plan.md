...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,922 default-feature public Rust API items and 4,070 evidence owners in `oracle/qualification-manifest.json`.
Of those upstream records, 165 are bound to ported Rust evidence, 2,072 remain semantic-mining work, 640 belong to twelve named deferred products, and nine are not applicable; 391 evidence owners are implemented, 17 are evidence-close, and 3,662 remain planned for CQ2 through CQ5.
