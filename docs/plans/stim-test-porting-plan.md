...
CQ0 discovery currently freezes 2,886 upstream records from all 103 C++ and 91 Python files, including 2,018 GTest records, 863 pytest semantic records, five source-symbol records, and explicit 64-bit, 128-bit, and 256-bit Stim word-size subcases, plus 1,922 default-feature public Rust API items and 3,891 evidence owners in `oracle/qualification-manifest.json`.
Of those upstream records, 165 are bound to ported Rust evidence, 2,051 remain semantic-mining work, 659 belong to twelve named deferred products, and 11 are not applicable; 476 evidence owners are implemented, 17 are evidence-close, and 3,223 remain planned for CQ2 through CQ5. The completed selected CQ2 `.stim`, `.dem`, and result-format slices have 32, 28, and 39 implemented evidence parents respectively and zero planned owners. Their qualification parents map exact selected upstream and exported-API owners while independently selected oracle-fixture and blocker-ledger parents remain direct evidence. This is semantic mapping, not file-level coverage promotion.
