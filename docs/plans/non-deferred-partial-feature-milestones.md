...
- Add exact positive tests for the locked honeycomb and toric generated-code missing-detector suffix cases, and add positive and negative tests for MPP products, observables, row-reduction behavior, Clifford propagation, `SPP` and `SPP_DAG` decomposition parity, repeat traversal, and unknown-input behavior. Do not add broader generated-code missing-detector negative or resource-boundary tests until an exact-subcase plan selects generated families, suffix comparators, resource behavior, oracle metadata, and benchmark policy.
