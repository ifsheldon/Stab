...
Status: Complete as of 2026-07-12. Production, executable-evidence, graph-cap, indexed-edge, and pre-reservation admission remediation is committed through `4c5901e`; all 52 cases have independent exact selectors, milestone-audit and full-code-review findings are closed, and clean PF6 allocation evidence identifies `HEAD=4c5901e2eaf03ddf0c8043b5655d943b70b92a70` with `local_modifications=false`, as recorded in `docs/plans/pfm-b5-analyzer-search-progress-report.md`.
...
- Retain the existing prefix/repeat/tail, huge odd observable, period-8, period-127, decomposition, remnant-edge, generated surface-coordinate generic reverse-fold, and folded-observable guard evidence.
- Prove nested billion-round gauge folding, saturating observational diagnostics, folded noisy `MPAD`, cumulative nested-probe admission including supported-unitary nested repeats, and the inclusive sixteen-detector local-decomposition boundary with a separately selected seventeen-detector rejection case.
...
- Use exact output for the selected finite WCNF corpus and deterministic DEM text, and canonical target-set or minimum-weight structural comparators for tie-sensitive search output. Generated structural comparators must additionally prove each returned mechanism exists in the source DEM. Treat sparse-ID and large folded-repeat WCNF compression as an intentional resource-hardening semantic deviation, not a universal byte-exact Stim claim.
- Route folded DEM access through PFM-B3; independently cap expanded nonzero mechanisms, per-error and aggregate source target occurrences, touched nodes, unique graph edges, persistent graph terms, search states, attempted transitions, per-state terms, aggregate stored state terms, hyperedge degree, edge incidences, SAT mechanisms, total target occurrences, variables, clauses, and literals. Retain the conservative SAT output-byte bound as redundant defense in depth instead of claiming that it is independently reachable behind stricter clause and literal limits.
...
- Use one shared shifted-recurrence summary in analyzer and sparse reverse-tracker repeat consumers instead of maintaining separate cycle-search loops.
...
- `pfm-b5-graphlike-generated-d25` and `pfm-b5-graphlike-generated-d11-r1000` are faithful pinned-Stim direct matches; clean final-audit ratios of 4.647x and 4.279x keep them report-only and outside the 1.25x gate.
...
- Every one of the 52 owned cases has an independently resolving exact Cargo selector; direct CLI fixtures bind both path and SHA-256 while still comparing live pinned Stim with Stab.
- Search and SAT resource limits fail with domain errors before uncontrolled traversal, graph construction, state, edge-incidence, clause, or literal growth; tests cross every independently claimed boundary, while the SAT output-byte guard remains tested as defense in depth at helper level.
...
- Keep `pf6-analyzer-mixed-top-level-fallback-rust` as evidence only for unsafe or still-unsupported bounded non-folded fallback, analyzer-budget cap preservation, and the selected folded-error non-masking guard it names.
- Keep `pf6-analyzer-generated-fold-loop-coordinates-rust` as evidence only for the selected generated surface-code coordinate comparison through the generic reverse-fold path with diagnostics proving bounded fallback was not used.
