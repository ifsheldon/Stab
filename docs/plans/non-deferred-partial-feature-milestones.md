...
- Promote `Circuit::inverse_qec` beyond the unitary subset only for selected no-flow QEC inverse cases with exact owned subcases. The selected noiseless plain reset-measure-detector target-list shape with one detector is implemented, including pinned `r_m_det`, multi-target record remapping, sparse detector subsets, duplicate detector-record parity, and empty-detector behavior, while broader detector-flow rewrites with interleaved operations, noisy measurement/reset, feedback, observables, repeats, and multi-instruction QEC inverse behavior remain active.
