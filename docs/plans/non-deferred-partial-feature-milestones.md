...
- Promote `Circuit::inverse_qec` beyond the unitary subset only for selected no-flow QEC inverse cases with exact owned subcases. The selected noiseless plain single-target reset-measure-detector triplet matching Stim's pinned `r_m_det` shape is implemented, while broader detector-flow rewrites, multi-target reset-measure-detector parity, noisy measurement/reset, feedback, observables, repeats, and multi-instruction QEC inverse behavior remain active.
