...
- Do not add a separate benchmark row for selected annotation-only search/SAT/WCNF repeat skipping unless it becomes a measurable hot path; the current slice is admission/resource evidence for avoiding repeat-count-scaled work.
...
- Keep exact positive and negative tests for the promoted `missing_detectors` row-reduction, Pauli-product, observable, Clifford-propagation, `SPP` and `SPP_DAG`, bounded repeat, selected folded final-repeat, honeycomb, and toric suffix evidence. Do not add broader MPP, pair-measurement, observable, gauge, Clifford-propagation, repeat-traversal, folded large-repeat, row-reduction, unknown-input, or generated-code missing-detector tests until an exact-subcase plan selects circuits, suffix comparators or source-owned invariants, resource behavior, oracle metadata, and benchmark policy.
