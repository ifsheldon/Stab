...
- Add Stab CLI tests for explicit rejections, invalid paths, nonexistent input files, unwritable output files, writer failures, malformed inputs, invalid formats, invalid observable side-output formats, unsupported `ptb64` output, feedback-inlining failures, and large input resource behavior; the selected `m2d` path-IO subset, including path-open precedence before converter setup, is covered by `pf7-m2d-path-io-rust`, selected `m2d` command-contract behavior is covered by `pf7-m2d-command-contract-rust`, and the selected `analyze_errors` flag and malformed-input subset is covered by `pf7-analyze-errors-flags-rust`.
...
- Supplement `pf7-m2d-cli-parity` and `pf7-analyze-errors-cli-parity`; the `m2d` path-IO subset is covered by `pf7-m2d-path-io-rust`, the `m2d` command-contract subset is covered by `pf7-m2d-command-contract-rust`, and the `analyze_errors` flag and malformed-input subset is covered by `pf7-analyze-errors-flags-rust`. Keep `pf7-legacy-dispatch-parity` implemented as the selected legacy-dispatch closure, with accepted aliases covered by `pf7-legacy-dispatch-accepted-rust`.
