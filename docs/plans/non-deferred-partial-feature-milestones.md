...
- Extend `missing_detectors` for selected generated honeycomb and toric suffix cases, plus any remaining MPP, pair-measurement, observable, gauge, and row-reduction cases that are not already implemented. The toric global-stabilizer suffix case is implemented; honeycomb generated-code suffix analysis remains active.
