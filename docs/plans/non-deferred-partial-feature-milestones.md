...
- Do not add a benchmark row for selected deterministic `MPAD` execution, selected stochastic `MPAD(p)` sampler or detection-sampling evidence, or selected noisy `MPAD(p)` analyzer pad-flip effects unless implementation changes a hot sampler, converter, detection-sampling, or analyzer path.
