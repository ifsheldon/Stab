...
- Do not add a benchmark row for selected deterministic `MPAD` execution or selected noisy `MPAD(p)` analyzer pad-flip effects unless implementation changes a hot sampler, converter, detection-sampling, or analyzer path.
...
- Keep exact positive and negative tests for the promoted `missing_detectors` row-reduction, Pauli-product, observable, `MPAD`, Clifford-propagation, `SPP` and `SPP_DAG`, bounded repeat, selected folded final-repeat, honeycomb, and toric suffix evidence. Do not add broader MPP, pair-measurement, observable, gauge, Clifford-propagation, repeat-traversal, folded large-repeat, row-reduction, unknown-input, or generated-code missing-detector tests until an exact-subcase plan selects circuits, suffix comparators or source-owned invariants, resource behavior, oracle metadata, and benchmark policy.
