...
The `pf2-time-reverse-flow-measurement` row measures the selected measurement-rich flow-transform bridge from the RPF2 side, now including the selected `MZZ` plus plain-qubit unitary suffix packet, selected measurement-ordering, selected plain reset-to-measurement over one or more unique qubit targets, selected measurement-to-reset, and measure-reset cases over one or more unique qubit targets including inverted result targets, and remains report-only for the same reason.
...
- Broader measurement-rich `time_reversed_for_flows` and broader transform-integration checks beyond the selected single-instruction unique-target measurement group, selected plain reset group over one or more unique qubit targets, selected measure-reset group over one or more unique qubit targets with inverted result-target support, and selected `MZZ` unitary-suffix packet.
