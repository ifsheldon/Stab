...
The refreshed corpus covers 46 cases and 148 generated flows per utility batch.
The current focused report-only probe used `target/benchmarks/rpf5-sweep-flow-generator-probe/baseline.json` and `target/benchmarks/rpf5-sweep-flow-generator-compare/compare.json` from Stab commit `9ad2aaaa3b9b127dbbc30e9310eb563a1d6e8416` with local modifications recorded as true.
It measured `stab_pf5_flow_generators_measurement_cases` at `3.044e5 cases/s` and `stab_pf5_flow_generators_measurement_flows` at `9.882e5 flows/s`.
...
just bench::smoke
just bench::baseline --only pf5-flow-generators-measurement-rich --out target/benchmarks/rpf5-sweep-flow-generator-probe
just bench::compare --only pf5-flow-generators-measurement-rich --baseline target/benchmarks/rpf5-sweep-flow-generator-probe/baseline.json --report target/benchmarks/rpf5-sweep-flow-generator-compare
...
- `circuit_flow_generators` for broader all-operation composed measurement-rich circuits beyond the promoted no-op annotation, ordinary-noise, tableau, measurement-record feedback, selected sweep-controlled Pauli sign-only no-op groups, heralded-record, and `SPP` or `SPP_DAG` decomposition subcases, unsupported feedback shapes, mixed sweep-plus-quantum groups, measurement-record-to-sweep controls, broader heralded-noise synthesis, folded repeat traversal beyond the current flow-row and materialized flattened-operation caps, noisy-flow checker semantics, and all-operation generator checks.
