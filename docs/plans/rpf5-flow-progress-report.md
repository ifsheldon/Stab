...
The `pf2-time-reverse-flow-measurement` row measures the selected single-instruction measurement-rich flow-transform bridge from the RPF2 side, now including selected measurement-ordering, selected reset-to-measurement, selected measurement-to-reset, and plain unique-target measure-reset cases, and remains report-only for the same reason.
...
The focused selected measurement-rich time-reversal probe used `target/benchmarks/pf2-time-reverse-measurement-ordering-probe/baseline.json` and `target/benchmarks/pf2-time-reverse-measurement-ordering-compare/compare.json`.
It measured `stab_circuit_time_reversed_for_flows_measurement` at `1.094e6 flows/s`.
...
The current selected reset-to-measurement and measurement-ordering slice promotes one noiseless plain single-target `R`, `RX`, or `RY` instruction, selected multi-record `M` and `MZZ` measurement ordering, and plain unique-target `MR`, `MRX`, and `MRY` measure-reset flow reversal in the same PF2-owned time-reversal row, rejects duplicate measurement targets, duplicate or inverted measure-reset targets, and unscoped observable or measurement-record terms for selected reset paths, refreshes the report-only benchmark corpus, and keeps multi-target or inverted reset-only operations, duplicate or inverted measure-reset groups, detector, feedback, noise, repeat, and multi-instruction QEC inverse shapes open.
...
- Broader measurement-rich `time_reversed_for_flows` and broader transform-integration checks beyond the selected single-instruction unique-target measurement group, selected single-target reset group, and selected plain unique-target measure-reset group.
