...
The `pf2-time-reverse-flow-measurement` row measures the selected measurement-rich flow-transform bridge from the RPF2 side, now including the selected `MZZ` plus plain-qubit unitary suffix packet, the exact pinned `flow_flip` packet, selected measurement-ordering, selected plain reset-to-measurement over one or more unique qubit targets, selected measurement-to-reset, and measure-reset cases over one or more unique qubit targets including inverted result targets, and remains report-only for the same reason.
...
The focused selected measurement-rich time-reversal probe used `target/benchmarks/pf2-flow-flip-probe-baseline/baseline.json` and `target/benchmarks/pf2-flow-flip-probe-compare/compare.json`.
It measured `stab_circuit_time_reversed_for_flows_measurement` at `9.212e5 flows/s` with the refreshed corpus including the selected `MZZ` plus plain-qubit unitary suffix packet, inverted result-target measure-reset cases, and the exact pinned `flow_flip` packet.
...
just bench::baseline --only pf2-time-reverse-flow-measurement --out target/benchmarks/pf2-flow-flip-probe-baseline
just bench::compare --only pf2-time-reverse-flow-measurement --baseline target/benchmarks/pf2-flow-flip-probe-baseline/baseline.json --report target/benchmarks/pf2-flow-flip-probe-compare
```
...
The docs/evidence sidecar found a P2 stale benchmark-evidence gap where this report still cited `pf2-inverted-measure-reset-*` artifacts after the `pf2-time-reverse-flow-measurement` corpus added the selected suffix packet and exact `flow_flip` packet; this report now cites `target/benchmarks/pf2-flow-flip-probe-baseline/baseline.json` and `target/benchmarks/pf2-flow-flip-probe-compare/compare.json`.
...
- Broader measurement-rich `time_reversed_for_flows` and broader transform-integration checks beyond the selected single-instruction unique-target measurement group, selected plain reset group over one or more unique qubit targets, selected measure-reset group over one or more unique qubit targets with inverted result-target support, selected `MZZ` unitary-suffix packet, and exact pinned `flow_flip` packet.
