...
Supersession note, 2026-07-10: PFM-B4 in `docs/plans/non-deferred-partial-feature-milestones.md` selects and implements the remaining active Rust flow-engine closure. Full generator-table GF(2) measurement solving for the selected generator surface is implemented without exhaustive fallback, all thirty-three ledger cases have independent selectors, and broader behavior outside the ledger now requires an explicit plan revision. Python binding behavior and broader PFM-B1 transform work remain separate.
