...
- The selected measurement-rich flow-transform subset validates flows through the sparse tracker and reverses flow endpoints for one noiseless plain unique-target `M`, `MX`, `MY`, `MXX`, `MYY`, or `MZZ` instruction group, with pinned Stim `M` and `MZZ` evidence, source-owned basis coverage for `MX`, `MY`, `MXX`, and `MYY`, selected multi-record measurement-ordering evidence, selected plain unique-target `R`, `RX`, and `RY` reset-to-measurement conversion, selected single-target `M`, `MX`, and `MY` measurement-to-reset conversion when the full flow batch has record dependence but no future Pauli dependence on the measured qubit, and the selected `MZZ` plus plain-qubit unitary suffix packet matching pinned `flow_through_mzz_h_cx_s`; the selected unique-target measure-reset subset covers `MR`, `MRX`, and `MRY`, including inverted result targets, by mapping reset-effect output terms into reversed measurement records and measurement-record dependencies back into reset effects.
...
The `pf2-time-reverse-flow-measurement` row measures the selected measurement-rich flow-transform bridge from the RPF2 side, now including the selected `MZZ` plus plain-qubit unitary suffix packet, selected measurement-ordering, selected plain unique-target reset-to-measurement, selected measurement-to-reset, and unique-target measure-reset cases including inverted result targets, and remains report-only for the same reason.
...
The focused selected measurement-rich time-reversal probe used `target/benchmarks/pf2-mzz-unitary-suffix-probe-baseline/baseline.json` and `target/benchmarks/pf2-mzz-unitary-suffix-probe-compare/compare.json`.
It measured `stab_circuit_time_reversed_for_flows_measurement` at `1.942e6 flows/s` with the refreshed corpus including the selected `MZZ` plus plain-qubit unitary suffix packet and inverted result-target measure-reset cases.
...
just bench::baseline --only pf2-time-reverse-flow-measurement --out target/benchmarks/pf2-mzz-unitary-suffix-probe-baseline
just bench::compare --only pf2-time-reverse-flow-measurement --baseline target/benchmarks/pf2-mzz-unitary-suffix-probe-baseline/baseline.json --report target/benchmarks/pf2-mzz-unitary-suffix-probe-compare
```
...
The docs/evidence sidecar found a P2 stale benchmark-evidence gap where this report still cited `pf2-inverted-measure-reset-*` artifacts after the `pf2-time-reverse-flow-measurement` corpus added the selected suffix packet; this report now cites `target/benchmarks/pf2-mzz-unitary-suffix-probe-baseline/baseline.json` and `target/benchmarks/pf2-mzz-unitary-suffix-probe-compare/compare.json`.
...
- Broader measurement-rich `time_reversed_for_flows` and broader transform-integration checks beyond the selected single-instruction unique-target measurement group, selected plain unique-target reset group, selected unique-target measure-reset group with inverted result-target support, and selected `MZZ` unitary-suffix packet.
