...
It does not complete the flow milestone because composed measurement-rich flow-generator synthesis, pair-measurement and MPP generators, solve-for-flow-measurements, failure explanations, `time_reversed_for_flows`, transform integration, variable-target gate flow metadata, and Python flow binding ergonomics remain open.
...
- `circuit_flow_generators` supports exact single-instruction generators for `M`, `MX`, `MY`, `R`, `RX`, `RY`, `MR`, `MRX`, `MRY`, and `MPAD`.
- Unpromoted measurement-rich generator shapes such as pair measurements, MPP, duplicate measure-reset targets, mixed measured/unitary instruction sequences, feedback, and repeat-contained measurements fail closed with an explicit unsupported generator error.
...
The generator row measures the promoted single-instruction measurement/reset/MPAD generator corpus through the Rust public `circuit_flow_generators` helper.
...
- `circuit_flow_generators` for composed measurement-rich circuits, pair-measurement, MPP, feedback, heralded-noise, and all-operation generator checks.
