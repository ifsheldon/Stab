...
The `pf2-time-reverse-flow-measurement` row measures the selected single-instruction measurement-rich flow-transform bridge from the RPF2 side, now including selected measurement-to-reset and plain single-target measure-reset cases, and remains report-only for the same reason.
...
The focused selected measurement-rich time-reversal probe used `target/benchmarks/pf2-time-reverse-measure-reset-probe/baseline.json` and `target/benchmarks/pf2-time-reverse-measure-reset-compare/compare.json`.
It measured `stab_circuit_time_reversed_for_flows_measurement` at `1.275e6 flows/s`.
...
The current selected measure-reset slice promotes one noiseless plain single-target `MR`, `MRX`, or `MRY` instruction plus selected single-target `M`, `MX`, and `MY` measurement-to-reset conversion in the PF2-owned time-reversal row, refreshes the report-only benchmark corpus, and keeps broader reset-only operations, inverted or multi-target measure-reset groups, detector, feedback, noise, repeat, and multi-instruction QEC inverse shapes open.
...
- Broader measurement-rich `time_reversed_for_flows` and broader transform-integration checks beyond the selected single-instruction measurement group and selected plain single-target measure-reset group.
