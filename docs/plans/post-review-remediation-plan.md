...
Work is organized into workstreams `WS1` through `WS8` plus a bookkeeping phase `P0`, ordered by risk reduction per unit of work.
...
- Before fixing a surface, flip its over-claiming rows in `docs/stab-feature-checklist.md` to reopened status and regenerate `docs/qualification-status.md`, so the dashboard is honest while fixes land.
...
- Keep one rolling progress report at `docs/plans/post-review-remediation-progress-report.md`, updated at each workstream completion with the evidence commands run and their revisions.
- WS2 additionally maintains `docs/plans/analyzer-consolidation-plan.md` (written before WS2b code) and records its stage gates there.
...
2. Add `M(1) 0` / `DETECTOR rec[-1]` oracle fixtures through `reference_sample`, `detect`, `m2d`, and skip-reference sampling.
...
3. Validate MPAD targets in {0, 1} in stab-model instruction validation so all consumers inherit the check, and exclude MPAD targets from qubit counting (`crates/stab-model/src/circuit/counts.rs:55-59`) to match `vendor/stim/src/stim/circuit/circuit_instruction.cc:276-286`.
...
2. A joint two-detector distribution test distinguishes whole-product collapse from first-term collapse at a fixed seed panel and passes only for the fixed implementation.
...
6. `MPAD 2` is rejected at model validation with a diagnostic matching Stim's error class, and `MPAD 1` no longer inflates `count_qubits`, both covered by tests.
...
Per D6, the forward engine and `GaugeTracker` will be retired; write `docs/plans/analyzer-consolidation-plan.md` before starting WS2b code, including the full equivalence test matrix.
...
Stage 4 (delete and split):

1. Delete `GaugeTracker` (`crates/stab-analysis/src/circuit_to_dem/gauge.rs`) and the forward sensitivity machinery; gauge removal then rewrites subsequent error symptoms by construction, matching `vendor/stim/src/stim/simulators/error_analyzer.cc:226-242`.
...
1. `docs/plans/analyzer-consolidation-plan.md` exists and its matrix fixtures are committed before any engine code changes (stage gate, checkable from history).
2. The stage 2 differential test ran green across the matrix, with the known-bug exception list exactly matching the review witnesses and nothing else.
3. After stage 3, every matrix entry byte-matches pinned Stim v1.16.0, including unpaired Pauli includes in both fold modes and gauge circuits; the executed review witness pair is a committed oracle row.
4. After stage 4, exactly one sensitivity-propagation engine exists: `rg` finds no gate-semantics `undo_`/propagation implementations outside `sparse_rev_frame_tracker.rs`, and `gauge.rs` is gone.
5. The structural quadratic is gone: an `analyze_errors` smoke benchmark on a large generated circuit (rotated surface code, distance 25, 1000 rounds) completes within the benchmark harness bound and shows at least an order-of-magnitude improvement over the recorded pre-fix measurement of the same input; the result is recorded as a diagnostic, not a promoted ratio.
6. No stab-analysis source file exceeds 1200 lines; clippy and fmt clean; `cargo test -p stab-analysis -p stab-core` passes.
7. The reopened `analyze_errors` checklist rows (decomposition, Pauli-target observables, gauge detectors) return to implemented status with their new fixture evidence listed.
...
3. `convert --in_format`: drop the `default_value = "01"` (`crates/stab-cli/src/convert.rs:22`) so the flag is required like Stim; add a CQ-CLI test asserting exit 1 and stderr class for the missing flag; correct the checklist row.
...
2. `MPP X0 *X1` and `MPP X0* X1` parse, round-trip to Stim's canonical printing, and byte-match pinned Stim through a `convert --in_format=stim --out_format=stim` oracle row; dangling line-end combiners still reject; `just rust::parser-fuzz` passes.
3. `stab convert` without `--in_format` exits 1 with the same stderr class as pinned Stim, as a committed CLI test; the checklist row no longer claims the old default.
...
Complete before any v0.2.0 release attempt.

1. Draft verification endpoint: `release_by_tag` (`ops/release/src/github.rs:408-416`) uses `GET /releases/tags/{tag}`, which returns published releases only, so the mandatory post-upload verification in `publish_draft` (`github.rs:86`) and `verify-remote-draft` (`github.rs:130-134`) fail against a draft.
   ...
   Extend the mocked-publisher tests to model 404-on-draft so the regression is representable, and rehearse once against a scratch repository and tag before release day.
...
## WS5: One Owner Per Invariant (M to L)
...
5. stab-model gate metadata: put `produces_results`-style flags and the alias list on `GateInfo` and derive the three string-matching sites (`crates/stab-model/src/circuit/counts.rs:40-52`, `crates/stab-model/src/circuit/api.rs:145-177`, `crates/stab-model/src/gate/metadata.rs:188-205`); unify the quadruplicated tag-escaping in `model_tag.rs`/`dem/tag.rs`.
...
6. Gate classification and aliases have one owner: the three string-matching sites are deleted and a completeness test asserts derived views agree with the gate table for every gate.
7. All duplication deletions preserve behavior: full workspace tests pass, and no checklist row changes state from this workstream alone.
...
1. Each closed gap carries a witness-derived test with an explicit time or work bound: the 100k-distinct-target MPP compiles within the bound; the `REPEAT`-with-mass-`SHIFT_COORDS` witness is either budget-rejected with a typed error or completes within the bound; the 1M-CX flow-generator witness and the 1M-target `MPAD` missing-detectors witness are budget-rejected or complete within their bounds.
2. New budget rejections use typed limit errors consistent with the existing `ResourceLimit` taxonomy and are documented as deliberate divergences where Stim would attempt the work.
3. An API-built `Circuit` with nesting deep enough to overflow the old recursive drop is constructed, cloned, compared, and dropped without abort in a test (or its construction is rejected at the ceiling by a typed error, matching whichever remediation is chosen).
4. `BitVec` allocation failure returns `StorageAllocationFailed` instead of aborting, covered by a test using an absurd bit length.
5. No behavior change on valid inputs: the full oracle implemented-only run stays green.

## WS7: Engine Performance Structure (M to L)

Required before Stim performance-ratio claims on these paths; benchmark-first per the performance-claims policy.
...
## WS8: Harness, Test-Quality, And Polish Batch (S each, batchable by crate)
...
1. Week 1: Phase P0, WS1, WS3 items 1 through 3, WS4 item 1.
2. Week 2: WS2a, remainder of WS3, WS4 items 2 through 5.
3. Weeks 3 and 4: WS2b stages 1 through 4, gated on `docs/plans/analyzer-consolidation-plan.md` and its equivalence matrix.
4. Then WS5 and WS6 in parallel as independent refactor batches.
5. Then WS7, followed by the full oracle corpus rerun, inventory regeneration, one `--tier full` qualification rerun, and un-reopening checklist rows with fresh evidence.
6. WS8 items batch into the above by crate; none block the sequence.
...
- Per phase boundary: `cargo test --workspace`, `just oracle::run --implemented-only`, `just oracle::record --check-clean`, inventory regeneration checks, and the stable-component commands from `CONTRIBUTE.md`.
...
2. Every reopened checklist row is restored with named fresh evidence, and `docs/qualification-status.md` regenerates with no reopened rows attributed to this review.
...
5. No source file exceeds 1200 lines, clippy and fmt are clean, and no new `#[allow]` of the panic-family lints exists in non-test code.
...
- WS2b is the only item with real architectural risk: it replaces an engine that passes a large existing suite, so byte-exact equivalence fixtures must exist before the old engine is deleted (stage 1 gate), and diagnostics-text changes must be reviewed as a compatibility surface (stage 3 gate).
