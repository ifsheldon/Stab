...
- 2026-08-06, revision 2: amended after the independent WS1 milestone audit (verdict: complete with spec follow-ups; both headline fixes re-verified by reverting them and reproducing the failures).
  Criterion 3 is amended because pinned Stim exposes no CLI reference-sample surface, so no oracle row can exist for `reference_sample`: the four CLI surfaces (detect 01, detect dets, m2d, `--skip_reference_sample` sampling) are required as committed exact-output oracle rows, and `reference_sample` itself is pinned by the `reference_flip_semantics` Rust suite across the general and direct-Z paths.
  Criterion 4 is amended because the sparse direct-Z admission (`cb285ea0`, merged before this plan) makes the literal `M 16000000` witness succeed cheaply by design: the criterion now requires that no public stab-engine function panics on any parseable circuit, with the huge-qubit-id witness returning `Ok` through the sparse direct-Z path and a typed storage error through every general-path public entry point, covered by tests.
  Criterion 2 is clarified: an exact cross-detector invariant over jointly sampled shots satisfies the statistical-test contract when the defect class makes one detector deterministic, per the contract's existing "joint distribution or invariant" wording.
  Reference task 2's probability-case clause is scoped to reference-side Rust pins (`M(0.5)`, `M(0)`, and the family-wide `p == 1` splits), because `p` in `(0, 1)` cannot be an exact-output oracle row and the mode-level flip ignore makes a per-family probability sweep redundant.
- 2026-08-07, revision 3: amended after the independent WS3 milestone audit (verdict: complete with spec follow-ups; items 1 through 7 probed byte-identically against the pinned binary by the auditor).
  WS3 criterion 2 is amended because pinned Stim's `convert` accepts only result formats and rejects `--in_format=stim` (probed), so no convert-based circuit round-trip surface exists: combiner byte parity is pinned through the `m4-parser-spaced-combiners-accept` analyze_errors row plus the Rust canonical round-trip pins.
  WS3 item 5 and decision D2 are amended to bless the implemented mechanism: broken-pipe-rooted failures return exit code 141 through the error chain instead of re-raising `SIGPIPE`, giving shell-visible parity (`$?` = 141, empty stderr) while remaining `WIFEXITED` where pinned Stim is `WIFSIGNALED`; a waitpid-status-aware harness can distinguish them, and this divergence is accepted because the workspace forbids the `unsafe` signal call absent a measured need.
  WS3 item 6's scanned legacy flag set is enumerated: `--convert`, `--sample`, `--detect`, `--m2d`, `--analyze_errors`, bare `--gen`, and the `--gen=`/`--sample=`/`--detect=` value forms relocate from any pre-`--` position (bare `--sample`/`--detect`/`--gen` carry their adjacent value); `--help` remains first-position-only because it is a help topic rather than an execution mode.
  WS3 criterion 8 is amended to "pinned byte-exactly in human and JSON modes" with the note that the JSON error format is a Stab extension pinned against Stab's own contract, not a Stim surface (the byte-exact JSON pin now exists).
  WS3 criterion 11 is amended to "every item whose behavior has a pinned-CLI-exercisable surface carries a committed oracle fixture row, and every other item carries a named Rust regression", with the change set defined at batch granularity (items 1 through 3 landed their rows in sibling Batch A commits).
- 2026-08-07, revision 4: amended after the independent WS2 milestone audit (verdict: incomplete pending committed evidence; the auditor re-derived all 126 matrix captures byte-identically from the pinned binary and reproduced the quadratic-gone diagnostic on both sides).
  WS2b Stage 2 and criterion 3 are amended to record the implemented comparator: the committed differential compared the reverse engine against the pinned-Stim captures (ground truth) rather than against the forward engine, its explicit exception list was the diagnosed fourteen-entry DEPOLARIZE-rounding set rather than the review witnesses (which byte-matched pinned Stim through the reverse engine from the start, proving the flip fixes them), and the stage closed with EMPTY exception lists once the fused-`fmadd` contraction diagnosis healed the rounding set; the anti-silent-blessing intent held because every unlisted divergence stopped the stage until diagnosed.
  WS2b criterion 6's "completes within the benchmark harness bound" clause is amended to the delivered form: a reproducible timed diagnostic recorded in the consolidation-plan ledger (the pre-flip engine exceeded a 300-second kill bound where the reverse engine completes in about 24 seconds, byte-identical to pinned Stim), with no benchmark-harness entry; adding a generously-bounded analyze_errors smoke benchmark stays a non-blocking follow-up.
  WS2b criterion 5's `rg` clause is scoped to the sensitivity-propagation invariant: remaining `undo_` sites either delegate Pauli propagation to the tracker or belong to the pre-existing flow-generator subsystem (`circuit_flow/generators.rs`), which computes stabilizer flow generators from `gate_tableau` and was never an owner of the analyzer's sensitivity invariant.
  WS2a criterion 2's transferred byte-exact rows and WS2b criterion 4's witness-pair rows landed as the six committed exact-output oracle rows `m10-analyze-errors-decompose-surface-{d3,d3-fold,d5,d5-fold}` and `m10-analyze-errors-pauli-include-{before,after}-error`, closing the audit's P1 and first P2 findings; WS2a criterion 3's third invariant (non-remnant components exist in the known set) gained its missing assertion in the decomposition invariant test.
...
2. Add `M(1) 0` / `DETECTOR rec[-1]` oracle fixtures through `reference_sample`, `detect`, `m2d`, and skip-reference sampling, plus p equal to 0 and p in (0, 1) cases across the noisy measurement families; the probability-case clause is scoped to reference-side Rust pins because p in (0, 1) cannot be an exact-output oracle row and the mode-level flip ignore makes a per-family sweep redundant (amended per the WS1 milestone audit).
...
2. A joint two-detector distribution test distinguishes whole-product collapse from first-term collapse under the statistical test contract below and passes only for the fixed implementation; an exact cross-detector invariant over jointly sampled shots satisfies the contract when the defect class makes one detector deterministic (amended per the WS1 milestone audit).
3. `M(1) 0` / `DETECTOR rec[-1]` fixtures byte-match pinned Stim for `detect` (01 and dets formats), `m2d`, and `--skip_reference_sample` sampling as committed exact-output oracle rows; `reference_sample` itself, having no pinned CLI surface, is pinned by the `reference_flip_semantics` Rust suite across the general and direct-Z paths (amended per the WS1 milestone audit).
4. No public function in stab-engine panics on any parseable circuit: the huge-qubit-id witness (`M 16000000`) returns `Ok` through the sparse direct-Z path by design, and general-path variants (for example `H 16000000` / `M 16000000` and huge measurement counts) return a typed storage error through every public entry point, covered by tests (amended per the WS1 milestone audit).
...
5. Broken pipe per D2: return exit code 141 with empty stderr for broken-pipe-rooted output failures (shell-visible parity with pinned Stim's silent `SIGPIPE` death; `WIFEXITED` versus `WIFSIGNALED` divergence accepted per Amendment Record revision 3); keep reporting for real I/O failures; add the qualification-plan broken-pipe test.
6. Legacy mode flags: scan the whole pre-`--` argument vector for exactly one legacy mode flag instead of first-position-only (`crates/stab-cli/src/lib.rs:392`); the scanned set is `--convert`, `--sample`, `--detect`, `--m2d`, `--analyze_errors`, bare `--gen`, and the `=`-value forms, with bare flags carrying their adjacent value (Amendment Record revision 3); keep multiple-flag rejection.
...
2. `MPP X0 *X1` and `MPP X0* X1` parse, round-trip to Stim's canonical printing, and byte-match pinned Stim through the `m4-parser-spaced-combiners-accept` analyze_errors oracle row (pinned Stim's `convert` rejects `--in_format=stim`, so no convert-based circuit round-trip surface exists; amended per the WS3 milestone audit); attached forms keep their current behavior; dangling line-end combiners still reject for MPP; `just rust::parser-fuzz` passes.
...
8. `stim detect --prepend_observables --append_observables` and the Stab equivalent both emit the deprecation warning before the combination error, with warning-before-error ordering pinned byte-exactly in human and JSON modes; the JSON error format is a Stab extension pinned against Stab's own contract (amended per the WS3 milestone audit).
...
11. Every item whose behavior has a pinned-CLI-exercisable surface includes a committed oracle fixture row, every other item includes a named Rust regression, and every item's checklist/docs update lands with its batch (amended per the WS3 milestone audit; items 5, 9, 10, and 11 have no pinned-CLI-exercisable oracle surface).
...
1. Draft verification endpoint: `release_by_tag` (`ops/release/src/github.rs:408-416`) uses `GET /releases/tags/{tag}`, which per GitHub's documentation returns published releases only, so the mandatory post-upload verification in `publish_draft` (`github.rs:86`) and `verify-remote-draft` (`github.rs:130-134`) fail against a real draft.
   ...
   Rehearse once against the fixed source-pinned scratch repository and a commit-derived throwaway tag before release day. Keep the production operator non-redirectable and provide a separate rehearsal-only operator that cannot publish crates or a GitHub release.
...
2. A documented rehearsal against public scratch repository `ifsheldon/Stab-release-rehearsal` (numeric repository ID `1342241032`) and its active no-bypass ruleset `21169813` completes the dedicated rehearsal `create-draft` path end to end under a commit-derived annotated tag: one private draft is created, all six assets are verified remotely, post-mutation verification passes, and the read-only rehearsal verifier passes against the live draft. The rehearsal revision, exact workflow run, tag, repository and ruleset identities, and output digests are recorded in the progress report.
3. Every reversible GitHub draft-stage command sequence in the rehearsal section of `docs/RELEASING.md` executes as written, including source mirroring, annotated-tag checks, exact-SHA dispatch, run-identity capture resolving to exactly one workflow run, artifact download, and remote draft verification. This criterion deliberately excludes `release::publish-reviewed`, production-tag creation or push, manual draft publication, and published-release verification because crates.io uploads and public production release state are irreversible and cannot be safely rehearsed against a scratch repository.
...
6. Smaller same-theme items: reuse `Circuit::count_*` in `circuit_detecting_regions.rs:621-706`; read tracker-owned counters in `circuit_inverse/reverse_flow.rs` instead of duplicating them; share one payload-byte estimator between `circuit_pass.rs` and `circuit_transforms.rs`; share one exact median implementation in ops/bench (`compare_evidence.rs:101` versus `statistics.rs:260`); delegate facade `Display`/equality to component types instead of copying text (`crates/stab-core/src/resources.rs:510-528`, `crates/stab-core/src/matched_error.rs:135-143`); add in-place canonicalization entry points to stab-analysis so `matched_error.rs:88-92` stops paying full clone round-trips; delete the panicking `CompiledDetectionConverter::reusable_detection_record` and `reusable_reference_sample` compatibility wrappers (`crates/stab-engine/src/detection/mod.rs:242`, `:258`), whose failure paths are unreachable only while the current compile caps hold (WS1 milestone-audit residual risk).
