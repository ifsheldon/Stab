...
This slice starts the M12 measurement gate by making the primary benchmark matrix explicit, adding release-profile compare report generation, recording row-level timing, variance, relative-ratio, pass/fail status, and allocation metadata, and adding profiler-note enforcement for rows slower than the hot-path threshold.
...
This slice covers only the matrix selection, compare-reporting infrastructure, profiler-note gate infrastructure, and allocation-tracking report plumbing.
...
| Report machine, compiler, Stim, Stab, benchmark parameters, median timing, variance, ratio, and status | Partially satisfied | `CompareReport` records machine metadata, Stim metadata, Stab commit and local-modification state, compare command metadata, per-row measurements, medians, optional variance, optional allocation data, relative ratio, pass/fail status, and profiler-note status; complete primary allocation reports remain pending. |
...
| `just bench::compare --profile release --primary` has no missing primary workloads | Missing | A full primary compare requires a complete pinned-Stim baseline and remains pending. |
