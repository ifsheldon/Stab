...
Broader profiler captures, optimization work across parser, sampler, detector, analyzer, and DEM hot paths, complete allocation reports for the primary matrix, source-owned per-row regression threshold values, source-owned completion baseline artifacts, and a source-owned first complete memory baseline remain pending M12 work.
...
No comparable beta failures remain in the latest local primary probe, but profiler notes, allocation baselines, regression thresholds, oracle completion, milestone audit, and full code review remain pending before M12 can be considered complete.
