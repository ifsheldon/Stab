...
Flip-probability and reset-flip settings match byte-exactly everywhere, so the divergence lives in the depolarizing-channel combination path; the leading suspect is that pinned Stim's `add_error_combinations` locally decomposes multi-symptom channel combinations unconditionally while Stab's `locally_decompose_combinations` only runs under `decompose_errors`, which also explains pinned Stim emitting `^`-separated classes without `--decompose_errors`.
