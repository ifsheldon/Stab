...
Disassembling the pinned binary resolved the residue without instrumentation: the frozen Linux AArch64 Release build contracts `add_error`'s `old * (1 - p) + (1 - old) * p` into one `fmadd` (`fma(old, 1 - p, (1 - old) * p)`, identical codegen in `add_error_in_sorted_jagged_tail` and across the oracle and probe builds), a fused single-rounding form that no composition of plain double expressions reproduces, which is why the black-box probe space was exhausted without a match.
The within-call `l + r - 2lr` conclusion recorded above was a misreading of that contraction: the vendor chains every combination slot through the same fused map merge with no pre-combination (`error_analyzer.cc:1658-1660`), and the fixtures that previously matched under plain rounding were exactly the cases (dyadic probabilities, single-merge classes) where fused and plain rounding coincide.
Replicating the contraction (`f64::mul_add` in `xor_probability`, also used by the decomposition re-merge that mirrors `error_analyzer.cc:1577`) and deleting the within-call subtotal healed all fourteen residual entries in BOTH fold modes: the reverse engine byte-matches all sixty-three matrix entries in both modes with empty differential divergence lists.
The forward analyzer keeps its pre-consolidation `l + r - 2lr` rounding everywhere (`merge_independent_probability_legacy`) because it matches pinned Stim on a different circuit subset than the vendor form, no single global formula can match both, and the engine is deleted at Stage 4; the matrix harness therefore tracks fold and nofold divergences separately (two versus eighteen entries after the fused-merge fix, the fold residue being the loop-free Pauli-include witnesses still routed to the forward engine by the public dispatch).
...
- Stage 2: complete. The reverse engine runs over the whole matrix in both fold settings through a test-scoped seam (`reverse_differential`); its divergence set was the same fourteen DEPOLARIZE-rounding entries in BOTH fold settings (the public fold path's sixteen-entry list carried two more only because the loop-free Pauli-include witnesses route to the forward engine on the public dispatch), and both Pauli-include review witnesses byte-match pinned Stim through the reverse engine in BOTH modes, proving the Stage 3 flip fixes the forward engine's headline direction bug outright. The residual fourteen entries were the pinned binary's `fmadd` contraction of `add_error`, resolved by disassembly as recorded in the diagnosis above: with `xor_probability` fused via `f64::mul_add` and the within-call subtotal deleted, the reverse engine byte-matches all sixty-three entries in both fold modes, both differential divergence lists are empty, the public fold list shrinks to the two forward-routed witnesses, and the full stab-analysis, stab-core, and stab-cli suites plus all eighty-one M10 oracle rows stay green. Stage 3 is unblocked on byte-exactness.
- Stage 3: complete. The public `circuit_to_detector_error_model` dispatch routes every request through `reverse_fold::try_analyze` in both fold settings; all sixty-three matrix entries byte-match their pinned-Stim captures in both modes with EMPTY divergence lists in both harnesses, the Pauli-include review witnesses byte-match in both modes (their fold and nofold captures are identical, so fold/no-fold equality holds across the loop-free matrix), and the unrolled path admits the same documented expansion budget the forward engine enforced. Diagnostics-surface changes, each enumerated with its pin: (1) invalid sweep or record forms in a controlled-Pauli's Pauli slot now reject on the analyzer path with the pinned `{gate} target {t} is not a qubit` class, the sign-only sweep no-op predicate is tightened to gate-order-valid control slots, and the flow solver, checker, and inverse transforms keep the pinned PFM-B4 lenient contract through an error-analysis-mode gate; (2) the tracker's old `does not support a measurement-record feedback target in this position` spelling is replaced by the vendor-shaped not-a-qubit rejection; (3) observable-containing gauges under `allow_gauge_detectors` now fail with the pinned `non-deterministic observables` message with the detector line suppressed like vendor `check_for_gauge`; (4) the reverse record-bound message joins the analyzer's `is out of range` class; (5) a dangling `ELSE_CORRELATED_ERROR` is rejected at body scope end (error_analyzer.cc:719-722), correcting Stage 0's silent-drop reading; (6) the catalogued `PAULI_CHANNEL_1(0.1792, 0.1008, 0.2592)` accept-divergence is RESOLVED by porting the vendor's approximate converter verbatim (including its `dc = ab_i - ac` Newton step), so the channel now rejects without `approximate_disjoint_errors` and byte-matches pinned Stim (`error(0.280000000000000026645352591003757) D0`) with it, and the gate semantic contract harness drives both compared circuits with the unit threshold the way the pinned binary must be driven. The corrected DEM bytes re-froze the three A7 pipeline diagnostic input digests, and both inventories regenerated (correctness `8423de93...`, performance `6fabde72...`), making prior CQ and PQ evidence historical per the digest policy. Verification: full workspace suite (3094 tests), all 474 implemented oracle rows, `record --check-clean` (139 rows), `matrix --check`, and both qualification checks pass from this tree.
- Stage 4: unblocked (Pass 2, Batch D).
