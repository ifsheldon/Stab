...
- Stage 2: complete for the differential deliverable. The reverse engine runs over the whole matrix in both fold settings through a test-scoped seam (`reverse_differential`); its divergence set is the same fourteen DEPOLARIZE-rounding entries in BOTH fold settings (the public fold path's sixteen-entry list carries two more only because the loop-free Pauli-include witnesses route to the forward engine on the public dispatch), and both Pauli-include review witnesses byte-match pinned Stim through the reverse engine in BOTH modes, proving the Stage 3 flip fixes the forward engine's headline direction bug outright. The residual fourteen entries all reduce to the DEPOLARIZE merge rounding recorded in the diagnosis above — present identically with folding disabled, so the remaining arithmetic gap is not confined to the fold machinery — which gates only the byte-exactness half of Stage 3's criterion; the instrumented-vendor investigation remains open.
