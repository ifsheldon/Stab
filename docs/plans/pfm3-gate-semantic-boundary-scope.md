...
- Core sampler surface: `CompiledSampler` and `Circuit::reference_sample` through the fixed-tableau contract, supported Hermitian `SPP` or `SPP_DAG` decomposition lowering, selected deterministic `MPP` Pauli-product records, selected deterministic `MPAD` pad records, selected stochastic `MPP(p)` Pauli-product measurement-record flip distributions, and selected stochastic `MPAD(p)` pad-record flip distributions.
- Core detection-conversion surface: `CompiledDetectionConverter` and public conversion helpers through the same fixed-tableau and supported Hermitian `SPP` or `SPP_DAG` subset plus selected deterministic `MPP`, deterministic `MPAD`, stochastic `MPP(p)`, and stochastic `MPAD(p)` reference mapping.
- Core detection-sampling surface: `sample_detection_events` frame and non-frame paths for the selected fixed-tableau, supported Hermitian `SPP` or `SPP_DAG`, selected deterministic `MPP`, selected deterministic `MPAD`, selected stochastic `MPP(p)`, and selected stochastic `MPAD(p)` cases.
...
- Stochastic `MPP(0.25)` records execute as independently sampled probabilistic Pauli-product measurement flips in sampler output, detection-conversion record mapping, non-frame detection sampling, and frame detection sampling where a deterministic Pauli observable selects the frame path.
...
- Stochastic `MPAD(0.25)` records execute as independently sampled probabilistic pad flips in sampler output, detection-conversion record mapping, non-frame detection sampling, and frame detection sampling where a deterministic Pauli observable selects the frame path.
...
- This slice does not promote full stochastic `MPP(p)` distribution parity beyond the selected sampler and detection-sampling record-flip evidence plus existing analyzer coverage.
- This slice does not promote full stochastic `MPAD(p)` distribution parity beyond the selected sampler and detection-sampling record-flip evidence plus selected analyzer pad-flip effects.
...
- `stochastic_mpp_executes_across_sampler_and_detection_surfaces` covers selected stochastic `MPP(p)` sampler four-bucket distribution, detection-converter reference mapping, non-frame detection-sampling distribution, and frame-path detection-sampling distribution.
...
- `stochastic_mpad_executes_across_sampler_and_detection_surfaces` covers selected stochastic `MPAD(p)` sampler four-bucket distribution, detection-converter reference mapping, non-frame detection-sampling distribution, and frame-path detection-sampling distribution.
...
- This slice does not add a primary benchmark gate or a new benchmark row for selected deterministic `MPP`, stochastic `MPP(p)`, deterministic `MPAD`, or stochastic `MPAD(p)`.
...
- `cargo test -p stab-core --test gate_semantic_execution stochastic_mpp --quiet`
- `cargo test -p stab-core --test gate_semantic_execution mpad --quiet`
- `cargo test -p stab-core --test gate_semantic_execution stochastic_mpad --quiet`
