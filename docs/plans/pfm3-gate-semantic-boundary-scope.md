...
- Core sampler surface: `CompiledSampler` and `Circuit::reference_sample` through the fixed-tableau contract, supported Hermitian `SPP` or `SPP_DAG` decomposition lowering, selected deterministic `MPP` Pauli-product records, selected deterministic `MPAD` pad records, and selected stochastic `MPAD(p)` pad-record flip distributions.
- Core detection-conversion surface: `CompiledDetectionConverter` and public conversion helpers through the same fixed-tableau and supported Hermitian `SPP` or `SPP_DAG` subset plus selected `MPP`, deterministic `MPAD` reference-sample and skip-reference behavior, and stochastic `MPAD(p)` reference mapping.
- Core detection-sampling surface: `sample_detection_events` frame and non-frame paths for the selected fixed-tableau, supported Hermitian `SPP` or `SPP_DAG`, selected deterministic `MPP`, selected deterministic `MPAD`, and selected stochastic `MPAD(p)` cases.
...
- This slice does not add a primary benchmark gate or a new benchmark row for selected deterministic `MPP`, deterministic `MPAD`, or stochastic `MPAD(p)`.
