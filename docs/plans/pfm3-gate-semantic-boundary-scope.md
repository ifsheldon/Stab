...
- Core analyzer surface: `circuit_to_detector_error_model` fixed-tableau propagation, supported Hermitian `SPP` or `SPP_DAG` state and gauge propagation, selected `MPP` ordering propagation, and selected `MPAD` measurement-record propagation.
...
- Oracle rows `pf3-gate-semantic-wide-rust`, `pf3-gate-mpp-execution-rust`, `pf3-gate-mpad-execution-rust`, `pf3-gate-spp-analyzer-rust`, `pf3-gate-spp-contract-rust`, `pf3-analyze-errors-spp-state-propagation-cli`, and `pf3-analyze-errors-spp-nondeterministic-cli` select the promoted evidence.
- Benchmark row `pf3-gate-semantic-wide` is report-only and measures only the selected fixed-tableau plus supported Hermitian `SPP` or `SPP_DAG` sampler, detection-conversion, and analyzer execution contract. The selected `MPP` and `MPAD` slices do not add benchmark rows because they are deterministic semantic bookkeeping contracts and do not change a hot throughput path.
