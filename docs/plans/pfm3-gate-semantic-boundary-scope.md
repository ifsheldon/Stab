...
- Core sampler surface: `CompiledSampler` and `Circuit::reference_sample` through the fixed-tableau contract, supported Hermitian `SPP` or `SPP_DAG` decomposition lowering, and selected deterministic `MPAD` pad records.
- Core detection-conversion surface: `CompiledDetectionConverter` and public conversion helpers through the same fixed-tableau and supported Hermitian `SPP` or `SPP_DAG` subset plus selected `MPAD` reference-sample and skip-reference behavior.
- Core detection-sampling surface: `sample_detection_events` frame and non-frame paths for the selected fixed-tableau, supported Hermitian `SPP` or `SPP_DAG`, and selected deterministic `MPAD` cases.
- Core analyzer surface: `circuit_to_detector_error_model` fixed-tableau propagation, supported Hermitian `SPP` or `SPP_DAG` state and gauge propagation, and selected `MPAD` measurement-record propagation.
...
- This slice does not promote full stochastic `MPAD(p)` distribution parity beyond existing sampler coverage.
...
- Oracle rows `pf3-gate-semantic-wide-rust`, `pf3-gate-mpad-execution-rust`, `pf3-gate-spp-analyzer-rust`, `pf3-gate-spp-contract-rust`, `pf3-analyze-errors-spp-state-propagation-cli`, and `pf3-analyze-errors-spp-nondeterministic-cli` select the promoted evidence.
- Benchmark row `pf3-gate-semantic-wide` is report-only and measures only the selected fixed-tableau plus supported Hermitian `SPP` or `SPP_DAG` sampler, detection-conversion, and analyzer execution contract. The selected `MPAD` slice does not add a benchmark row because it is a deterministic semantic bookkeeping contract and does not change a hot throughput path.
...
- This slice does not add a primary benchmark gate or a new benchmark row for `MPAD`.
