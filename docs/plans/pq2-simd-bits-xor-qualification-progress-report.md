...
The audit also exposed two genuine under-specifications: the plan does not require a machine-readable receipt for the standalone closure-command sequence, and it does not define whether allocation instrumentation must cover both implementations and every scale. Both remain visible as open entries in `docs/plans/milestone-spec-gaps.md`; neither changes the implemented AArch64 timing contract or retroactively broadens the allocation claim.
...
6. Split `report.rs`, `validation.rs`, and `worker.rs` by invariant ownership before the next qualification slice adds behavior; they currently contain 1,191, 1,189, and 1,187 lines.
7. Replace or constrain the broad `Reworked` validation exemption with a structured exact-replacement mapping before another heterogeneous inherited row relies on it for threshold eligibility.
