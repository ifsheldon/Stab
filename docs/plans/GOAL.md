...
Program checkpoint: CQ0 is source-current at correctness inventory digest `4dbbb4b2cda3117bdd3d3ddfcd30b55f09e6f401352e3e86130222189d47791f`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory schema version 2 and digest `a76090c996ad404c1cb8bfa85066e286c6f40b32754b3750e984375f7ca90025`: seventeen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 51 independently reported scale outcomes. The inventory digest binds the separate identity and non-identity Clifford-string groups to exact correctness parents, frozen vectors, comparator sources, independent thresholds, reviewed scalable scalar oracles, normalized statistics, and source-owned independent-throughput timing only for the exact identity group. Runtime-group schema version 5 separately binds both groups to `benchmarks/profiler-notes/qualification/perfq-m6-clifford-string.md`; neither contract inherits Clifford timing evidence. Clean pre-migration revision `127d6661a9e00872fc4aa4c0b0d27171e005afa5` published and replayed both group completion receipts, including identity completion report SHA-256 `78fc10ca29e432641f3d978ed871c4b96d1ba344d714c20bf726f574239d2126`, which authorized retirement of only the inherited `m6-clifford-string` timing threshold and exact identity/small replacement mapping. The source-current inventory performs that focused retirement while preserving the legacy memory baseline for PQ6. Packed-storage optimization revision `2a0ab88c44eeeaae1714b0976089ecc1809203f3` replaces scalar per-qubit multiplication with six bit planes and an isolated portable-SIMD kernel; review-fix revision `9c672ef3c12c3fe68632e8609a58ae98714bc144` restores constant-time point metadata updates and allocation-free repeated randomization. Qualification report schema version 30 binds the checked timing policy, common semantic batch, per-implementation selected counts and work totals, selected output digests, selected/common equality at matching counts, and normalized ratio reconstruction. The immutable diagnostic at `target/benchmarks/qualification/perfq-m6-clifford-identity-independent-schema30-review-final-20260719` binds group-contract SHA-256 `e93c4ffd6ee89ef5c08ca901f95b7f7697940e9122f414495c2ee0872f0db608`, selected 408,951 Stim and 28,225,158 Stab iterations, proved exact common semantics at 408,951 iterations, and passed with median `0.014479x` and upper bound `0.014488x`. Offline replay passed for report SHA-256 `311f975a5b8789ca764d810168c1dfad95758f1f275a8921936d5b3c5228a07b`; the report remains non-promotable because the checkout was dirty and diagnostic-mode `--allow-unverified-host` was used. Clean reviewed iterator evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632`, Pauli multiplication revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb`, transpose revision `f912cc3af1f13cc9fab798d69937c155d37d83a0`, and earlier kernel evidence remain accepted historical evidence under their recorded inventories. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work-unit, output-identity, source-owned batching-policy, or migration rule may be relaxed.
CQ2 implementation and exact ownership remain complete for eight selected domains and 271 implemented parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. Clean correctness revision `3f2f382627c8421de0a668819d467a9f252de20f` passed PR, full, and soak under preceding digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`; focused revision `ac20ffca` passed the exact three Clifford prerequisites under preceding digest `c50f27fd097ac870c987d1f91c44d9e6a75510ed4d51ec44853dbc328f0b1fa7`. Both are historical evidence for their exact producers and inventories. The reviewed core and worker source changes make a fresh same-revision focused three-case prerequisite mandatory before dependent timing. Exact historical request, report, completion, and preflight digests are recorded in `docs/plans/cq2-deterministic-qualification-progress-report.md`.
The active execution checkpoint is the eleventh PQ2 Clifford-string slice. Clean pre-migration revision `127d6661a9e00872fc4aa4c0b0d27171e005afa5` completed worker reproducibility, exact focused CQ, both adapter probes, all 12 full and soak scale reports and regressions, four replayed rollups, and two replayed completion receipts under performance inventory `0ee3639389860799298164c94c647fcab45b03c9d67b941b1aad12c6e5e06df5`. The source-current inventory now retires only the authorized inherited timing threshold and identity/small mapping. Next commit this focused migration, then repeat the complete two-group correctness, worker, probe, timing, regression, rollup, and completion chain from one clean post-migration revision before promoting any current-inventory ratio. Keep every failed, noisy, or host-unverified `1.25x` outcome visible and do not promote diagnostics.
The first five proving groups passed on the controlled Linux AArch64 host under preceding performance inventories and remain historical after later inventory or shared-worker changes. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` closes the fifth group's historical AArch64 evidence chain without weakening the `1.25x` gate. Both under-specifications revealed by that audit are now resolved in `docs/plans/milestone-spec-gaps.md`: Stab allocation instrumentation covers every dense-XOR scale plus the accepted maximum, and clean implementation revision `b208a359f3f7676e2b07d64a5dc8caca208abf6a` adds completion receipt schema version 1 for every later executable slice. The sixth `not_zero`, seventh sparse-XOR, eighth BitMatrix transpose, ninth Pauli multiplication, and tenth split Pauli iterator slices are complete on controlled Linux AArch64 at their recorded inventories; their progress reports record exact correctness, worker, report, regression, rollup, completion, audit, and review evidence. Clean revision `3f2f382627c8421de0a668819d467a9f252de20f` remains the latest complete 271-parent CQ2 execution checkpoint under its historical inventory; the source-current inventory requires fresh focused Clifford execution. Native x86-64 execution remains an unclaimed evidence target, and x86 adapter builds must prove that they inherit Stim's resolved machine flags before producing any ratio.
...
Preserve clean CQ2 evidence revision `3f2f382627c8421de0a668819d467a9f252de20f`, focused revision `ac20ffca`, and their exact historical report chains. The machine contract intentionally rejects replay from any dirty checkout or different `HEAD`; every promotable Clifford run must generate and consume focused exact CQ prerequisites from the same clean revision as its performance workers. Performance consumer commits `9be47ff2`, `0a9b228d`, and `7d0c07eb` add coherent historical 6/3 and current 7/4 schema support, reject mixed families, require complete passing stream and artifact receipts, and reconstruct the global statistical attempt ledger from producer-authentic deterministic and statistical fixtures. The exact legacy Clifford pair remained mapped only to identity/small throughout pre-migration timing, and replayed identity completion at revision `127d6661a9e00872fc4aa4c0b0d27171e005afa5` authorized its focused timing retirement. The source-current inventory removes only that threshold and mapping, preserves the memory row, and requires the entire two-group chain to be regenerated from the clean migration revision before any ratio becomes current. Proceed in the exact item order defined by `docs/plans/comprehensive-stim-performance-qualification-plan.md`.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `a76090c996ad404c1cb8bfa85066e286c6f40b32754b3750e984375f7ca90025` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
...
The active eleventh slice is the two-contract Clifford-string plan in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. `PERFQ-M6-CLIFFORD-STRING` owns only the exact pinned identity workload, while `PERFQ-M6-CLIFFORD-STRING-NON-IDENTITY` covers equal-width public in-place multiplication over the pinned Stim 24-name order and a deterministic complete 24-by-23 non-identity composition cycle. Their frozen vector, exact correctness, independent output, zero-allocation, 62-receipt hostile matrix, source-shape, private-worker, scalable-oracle, and legacy-retirement contracts are implemented and independently reviewed; the packed portable-SIMD path addresses the measured scalar architecture failure. The complete pre-migration chain at revision `127d6661a9e00872fc4aa4c0b0d27171e005afa5` authorizes the focused timing migration now represented by the source-current inventory. Do not let the O(1) identity fast path substitute for non-identity work, and do not promote direct, dirty, host-unverified, diagnostic-mode, or pre-migration reports under the post-migration inventory. Finish the clean post-migration two-group rerun, final audit, final review, and progress report in that order.
