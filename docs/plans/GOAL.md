...
The source-current Clifford-string PQ2 contract is closed on controlled Linux AArch64. Preserve this completed order and do not reopen its evidence merely to produce a newer aggregate digest:

1. **Completed at revision `859bf20`:** commit the Clifford short-right-operand metadata fix; all-path pre-mutation admission; retained repository descriptors and handle-relative Git, source, correctness, and pinned-Stim inventory access; exact correctness-case equality before unrequested receipt access; early formal-session binding with dual-error preservation; fallible terminal abort for every active staged-write failure; exact descriptor, length, digest, directory-identity, and child-name-set binding; descriptor-checked rollback and cleanup; production-dispatch regressions; generated checklist-count ownership; refreshed inventories; and synchronized source documentation. Do not generate promotable evidence from an uncommitted tree.
2. **Completed at revision `859bf20`:** generate and replay the exact three-case focused CQ full report for `cq2-algebra-clifford-string-api-contract`, `cq2-algebra-clifford-group-contract`, and `cq2-algebra-resource-clifford-growth` using their checked generated case IDs.
3. **Completed at revision `859bf20`:** reproduce both private workers and pass both Clifford adapter probes with private Stab build-receipt schema version 5, adapter receipt schema version 11, contract-preflight schema version 12, exactly 212 ordered contract probes, byte-reproducible worker identities, and all 72 Clifford receipts.
4. **Completed at revision `859bf20`:** generate all 12 unique full and soak group-scale reports into previously absent output directories on the verified controlled Linux AArch64 host. The machine reports prove unchanged cumulative swap counters; the operator separately observed the `finally`-guarded swap disable and restoration around the timing window.
5. **Completed at revision `859bf20`:** replay and regress every report, produce and replay four architecture-scoped rollups, and produce and independently replay one completion receipt per Clifford group.
6. **Completed:** synchronize `docs/plans/pq2-clifford-string-qualification-progress-report.md` with the exact revision, inventories, schemas, worker identities, artifact hashes, ratios, confidence bounds, host evidence, resource observations, migration authorization, and retained failed or rejected attempts. Operator-observed attempts without artifacts remain labeled non-evidence.
7. **Completed:** run `milestone-audit` and independent `full-code-review` over qualification lifecycle, migration authorization, Clifford correctness and SIMD behavior, hostile inputs, evidence, and documentation. The source-current reviews found no unresolved P0 through P3 issue and no newly revealed under-specification.
8. **Completed:** run the repository verification commands and synchronize the evidence documentation. The source-current schema-version-31 chain is accepted only for controlled Linux AArch64; native x86-64 and PQ6 remain separate later obligations.

Preserve clean CQ2 evidence revision `3f2f382627c8421de0a668819d467a9f252de20f`, focused revision `ac20ffca`, and their exact historical report chains. The machine contract intentionally rejects replay from a dirty checkout or different `HEAD`; every promotable Clifford run must generate and consume focused exact CQ prerequisites from the same clean revision as its performance workers. Historical correctness schema families remain supported only as explicitly modeled evidence and cannot be mixed. The exact legacy Clifford pair remained mapped only to identity/small throughout pre-migration timing, and replayed identity completion at revision `127d6661a9e00872fc4aa4c0b0d27171e005afa5` authorized its focused timing retirement. `benchmarks/qualification-threshold-migrations.json` now machine-binds that authorization to the exact replacement group, measurement, scale, report and preflight hashes, revisions, inventories, and retained memory row. Current inventory `a47866ba5eab70392dd2754391d3d7d8588567a7cbfc1f81a569be813804ce51` removes only that timing threshold and mapping, preserves the memory row, and requires the entire two-group chain to be regenerated from the clean review-fix revision before any ratio becomes current. Proceed in the exact item order defined by `docs/plans/comprehensive-stim-performance-qualification-plan.md`.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `a47866ba5eab70392dd2754391d3d7d8588567a7cbfc1f81a569be813804ce51` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
...
The eleventh slice is the two-contract Clifford-string plan in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. `PERFQ-M6-CLIFFORD-STRING` owns only the exact pinned identity workload, while `PERFQ-M6-CLIFFORD-STRING-NON-IDENTITY` covers equal-width public in-place multiplication over the pinned Stim 24-name order and a deterministic complete 24-by-23 non-identity composition cycle. Clean revisions `91f62d0a78659da2e8e264a6968b3c6cd32456de`, `da7c787d1e9f49110d7054868b146b5fb7d7bda4`, `29a29d5f68767e4ab131b051c88f6b77417e0338`, and `0b86f07881198c57df1237b23a7d7c0084f2a272` retain historical chains under their exact contracts, with the latter two review-rejected. Clean revision `859bf202bdd4bdfbca07e9b1d647afb1b0542846` closes the accepted source-current machine chain with exact correctness, reproducible workers, two probes, 12 first-attempt passing reports and regressions, four replayed rollups, two independently replayed completion receipts, zero timed Stab allocations at every runtime width and accepted maximum, and the preserved M12 memory baseline. Do not let the O(1) identity fast path substitute for non-identity work, overwrite or mutate an existing formal artifact, or promote nondirect, dirty, host-unverified, diagnostic-mode, operator-observed, rejected-host, review-rejected, or pre-migration results. Milestone audit and independent follow-up review are recorded in `docs/plans/pq2-clifford-string-qualification-progress-report.md` with no unresolved confirmed finding.
