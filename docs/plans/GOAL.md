...
- P2 has consolidated all six result codecs on `RecordFormat`, removed facade-only circuit path helpers and forwarding modules, removed fictitious backend selection, moved the CLI onto direct model, record, analysis, and engine ownership with private typed diagnostic composition, and deleted all three execution compatibility adapters. Measurement sampling, detection conversion, and DEM sampling now each have one compiler-plan-session-sink path; the remaining P2 work is facade domain-error removal and executable consumer fixtures.
