...
The active work is independent review closure of the twelfth PQ2 slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. Implementation, exact correctness ownership, focused legacy migration, retained memory-baseline checks, the complete source-current post-fix machine chain, accepted-maximum report-only memory observations, documentation synchronization, and milestone audit are complete. Continue in this exact order:

1. Preserve every `9497df0` CQ, formal report, rollup, and completion artifact unchanged and preserve the clean `dade78f` accepted-maximum memory observations. Do not rerun a stable timing producer or overwrite a retained path. Treat the pre-migration chain only as migration authorization and retain the `7c3e553` chain only as review-rejected historical evidence.
2. Commit the milestone-audit documentation closure separately from any later implementation fix.
3. Spawn independent GPT-5.6/max `full-code-review` lanes for core DEM correctness, worker and adapter equivalence, hostile-input and artifact lifecycle, benchmark science and migration, and documentation consistency. Give each lane the full twelfth-slice diff and evidence contract, require P0 through P3 findings with exact file references, and keep lane conclusions independent until synthesis.
4. Resolve every confirmed review finding and rerun all affected targeted checks. Regenerate source-owned evidence from a new clean revision whenever a fix changes an evidence identity or performance contract; do not carry forward an old ratio across that boundary.
5. Run full repository verification, commit focused fixes and final evidence documentation, and advance this execution contract only when independent review is closed, source-current docs agree, both final completion receipts replay from the accepted revision, swap is restored, no qualification process remains, and the worktree is clean.
