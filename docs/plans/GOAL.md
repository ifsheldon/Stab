...
4. In progress: extract `stab-analysis` over model and algebra only. Gate semantic projections, circuit and DEM tag stripping, circuit tableau conversion, simplification, decomposition, bounded circuit and DEM flattening, noise removal, DEM probability rounding, repetition/surface/color generation, MBQC decomposition, unsigned flow checking/generation/solving, sparse reverse-frame tracking, unitary and selected QEC inversion, tracker-driven flow reversal, bounded feedback lowering, detecting regions, missing-detector analysis, circuit-to-DEM lowering and loop folding, and XYZ error-probability decomposition have moved; graph and hypergraph search, SAT, and matching remain. Keep every slice free of records, execution, CLI, and ops.
