...
The first two proving groups passed on the controlled Linux AArch64 host under the preceding performance inventory. Clean revision `ba70a52025fdd4122ac97cec263725b2ec56e431` produced twelve passing, non-noisy full and soak measurements plus four replayed architecture rollups from one correctness request and one reproducible six-digest worker identity. Parser medians range from `0.897744x` to `0.970298x` with worst upper bound `0.974833x`; canonical-print medians range from `0.372912x` to `0.376075x` with worst upper bound `0.398775x`. Those reports are now historical because graduating the gate-hash group changed the global inventory and shared worker. The active owner action is to commit the third group, run its exact gate-name CQ preflight, prove worker reproducibility including pre-barrier rejection by both sealed workers for an 83-item partial gate sweep, and publish clean full and soak AArch64 scale families plus rollups without weakening the `1.25x` gate. Do not rerun parser or printer merely to conceal this interim inventory transition; rerun them when a later program checkpoint needs simultaneous source-current evidence. Native x86-64 evidence remains a separate unclaimed host dependency.
