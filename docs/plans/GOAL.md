...
- PQ2 gate and bit-kernel evidence: `docs/plans/pq2-gate-name-hash-qualification-progress-report.md` and `docs/plans/pq2-simd-word-popcount-qualification-progress-report.md`.
...
The first three proving groups passed on the controlled Linux AArch64 host under preceding performance inventories. Clean revision `ba70a52025fdd4122ac97cec263725b2ec56e431` produced parser and canonical-print evidence, and clean revision `c76b7071fc4d7ac358ef3a2fffc053ea675bd05f` produced gate-hash evidence with medians from `0.931886x` to `0.932764x` and worst upper bound `0.933289x`; all are historical after the popcount graduation changed the global inventory and shared workers. Clean revision `56dfe7569c6da48ffe76bde18f21ff43095f029b` closes the corrected fourth-group AArch64 evidence chain without weakening the `1.25x` gate. The next finite dependency-ordered contract is the fifth PQ2 slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`: graduate `PERFQ-M5-SIMD-BITS` as the exact dense `xor-complete-vector` phase, split it from the heterogeneous legacy row, bind both exact CQ owners and isolated Stim comparator source, generalize adapter source receipts without weakening existing evidence, and prove exact two-vector input, final-state output, bounded-width, allocation, scale, and `1.25x` acceptance contracts. Apply the same clean CQ, reproducibility, probe, full, soak, regression, rollup, audit, and GPT-5.6/max review chain. Rerun the complete 271-parent CQ2 family at the next simultaneous-current program checkpoint instead of relabeling historical reports. Native x86-64 execution remains an unclaimed evidence target, and x86 adapter builds must prove that they inherit Stim's resolved machine flags before producing any ratio.
