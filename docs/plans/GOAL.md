...
Program checkpoint: CQ0 is source-current at correctness inventory digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. Core revision `8503f458eac09fe94dba54cf8d6e16e88e195df4` strengthens the dedicated Pauli iterator owner with an independent sparse enumerator, exact pinned scales, word boundaries, borrowed-storage reuse, restart, and public resource limits. Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` published and replayed the exact three-case correctness prerequisite, reproducible workers, both complete adapter probes, 12 first-attempt passing reports and regressions, four rollups, and two completion receipts under performance inventory `ad3b6640e04855ac76d4851f856bb405e7c80c55dbcd67b204d67ea41d40c1eb`. Those receipts authorized retirement of only the bundled `m6-pauli-iter` timing threshold and its two exact mappings. PQ0 is source-current at performance inventory digest `adb7f822ff892b3d85b362130f048d7fe76f6e314349e5f42e71f54389a771d7`: seventeen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule across three scales, for 51 independently reported scale outcomes. This digest adds the separate identity and non-identity Clifford-string contracts, their exact correctness bindings, frozen vectors, comparator sources, and thresholds; it does not inherit or promote any Clifford timing evidence. The inherited iterator row is superseded, and its memory baseline remains guarded for PQ6. Clean reviewed iterator evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632` remains accepted historical AArch64 evidence under recorded performance inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf`, with all-output correctness at the accepted maximum, 12 first-attempt passing reports and regressions, four rollups, two completion receipts, completed milestone audit, and two clean final GPT-5.6/max review lanes. Clean revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb` remains accepted historical Pauli multiplication evidence under its recorded inventories, and clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` remains accepted historical transpose evidence under its recorded inventory. Earlier `not_zero`, sparse-XOR, and dense-XOR outcomes likewise remain historical under their exact identities. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
CQ2 implementation, ownership, and all-domain execution are complete for its eight selected domains and 271 implemented exact parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. Clean correctness revision `3f2f382627c8421de0a668819d467a9f252de20f` includes the hardened owner-admission and report-relocation controller, passed the PR tier's 145 selected parents and all 271 parents in both full and soak under digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`, replayed every report, and passed one exact preflight per tier with `local_modifications=false`. This is accepted source-current evidence at its recorded clean revision, not a claim that its receipts replay from a later checkout. Exact request, report, completion, and preflight digests are recorded in `docs/plans/cq2-deterministic-qualification-progress-report.md`.
The active execution checkpoint is the eleventh PQ2 Clifford-string slice. Apply the paired performance harness to the exact identity and complete non-identity public in-place multiplication workloads, bind each runtime group to exact passing CQ2 prerequisites and independent output digests, implement the frozen small, medium, large, and accepted-maximum contracts, and keep failed or noisy 1.25x outcomes visible. Do not promote any ratio until its exact current-digest correctness preflight, comparator, work count, output, allocation, hostile-input, rollup, and completion obligations validate.
The first five proving groups passed on the controlled Linux AArch64 host under preceding performance inventories and remain historical after later inventory or shared-worker changes. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` closes the fifth group's historical AArch64 evidence chain without weakening the `1.25x` gate. Both under-specifications revealed by that audit are now resolved in `docs/plans/milestone-spec-gaps.md`: Stab allocation instrumentation covers every dense-XOR scale plus the accepted maximum, and clean implementation revision `b208a359f3f7676e2b07d64a5dc8caca208abf6a` adds completion receipt schema version 1 for every later executable slice. The sixth `not_zero`, seventh sparse-XOR, eighth BitMatrix transpose, ninth Pauli multiplication, and tenth split Pauli iterator slices are complete on controlled Linux AArch64 at their recorded inventories; their progress reports record exact correctness, worker, report, regression, rollup, completion, audit, and review evidence. Clean revision `3f2f382627c8421de0a668819d467a9f252de20f` is the source-current complete 271-parent CQ2 checkpoint. Native x86-64 execution remains an unclaimed evidence target, and x86 adapter builds must prove that they inherit Stim's resolved machine flags before producing any ratio.
...
Preserve clean CQ2 evidence revision `3f2f382627c8421de0a668819d467a9f252de20f` and its exact report chain. In this program, source-current means that evidence is accepted for the producer and inventory contract recorded in its clean revision. A documentation-only commit that leaves those contracts and both inventory digests unchanged does not alter that scientific status, but the machine contract intentionally rejects replay from any dirty checkout or different `HEAD`. Any source change affecting the correctness runner or inventory makes the all-domain refresh historical. Every later promotable performance run, including the eleventh Clifford-string slice, must generate and consume focused exact CQ prerequisites from the same clean revision as its performance workers instead of presenting the `3f2f382` receipts as current-checkout preflight. Performance consumer commits `9be47ff2`, `0a9b228d`, and `7d0c07eb` add coherent historical 6/3 and current 7/4 schema support, reject mixed families, require complete passing stream and artifact receipts, and reconstruct the global statistical attempt ledger from producer-authentic deterministic and statistical fixtures. A clean `7d0c07eb` handoff probe made the real gate-lookup product runner consume a same-revision focused CQ prerequisite and complete paired execution on a verified host. Proceed in the exact item order defined by `docs/plans/comprehensive-stim-performance-qualification-plan.md`, starting with checked vectors and strengthened correctness rather than formal timing.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `adb7f822ff892b3d85b362130f048d7fe76f6e314349e5f42e71f54389a771d7` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
...
The active eleventh slice is the two-contract Clifford-string plan in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. Existing `PERFQ-M6-CLIFFORD-STRING` owns only the exact pinned identity workload, while new `PERFQ-M6-CLIFFORD-STRING-NON-IDENTITY` must cover equal-width public in-place multiplication over the pinned Stim 24-name order and a deterministic complete 24-by-23 non-identity composition cycle. Do not let Stab's O(1) identity fast path substitute for the non-identity measurement, do not accept a callback without the specified compiler fences, opaque operands, and zero-seeded result-derived successful-call witness, and do not begin formal timing until the frozen vector file, exact correctness, independent output, allocation, 62-receipt hostile matrix, and worker-receipt contracts land. Every rejected Clifford request must be the frozen single-field mutation of its named accepted baseline so an earlier guard cannot impersonate the intended rejection branch.
