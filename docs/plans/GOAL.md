...
Program checkpoint: CQ0 is source-current at correctness inventory digest `4dbbb4b2cda3117bdd3d3ddfcd30b55f09e6f401352e3e86130222189d47791f`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory digest `18f2b401f386ec3fb459a2d26497a96b881f03d70ca9089665323b1efb61eff4`: seventeen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 51 independently reported scale outcomes. This digest binds the separate identity and non-identity Clifford-string groups to exact correctness parents, frozen vectors, comparator sources, independent thresholds, the reviewed scalable scalar oracles, and `benchmarks/profiler-notes/qualification/perfq-m6-clifford-string.md`; it inherits no Clifford timing evidence. The inherited `m6-clifford-string` threshold is now an exact Stim/Stab measurement pair mapped only to identity/small, and validation rejects non-identity or other-scale substitution. Packed-storage optimization revision `2a0ab88c44eeeaae1714b0976089ecc1809203f3` replaces scalar per-qubit multiplication with six bit planes and an isolated portable-SIMD kernel; review-fix revision `9c672ef3c12c3fe68632e8609a58ae98714bc144` restores constant-time point metadata updates and allocation-free repeated randomization. Direct diagnostics remain non-promotable until clean paired qualification completes. Clean reviewed iterator evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632`, Pauli multiplication revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb`, transpose revision `f912cc3af1f13cc9fab798d69937c155d37d83a0`, and earlier kernel evidence remain accepted historical evidence under their recorded inventories. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `18f2b401f386ec3fb459a2d26497a96b881f03d70ca9089665323b1efb61eff4` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
