...
4. In progress: extract `stab-analysis` over model and algebra only. Gate semantic projections, circuit tag stripping, circuit tableau conversion, simplification, decomposition, bounded flattening, noise removal, repetition/surface/color generation, MBQC decomposition, unsigned flow checking/generation/solving, sparse reverse-frame tracking, unitary and selected QEC inversion, tracker-driven flow reversal, and bounded feedback lowering have moved; detecting regions, missing detectors, DEM analysis/search, and matching remain. Keep every slice free of records, execution, CLI, and ops.
