...
- `stab-algebra`, `stab-analysis`, `stab-bits`, `stab-model`, and `stab-records` build on Stable Rust 1.97.1. `stab-model` physically owns the complete circuit and DEM compatibility models. The current `stab-analysis` slices physically own gate semantic projections, fixed-shape unitary metadata, gate and circuit tableau lowering, circuit simplification and decomposition, recursive circuit tag removal, bounded flattening, noise removal, repetition/surface/color generation, MBQC decomposition, unsigned flow checking/generation/solving, sparse reverse-frame tracking, unitary and selected QEC inversion, tracker-driven flow reversal, bounded feedback lowering, detecting regions, missing-detector analysis, and typed analysis resource admission. `stab-core` preserves the established public facade and still owns the remaining DEM analysis and execution implementations; portable SIMD is temporarily absent until a dependency-free raw kernel owner restores a distinct measured implementation.
...
4. In progress: extract `stab-analysis` over model and algebra only. Gate semantic projections, circuit tag stripping, circuit tableau conversion, simplification, decomposition, bounded flattening, noise removal, repetition/surface/color generation, MBQC decomposition, unsigned flow checking/generation/solving, sparse reverse-frame tracking, unitary and selected QEC inversion, tracker-driven flow reversal, bounded feedback lowering, detecting regions, and missing-detector analysis have moved; DEM analysis, DEM transforms, search, SAT, and matching remain. Keep every slice free of records, execution, CLI, and ops.
