...
- `stab-algebra`, `stab-analysis`, `stab-bits`, `stab-model`, and `stab-records` build on Stable Rust 1.97.1. `stab-model` physically owns the complete circuit and DEM compatibility models, and `stab-analysis` physically owns every implemented pure analysis surface, including error matching and provenance values. The first `stab-engine` slice physically owns compilation-request fingerprints and execution-side biased randomization, while `stab-core` preserves thin compatibility reexports. Sampling, detection, DEM execution, and portable SIMD remain to be extracted.
