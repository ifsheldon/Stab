...
- Current first-group PQ2 evidence: `docs/plans/pq2-circuit-parse-qualification-progress-report.md`.
...
The first proving group is `PERFQ-M4-CIRCUIT-PARSE`. Its schema-version-3 source contract, materialized-source worker receipts, exact cap probes, canonical report replay, and architecture-scoped rollups are implemented and reviewed. Clean revision `f537b3677d91c1f6ee17b916bb08412691fc3989` produced current correctness-bound PR diagnostic evidence plus promotable full and soak reports for all three scales on the controlled Linux AArch64 host. Both AArch64 rollups replay, all source reports bind one exact six-digest worker identity, and no measurement is noisy. All six promotable measurements fail the unchanged `1.25x` target: soak medians are `1.281841x`, `1.273620x`, and `1.351942x` for 64, 4,096, and 65,536 instructions. The active owner action is to profile and optimize parser, instruction-construction, replacement, or destruction throughput without changing semantic work, fixtures, scales, comparator, or gate, then regenerate the complete correctness-bound AArch64 full and soak families and rollups from one clean commit. `perf_event_paranoid=4` blocks fresh local stack attribution, so use an authorized controlled host configuration or another controlled AArch64 host instead of guessing from source. Native x86-64 evidence remains a separate unclaimed host dependency. See `docs/plans/pq2-circuit-parse-qualification-progress-report.md`; do not treat this one failed group as completion of `PERF-CIRCUIT-MODEL` or PQ2.
