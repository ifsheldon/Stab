...
- Current first two PQ2 product-group reports: `docs/plans/pq2-circuit-parse-qualification-progress-report.md` and `docs/plans/pq2-circuit-canonical-print-qualification-progress-report.md`.
...
Program checkpoint: CQ0 is source-current at correctness inventory digest `b80801fea6eae550feecf40489259de56123f6f3331b747d52c323d576fd0285`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,743 evidence parents. Of those parents, 579 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first two PQ2 product contracts are source-current at performance inventory digest `f3c4009044b0bafcd877f76798c7f4f08c475c0877b85f68d22ae0449e3ddb8f`; exactly `PERFQ-M4-CIRCUIT-PARSE` and `PERFQ-M4-CIRCUIT-CANONICAL-PRINT` are graduated to implemented exact runtime groups. The legacy canonical-printer row is superseded and its M12 beta, timing-regression, and memory waivers are retired. No threshold or comparator fidelity rule was relaxed.
CQ2 implementation and ownership remain complete for its eight selected domains and 270 implemented exact parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 59, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. The clean all-domain PR, full, and soak reports from revision `bae9e01cb3fedaf9d37958e6827b064c635b9898` remain historical after later owner changes. Current clean exact execution for the three parser and canonical-printer prerequisites passed on revision `ba70a52025fdd4122ac97cec263725b2ec56e431`; rerun the full 270-case CQ2 family before citing all-domain execution as current-digest evidence, but do not block an exact dependent performance group whose own current-digest prerequisites and preflight pass.
...
The first two proving groups are complete on the controlled Linux AArch64 host. Clean revision `ba70a52025fdd4122ac97cec263725b2ec56e431` produced twelve passing, non-noisy full and soak measurements plus four replayed architecture rollups from one correctness request and one reproducible six-digest worker identity. Parser medians range from `0.897744x` to `0.970298x` with worst upper bound `0.974833x`; canonical-print medians range from `0.372912x` to `0.376075x` with worst upper bound `0.398775x`. Every source report passed the unchanged `1.25x` median and upper-confidence-bound gate. Process RSS remains a separate claim: canonical printing is about `1.32x` to `1.35x` Stim peak RSS on this host, while parser RSS is higher at small and medium scales and lower at large scale. Native x86-64 evidence remains a separate unclaimed host dependency. Programmatically constructed circuits deeper than the parser's 256-level admission limit remain a CQ6/PQ6 resource-boundary risk, and the flat canonical-print ratio does not claim tag-heavy, float-heavy, target-heavy, repeat-heavy, or public file-output performance. See both PQ2 circuit progress reports; do not treat these groups as completion of `PERF-CIRCUIT-MODEL` or PQ2.
...
Do not treat PQ0's 14 retained rows as qualified evidence: the current inventory still reports 158 missing correctness preflights, 158 missing output digests, 58 asymmetric CLI rows, 73 missing comparators, 123 missing scale families, and 20 heterogeneous selections. The two implemented circuit contracts replace only their exact parse and canonical-serialization groups. The active owner action is to define the next finite PQ2 executable slice from the checked planned inventory, bind it to exact current-digest CQ prerequisites, and implement one phase-specific equivalent-work group at a time without reopening the completed M4 evidence unless shared source changes.
