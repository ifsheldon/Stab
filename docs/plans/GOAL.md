...
- Exact ML admits at most 20 detectors, one observable, 256 mechanisms, 65,536 represented instruction visits, and `2^21` joint states. Its mutually exclusive probability workspaces are bounded to 16 MiB for the primary pass and 32 MiB for tie resolution; pair updates are bounded to `2^28` per pass and `2^29` total when tie resolution runs.
