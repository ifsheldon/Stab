...
5. In progress: circuit sampling, measurement-to-detection conversion, and circuit detection sampling are extracted into `stab-engine`; move DEM sampling next, then reference-sample trees and sampled-flow execution without changing facade or CLI behavior.
