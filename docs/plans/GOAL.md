...
Program checkpoint: CQ0 is source-current at correctness inventory digest `3db44922e3310cb3a573fcff3b28d5eea5d28e0d6975e0856965c601ecc23c72`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. The digest changed because the exact Pauli multiplication owner now exercises direct non-identity in-place multiplication through 1,000,000 qubits. PQ0 is source-current at performance inventory digest `84d5ab682acda2a847972a74c5d58443fde8d2c820e62e46b634562e7c918e46`: thirteen PQ2 product contracts are executable, including the new scale-bound `PERFQ-M6-PAULI-STRING` contract. Pauli timing, regression, migration, and completion evidence are not yet accepted. Clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` remains accepted historical transpose evidence under its recorded inventory, with in-place median ratios from `0.366307x` to `0.684489x`, worst confidence-interval upper bound `0.686065x`, allocating median ratios from `0.397088x` to `0.657658x`, and worst upper bound `0.660876x`. Earlier clean revisions `e660c91cff142b611f52a0a28a36e0a3d15670ed` and `1264d885087761b19b37beded47811cc0c117e4d` remain historical authorization and diagnostic evidence under their exact worker identities. The focused migration supersedes only the heterogeneous `m5-simd-bit-table` timing threshold; its memory baseline remains active. Clean revisions `60b732c77f1828058fbd65ec6c5c4ad582467fd1` and `7b43b46d1c08f669264d009b8d3872ce86838f0e` retain accepted historical AArch64 `not_zero` and sparse-XOR evidence under their recorded inventories. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
The active ninth PQ2 slice is `PERFQ-M6-PAULI-STRING` in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. Its source-owned worker, independent pinned-Stim comparator, exact small, medium, and large scale mappings, schema-version-9 preflight, and schema-version-27 report contract are executable at the current inventories; no performance outcome is accepted yet. It qualifies only equal-width public `PauliString::right_multiply_in_place_returning_log_i_scalar` against pinned Stim with deterministic non-identity operands at 10,000, 100,000, and 1,000,000 qubits.
Do not use the inherited identity-only `m6-pauli-string` ratio as evidence: Stab's public identity-right fast path makes that legacy comparison O(1) on the Stab side. Keep the legacy row-level and three exact pair-level `1.25x` thresholds active until the faithful group has a clean pre-migration completion receipt, then retire only those timing thresholds through the scale-aware migration and regenerate the complete evidence chain from one clean post-migration revision.
