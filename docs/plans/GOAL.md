...
Program checkpoint: CQ0 is source-current at correctness inventory digest `deb6c025854e0e9dc555b45ee5afda33ac22b31c307d41d01731fa320a399f73`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,741 evidence parents. Of those parents, 577 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first PQ2 product contract are source-current at performance inventory digest `ce0c451a9c3123be95d3b9606b96a7ce26e3b26f09f543ae7d2e9e0345e86d54`; exactly `PERFQ-M4-CIRCUIT-PARSE` is graduated to an implemented exact runtime group, and no threshold, waiver, or comparator fidelity rule was relaxed.
CQ2 is complete on clean committed revision `bae9e01cb3fedaf9d37958e6827b064c635b9898`. Its eight selected domains contain 270 implemented exact parents and zero planned parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 59, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. Clean current-digest execution passed 144 of 144 PR cases, 270 of 270 full cases, and 270 of 270 soak cases with `local_modifications=false`; offline report regeneration and controller-bound exact preflight also passed. The Algebra slice assigns all 654 selected exported Rust API items, dispositions 347 pinned-upstream records as 12 ported Rust, 197 semantic-mining, and 138 deferred-product records, and closes typed Flow, Tableau, repeat-work, solver, iterator, random, and unitary resource contracts after milestone audit and GPT-5.6/max full-code-review.
...
The first proving group is `PERFQ-M4-CIRCUIT-PARSE`. Its schema-version-3 source contract is implemented with three exact input identities, symmetric `circuit-parse` and `parse` workers, exact CQ2 ownership, output identity, owner, profiler note, and report-bound timing and memory evidence. Commit that contract before regenerating evidence, then run `just bench::qualification-worker-reproducibility` from the clean unchanged commit before producing a current-commit CQ preflight, PR diagnostic report, clean full and soak reports for all three scales, regression replay, and separate fail-closed AArch64 full and soak scale-family rollups. The reproducibility check must hash the worker source from each materialized build commit, require each sealed binary to confirm its receipt source and build identity through the protocol, and prove both binaries reject the first unsupported circuit-parse scale before the start barrier. Every source report in one rollup must use the exact same Stim and Stab worker source, build fingerprint, and binary digest. Each rollup must be produced from the same clean commit as its source reports, record the producer repository state, and pass `just bench::qualification-rollup-report --input <rollup-directory>` so canonical reconstruction, exact source bindings, and compare-and-swap publication are proven. Run milestone audit and GPT-5.6/max full-code-review before accepting the slice. Existing reports from revision `969f399c93c6540022f8ca5aeb9f0c26ed13a49f` predate the current schema and are historical. Native x86-64 evidence is a separate host dependency and must remain unclaimed until collected on a controlled x86-64 host. Do not treat landing the harness seam, one scale, one tier, or one architecture as completion of the circuit-model feature.
...
Do not treat PQ0's 14 retained rows as qualified evidence: 158 of 159 active inherited rows still record missing correctness preflight and output-digest evidence, 58 CLI rows remain asymmetric, 73 rows lack a current comparator, 123 lack required scale families, and 20 select heterogeneous upstream measurements. The implemented circuit-parse contract removes those exact inventory deficiencies only for its own row; its measured AArch64 ratios remain failed until both the median and upper confidence bound meet `1.25`.
