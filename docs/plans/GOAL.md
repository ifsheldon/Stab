...
Program checkpoint: CQ0 is source-current at correctness inventory digest `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at post-migration performance inventory digest `a3be3a7404d6dbc4bcffae0e3dea52e6b93797102dc5b08a776323044efdabcf`: nineteen PQ2 product contracts remain executable, each with one exact `1.25x` rule over three scales, for 57 independently reported outcomes. Clean pre-migration revision `d9e2405d18cfff05d9b5d908525394476b0edcbc` passed and replayed both DEM adapter probes, worker reproducibility, the exact focused CQ prerequisite, all twelve first-attempt full and soak scale reports and regressions, four rollups, and both completion receipts under preceding performance digest `a98f57cf194f3a021d321266656cf688c9f7780fb39fa337475e8132411eb88a`. Parse medians range from `1.102449x` through `1.159075x`, print medians range from `0.559351x` through `0.580445x`, and the worst confidence-interval upper bound is `1.167830x`. Focused migration commit `1cfecd64cde4a5effdf07fdaabdbe51017e25a4a` retired only the exact legacy parse timing threshold and print no-ratio waiver, preserved both memory baselines, and marked both old rows superseded and non-primary. Schema-version-2 `benchmarks/qualification-threshold-migrations.json` binds both authorizations, their distinct guard kinds, the complete replacement scale families, migration revision and inventory, and retained memory evidence. Clean post-migration revision `7c3e55301b3f098497613d7dad2d624dc08a4dda` passed both probes, worker reproducibility, focused correctness, all twelve first-attempt reports, regressions, and four rollups, but independent parse-completion replay exposed that semantic contract probes were mislabeled as timing evidence and could reject a legitimate zero-duration clock reading. Preserve that chain as review-rejected historical evidence and do not rerun or relabel it. The corrected harness uses private Stab build-receipt schema version 6, adapter receipt schema version 12, worker-protocol schema version 4, contract-preflight schema version 14 with 228 probes, and qualification report schema version 33. Clean post-fix revision `9497df0350cb33dcd249ea12fda802b5a68efe00` passed and replayed both probes, worker reproducibility, the exact focused CQ prerequisite, all twelve first-attempt reports and regressions, four rollups, and both completion receipts. Parse medians range from `1.090387x` through `1.145890x`, print medians range from `0.569331x` through `0.589777x`, and the worst confidence-interval upper bound is `1.150114x`. That chain is source-current machine evidence but remains pending milestone audit, independent review, and final verification. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence only under its recorded inventory. Setup and peak RSS remain report-only until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, batching policy, publication-retention rule, or migration rule may be relaxed.
...
The active work is final review closure of the twelfth PQ2 slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. Implementation, exact correctness ownership, focused legacy migration, retained memory-baseline checks, the source-current post-fix prerequisite chain, all twelve formal reports, all report replays and regressions, four rollup replays, and both completion replays are complete. Continue in this exact order:

1. Preserve every `9497df0` CQ, formal report, rollup, and completion artifact unchanged. Do not rerun a stable timing producer or overwrite a retained path. Treat the pre-migration chain only as migration authorization and retain the `7c3e553` chain only as review-rejected historical evidence.
2. Finish source-owned documentation synchronization with the exact `9497df0` artifact paths, hashes, ratios, confidence bounds, report-only RSS observations, replay and regression results, zero timing reruns, restored swap, and explicit pending-review status. Commit this evidence-only update separately from any later implementation fix.
3. Run `milestone-audit` over every twelfth-slice task, test, benchmark, migration, memory-retention rule, evidence artifact, and acceptance criterion. Fix every confirmed implementation or evidence issue. If a fix changes a worker, adapter, fixture, schema, runtime contract, correctness prerequisite, source inventory, or measured hot path, regenerate the complete affected prerequisite and performance chain from the new clean revision instead of relabeling `9497df0`. Log only genuine newly revealed under-specification in `docs/plans/milestone-spec-gaps.md`.
4. Spawn independent GPT-5.6/max `full-code-review` lanes for core DEM correctness, worker and adapter equivalence, hostile-input and artifact lifecycle, benchmark science and migration, and documentation consistency. Give each lane the full twelfth-slice diff and evidence contract, require P0 through P3 findings with exact file references, and keep lane conclusions independent until synthesis.
5. Resolve every confirmed review finding and rerun all affected targeted checks. Regenerate source-owned evidence from a new clean revision whenever a fix changes an evidence identity or performance contract; do not carry forward an old ratio across that boundary.
6. Run full repository verification, commit focused fixes and final evidence documentation, and advance this execution contract only when audit and review are closed, source-current docs agree, both final completion receipts replay from the accepted revision, swap is restored, no qualification process remains, and the worktree is clean.
