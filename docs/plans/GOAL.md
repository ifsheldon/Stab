...
- Exact revision `67bd94eccd803d490cf96321eaeba0e96f691927` passed GitHub Actions run `32438708595`, PR correctness (729 of 729), broad full and soak correctness (962 of 962 each), all eleven exact full prerequisites, the live result-format and implemented oracle suites, and 46 worker reproducibility receipts.
- All seventeen non-DEM release groups passed 102 full and soak reports and produced 34 replayed rollups. DEM parse published all eighteen reports, but coordinate-sparse-large/full failed Stim parity at median `1.223931x` and confidence upper bound `1.271272x`; the verified-host result was preserved without rerun, waiver, or threshold change. Every `67bd94ec` artifact becomes historical with the source repair.
- The replacement parser keeps the two-item nested-body storage win while restoring a direct preallocated top-level vector. Alternating diagnostics preserve exact identities; all flat-error and coordinate-sparse medians improve, folded medians remain neutral, and separate five-pair large panels show 1.9 percent coordinate and 2.7 percent folded gains. These dirty-tree diagnostics guide the fix but are not formal evidence.
...
1. Regenerate checked correctness and performance identities, complete milestone audit, full code review, broad verification, and a focused parser/status commit; push it and require exact-revision `Rust` and `Qualification Contracts` CI before producing replacement evidence.
2. Regenerate and replay PR, full, and soak correctness, all 11 exact prerequisites, the live result-format corpus, the implemented oracle matrix, and 46 worker reproducibility receipts at unique replacement paths; from that clean revision also run `pq2-dem-parse-adapter-smoke` and `pq2-dem-canonical-print-adapter-smoke` before formal timing.
