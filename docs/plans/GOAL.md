...
Program checkpoint: CQ0 is source-current at correctness inventory digest `5d795e831bc20b3f2780ca72c1eaea7c75387388d38f8e37f4539254a41e821b`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory digest `1d38c155acbaf78234f9b92857cfef8c25ffa059a4a9e9756b272a72272dfd0d`: twelve PQ2 product contracts are executable, including independent allocating and in-place transpose groups for the four public transpose rustdoc paths. Clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` passed both exact correctness prerequisites, worker reproducibility, both public probes, all 12 first-attempt full and soak reports, all report replays and regressions, four rollups, and both completion receipt replays. In-place median ratios range from `0.370154x` to `0.695896x`, with worst confidence-interval upper bound `0.696808x`; allocating median ratios range from `0.398336x` to `0.673927x`, with worst upper bound `0.679028x`. That complete chain authorized superseding only the heterogeneous `m5-simd-bit-table` timing threshold; its memory baseline remains active, and no pre-migration report is relabeled as source-current post-migration evidence. Clean post-migration revision `1264d885087761b19b37beded47811cc0c117e4d` also completed and replayed the full chain, with in-place medians `0.370966x` to `0.700434x` and worst upper bound `0.702625x`, and allocating medians `0.396103x` to `0.666317x` and worst upper bound `0.669186x`. Independent review then tightened the exact in-place edge oracle and moved semantic-work overflow rejection before fixture allocation and barrier consumption in both workers. Those changes intentionally invalidate the old fingerprints, so both clean chains are historical and the complete source-current chain must be regenerated. Clean revisions `60b732c77f1828058fbd65ec6c5c4ad582467fd1` and `7b43b46d1c08f669264d009b8d3872ce86838f0e` retain accepted historical AArch64 `not_zero` and sparse-XOR evidence under their recorded inventories, including accepted-maximum allocation, regression, rollup, completion, audit, and review closure. The sparse-XOR ratios range from `0.965755x` to `1.026014x`, with worst confidence-interval upper bound `1.034133x`. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
Do not treat PQ0's 12 retained rows as qualified evidence: the current inventory reports 158 missing correctness preflights, 158 missing output digests, 58 asymmetric CLI rows, 73 missing comparators, 123 missing scale families, and 20 heterogeneous selections. The twelve implemented contracts replace only their exact parse, canonical-serialization, all-gate-name-hash, toggle-plus-popcount, complete-vector dense-XOR, three position-specific `not_zero`, sparse row-XOR, sparse item-toggle, allocating transpose, and square in-place transpose groups.
...
The exact API ownership, scalar-reference and allocation tests, safe blocked kernel, sealed workers, independent adapter, invocation preflight, runtime groups, probes, and generated inventory are implemented. Do not add a public raw-word constructor, a public preallocated transpose API, architecture-specific intrinsics, or a zero-filled no-op-friendly fixture to make the comparison easier.
...
The clean first-stage evidence at `e660c91cff142b611f52a0a28a36e0a3d15670ed` and clean post-migration evidence at `1264d885087761b19b37beded47811cc0c117e4d` remain historical authorization and diagnostic evidence only after review tightened the exact in-place edge oracle and the fingerprinted worker contract.
...
The focused migration supersedes the legacy `m5-simd-bit-table` timing threshold while retaining its exact upstream provenance and M12 memory baseline. Commit the review fixes and regenerated inventory, then rerun both exact CQ prerequisites, worker reproducibility, both probes, all 12 full and soak reports, all report replays and regressions, four rollups, and two completion receipts from that one clean revision before claiming closure. The 90-receipt preflight must prove valid-shape semantic-work overflow rejection before fixture allocation and before consuming the enabled start barrier for both implementations and both transpose methods. PQ6 alone may replace the memory baseline.
