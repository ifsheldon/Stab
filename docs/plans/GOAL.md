...
The active execution checkpoint is the eleventh PQ2 Clifford-string slice. The runtime contracts, frozen cross-language vectors, allocation checks, hostile probes, source-shape tests, packed portable-SIMD implementation, scalable independent scalar oracles, packed-storage review fixes, and exact legacy identity/small replacement mapping are present. Next commit the refreshed inventories, reproduce both private workers, publish and replay the exact same-revision CQ prerequisites, and rerun both adapter probes. Then run both groups at all three scales for full and soak only on a host with no swap-counter movement, replay every report and regression, build architecture-scoped rollups and completion receipts, perform the legacy identity-row migration only after the replayed pre-migration identity completion authorizes it, and repeat the complete two-group chain from one clean post-migration revision. Keep every failed, noisy, or host-unverified `1.25x` outcome visible and do not promote diagnostics.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `0ee3639389860799298164c94c647fcab45b03c9d67b941b1aad12c6e5e06df5` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
