...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first four PQ2 product contracts are source-current at performance inventory digest `877df12bf1b3d63da92289e22f117097cedbc20860d165c90b41554aa110263b`; `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, and `PERFQ-M5-SIMD-WORD` are graduated to implemented runtime groups. Milestone audit rejected the initial popcount acceptance evidence because the Stim comparator did not reproduce the exact architecture-dependent upstream loop, the output digest omitted three fixture-fingerprint lanes, and output work crossed the timing boundary. The corrected popcount source contract binds the exact `ptr_simd[k].popcount()` comparator, CMake-resolved `libstim` compile flags, canonical eight-field output encoding, fixed odd and even vectors, the accepted maximum, all three rejection classes, both exact comparator-source digests, and 18 report-bound receipts from the probes that actually executed. The subsequent GPT-5.6/max review found that a refingerprinted receipt set could still be transplanted between worker pairs and that the frozen protocol output literal was not owned by a shared vector contract; report schema version 18 now includes both workers' six exact source, build-fingerprint, and binary identities in the preflight digest, cross-checks them during replay, and derives both invocation and worker tests from one canonical protocol vector. A clean committed qualification cycle remains required before any corrected popcount ratio is promotable. The first three groups' passing AArch64 reports and all earlier popcount reports remain historical after the global performance inventory, shared workers, or report evidence contract changed. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. The legacy canonical-printer row is superseded and its M12 beta, timing-regression, and memory waivers are retired; the legacy `m5-simd-word` M12 threshold remains active until explicit replacement migration. No threshold or comparator fidelity rule may be relaxed.
...
The first three proving groups passed on the controlled Linux AArch64 host under preceding performance inventories. Clean revision `ba70a52025fdd4122ac97cec263725b2ec56e431` produced parser and canonical-print evidence, and clean revision `c76b7071fc4d7ac358ef3a2fffc053ea675bd05f` produced gate-hash evidence with medians from `0.931886x` to `0.932764x` and worst upper bound `0.933289x`; all are historical after the popcount graduation changed the global inventory and shared workers. Clean revision `38a2d5eab2fec3211eb9466899c6afd0ba91c4ca` produced three passing CQ prerequisites, reproducible private workers, and three apparently fast full-tier popcount reports, but audit invalidated those ratios because their comparator, output identity, and timing boundary did not satisfy the declared contract. The active owner action is to close review on the corrected fourth-group source, commit it, rerun its three exact CQ prerequisites and dependent preflight, prove receipt-bound Stim compile flags, exact comparator-source digests, and all 18 report-bound worker-preflight receipts, prove private-worker reproducibility including fixed odd and even outputs, actual accepted-maximum execution, and all three pre-barrier rejection classes, run the adapter probe, and publish clean full and soak AArch64 scale families plus rollups without weakening the `1.25x` gate. Do not rerun parser, printer, or gate merely to conceal this interim inventory transition; rerun them when a program checkpoint requires simultaneous source-current evidence. Native x86-64 execution remains an unclaimed evidence target, but x86 builds must still prove that the adapter inherits Stim's resolved machine flags before they can produce any ratio.
