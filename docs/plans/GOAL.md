...
- Exact ML admits at most 20 detectors, one observable, 256 mechanisms, 65,536 represented instruction visits, and `2^21` joint states. Its mutually exclusive directed-interval and exact-dyadic probability workspaces are each bounded to 32 MiB; exact tables above that cap fail with a typed resource error, and pair updates are bounded to `2^28` per pass and `2^29` total when exact certification runs.
