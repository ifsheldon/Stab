...
- `stab-algebra`, `stab-analysis`, `stab-bits`, `stab-model`, and `stab-records` build on Stable Rust 1.97.1. `stab-model` physically owns the complete circuit and DEM compatibility models. The current `stab-analysis` slices physically own gate semantic projections, fixed-shape unitary metadata, gate and circuit tableau lowering, circuit simplification and decomposition, recursive circuit tag removal, and typed analysis failures. `stab-core` preserves the established public facade and still owns the remaining analysis and execution implementations; portable SIMD is temporarily absent until `stab-kernels-simd` is extracted as a dependency-free raw kernel owner.
...
4. In progress: extract `stab-analysis` over model and algebra only. Gate semantic projections, circuit tag stripping, circuit tableau conversion, simplification, and decomposition have moved; transforms, flows, generation, DEM analysis/search, matching, MBQC, and reverse tracking remain. Keep every slice free of records, execution, CLI, and ops.
