...
Active execution goal as of 2026-07-23.
...
Program checkpoint: CQ0 is source-current at correctness inventory digest `9b110280da2ac9c61568265a19c6b41ead79aa1bec6c8943ecee6a5b6d39df5`, with 2,886 upstream records, 1,986 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at post-migration performance inventory digest `30e9df3e8004b59e43716dbb9e7aa847f472811e0adba74c43ef6bc7b243d498`: nineteen PQ2 product contracts remain executable, each with one exact `1.25x` rule over three scales, for 57 independently reported outcomes. Clean revision `9497df0350cb33dcd249ea12fda802b5a68efe00` retains a complete historical DEM chain with worst confidence-interval upper bound `1.150114x`, and `dade78f7aa91af98e2f79441ec9695c37436a21c` retains historical accepted-maximum RSS observations. Five independent GPT-5.6/max review lanes later rejected those artifacts as current evidence after confirming five pinned-Stim DEM correctness gaps, asymmetric C++ post-return assignment timing, a mismatched C++ peak-RSS phase boundary, and stale PQ0 counts. The source-current contract repairs every finding: a zero-capable `DemRepeatCount`, Stim-compatible empty arguments, zero repeats, 60-bit text limits, ASCII lexical boundaries and standalone target case, output-returning C++ timing for every return-valued workload, and pre-digest RSS capture. No repaired-source machine chain exists yet. Preserve every earlier artifact under its exact inventory, never overwrite a retained path, and do not carry forward an old ratio or RSS observation. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence only under its recorded inventory. Setup and peak RSS remain report-only until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, batching policy, publication-retention rule, or migration rule may be relaxed.
...
CQ2 implementation and exact ownership remain complete for eight selected domains and 271 implemented parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. The source-current inventory is `9b110280da2ac9c61568265a19c6b41ead79aa1bec6c8943ecee6a5b6d39df5`; it adds twelve `DemRepeatCount` API records to the strengthened exact DEM model case and has no current all-domain execution claim. Clean historical all-domain and focused evidence remains valid only under each recorded producer and inventory. Exact historical digests remain recorded in `docs/plans/cq2-deterministic-qualification-progress-report.md`.
...
The active work is repaired-source closure of the twelfth PQ2 slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. The first independent review is complete and every confirmed source finding is implemented, but all machine evidence at `9497df0` and `dade78f` is historical because the correctness inventory, shared adapter timing boundary, peak-RSS boundary, and source digests changed. Continue in this exact order:

1. Preserve every existing CQ, formal report, rollup, completion, and accepted-maximum memory artifact unchanged. Verify `/swap.img` is enabled before and after any host preparation. Never reuse an artifact directory, rerun a stable result from the same revision, or relabel an old ratio or RSS observation under the repaired inventory.
2. Finish source-contract validation: run the exact DEM tests, full `stab-core` suite, correctness regeneration and check, performance regeneration and check, benchmark smoke, focused `stab-bench` tests, formatting, Clippy, and staged pre-commit. Review generated inventory diffs instead of accepting digest churn blindly.
3. Commit the correctness, adapter, inventory, and synchronized status repairs as a focused source-contract commit. All evidence producers after this point must report that exact clean revision and `local_modifications=false`.
4. From that clean revision, run both source-owned DEM adapter probes and private-worker reproducibility. Publish a fresh exact focused CQ full prerequisite for the strengthened DEM case, replay it independently, and pass exact preflight using its request and completion digests.
5. Repeat both accepted-maximum DEM probes in `memory` mode at 524,288 items and record Stim and Stab setup and peak RSS from the corrected pre-digest phase boundary. These observations remain report-only and do not claim PQ6 growth acceptance.
6. Publish exactly one fresh full and one fresh soak report for every parse and print scale. Disable swap only around each formal timing producer using an unconditional restoration trap, restore `/swap.img` immediately afterward, and retain failed, noisy, host-rejected, or controller-rejected first attempts. Do not rerun a non-noisy result merely because it fails the `1.25x` gate.
7. Independently replay all twelve reports and run the exact source-owned regression check for each. Publish and replay separate parse-full, parse-soak, print-full, and print-soak rollups, then publish and independently replay separate parse and print completion receipts.
8. Update the progress report with exact revision, inventory, request, report, preflight, Markdown, rollup, completion, worker, adapter, and memory hashes; all ratios and confidence intervals; retained failure dispositions; commands; swap restoration; and unclaimed native x86-64 and PQ6 scope.
9. Run `milestone-audit` against the repaired source and fresh evidence. Fix implementation or evidence findings; log only genuine newly revealed under-specification in `docs/plans/milestone-spec-gaps.md`.
10. Spawn five new independent GPT-5.6/max `full-code-review` lanes for core DEM correctness, worker and adapter equivalence, hostile-input and artifact lifecycle, benchmark science and migration, and documentation consistency. Give each lane the complete repaired-source diff and fresh evidence contract, require P0 through P3 findings with exact file references, and synthesize only after all lanes finish.
11. Resolve every confirmed final-review finding and regenerate only the evidence whose bound source identity changed. Run full repository verification, commit focused final evidence documentation, and advance this execution contract only when both completion receipts replay from the accepted revision, source-current docs agree, swap is restored, no qualification process remains, and the worktree is clean.
