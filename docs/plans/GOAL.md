...
- Exact revision `b397052d` passed GitHub Actions run `31267356816` and produced passing PR (728 of 728) and full (961 of 961) correctness reports, but the subsequent packed-B8 repair changed the public correctness and performance inventories. Those reports remain historical; no source-current correctness or performance chain exists for release authorization.
- Commit `17871e75` restores representation-preserving `b8` to `b8` conversion through the shared record reader. Its clean focused diagnostic measured the wide M7 row at `0.043610x` pinned Stim after the pre-fix source measured `2.616x`; the unchanged `1.25x` beta status passes. Correctness inventory `3efea1b3...` and performance inventory `7525cc9b...` now bind the new public packed-frame method.
...
1. Commit this B8 regression checkpoint, push the resulting clean revision, and require exact-revision `Rust` and `Qualification Contracts` CI.
