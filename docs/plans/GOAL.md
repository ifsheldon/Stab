...
Status: Active. Milestones A0 through A8 are complete. A9 is pre-evidence while the final completion-replay and release-boundary repairs await push, exact-revision CI, and one clean audit pass.
...
- Focused repairs now bind replay inputs through retained descriptors, recompute final descriptor-rooted Git state, validate release toolchains across targets, recheck the remote tag after upload, bind workflow dispatch to the exact tag commit, and quote typed `just` arguments while building each local operator in a fresh owner-only target.
...
1. Synchronize the progress report, release procedure, architecture plan, and contributor workflow with the repaired contracts.
...
5. Produce fresh controlled AArch64 full and soak evidence for all 19 release groups, both accepted-maximum DEM memory receipts, 38 rollups, and one 138-report `a9-release` completion under soft `RLIMIT_NOFILE=1024`; replay the completion once.
