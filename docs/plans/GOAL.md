...
Program checkpoint: CQ0 is source-current at correctness inventory digest `7e42ddddd662593b56f0bd67885b74babf9a96319de990e4f2cb6218638edea5`; PQ0 is dependency-regenerated at performance inventory digest `67bcbfcf2d991c883b6d889bf48b4d9b8c09bcb52bdbd6dc1e041b6162a30193`. The previous clean correctness and dependent PQ1 reports from revision `d0ecafd62794daad0ab5eb63d54c481a5e32a30b` remain historical Generation-checkpoint evidence because later Algebra ownership and resource-admission changes changed both frozen digests. Their 1.014015, 1.015160, and 1.015225 median ratios measure only the synthetic adapter protocol and are not Stab product-performance evidence. Publish new clean reports only after the Algebra slice has exact parents and passes audit and review.
The active milestone is CQ2: qualify deterministic models, formats, gates, bit kernels, circuit APIs, generation, and stabilizer algebra at exact case and exported-API granularity before PQ2 times those surfaces. The selected `.stim`, `.dem`, result-format, gate-contract, bit-kernel, circuit-API, and Generation domains are source-complete and accepted with 29, 28, 39, 59, 12, 24, and 25 implemented evidence parents respectively and zero planned owners. The Generation slice assigns all 107 selected exported Rust API items, dispositions all 15 relevant pinned-upstream records, preserves the Python `Circuit.generated` string-dispatch symbol as one product deferral, and adds exact family, parameter, no-noise matrix, helper, and pre-allocation resource contracts. The reviewed Algebra classifier selects 217 portable upstream records, explicitly defers 139 product records, rejects unexposed C++ helpers as not applicable, and now assigns 650 exported Rust API items after adding the typed resource contract. CQ2 has 217 implemented owners and 457 planned owners across 674 selected owners. The deterministic Algebra materialization prerequisite is implemented with typed Pauli, Clifford, Tableau, random-Tableau, stabilizer-solver, iterator, circuit-conversion, and unitary-conversion admission plus exact boundary tests. The active implementation blocker remains `CQ-ALGEBRA`, where 457 of 458 owners are planned until exact semantic parents are mapped and the complete slice passes audit and review.
