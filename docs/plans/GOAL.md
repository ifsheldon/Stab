...
Program checkpoint: CQ0 is source-current at correctness inventory digest `fbaa2bdf8bc0eea01b2aca385a4e537de11c3b35099671cca0e2775950c2fbb0`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. The third DEM parser optimization moved two rustdoc source lines without changing any selected parent, owner, selector, disposition, case ID, or count. PQ0 is source-current at performance inventory schema version 2 and digest `a98f57cf194f3a021d321266656cf688c9f7780fb39fa337475e8132411eb88a`: nineteen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 57 independently reported scale outcomes. Runtime-group schema version 5 binds independent direct DEM parse and canonical-print contracts, their exact correctness parent, frozen eight-item fixture, three scales, pinned-Stim comparator sources, output identities, and source-owned profiler note while preserving the separate Clifford contracts and source-owned independent-throughput timing only for Clifford identity. The clean Git revision, private Cargo build, sealed Stab binary digest, and worker identity bind the private parser implementation. The checked threshold-migration ledger continues to bind every completed legacy retirement and preserved memory baseline. Clean revision `ca9fd68d3856e9cac9cc6da16433947d056e8848` passed parse/small/full and parse/medium/full but retained and replayed a stable parse/large/full failure at median `1.248221x` and confidence-interval upper bound `1.263920x`; it was not rerun. Clean implementation commit `430428ea93a40af25a352746acc2bc517e7ad1fd` contains the third optimization but has no formal timing result yet. Print has no formal timing evidence, and neither DEM contract has migration authorization. The source-current harness uses private Stab build-receipt schema version 6, adapter receipt schema version 12, contract-preflight schema version 13 with 228 probes, and qualification report schema version 32. No prior correctness or performance report is relabeled as current after its worker, hostile-request, publication, migration, fixture, receipt, hot-path, or inventory contract changes. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence under its recorded inventory. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, source-owned batching policy, publication-retention rule, or migration rule may be relaxed.
