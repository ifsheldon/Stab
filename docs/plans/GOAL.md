...
Program checkpoint: CQ0 is source-current at correctness inventory digest `e5635ca4a2c217ee79768d35336c50611a9f12f86edded8a7f01fce1ca72add4`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. The second DEM parser optimization moved the same 108 rustdoc source lines without changing any selected parent, owner, selector, disposition, case ID, or count. PQ0 is source-current at performance inventory schema version 2 and digest `d27b71d205ec938a77c9981fb524e70c9b17eeafeadf3ade6367632aeb3b1693`: nineteen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 57 independently reported scale outcomes. Runtime-group schema version 5 binds independent direct DEM parse and canonical-print contracts, their exact correctness parent, frozen eight-item fixture, three scales, comparator sources, output identities, and source-owned profiler note while preserving the separate Clifford contracts and source-owned independent-throughput timing only for Clifford identity. The checked threshold-migration ledger continues to bind every completed legacy retirement and preserved memory baseline; neither DEM contract has migration authorization or passing second-optimization timing evidence yet. Historical clean revision `d8de73d50fbeb4e001ea38c784d1fdcfc76dad76` retained and replayed a stable first-optimization parse/small/full failure at median `1.372971x` and confidence-interval upper bound `1.377375x`; clean implementation commit `e9b94d524167b2ca7bb953e525abc26e77641629` has no formal timing result yet. The source-current harness uses private Stab build-receipt schema version 6, adapter receipt schema version 12, contract-preflight schema version 13 with 228 probes, and qualification report schema version 32. No prior correctness or performance report is relabeled as current after its worker, hostile-request, publication, migration, fixture, receipt, hot-path, or inventory contract changes. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence under its recorded inventory. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, source-owned batching policy, publication-retention rule, or migration rule may be relaxed.
