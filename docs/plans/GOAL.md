...
Program checkpoint: CQ0 is source-current at correctness inventory digest `b9ed22cfcf637e740e68501b32b764015e937e6bc8d75ced3328f62ecc20cf40`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. The DEM parser optimization moved 108 rustdoc source lines without changing any selected parent, owner, selector, disposition, case ID, or count. PQ0 is source-current at performance inventory schema version 2 and digest `acd913e0966b9968b24c06bd2fed678f4608f0ff1ab82735dab5ebc109f119b3`: nineteen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 57 independently reported scale outcomes. Runtime-group schema version 5 binds independent direct DEM parse and canonical-print contracts, their exact correctness parent, frozen eight-item fixture, three scales, comparator sources, output identities, and source-owned profiler note while preserving the separate Clifford contracts and source-owned independent-throughput timing only for Clifford identity. The checked threshold-migration ledger continues to bind every completed legacy retirement and preserved memory baseline; neither DEM contract has migration authorization or promotable post-optimization timing evidence yet. The source-current harness uses private Stab build-receipt schema version 6, adapter receipt schema version 12, contract-preflight schema version 13 with 228 probes, and qualification report schema version 32. No prior correctness or performance report is relabeled as current after its worker, hostile-request, publication, migration, fixture, receipt, hot-path, or inventory contract changes. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence under its recorded inventory. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, source-owned batching policy, publication-retention rule, or migration rule may be relaxed.
