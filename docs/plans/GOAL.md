...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and eight PQ2 product contracts are source-current at performance inventory digest `315c1d985af62f08068cd273a0e06399aaa7dd85ff3009309e284bc46aaaaf3d`: the first five historical groups plus independent early-hit, all-zero, and late-hit `not_zero` groups. The current `not_zero` contracts force an optimizer-opaque immutable input reference on both workers so calibration measures every requested scan instead of a precomputed Boolean. The timing policy records the 2-second independent ceiling and bounded 10-second wide-ratio common ceiling without changing equal-work pairing or the `1.25x` gate. The inventory binds the legacy XOR threshold pair to `PERFQ-M5-SIMD-BITS/xor-complete-vector` and the legacy `not_zero` pair only to `PERFQ-M5-SIMD-BITS-NOT-ZERO-EARLY/not-zero`. The legacy M12 guards stay active until their exact migration evidence lands. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`: two exact passing CQ prerequisites, reproducible sealed workers, 30 report-bound contract-preflight receipts, six passing non-noisy full and soak AArch64 reports, six successful report and regression replays, and two replayed rollups. Median ratios range from `0.374633x` to `0.559828x`, with worst confidence-interval upper bound `0.561257x`; `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md` records that exact historical scope. No `not_zero` ratio is accepted yet. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold or comparator fidelity rule may be relaxed.
...
## Active Sixth-Slice Finish Order
