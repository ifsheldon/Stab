...
- P2 has consolidated all six result codecs on `RecordFormat`, removed facade-only circuit path helpers and forwarding modules, removed fictitious backend selection, and started direct CLI ownership. The remaining P2 work is complete CLI component migration, domain errors, compatibility-adapter deletion, and executable consumer fixtures.
