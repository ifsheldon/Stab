...
Active execution goal as of 2026-07-18.
...
Program checkpoint: CQ0 is source-current at correctness inventory digest `c50f27fd097ac870c987d1f91c44d9e6a75510ed4d51ec44853dbc328f0b1fa7`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory digest `f60aab6b68859a1aeb5a12e90122459ba70a404d94f3d95d7ad0c47b114286b0`: seventeen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 51 independently reported scale outcomes. This digest binds the separate identity and non-identity Clifford-string groups to exact correctness parents, frozen vectors, comparator sources, independent thresholds, and `benchmarks/profiler-notes/qualification/perfq-m6-clifford-string.md`; it inherits no Clifford timing evidence. Packed-storage optimization revision `2a0ab88c44eeeaae1714b0976089ecc1809203f3` replaces scalar per-qubit multiplication with six bit planes and an isolated portable-SIMD kernel after clean focused probes exposed diagnostic ratios of `21.933884x` and `238.618379x`. Direct post-change diagnostics are promising but remain non-promotable until clean paired qualification completes. Clean reviewed iterator evidence revision `afaf0bf7f236b9f6ae6f72c19bbbdfea94d26632`, Pauli multiplication revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb`, transpose revision `f912cc3af1f13cc9fab798d69937c155d37d83a0`, and earlier kernel evidence remain accepted historical evidence under their recorded inventories. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, equal-work, output-identity, or migration rule may be relaxed.
CQ2 implementation and exact ownership remain complete for eight selected domains and 271 implemented parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 60, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. Clean correctness revision `3f2f382627c8421de0a668819d467a9f252de20f` passed PR, full, and soak under preceding digest `4c940e983df10a7c95cc512939f4a0cce79f1865e141739af9378db581ea5f87`; it is historical evidence for its exact producer and inventory. The Clifford source change makes a fresh same-revision focused three-case prerequisite mandatory before dependent timing. Exact historical request, report, completion, and preflight digests are recorded in `docs/plans/cq2-deterministic-qualification-progress-report.md`.
The active execution checkpoint is the eleventh PQ2 Clifford-string slice. The runtime contracts, frozen cross-language vectors, allocation checks, hostile probes, source-shape tests, and packed portable-SIMD implementation are present. Next freeze the refreshed inventories in a clean commit, prove private-worker reproducibility, publish and replay the exact same-revision CQ prerequisites, run both groups at all three scales for full and soak, replay every report and regression, build architecture-scoped rollups and completion receipts, perform the legacy identity-row migration only after pre-migration completion authorizes it, then run milestone audit and independent GPT-5.6/max full-code-review lanes. Keep every failed or noisy `1.25x` outcome visible and do not promote dirty diagnostics.
...
Preserve clean CQ2 evidence revision `3f2f382627c8421de0a668819d467a9f252de20f` and its exact historical report chain. The machine contract intentionally rejects replay from any dirty checkout or different `HEAD`, and packed Clifford revision `2a0ab88c44eeeaae1714b0976089ecc1809203f3` makes that all-domain refresh historical under the current correctness inventory. Every promotable Clifford run must generate and consume focused exact CQ prerequisites from the same clean revision as its performance workers. Performance consumer commits `9be47ff2`, `0a9b228d`, and `7d0c07eb` add coherent historical 6/3 and current 7/4 schema support, reject mixed families, require complete passing stream and artifact receipts, and reconstruct the global statistical attempt ledger from producer-authentic deterministic and statistical fixtures. Proceed in the exact item order defined by `docs/plans/comprehensive-stim-performance-qualification-plan.md`, beginning with a clean inventory commit and reproducible workers before formal timing.
...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration, and migration commit `d706634eeaa536b2ce48d3dc9431b4feb513317f` retired only the bundled timing threshold and two exact small-scale mappings. The accepted historical post-migration chain at inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf` has 12 first-attempt passing reports and regressions, four replayed rollups, two replayed completion receipts, and median ratios from `0.025664x` to `0.568566x` with worst upper bound `0.570628x`. No ratio is promoted under current inventory `f60aab6b68859a1aeb5a12e90122459ba70a404d94f3d95d7ad0c47b114286b0` until its exact group is rerun from one clean revision. `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6.
...
The active eleventh slice is the two-contract Clifford-string plan in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. `PERFQ-M6-CLIFFORD-STRING` owns only the exact pinned identity workload, while `PERFQ-M6-CLIFFORD-STRING-NON-IDENTITY` covers equal-width public in-place multiplication over the pinned Stim 24-name order and a deterministic complete 24-by-23 non-identity composition cycle. Their frozen vector, exact correctness, independent output, zero-allocation, 62-receipt hostile matrix, source-shape, and private-worker contracts are implemented; the packed portable-SIMD path addresses the measured scalar architecture failure. Do not let the O(1) identity fast path substitute for non-identity work, and do not promote the direct diagnostics. Finish clean reproducibility, focused CQ, full and soak timing, regressions, rollups, completion, migration, audit, review, and documentation in that order.
