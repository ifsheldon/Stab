...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and ten PQ2 product contracts are source-current at performance inventory digest `8cc3ab3eb88faaf539c3c0eabaf3865ad421d8f67b14549cb4c7acc71faf2406`: the first five historical groups, independent early-hit, all-zero, and late-hit `not_zero` groups, and independent sparse row-XOR and item-toggle groups. Clean revision `60b732c77f1828058fbd65ec6c5c4ad582467fd1` supplies the complete passing AArch64 `not_zero` evidence chain, including accepted-maximum allocation coverage and final audit and review closure. Clean revision `e2f6292f473b034d8886fc100039c7a78c4a3989` supplies the passing first-stage sparse-XOR correctness, reproducibility, twelve reports, twelve regressions, four rollups, and two replayed completion receipts under pre-migration inventory `2d9cb3e3e2dc36a29c31964480f9b735e2411b26f4ba2b3ac66ed6791b617dc0`. Those receipts authorize the narrow source-current migration that retires only the two legacy sparse-XOR timing pairs and replacement markers while preserving the M12 memory baseline. The required complete post-migration evidence rerun, audit, review, and progress report are still pending and block source-current sparse-XOR closure. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
## Seventh-Slice Execution Contract

1. Implement `PERFQ-M5-SPARSE-XOR` and `PERFQ-M5-SPARSE-XOR-ITEM` as independent runtime groups exactly as specified by the seventh executable slice in the performance plan.
2. Use the existing exact CQ2 owner `cq-evidence-qualification-bea77c19e9ae0b24`; do not replace it with broad bit tests, the legacy benchmark row, or file-level upstream provenance.
3. Freeze the exact upstream fixtures and actual operation counts before timing: 1,997 row XORs per complete 1,000-row callback and seven toggles per complete item callback. Every scale and cap must be a positive integer number of complete callbacks.
4. Prime capacity with exactly two untimed callbacks on both implementations, prove that canonical state is restored, and instrument zero timed Stab allocations at all three scales and the accepted maximum. Do not infer zero allocation from RSS or source inspection.
5. Bind the new isolated C++ comparator, Rust worker module, canonical input and output encodings, report schema, 58-probe preflight, worker identities, and exact partial-sweep and first-over-cap failures through offline replay.
6. Keep the two legacy sparse-XOR M12 threshold pairs and exact replacement mappings active until both first-stage completion receipts pass from one clean revision. Clean revision `e2f6292f473b034d8886fc100039c7a78c4a3989` passed and replayed both first-stage completion receipts, so retire only those timing pairs, regenerate the inventory, and rerun the entire seventh-slice chain from one post-migration clean revision before calling it current.
7. Publish and replay 12 final promotable reports, 12 regressions, four scale-family rollups, and two completion receipts on controlled Linux AArch64. Preserve any failed or noisy result and add the required profiler note instead of rerunning toward a pass.
8. Run milestone audit and an independent GPT-5.6/max full-code-review subagent after final post-migration evidence. Fix all confirmed findings, rerun affected evidence, and record only genuine newly revealed under-specification.
9. Update `docs/plans/pq2-sparse-xor-qualification-progress-report.md`, this goal, the performance plan, PQ0 and PQ1 reports, CQ2 report, benchmark README, generated inventories, runtime contracts, and migration documentation in the same closure sequence.
10. Do not reopen completed `not_zero` implementation or claim native x86-64 or PQ6 memory qualification. Later shared-worker changes may make its AArch64 evidence historical, but they do not erase the accepted sixth-slice result.
