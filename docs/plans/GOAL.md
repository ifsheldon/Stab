...
- PQ2 gate and bit-kernel evidence: `docs/plans/pq2-gate-name-hash-qualification-progress-report.md`, `docs/plans/pq2-simd-word-popcount-qualification-progress-report.md`, and `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md`.
...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first five PQ2 product contracts are source-current at performance inventory digest `8b4735f7d651e74d3029014a4bf0c4580d85462295d66c93d8e090d3433c3958`; `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, `PERFQ-M5-SIMD-WORD`, and `PERFQ-M5-SIMD-BITS` are graduated to implemented runtime groups. The hardened inventory binds the legacy `simd_bits_xor_10K` and `stab_simd_bits_xor_10K` threshold pair explicitly to executable measurement `PERFQ-M5-SIMD-BITS/xor-complete-vector`; unmapped legacy pairs remain active and visible. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`: two exact passing CQ prerequisites, reproducible sealed workers, 30 report-bound contract-preflight receipts, six passing non-noisy full and soak AArch64 dense-XOR reports, six successful report and regression replays, and two replayed rollups. Median ratios range from `0.374633x` to `0.559828x`, with worst confidence-interval upper bound `0.561257x`; `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md` records the exact evidence and scope. All five groups' passing AArch64 reports are historical after the global performance inventory changed. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. The legacy canonical-printer row is superseded and its M12 beta, timing-regression, and memory waivers are retired; the legacy `m5-simd-word` threshold and both legacy `m5-simd-bits` XOR and `not_zero` submeasurement thresholds remain active until explicit replacement migration. No threshold or comparator fidelity rule may be relaxed.
...
The first five proving groups passed on the controlled Linux AArch64 host under preceding performance inventories and remain historical after later inventory or shared-worker changes. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` closes the fifth group's historical AArch64 evidence chain without weakening the `1.25x` gate. Milestone audit and independent GPT-5.6/max full code review found no confirmed implementation or evidence-integrity defect; their command-record, documentation-synchronization, digest-label, and CQ-owner findings are resolved in `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md`. Both under-specifications revealed by that audit are now resolved in `docs/plans/milestone-spec-gaps.md`: Stab allocation instrumentation covers every dense-XOR scale plus the accepted maximum, and clean implementation revision `b208a359f3f7676e2b07d64a5dc8caca208abf6a` adds completion receipt schema version 1 for every future executable slice. `docs/plans/pq2-completion-receipt-progress-report.md` records its milestone audit, independent review closure, durable publication boundary, and explicit prospective scope. Historical dense-XOR evidence predates that receipt and is not retroactively relabeled. Select the next finite dependency-ordered PQ2 runtime group from the remaining planned inventory instead of presuming that another heterogeneous legacy row is one valid ratio. The checked inventory requires a structured exact-replacement mapping before a heterogeneous inherited row can point at a primary runtime threshold. `report.rs`, `validation.rs`, and `worker.rs` are split by invariant ownership, and worker receipt schema version 2 binds every executable worker source module. Every subsequent executable PQ2 slice must publish and replay its completion receipt before milestone audit and independent review. Rerun the complete 271-parent CQ2 family at the next simultaneous-current program checkpoint instead of relabeling historical reports. Native x86-64 execution remains an unclaimed evidence target, and x86 adapter builds must prove that they inherit Stim's resolved machine flags before producing any ratio.
