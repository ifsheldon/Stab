...
1. Finish and commit the contract-evidence correction as one clean source-contract revision. Worker protocol version 4 must accept finite zero elapsed time only for `contract`; `timing` and `memory` must still require finite positive time. Contract-preflight schema version 14 must bind `contract` on every accepted and rejected receipt, report schema version 33 must reject stale reports, and the regenerated performance inventory must bind the changed adapter source. Verify `just bench::qualification-regenerate --check`, `just bench::qualification-check`, the complete `stab-bench` test suite, formatting, Clippy, benchmark smoke, and staged pre-commit before committing.
2. Preserve the complete `7c3e553` report and rollup chain plus its published parse completion. Record the independent replay failure exactly; do not overwrite, rerun, or use any part of that chain as current evidence.
3. Do not edit source during the replacement evidence chain. From the exact clean protocol-fix revision, rerun both DEM adapter probes, private-worker reproducibility, and the exact focused one-case CQ full producer, replay, and preflight into new unique artifact roots.
4. Produce all twelve post-fix timing reports: parse and print, full and soak, at small, medium, and large scales. Disable `/swap.img` only around each timing producer with an exit trap that always restores it, verify restoration immediately, and replay and regression-check each artifact before starting the next. Keep the first complete result; rerun only one complete group-scale attempt when the source-owned noise rule classifies the first as noisy.
5. Publish and replay separate parse-full, parse-soak, print-full, and print-soak architecture-scoped rollups from the passing reports, then publish and independently replay one parse completion and one print completion. Every artifact must bind the clean revision, current correctness digest, current performance digest, exact worker identities, contract-preflight identity, and one controlled AArch64 CPU identity.
6. Update the DEM progress report with the exact post-fix revision, artifact paths, hashes, ratios, confidence bounds, peak-RSS observations, replay and regression results, and any retained failed or noisy attempts. Explicitly distinguish accepted post-fix evidence from pre-migration authorization and the review-rejected `7c3e553` chain.
7. Run `milestone-audit` over every twelfth-slice task, test, benchmark, migration, memory-retention, evidence, and acceptance criterion. Fix every confirmed implementation or evidence issue and rerun the affected machine chain from the fix revision. Log only genuine newly revealed under-specification in `docs/plans/milestone-spec-gaps.md`.
8. Spawn independent GPT-5.6/max `full-code-review` lanes for core DEM correctness, worker and adapter equivalence, hostile-input and artifact lifecycle, benchmark science and migration, and documentation consistency. Fix every confirmed P0 through P3 issue and regenerate affected evidence from the final clean revision.
9. Run full repository verification, commit focused fixes and final evidence documentation, and advance this execution contract only when both post-fix completion receipts replay, audit and review are closed, source-current docs agree, swap is restored, and the worktree is clean.
