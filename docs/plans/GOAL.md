...
- P2 has consolidated all six result codecs on `RecordFormat`, removed facade-only circuit path helpers and forwarding modules, removed fictitious backend selection, moved the CLI onto direct model, record, analysis, and engine ownership with private typed diagnostic composition, and deleted `CompiledSampler` plus its materialized, byte, and callback routes. Measurement sampling now has one compiler-plan-session-sink path; the remaining P2 work is detection and DEM adapter deletion, facade domain-error removal, and executable consumer fixtures.
