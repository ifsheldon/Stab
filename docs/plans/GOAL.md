...
Program checkpoint: CQ0 is current at correctness inventory digest `cd82f99bd3f02446187b55c873e396e08c234f86693ba5f5734882fbe4814b56`; PQ0 is dependency-regenerated at performance inventory digest `2cd3f0cd44f2450297dc87abec98156eb717127483a051f8dd48ee29bdc39fcd`. CQ1 refresh evidence from revision `f62cdf135f6804419809e74d9e68c66551adf6e3` passes PR 303/303, full 457/457, and soak 457/457; the required current-digest PQ1 report-only refresh must run from the committed documentation and checklist synchronization revision before this checkpoint is promoted.
The active milestone is CQ2: qualify deterministic models, formats, gates, bit kernels, and stabilizer algebra at exact case and exported-API granularity before PQ2 times those surfaces. The selected `.stim` and `.dem` domains are complete with 32 and 28 implemented evidence parents respectively and zero planned owners. The `.dem` slice comprises 17 qualification parents, eight direct oracle-fixture parents, and three blocker-ledger parents; it maps all selected exact upstream and exported Rust API contracts without claiming deferred Python object shape or C++ convenience APIs. CQ2 now has 204 implemented owners and 2,276 planned owners across 2,480 selected owners, so continue with `CQ-RESULT-FORMATS` instead of treating the completed format-model slices as CQ2 completion.
