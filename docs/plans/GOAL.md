...
Program checkpoint: CQ0 is source-current at correctness inventory digest `deb6c025854e0e9dc555b45ee5afda33ac22b31c307d41d01731fa320a399f73`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,741 evidence parents. Of those parents, 577 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is dependency-regenerated at performance inventory digest `101ecb8ba8853522a234be0437e3779007428a6a8749f4fd01c77a7fd7131345`; no disposition, threshold, waiver, comparator classification, or unresolved count was relaxed during the CQ2 refresh.
...
The first proving group is `PERFQ-M4-CIRCUIT-PARSE`. Finish its schema-version-2 source-owned scales, symmetric `circuit-parse` and `parse` worker pair, exact CQ2 preflight binding, semantic digest, PR diagnostic run, clean full and soak three-scale runs, regression evidence, audit, review, and documentation before using the same runtime shape for the remaining PQ2 groups. Do not treat landing the harness seam or one scale as completion of the circuit-model feature.
