...
Program checkpoint: CQ0 is source-current at correctness inventory digest `648e7ea5a66997a810498dc871257bd654c7f9af9304651d43a88103eded0289`, with 2,886 upstream records, 1,974 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory schema version 2 and digest `8094995692c48c98723467cf6e90c3c685797fb4a02bf0efe15c273d844fbfef`: nineteen PQ2 product contracts are executable, each with one source-owned `1.25x` measurement rule over three scales, for 57 independently reported scale outcomes. Runtime-group schema version 5 now also binds independent direct DEM parse and canonical-print contracts, their exact correctness parent, frozen eight-item fixture, three scales, comparator sources, and output identities while preserving the separate Clifford contracts and source-owned independent-throughput timing only for Clifford identity. The checked threshold-migration ledger continues to bind every completed legacy retirement and preserved memory baseline; neither DEM contract has migration authorization or promotable timing evidence yet. The source-current harness uses private Stab build-receipt schema version 6, adapter receipt schema version 12, contract-preflight schema version 13 with 228 probes, and qualification report schema version 32. No prior correctness or performance report is relabeled as current after its worker, hostile-request, publication, migration, fixture, or receipt contract changes. Clean reviewed Clifford, iterator, Pauli multiplication, transpose, and earlier kernel evidence remains accepted historical evidence under its recorded inventory. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator fidelity, semantic-work unit, output identity, source-owned batching policy, publication-retention rule, or migration rule may be relaxed.
...
The active work is the twelfth PQ2 slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`: direct in-memory DEM parsing and canonical serialization. Execute it in this exact order:

1. Add the two planned runtime contracts, exact CQ binding, frozen eight-item fixture family, fixed scales, accepted maximum, first rejection, output normalization, strict `1.25x` rules, source ownership, and legacy replacement targets to the checked inventory before production implementation. Inventory tests must fail for the intentionally absent worker, adapter, probes, receipts, and migration evidence.
2. Add Rust and pinned-C++ fixture tests first. Freeze all input bytes and digests, normalized output bytes and digests, odd and even requests, parser/printer equality, accepted maximum, incomplete-cycle rejection, zero rejection, first-over-cap rejection, wrong-measurement rejection, and work-overflow rejection. Confirm the new inventory and dispatch tests fail for the intended missing support.
3. Implement `PERFQ-M10-DEM-PARSE-CONTRACT` as direct owned-model parsing and `PERFQ-M10-DEM-PRINT-CONTRACT` as direct owned-string canonical serialization. Keep fixture generation and printer model parsing outside timing, retain owned-result allocation and replacement or destruction inside timing, use optimizer-opaque consumption, and hash only the final normalized canonical text outside timing.
4. Isolate DEM worker, comparator, probe, invocation, and boundary logic in dedicated modules so existing files remain below 1,200 lines. Add every source to the ordered private-worker or adapter receipt and bump the private build, adapter, preflight, and report schemas exactly as the twelfth-slice plan specifies.
5. Extend the canonical preflight to exactly 228 ordered receipts and add the two source-owned adapter probes. Prove byte-reproducible private workers, exact cross-worker fixture and output identity, accepted maximum handling, and first-over-cap rejection before an enabled start barrier.
6. Run targeted Rust tests, adapter compilation, inventory generation, `just bench::qualification-list`, both adapter probes, and private-worker reproducibility until the implementation contract is green. Preserve first faithful failures and do not optimize or weaken the fixture or gate before recording them.
7. Commit a clean pre-migration implementation revision. From that exact revision, generate and replay the focused one-case CQ full prerequisite, all twelve full and soak group-scale reports, twelve regressions, four architecture-scoped rollups, and two independently replayed completion receipts on the controlled Linux AArch64 host. Keep swap disabled only during timing through the existing restoration guard and preserve rejected or noisy artifacts.
8. Use the replayed parse and print completion receipts to authorize only the exact legacy timing changes named by the plan. Preserve both M12 memory baselines for PQ6, commit the focused migration separately, then regenerate the entire two-group machine chain from one clean post-migration revision before claiming source-current ratios.
9. Run `milestone-audit`, then spawn independent GPT-5.6/max `full-code-review` lanes for core DEM correctness, worker and adapter equivalence, hostile-input and artifact lifecycle, benchmark science and migration, and documentation consistency. Fix every confirmed finding, rerun affected evidence from the fix revision, and log only genuine newly revealed under-specification.
10. Publish `docs/plans/pq2-dem-parse-print-qualification-progress-report.md` with exact revisions, inventory and schema digests, worker identities, artifacts, ratios, confidence bounds, resource observations, migration authorization, audit closure, review closure, rejected attempts, and remaining x86-64 and PQ6 scope. Run repository verification, commit focused changes, and update this section to the next unfinished PQ2 slice only after the DEM pair satisfies every acceptance criterion.
