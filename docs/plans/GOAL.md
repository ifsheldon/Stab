...
Program checkpoint: CQ0 is source-current at correctness inventory digest `a739d350eeb3455d4a0b386f8a257d3d4fe01d417d7d11d8a269229d68a6a103`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at post-migration performance inventory digest `e79edf2e1eaa49a801606245d4a845d47a1d000ed527c9669d95e091c4480237`: thirteen PQ2 product contracts are executable, including `PERFQ-M6-PAULI-STRING`, and the identity-only legacy Pauli timing row is superseded while its memory baseline remains active for PQ6. Clean revision `cd1e33e10f45995ccaca498547ff5aa88bfe51bb` passed the strengthened exact two-case Pauli correctness preflight, reproducible workers, adapter probe, all three full and all three soak timing reports on their first attempt, every report replay and regression, both rollups, and completion receipt plus replay at the source-current inventories. Its six median ratios range from `1.001956x` to `1.032352x`, with worst confidence-interval upper bound `1.032881x`. The pre-migration chain at `3a0fcd814f8d1a9441420ab85edf3d757572ba93` remains historical authorization for focused migration commit `42c132f2c49538364649cd90962166223c72b4c6`; it is not relabeled as current. Clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` remains accepted historical transpose evidence under its recorded inventory. Earlier `not_zero`, sparse-XOR, dense-XOR, and transpose outcomes likewise remain historical under their exact identities. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
The active tenth PQ2 slice is the split Pauli-string iterator contract in `docs/plans/comprehensive-stim-performance-qualification-plan.md`: `PERFQ-M6-PAULI-ITER` owns X/Z weights 2 through 5, and `PERFQ-M6-PAULI-ITER-SINGLETON` owns X/Y/Z weight 1. They must retain separate correctness summaries, scale families, timing outcomes, rollups, and completion receipts while preserving the exact upstream constructor-plus-borrowed-traversal lifecycle.
Keep the bundled `m6-pauli-iter` timing threshold active until both exact replacement groups pass and replay from one clean pre-migration revision. Retire only its timing threshold and two exact small-scale mappings after both receipts authorize migration, preserve and guard the memory baseline for PQ6, then regenerate the complete two-group chain from one clean post-migration revision before closure.
Refactor the near-limit worker owner before adding the new workload variants, freeze exact hostile-boundary and overflow receipts before timing, and do not use the current legacy row ratio as product evidence. Publish `docs/plans/pq2-pauli-string-iterator-qualification-progress-report.md` only after exact CQ publication, worker reproducibility, both probes, 12 full and soak reports, 12 regressions, four rollups, two completion receipts, milestone audit, and GPT-5.6/max review closure all pass.
