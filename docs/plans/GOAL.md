...
Program checkpoint: CQ0 is source-current at correctness inventory digest `4d9faa21e318eeebc4614c7bf62491bb2db73b5db57ae3dab7d0f19f3fda7cad`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,743 evidence parents. Of those parents, 579 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first PQ2 product contract are source-current at performance inventory digest `9687d45fa57b97388e7a8c7b1676f2545619ff3185c19aa714583b25d1680924`; exactly `PERFQ-M4-CIRCUIT-PARSE` is graduated to an implemented exact runtime group, and no threshold, waiver, or comparator fidelity rule was relaxed.
...
The first proving group is `PERFQ-M4-CIRCUIT-PARSE`. Its schema-version-3 source contract, materialized-source worker receipts, exact cap probes, canonical report replay, and architecture-scoped rollups are implemented and reviewed. Clean revision `d2df9766c5e3543c8df016db31f48f552354d79f` produced current correctness-bound full and soak reports for all three scales on the controlled Linux AArch64 host. Both AArch64 rollups replay, all six regression checks pass, every source report binds one exact six-digest worker identity, and no measurement is noisy. All six measurements pass the unchanged `1.25x` target: medians range from `0.859679x` to `0.917843x`, and the worst confidence-interval upper bound is `0.926499x`. The parser optimization blocker is closed. The active owner action is to implement the next source-owned PQ2 runtime group and its exact correctness prerequisites; native x86-64 circuit-parse evidence remains a separate unclaimed host dependency. See `docs/plans/pq2-circuit-parse-qualification-progress-report.md`; do not treat this one passing group as completion of `PERF-CIRCUIT-MODEL` or PQ2.
...
Do not treat PQ0's 14 retained rows as qualified evidence: 158 of 159 active inherited rows still record missing correctness preflight and output-digest evidence, 58 CLI rows remain asymmetric, 73 rows lack a current comparator, 123 lack required scale families, and 20 select heterogeneous upstream measurements. The implemented circuit-parse contract removes those exact inventory deficiencies only for its own row, and its controlled AArch64 full and soak evidence now passes both the median and upper-confidence-bound requirements at the unchanged `1.25x` gate.
