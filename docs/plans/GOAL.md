...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and eight PQ2 product contracts are source-current at performance inventory digest `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0`: the first five historical groups plus independent early-hit, all-zero, and late-hit `not_zero` groups. The current `not_zero` contracts force an optimizer-opaque immutable input reference on both workers so calibration measures every requested scan instead of a precomputed Boolean. The timing policy records the 2-second independent ceiling and bounded 20-second wide-ratio common ceiling without changing equal-work pairing or the `1.25x` gate. Clean revision `817d0fe870fd1b02c8e30f18e534e35df705a1ee` supplies historical passing evidence at immediately preceding inventory `c2362b6fda35626e5f571399512067ab406f92a22692c162ae70002ef7a651f8`: 18 first-attempt full and soak AArch64 measurements, six replayed rollups, and three replayed completion receipts, with median ratios from `0.030337x` to `0.663540x` and every bootstrap confidence-interval upper bound below `0.664x`. Independent review confirmed the implementation and comparator but found that the accepted maximum width was absent from allocation-counted testing and that post-migration closure requires evidence at the source-current inventory. Commit `fd016d5dc4afbe539c9a5a44242694693edfc622` fixes the allocation boundary. Do not mark the sixth slice closed until all correctness, worker, adapter, full, soak, regression, rollup, and completion artifacts are regenerated and replayed from one later clean source revision at `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0`. The source-current inventory retains only the still-active XOR replacement mapping and threshold pair. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold or comparator fidelity rule may be relaxed.
