...
Clean pre-migration revision `f2388dccc01abb7ef89e5f56d9062c6656837470` authorized the narrow iterator timing migration. The bundled timing threshold and both exact small-scale mappings are retired in source-current inventory `48eacf03a2ecdca917c05aade52b7e17c9ead1be8b75b203e1d43c2f3b3b7dbf`, while `benchmarks/m12-primary-memory-baseline.json` remains explicitly guarded for PQ6. Regenerate the complete two-group chain from one clean post-migration revision before closure.
The migration checkpoint has split the near-limit worker owner, added both workload variants, frozen exact hostile-boundary and overflow receipts, raised the private Stab build receipt to schema version 3, the adapter receipt and contract preflight to schema version 10 with 140 receipts, and the qualification report to schema version 28. Do not use the retired legacy row ratio, pre-migration reports as post-migration evidence, or diagnostic probe timing as product evidence. Publish `docs/plans/pq2-pauli-string-iterator-qualification-progress-report.md` only after the post-migration exact CQ publication, worker reproducibility, both probes with their complete boundary matrices, 12 full and soak reports, 12 regressions, four rollups, two completion receipts, milestone audit, and GPT-5.6/max review closure all pass.
