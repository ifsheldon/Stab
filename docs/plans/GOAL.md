...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first five PQ2 product contracts are source-current at performance inventory digest `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`; `PERFQ-M4-CIRCUIT-PARSE`, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, `PERFQ-M4-GATE-LOOKUP`, `PERFQ-M5-SIMD-WORD`, and `PERFQ-M5-SIMD-BITS` are graduated to implemented runtime groups. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies two exact passing CQ prerequisites, reproducible sealed workers, 30 report-bound contract-preflight receipts, six passing non-noisy full and soak AArch64 dense-XOR reports, six successful report and regression replays, and two replayed rollups. Median ratios range from `0.374633x` to `0.559828x`, with worst confidence-interval upper bound `0.561257x`; `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md` records the exact evidence and scope. The first four groups' passing AArch64 reports remain historical after the global performance inventory and shared workers changed. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. The legacy canonical-printer row is superseded and its M12 beta, timing-regression, and memory waivers are retired; the legacy `m5-simd-word` threshold and both legacy `m5-simd-bits` XOR and `not_zero` submeasurement thresholds remain active until explicit replacement migration. No threshold or comparator fidelity rule may be relaxed.
...
The first four proving groups passed on the controlled Linux AArch64 host under preceding performance inventories and remain historical after later inventory or shared-worker changes. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` closes the fifth group's AArch64 evidence chain without weakening the `1.25x` gate. Milestone audit and independent GPT-5.6/max full code review found no confirmed implementation or evidence-integrity defect; their command-record, documentation-synchronization, digest-label, and CQ-owner findings are resolved in `docs/plans/pq2-simd-bits-xor-qualification-progress-report.md`, and both genuine under-specifications remain open in `docs/plans/milestone-spec-gaps.md`. Select the next finite dependency-ordered PQ2 runtime group from the remaining planned inventory instead of presuming that another heterogeneous legacy row is one valid ratio. Before adding behavior, split `report.rs`, `validation.rs`, and `worker.rs` by invariant ownership, and require a structured exact-replacement mapping before another heterogeneous inherited row relies on the broad `Reworked` validation exemption. Rerun the complete 271-parent CQ2 family at the next simultaneous-current program checkpoint instead of relabeling historical reports. Native x86-64 execution remains an unclaimed evidence target, and x86 adapter builds must prove that they inherit Stim's resolved machine flags before producing any ratio.
