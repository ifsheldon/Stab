...
Program checkpoint: CQ0 is source-current at correctness inventory digest `4d9faa21e318eeebc4614c7bf62491bb2db73b5db57ae3dab7d0f19f3fda7cad`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,743 evidence parents. Of those parents, 579 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and the first two PQ2 product contracts are source-current at performance inventory digest `b6bf408c54461d65670200fe701f7fff3e5e0470509d8fc89aeab729a242781b`; exactly `PERFQ-M4-CIRCUIT-PARSE` and `PERFQ-M4-CIRCUIT-CANONICAL-PRINT` are graduated to implemented exact runtime groups, and no threshold, waiver, or comparator fidelity rule was relaxed.
CQ2 implementation and ownership remain complete for its eight selected domains and 270 implemented exact parents: `.stim` 29, `.dem` 28, result formats 39, gate contract 59, bit kernels 12, circuit API 24, Generation 25, and Algebra 54. The clean all-domain PR, full, and soak reports from revision `bae9e01cb3fedaf9d37958e6827b064c635b9898` became historical when the reviewed negative-zero analyzer owners refreshed the global correctness inventory. Current clean exact execution for the two circuit-parser prerequisites passed on revision `d2df9766c5e3543c8df016db31f48f552354d79f`; rerun the full 270-case CQ2 family before citing all-domain execution as current-digest evidence, but do not block an exact dependent performance group whose own current-digest prerequisites and preflight pass.
...
The first proving group is `PERFQ-M4-CIRCUIT-PARSE`. Its schema-version-3 source contract, materialized-source worker receipts, exact cap probes, canonical report replay, and architecture-scoped rollups are implemented and reviewed. Clean revision `d2df9766c5e3543c8df016db31f48f552354d79f` produced six passing AArch64 full and soak measurements at the preceding performance inventory, with medians from `0.859679x` to `0.917843x` and worst confidence-interval upper bound `0.926499x`. Those artifacts are historical after the shared worker and runtime inventory gained canonical printing. The second group, `PERFQ-M4-CIRCUIT-CANONICAL-PRINT`, now owns a symmetric `serialize` workload, the same exact three-scale fixture family, dedicated canonical-printer and round-trip CQ prerequisites, final-output digest equality, process RSS, and the unchanged `1.25x` gate. The active owner action is to commit the sealed worker sources, prove private-worker reproducibility, then regenerate exact correctness-bound full and soak reports and architecture rollups for both groups from one clean revision. Native x86-64 evidence remains a separate unclaimed host dependency. See `docs/plans/pq2-circuit-parse-qualification-progress-report.md`; do not treat either group as completion of `PERF-CIRCUIT-MODEL` or PQ2.
...
Do not treat PQ0's 14 retained rows as qualified evidence: 158 of 159 active inherited rows still record missing correctness preflight and output-digest evidence, 58 CLI rows remain asymmetric, 73 rows lack a current comparator, 123 lack required scale families, and 20 select heterogeneous upstream measurements. The two implemented circuit contracts replace only their exact parse and canonical-serialization groups. The passing parser measurements at the preceding inventory remain historical until both groups are regenerated from the current shared worker and inventory.
