...
Program checkpoint: CQ0 is source-current at correctness inventory digest `5d795e831bc20b3f2780ca72c1eaea7c75387388d38f8e37f4539254a41e821b`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at performance inventory digest `3d8da16998e4ea387f025a55e61b1c8c0ce8a3202804633aae4083dc4647d2cb`: twelve PQ2 product contracts are executable, including independent allocating and in-place transpose groups for the four public transpose rustdoc paths. Clean pre-migration revision `e660c91cff142b611f52a0a28a36e0a3d15670ed` passed both exact correctness prerequisites, worker reproducibility, both public probes, all 12 first-attempt full and soak reports, all report replays and regressions, four rollups, and both completion receipt replays. Its median ratios range from `0.370154x` to `0.695896x`, with worst confidence-interval upper bound `0.696808x`. That complete chain authorized superseding only the heterogeneous `m5-simd-bit-table` timing threshold; its memory baseline remains active, and no pre-migration report is relabeled as source-current post-migration evidence. Clean revisions `60b732c77f1828058fbd65ec6c5c4ad582467fd1` and `7b43b46d1c08f669264d009b8d3872ce86838f0e` retain accepted historical AArch64 `not_zero` and sparse-XOR evidence under their recorded inventories, including accepted-maximum allocation, regression, rollup, completion, audit, and review closure. The sparse-XOR ratios range from `0.965755x` to `1.026014x`, with worst confidence-interval upper bound `1.034133x`. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. No prior correctness report is relabeled as current after the transpose resource contract changed the correctness digest. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
The clean first-stage evidence at `e660c91cff142b611f52a0a28a36e0a3d15670ed` satisfies both exact CQ cases, every frozen input and output receipt, all four rejection classes for both workloads and workers, worker reproducibility, both public probes, allocation instrumentation, scalar-reference kernel tests, 12 passing timing reports, four rollups, and two completion receipts.
...
The focused migration supersedes the legacy `m5-simd-bit-table` timing threshold while retaining its exact upstream provenance and M12 memory baseline. Commit that migration, then rerun both exact CQ prerequisites, worker reproducibility, both probes, all 12 full and soak reports, all report replays and regressions, four rollups, and two completion receipts from one clean post-migration revision before claiming closure. PQ6 alone may replace the memory baseline.
