...
Program checkpoint: CQ0 is source-current at correctness inventory digest `3db44922e3310cb3a573fcff3b28d5eea5d28e0d6975e0856965c601ecc23c72`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 is source-current at post-migration performance inventory digest `7eedf59cb65d2bd244accc56973d7831001191cd62511c56b05a5cd7ed612ac6`: thirteen PQ2 product contracts are executable, including `PERFQ-M6-PAULI-STRING`, and the identity-only legacy Pauli timing row is superseded while its memory baseline remains active for PQ6. Clean pre-migration revision `3a0fcd814f8d1a9441420ab85edf3d757572ba93` passed all three full and all three soak Pauli reports on the first attempt, every report replay and regression, both rollups, and completion receipt plus replay at performance inventory `84d5ab682acda2a847972a74c5d58443fde8d2c820e62e46b634562e7c918e46`. Its six median ratios range from `1.002254x` to `1.031540x`, with worst confidence-interval upper bound `1.031846x`; this evidence authorized focused migration commit `42c132f2c49538364649cd90962166223c72b4c6` but remains historical until the post-migration chain is regenerated. Clean revision `f912cc3af1f13cc9fab798d69937c155d37d83a0` remains accepted historical transpose evidence under its recorded inventory. Earlier `not_zero`, sparse-XOR, dense-XOR, and transpose outcomes likewise remain historical under their exact identities. No prior correctness or performance report is relabeled as current after its bound source contract changes. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
The active ninth PQ2 slice is `PERFQ-M6-PAULI-STRING` in `docs/plans/comprehensive-stim-performance-qualification-plan.md`. Its source-owned worker, independent pinned-Stim comparator, schema-version-9 preflight, and schema-version-27 report contract are executable at the current inventories. The clean pre-migration chain passed all six scale and tier points and authorized the narrow legacy migration; the complete source-current post-migration chain remains pending. It qualifies only equal-width public `PauliString::right_multiply_in_place_returning_log_i_scalar` against pinned Stim with deterministic non-identity operands at 10,000, 100,000, and 1,000,000 qubits.
Do not use the inherited identity-only `m6-pauli-string` ratio as evidence: Stab's public identity-right fast path makes that legacy comparison O(1) on the Stab side. Migration commit `42c132f2c49538364649cd90962166223c72b4c6` retired only its row-level and three exact pair-level timing thresholds and scale mappings. Preserve the memory baseline, and regenerate correctness preflight, worker identity, probe, six reports, six regressions, two rollups, and one completion receipt from one clean post-migration revision before claiming closure.
