...
PFM-B4 is now the active milestone. Its implementation and source-owned evidence are in place: all forty-nine cases have exact one-test selectors, detecting-region and missing-detector evidence is focused, the flow engine is `33/33 implemented`, the exhaustive solver fallback is removed, and generator plus sparse-checker dispatch share typed reverse-transition classification. Successive GPT-5.6/max reviews exposed and drove fixes for `MPAD` simulation width and record ordering, reverse-engine selection, ignored-only repeat and row-cap behavior, quadratic idle-width repeat setup, gate-specific sweep-only feedback handling, duplicate reset semantics, flow ordering, Pauli-projection solver evidence, benchmark semantic work, and idle-qubit normalization. Core, benchmark, and compatibility finding-closure reviews are clean. Finish the first implementation commit and both clean committed-HEAD allocation reports before advancing to PFM-B1; do not reopen PFM-B0, PFM-B2 contract groundwork, or PFM-B3 unless their frozen contracts change.
