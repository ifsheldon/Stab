...
Program checkpoint: CQ0 is source-current at correctness inventory digest `ccb80eb4b660a375b59460c3b7fa03a810abd6f868735b566735378105db22b2`, with 2,886 upstream records, 1,972 default-feature public Rust API items, and 1,744 evidence parents. Of those parents, 580 are implemented, 17 are evidence-close, and 1,147 remain planned for later qualification. PQ0 and eight PQ2 product contracts are source-current at performance inventory digest `0161ab09015487ee2a1298be8dafe7c744b426b28a4e9fbdbd688e775c1655a0`: the first five historical groups plus independent early-hit, all-zero, and late-hit `not_zero` groups. Clean revision `60b732c77f1828058fbd65ec6c5c4ad582467fd1` supplies the complete passing AArch64 `not_zero` evidence chain, including accepted-maximum allocation coverage and final audit and review closure. The next dependency-ordered PQ2 work is the seventh executable slice in `docs/plans/comprehensive-stim-performance-qualification-plan.md`: split inherited sparse XOR into independent 1,000-row symmetric-difference and seven-item-toggle contracts, bind both to exact CQ2 case `cq-evidence-qualification-bea77c19e9ae0b24`, and preserve the `1.25x` gate without aggregating the operations. The source-current inventory retains the active XOR replacement mapping and both sparse-XOR legacy threshold pairs until their exact clean completion receipts authorize narrow migrations. Clean revision `5d226c94ece70f96d0b771f9c8cde7464ccd261b` supplies historical dense-XOR evidence at performance inventory `fb50789c58786219c807c79e87202396b17563ee7cb584bcda4d3379007ed716`, with median ratios from `0.374633x` to `0.559828x` and worst confidence-interval upper bound `0.561257x`. Setup and peak RSS remain report-only observations until PQ6 defines a machine-checked growth rule. No threshold, comparator-fidelity, equal-work, output-identity, or migration rule may be relaxed.
...
6. Keep the two legacy sparse-XOR M12 threshold pairs and exact replacement mappings active until both first-stage completion receipts pass from one clean revision. Then retire only those timing pairs, regenerate the inventory, and rerun the entire seventh-slice chain from one post-migration clean revision before calling it current.
