...
It is not an RPF5 completion report because broader detecting-region target shapes outside the promoted positive set and source-owned fail-closed set, broader generated-code cases beyond the promoted repetition-code and selected memory-Z and memory-X surface-code samples, broader gauge behavior, missing-detector families, and measurement-rich flow-transform integration remain active work.
...
- `MR`/`MRX`/`MRY`, `QUBIT_COORDS`, and `SHIFT_COORDS` are accepted for detecting-region traversal, promoting the generated repetition-code shape from pinned Stim's target-filter example plus selected generated memory-Z and memory-X surface-code slices.
...
The existing report-only detecting-region benchmark rows cover repeat traversal, target filtering, Clifford propagation, generated repetition-code extraction, and selected generated rotated memory-Z surface-code extraction, but they should not be cited as direct performance evidence for the feedback, sweep-control no-op, `MPAD` record-drop, heralded-record-drop, `MPP` product-measurement, `SPP`/`SPP_DAG` unitary-product target-shape branches, or the unrotated memory-Z and memory-X surface-code exact-output fixtures.
...
The generated surface-code slices promote narrow source-owned memory-Z and memory-X surface-code detecting-region samples for the Rust generator surface, not arbitrary generated-code parity.
The rotated memory-Z owned positive subcases are generated `surface_code:rotated_memory_z` with distance 3 and rounds 3, default-like all-detector plus all-observable target count of 25, default-like all-tick selection from 0 through 20, and exact selected-region extraction for D0, D4, and L0 across Stab ticks 0 through 5.
...
The memory-X owned positive subcases are generated `surface_code:rotated_memory_x` and `surface_code:unrotated_memory_x` with distance 3 and rounds 3, default-like all-detector plus all-observable target counts of 25 and 37, default-like all-tick selection from 0 through 20, and exact selected-region extraction for D0, D4, and L0 across Stab ticks 0 through 5.
The memory-X exact expectations come from pinned Stim v1.16.0 `Circuit.detecting_regions` and are recorded in [pfm5-detecting-regions-surface-memory-x-scope.md](pfm5-detecting-regions-surface-memory-x-scope.md).
...
The benchmark row for this slice is a non-primary report-only Rust utility workload measuring the same selected generated rotated memory-Z surface-code target and tick set through `circuit_detecting_regions_for_targets`; no separate benchmark row is added for the unrotated memory-Z or memory-X exact-output fixtures.
...
The owned negative subcases keep unsupported feedback positions, source-owned fail-closed non-`CZ` record-record, sweep/sweep, and record/sweep groups, broader generated-code regions beyond the promoted repetition-code and selected memory-Z and memory-X surface-code cases, and broader gauge-specific behavior fail-closed or partial until those surfaces are explicitly promoted.
...
- `detecting_regions_generated_surface_code_memory_x_basis_regions`
...
The generated surface-code benchmark row uses the selected D0, D4, and L0 targets plus the first six ticks for the distance-3 rounds-3 generated rotated memory-Z surface-code circuit, then times extraction through the additive `DemTarget` API; memory-X exact-output fixtures intentionally reuse this report-only performance path without adding a separate row.
...
cargo test -p stab-core detecting_regions_generated_surface_code_memory_x_basis_regions --quiet
```
...
The pinned-Stim `detslice-text` reproduction also passed for the generated repetition-code selected-region expectations recorded above, the generated rotated memory-Z surface-code reproduction passed for the selected D0, D4, and L0 targets across Stim ticks 1 through 6, and the unrotated memory-Z plus rotated and unrotated memory-X exact regions were reproduced from pinned Stim v1.16.0 `Circuit.detecting_regions`.
...
Milestone audit status is complete for the target-filter, generated repetition-code, generated rotated memory-Z surface-code, generated unrotated memory-Z surface-code, generated rotated and unrotated memory-X surface-code, unsigned Clifford, and ignored-anticommutation slices while broader RPF5 remains partial.
The current memory-X generated surface-code milestone-audit and full-code-review sidecars found no findings for the scoped selected-region evidence, documentation sync, oracle metadata, and no-new-benchmark policy.
...
The generated memory-X surface-code refresh adds exact D0, D4, and L0 selected-region evidence for the distance-3 rounds-3 `surface_code:rotated_memory_x` and `surface_code:unrotated_memory_x` Rust generator surfaces while leaving broad surface-code region-table parity active and without adding a separate performance row.
...
- Broader detecting-region target shapes outside the promoted positive set and source-owned fail-closed set remain active, especially unsupported feedback placements and any shapes not covered by measurement inversions, selected feedback placements, selected gate-order-valid sweep-controlled Pauli sign-only no-op groups, selected `CZ` classical-only bit-bit groups, explicit non-`CZ` classical-bit rejection, `MPAD`, heralded record-producing noise, `MPP`, and `SPP`/`SPP_DAG` Pauli products; broader generated-code regions beyond the promoted repetition-code and selected memory-Z and memory-X surface-code cases and broader gauge behavior also remain active.
- Missing-detector generated-code suffix analysis beyond the promoted honeycomb and toric cases is locked by [pfm5-missing-detectors-generated-boundary-scope.md](pfm5-missing-detectors-generated-boundary-scope.md) and remains under-specified until exact generated families, suffix comparators, resource behavior, oracle metadata, and benchmark policy are selected; broader flow-dependent utility behavior remains active.
