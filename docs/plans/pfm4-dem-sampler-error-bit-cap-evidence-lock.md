...
- Direct detection-event sampling remains free to fold selected repeated stochastic, deterministic, and zero-probability bodies because it does not expose per-error occurrence bits.
- Non-selected nested or shifted repeated stochastic direct-sampling bodies still reject excessive sampled-error application work before walking the folded tree.
...
- `crates/stab-core/tests/dem_sampler.rs` has `pf4_dem_sampler_folded_repeat_sampling_and_materialized_error_caps`, proving sampled-error materialization, streamed sampled-error output, and non-selected stochastic sampled-work caps.
...
It is optimizing nested or shifted repeated stochastic direct detection-event sampling where dense detector and observable output could avoid per-occurrence work without changing the flat sampled-error record contract.
