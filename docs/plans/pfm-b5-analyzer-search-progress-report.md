...
Graphlike and hypergraph searches enforce independent state, transition, per-state payload, and aggregate stored-payload budgets, and hypergraph adjacency interns each edge once instead of cloning its detector set into every incident node.
Search traversal rejects an oversized nonzero error target list before normalization or shifted-target allocation, and zero-probability source errors retain Stim-compatible failure diagnostics instead of being reported as an absent error declaration.
SAT/WCNF encoding returns the canonical trivial UNSAT problem before flattening models with no observables or no source error declarations, then preflights mechanisms, target occurrences, variables, stored clauses, literals, and output bytes before clause allocation for nontrivial models.
...
- One cumulative recurrence-probe work budget across nested circuit entries and instructions.
...
- Stim-compatible zero-probability diagnostics and independently bounded variable-sized graphlike and hypergraph search states.
...
`just oracle::blockers --check-selectors` reports all 48 PFM-B5 cases implemented with no shared selectors, eleven supporting oracle rows, and eleven supporting benchmark rows.
...
An analyzer unit regression crosses the cumulative nested-probe work limit, and the fallback integration tests enter a genuinely unsupported instruction path before independently crossing repeat-count, aggregate-repeat-iteration, and expanded-instruction limits.
...
Graphlike and hypergraph unit regressions independently cross per-state and aggregate stored-payload limits before persistent state cloning.
...
Analyzer recurrence discovery is limited to 1,000,000 cumulative work units across nested circuit entries and probed instructions and uses memory proportional to canonical boundary state, one loop body, and emitted compact DEM output.
...
Each search state is capped at 65,536 detector and observable terms, aggregate persisted map, predecessor, and queue copies are capped at 5,000,000 terms, and each traversed nonzero error mechanism is capped at 65,536 source target occurrences before normalization.
...
A parameterized touched-detector test crosses the effective-node boundary without allocating one million nodes; graphlike and hypergraph tests cross per-state and aggregate payload budgets; and SAT tests prove the single total target-occurrence boundary plus early trivial UNSAT handling for a large detector-only shifted repeat.
...
The second-review remediation currently passes:
...
Status: Reopened after the required second full-code review found additional defects.
...
| Nested, coordinate, gauge, folded `MPAD`, generated QEC, and cross-iteration behavior | Satisfied | `crates/stab-core/tests/dem_analyzer_pfm_b5.rs`; ledger analyzer cases |
| Finite graphlike and hypergraph closure corpora | Satisfied | Semantic corpus, zero-probability diagnostics, and payload-budget tests across nine graphlike and ten hypergraph ledger cases |
