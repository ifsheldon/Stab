...
Resolution: Partially narrowed by the selected single-instruction measurement-rich `time_reversed_for_flows` slice, which promotes pinned `M` and `MZZ` examples with selected measurement-ordering evidence, selected single-target `R`, `RX`, and `RY` reset-to-measurement conversion, selected single-target `M`, `MX`, and `MY` measurement-to-reset conversion, and selected plain unique-target `MR`, `MRX`, and `MRY` measure-reset flow reversal using the existing sparse flow verifier and logs broader reset-only operations, duplicate or inverted measure-reset shapes, detectors, feedback, noise, repeats, and multi-instruction QEC inverse behavior as still active or under-specified. The selected pinned feedback loop-refolding and nested bounded-repeat detector-parity cases are now covered separately by `Circuit::with_inlined_feedback`.
