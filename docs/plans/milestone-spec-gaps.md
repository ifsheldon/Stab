...
Resolution: Partially narrowed by the selected single-instruction measurement-rich `time_reversed_for_flows` slice, which promotes pinned `M` and `MZZ` examples using the existing sparse flow verifier and logs broader resets, detectors, feedback, noise, repeats, and multi-instruction QEC inverse behavior as still active or under-specified. The selected pinned feedback loop-refolding case is now covered separately by `Circuit::with_inlined_feedback`.
...
Resolution: Resolved by the PF2 bounded loop-refolding slice. `Circuit::with_inlined_feedback` now covers the pinned `transform_without_feedback.loop` shape with exact output and DEM preservation, plus an excessive-repeat-work rejection. Broader repeat-contained feedback parity remains future work under the full feedback-inlining transform checklist row.
