...
Resolution: partially addressed by implementing `--err_out`, `--err_out_format`, `--replay_err_in`, and `--replay_err_in_format` for `01`, `b8`, `r8`, `hits`, and `dets`; `ptb64` streams and the exact M11 acceptance matrix remain pending plan update.
