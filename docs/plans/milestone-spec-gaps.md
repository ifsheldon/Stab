...
Resolution: remove the placeholder limits call, retain fixed sampler representability checks as compiler semantics, introduce backend selection with scalar as A4's only registered backend, and reject explicit portable SIMD before compilation. A6's measured raw XOR and Clifford kernels are build-time leaf acceleration, not a distinct sampling implementation; a later packed-frame milestone may register portable SIMD only after it owns a separate executable plan, plan fingerprint, semantic-equivalence evidence, and phase-specific performance evidence.
...
Resolution: M9 now treats generated-fixture acceptance as converting the same sampled measurements through equivalent `01` text and `b8` bit-packed `m2d` routes for M7 repetition, rotated-surface, unrotated-surface, and color-code circuits with appended observables. Evidence is `cargo test -p stab-cli m2d_converts_the_same_generated_measurements_in_text_and_bitpacked_formats --quiet` and the oracle manifest row `coverage-simulators-measurements-to-detection-events-generated`. Independent `sample` and `detect` commands are not compared byte-for-byte because exact random streams across different SIMD batching paths are outside the compatibility contract. Existing hand-authored M9 oracle rows continue to cover `dets` label formatting.
...
Resolution: `docs/plans/rust-stim-drop-in-rewrite.md` defined `--skip_loop_folding` acceptance as output-compatible repeat-circuit behavior for M8 and deferred compact repeat-preserving `ReferenceSampleTree` construction plus its performance parity to M12. Evidence is the implemented oracle fixture `m8-sample-skip-loop-folding` and the structural row `coverage-util-top-reference-sample-tree`. The later P4 program separately promoted execution-time reference-repeat reuse and made the flag select iterative execution; it did not claim compact tree construction.
...
Resolution: M6 now states that the public Rust API starts with owned `PauliString`, `FlexPauliString`, `CliffordString`, and `Tableau` values, public `PauliStringRef` parity is not required unless later parity or performance work proves it necessary, and Python-only binding behavior remains semantic-mining input until the Python API milestone. `crates/stab-algebra/tests/stabilizers.rs` checks that real `PauliString` rejects imaginary, lowercase, and sparse-style text while `FlexPauliString` accepts phase-general dense and sparse text with canonical display; manifest rows `coverage-stabilizers-pauli-string`, `coverage-stabilizers-flex-pauli-string`, and `coverage-stabilizers-pauli-string-ref` track the split.
...
## 2026-07-30 - A6: Final Ownership-Continuity Matrix

Status: Resolved
Revealed by: final A6 benchmark audit after the focused M5 and M6 diagnostic round.
Current text: A6 required source-current evidence for every moved call path, while its closure checklist named only focused M5 and M6 correctness and timing diagnostics.
Gap: the broad requirement and narrow checklist allowed an agent either to omit changed parser, records, engine, DEM, analysis, and PF paths or to invent a different subset during closure.
Proposed amendment: freeze one exact final matrix before formal execution and preserve all existing comparator classes, thresholds, waivers, and semantic witnesses.
Resolution: the A6 plan and extraction map now require all 166 pre-M12 manifest rows, covering M4 through M11 and PF1 through PF7, from one fresh pinned-Stim baseline and one warmed three-run Stab comparison. Threshold-eligible rows retain their existing policies, and any report-only phase more than 15% slower than its latest clean extraction report requires profiling plus a fix or source-owned explanation.

## 2026-07-30 - A6: Canonical Qualification Package Ownership

Status: Resolved
Revealed by: final extraction audit after the physical bits and algebra crates existed.
Current text: A6 required physical component extraction and regenerated qualification ownership, but did not state whether canonical semantic tests could continue executing the facade package.
...
## 2026-07-30 - A6: Evidence Scope Exceeded The Extraction Risk

Status: Resolved
Revealed by: final requirement-by-requirement A6 milestone audit before replacement timing.
Current text: A6 said unrelated rows were continuity checks only when extraction changed their executed call path, but also required the complete 166-row matrix, historical instrumentation backports for every comparable report-only phase, mandatory profiler receipts for reproduced crossings, and one focused-evidence publication object.
...
## 2026-08-01 - A7: Evidence Revision And Status Closure

Status: Resolved
Revealed by: final A7 evidence planning after reopening the decoder checklist row
Current text: A7 required source-current correctness, diagnostic timing, exact-revision CI, synchronized generated inventories, a completed checklist row, and an append-only closure report.
...
Resolution: the first repair named six closure paths and added ancestry plus full-history path validation. The later completion-artifact-authentication amendment below supersedes that shape with one canonical checked manifest, an exact seven-path allowlist, a clean checked state, and at most one descendant. Product code, tests, fixtures, workflows, inventories, policies, README, checklist, migration guide, and release operations remain evidence-invalidating changes.

## 2026-08-02 - A9: Completion Artifact Authentication

Status: Resolved
Revealed by: independent pre-evidence milestone audit and full-code-review of the first A9 generated-status implementation.
Current text: A9 required the generated dashboard to validate a checkpoint digest, inventory identities, measured-commit ancestry, and an allowlisted status descendant.
Gap: the checkpoint supplied its own digest and summary but the dashboard did not open the named completion artifact. A fabricated path and digest could therefore claim current release completion without a schema-version-3 manifest, exact release scope, environment result, memory result, or per-group correctness evidence. The first implementation also accepted uncommitted allowlisted changes and any number of allowlisted descendant commits.
Proposed amendment: make a byte-identical canonical completion manifest a checked source artifact, generate its checkpoint only after deterministic replay, validate the checked bytes and complete current contract in the dashboard, require a clean worktree in check mode, and permit at most one committed status descendant.
Resolution: checkpoint schema 2 references `benchmarks/qualification-completion-manifest.json` by exact SHA-256. `just qualification::completion-checkpoint` first replays the schema-version-3 completion from a clean measured revision, then writes the canonical manifest and checkpoint. Status validation opens the checked regular file, verifies canonical bytes, scope, Stim identity, inventories, parity and regression policies, raw-work timing boundary, environment, memory status, exact release matrix, measured Git commit, clean checked state, and at most one seven-path descendant. Missing, altered, stale, noncanonical, scope-incomplete, dirty, or multiply descended claims fail closed.

## 2026-08-02 - A9: Final Preflight And One-Descendant Release Order

Status: Resolved
Revealed by: independent pre-evidence milestone audit and full-code-review of the coordinated publication sequence.
Current text: the first A9 sequence generated a final package preflight before formal evidence, created a status descendant afterward, and then required publication to use both that stale preflight and current `HEAD`. Release documentation also expected post-publication URLs to be committed despite the one-descendant limit.
...
## 2026-08-04 - A9: First-Failure Evidence Retention

Status: Resolved
Revealed by: milestone audit and full code review after the first late-hit soak-large qualification command could not publish a report
Current text: performance qualification required every failed or noisy promotable report to carry a source-owned profiler-note path and digest, while group-specific slices required the first unfavorable result to remain visible and allowed a profiler note to be added only after observing that result.
...
## 2026-08-22 - WS4/A9: Reversible Release-Rehearsal Scope

Status: Resolved
Revealed by: pre-evidence audit of the pending WS4 scratch-repository rehearsal
Current text: WS4 required every command sequence in `docs/RELEASING.md` to execute against a rehearsal setup, but the only GitHub operator was hard-coded to the production repository and required authenticated A9 release completion.
Gap: literal compliance would require irreversible crates.io publication, creation of the protected production tag, and manual publication of a public GitHub release. Redirecting the production operator through runtime flags would also weaken its fixed-destination safety contract, while the completion requirement prevented the rehearsal from finding release defects before expensive formal evidence.
Proposed amendment: keep the production operator fixed and non-redirectable; add a separate rehearsal-only binary and workflow pinned to one numeric scratch-repository identity, active no-bypass tag ruleset, and source-derived annotated tag; authorize it through architecture and ordinary qualification-status checks; and scope the executable rehearsal to the complete reversible GitHub draft-stage sequence.
Resolution: the production lane remains fixed to `ifsheldon/Stab`, ruleset `20419793`, and `v0.2.0`. The rehearsal lane is fixed to public scratch repository `ifsheldon/Stab-release-rehearsal` (ID `1342241032`), ruleset `21169813`, and `v0.2.0-rehearsal-<40-character-source-commit>`. Its binary exposes no crates.io publication or public-release operation, implements the same token-free Cargo-sandbox re-exec boundary needed by artifact and authorization work, and can create only a private draft. The amended WS4 criteria explicitly exclude irreversible production steps while retaining source mirroring, exact-tag dispatch, bounded workflow-scoped exact-SHA run discovery, six-asset verification, and read-only live-draft verification. Architecture policy requires both fixed workflow paths to exist. One exact three-document rehearsal-record descendant may name the external run without changing executable or normative source; any broader change invalidates the rehearsal.
Retirement note (2026-08-24): [the pre-0.2 entropy plan](pre-0.2-code-entropy-reclamation-plan.md) E1 superseded this resolved operating design after the scratch repository was deleted. The rehearsal workflow, binary, fixed target, recipes, and two-workflow architecture requirement are historical only; the production release lane is now the sole executable owner.

## 2026-08-23 - A9: Sealed Swap-Device State

Status: Open for a later qualification-schema revision
Revealed by: post-evidence milestone audit of the `515e4a04` A9 completion
Current text: A9 requires the operator to disable recorded swap immediately before formal timing, restore the exact prior configuration afterward, and preserve host evidence showing no swap-counter movement.
