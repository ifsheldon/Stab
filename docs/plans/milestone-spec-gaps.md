...
Resolution: Exact closure scope is selected by PFM-B1 in `docs/plans/non-deferred-partial-feature-milestones.md`. The entry remains open until the named C++ and Python semantic subcases, bounded MPAD matrix, generated-surface reversal, shared reverse-flow implementation, tests, oracle rows, benchmark evidence, audit, and review are complete.
...
Resolution: Exact closure scope is selected by PFM-B3 in `docs/plans/non-deferred-partial-feature-milestones.md`. The shared folded visitor, seven named consumer migrations, materialization rationales, differential and resource tests, focused oracle rows, and benchmark evidence are implemented in `docs/plans/pfm-b3-folded-dem-traversal-progress-report.md`; this entry remains open only until the required milestone audit and full-code-review close without unresolved implementation findings.
...
Resolution: PFM-B4 in `docs/plans/non-deferred-partial-feature-milestones.md` selects evidence closure instead of speculative feature growth. The entry remains open until the two named pinned upstream cases and existing promoted target, generated-code, gauge, repeat, and resource evidence are independently selectable, audited, reviewed, and synchronized with the checklist and inventory.
...
Resolution: PFM-B4 in `docs/plans/non-deferred-partial-feature-milestones.md` selects evidence closure instead of an unbounded generated-code or stabilizer-rank matrix. The entry remains open until the three named pinned upstream cases and existing promoted row-reduction, observable, gate, repeat, and resource evidence are independently selectable, audited, reviewed, and synchronized with the checklist and inventory.
...
Resolution: Exact closure scope is selected by PFM-B4 in `docs/plans/non-deferred-partial-feature-milestones.md`. The entry remains open until named C++ and Python semantic subcases are split into stable ids, flow transitions are shared, exhaustive subset solving is replaced by GF(2) elimination, generated and resource tests pass, and oracle, benchmark, audit, review, and documentation evidence is complete.
