...
Status: Open
Revealed by: PFM0 evidence-lock refresh after promoting the selected reset-measure-detector, two-to-one, `m_det`, MPP, noisy MZZ, observable include, noisy measurement, noisy measure-reset, noisy measure-reset detector-flow, pass-through, and selected measurement-rich `time_reversed_for_flows` slices.
Current text: PFM2 says broader detector-flow rewrites, broader observable-aware QEC inverse rewrites, feedback, repeats, and multi-instruction QEC inverse behavior remain active after listing the selected implemented packets.
Gap: the selected evidence is now a collection of exact pinned or source-owned packets with tests, oracle rows, and report-only benchmark coverage where relevant, but the remaining broad phrase does not name exact circuits, detector counts, observable shapes, feedback shapes, noise models, repeat structures, comparator class, resource boundaries, oracle metadata, or benchmark policy. Treating it as implementation-ready would require inventing new QEC inverse or measurement-rich transform scope after the fact.
Proposed amendment: keep the currently promoted exact packets as the complete PFM2 QEC inverse and selected measurement-rich time-reversal evidence until a future plan names additional exact circuits, positive and negative tests, comparator behavior, oracle rows, benchmark or no-benchmark rationale, and resource-boundary behavior. Do not implement broader QEC inverse, detector-flow, observable-aware, feedback, repeat, or multi-instruction transform behavior from broad checklist wording alone.
Resolution: Pending an exact-subcase plan for any broader PFM2 QEC inverse or measurement-rich transform family beyond the selected packets already named in the roadmap and reports.
...
Resolution: Pending exact-subcase plans for broader PFM4 folded traversal, coordinate traversal, or generated-loop analyzer output beyond the selected promoted evidence.
...
Resolution: Pending an exact-subcase plan for any detecting-region target-shape, generated-code, or gauge family beyond the selected promoted evidence.

## 2026-07-08 - PFM5: Broader Missing-Detector Utility Boundary

Status: Open
Revealed by: PFM0 evidence-lock refresh after promoting selected `missing_detectors` row-reduction, observable, Clifford-propagation, Pauli-product, repeat-traversal, folded final-repeat, and generated-code suffix slices.
...
Gap: the promoted `missing_detectors` evidence now names exact row-reduction, repeated MPP, pair-measurement, record-only observable, ignored Pauli observable, tableau-backed Clifford, `SPP`, `SPP_DAG`, bounded repeat, selected folded final-repeat, honeycomb, and toric suffix cases. The remaining phrase does not name exact circuits, unknown-input modes, gauge modes, stabilizer products, row-reduction rank patterns, folded repeat bodies, deterministic-loop families, comparator class, oracle metadata, resource limits, or benchmark policy. Treating the broad phrase as implementation-ready would require inferring a whole upstream utility-matrix target from the already promoted examples.
...
Resolution: Pending exact-subcase plans for broader `missing_detectors` utility families beyond the selected promoted evidence.
...
Resolution: Pending exact-subcase plans for broader flow-generator, solver, diagnostic, folded-repeat, or transform-integration families beyond the selected promoted evidence.

## 2026-07-08 - PFM6: Broader Analyzer Search And Sparse-Tracker Boundary

Status: Open
Revealed by: PFM0 evidence-lock refresh after promoting selected generated-QEC analyzer, prefix/repeat/tail folded analyzer, loop-carried observable folded analyzer, loop-folded decomposition, folded observable guard, mixed-top-level fallback, direct and generated search, SAT/WCNF, sparse reverse tracker, and matched-error canonicalization slices.
...
Gap: the promoted PFM6 evidence is exact and source-owned, including the selected huge odd-repeat loop-carried observable case, but the remaining broad phrases do not name generated circuits, exact DEM outputs, other loop-carried observable periods, search tie cases, SAT or WCNF encodings, sparse-tracker consumers, variable-target unitary families, value-object fields, comparator behavior, resource limits, oracle metadata, or benchmark policy. Treating the broad phrases as implementation targets would recreate the original planning failure of selecting work from whole upstream files.
Proposed amendment: keep the current PFM6 slices as the selected analyzer, search, sparse-tracker, and matched-error evidence. Do not add broader generated analyzer, loop-carried observable, search, SAT/WCNF, sparse-tracker consumption, variable-target unitary, or value-object hardening rows until a future plan names exact subcases, positive and negative tests, comparator behavior, resource boundaries, oracle metadata, and benchmark or no-benchmark rationale.
...
Status: Open
Revealed by: PFM3 evidence-lock refresh after comparing the implemented fixed-tableau, `SPP` or `SPP_DAG`, selected deterministic `MPP`, and selected deterministic `MPAD` execution evidence against the broad "remaining legal-gate execution" wording.
Current text: PFM3 says non-tableau legal operations remain active or explicitly rejected after the fixed-tableau gate contract, supported Hermitian `SPP` or `SPP_DAG` execution slices, selected deterministic `MPP` execution slice, and selected deterministic `MPAD` execution slice landed.
Gap: the selected evidence now covers all 46 fixed-tableau gates across sampler, detection-conversion, and analyzer circuits, supported Hermitian `SPP` or `SPP_DAG` execution and anti-Hermitian rejection across the promoted sampler, detection-conversion, detector-frame, and analyzer surfaces, selected deterministic `MPP` Pauli-product measurement execution across the promoted sampler, detection-conversion, non-frame detection-sampling, frame detection-sampling, and analyzer surfaces, and selected deterministic `MPAD` measurement-pad execution across those same surfaces. The remaining broad phrase does not name additional legal gate families, execution surfaces, accepted or rejected target shapes, comparator class, resource behavior, oracle metadata, or benchmark policy. Treating it as implementation-ready would require inventing scope after the fact.
Proposed amendment: keep the fixed-tableau plus supported Hermitian `SPP` or `SPP_DAG` plus selected deterministic `MPP` and `MPAD` execution contract as the complete gate semantic evidence currently selected by PFM3. Do not add another legal-gate execution row until a future plan names exact gate families, execution surfaces, accepted and rejected behavior, positive and negative tests, resource-boundary behavior, oracle metadata, and benchmark or no-benchmark rationale.
...
Resolution: Resolved for the generated surface-code coordinate dependency by the bounded mixed-top-level fallback in `circuit_to_detector_error_model` for unsafe or still-unsupported `fold_loops=true` shapes. The fallback uses the existing capped non-folded analyzer, `pf6_dem_analyzer_fallback_uses_bounded_unfolded_for_unsafe_tail_dependency` proves semantic equality with non-folded analysis for an unsafe prefix/repeat/tail dependency, the `pf6_dem_analyzer_fallback_preserves_` tests prove repeat-count, aggregate repeat-iteration, and expanded-instruction caps remain active, `pf6_dem_analyzer_fallback_does_not_mask_prefixed_repeat_errors` proves selected folded prefixed-repeat errors are not silently replaced by fallback output, and `pf6_dem_generated_surface_code_fold_loop_coordinates_match_circuit` proves the pinned generated surface-code coordinate comparison. The later `pf6-analyzer-prefix-repeat-tail-folding-rust` row promotes a selected true-folded detector-chain slice. Broader true folded generated-loop analyzer output is now governed by the 2026-07-08 PFM4 and PFM6 open entries until exact subcases are selected.
