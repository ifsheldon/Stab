...
Status: Open
Revealed by: M12 primary compare evidence after reclassifying `m8-sample-high-repeat-contract`, `m9-m2d-bitpacked-contract`, `m10-analyze-errors-high-repeat-contract`, and four M11 sample_dem rows from `contract-only` to faithful public `stim-cli` baselines.
