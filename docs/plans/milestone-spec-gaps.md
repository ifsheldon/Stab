...
Status: Open
Revealed by: M12 primary compare evidence after reclassifying `m9-m2d-bitpacked-contract` and `m10-analyze-errors-high-repeat-contract` from `contract-only` to faithful public `stim-cli` baselines.
