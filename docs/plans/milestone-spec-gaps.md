...
Resolution: partially addressed by adding implemented exact oracle rows for deterministic sparse detector ids, dense detector targets, repeated detector shifts, high detector ids with `b8` output, correlated detector combinations, observable side output, and detector-observable correlation through deterministic side-output fixtures. The remaining matrix decision is whether noisy sparse, dense, repeated, high-detector, and observable-only cases must be runnable oracle rows or can remain covered by direct Rust statistical and structural tests plus benchmark representatives.
