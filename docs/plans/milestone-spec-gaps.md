...
Resolution: partially addressed by implementing `--err_out`, `--err_out_format`, `--replay_err_in`, and `--replay_err_in_format` for `01`, `b8`, `r8`, `hits`, and `dets`, then adding detector, observable, error, and replay stream support for `ptb64`; the exact M11 acceptance matrix and Stab-only observable append/prepend compatibility policy remain pending plan update.
