...
Gap: the selected evidence is now a collection of exact pinned or source-owned packets with tests, oracle rows, and report-only benchmark coverage where relevant, but the remaining broad phrase does not name exact circuits, detector counts, observable shapes, feedback shapes, noise models, repeat structures, comparator class, resource boundaries, oracle metadata, or benchmark policy. For MPAD specifically, the promoted selected record-tail observable-flow packet still leaves observable terms outside selected record-only unique-id tails, Pauli-observable tails, duplicate observable-id merging, interleaved operations, repeats, feedback, and multi-instruction MPAD rewrites unselected. Treating either broad phrase as implementation-ready would require inventing new QEC inverse or measurement-rich transform scope after the fact.
