...
- Observable terms in `Circuit::time_reversed_for_flows` batches through `MPAD`.
...
Pinned Stim v1.16.0 probes used for the promoted Pauli-only and measurement-record flow cases:
...
Pinned Stim v1.16.0 also accepts selected observable-output terms such as `MPAD 0; OBSERVABLE_INCLUDE(0) rec[-1]` with `1 -> obs[0]`, but that observable flow behavior is intentionally not promoted in this slice because broader MPAD observable flow semantics still need exact tests, comparator rules, and resource policy.

Primary test evidence:

- `cargo test -p stab-core --test circuit_inverse_qec_mpad --quiet`

This test covers selected direct `inverse_qec` output, empty-flow, Pauli-only, and measurement-record `time_reversed_for_flows`, observable and unsatisfied-flow rejection, nearby fail-closed shapes, and `keep_measurements` rejection.
