...
This note locks the selected `MPAD` record-tail slice for `Circuit::inverse_qec` and empty-flow `Circuit::time_reversed_for_flows`.
...
- Non-empty `Circuit::time_reversed_for_flows` batches through `MPAD`, including Pauli-only flows.
...
This test covers selected direct `inverse_qec` output, empty-flow `time_reversed_for_flows`, explicit non-empty-flow rejection, nearby fail-closed shapes, and `keep_measurements` rejection.
