...
P2 retirement note, 2026-08-27: the scoped graph/vector `simulator_cross_checks` file was later deleted because its test-local vector engine duplicated owned algebra and circuit-to-tableau coverage without exercising a public graph or vector simulator. The deferred product boundary remains unchanged; only the independently owned `unitary_to_tableau` subset remains implemented evidence.

## Closed Row

| Checklist row | Current status | Closure boundary |
| --- | --- | --- |
| `Conversion to or from numpy, state vectors, and arbitrary unitaries` | Deferred with scoped Rust subset | Stab keeps the M6-owned `unitary_to_tableau` Rust subset, while Python/numpy conversion APIs, state-vector APIs, arbitrary unitary conversion APIs, `tableau_to_unitary`, random tableau/unitary round trips, and public graph/vector simulator products remain deferred. |
...
- `cargo test -p stab-algebra --test stabilizers_vs_amplitudes --quiet`
...
- `docs/plans/m12-progress-report.md` preserves the historical scoped graph/vector cross-check and records its later P2 retirement without promoting public simulator parity.
...
- `docs/plans/stim-test-porting-plan.md` leaves graph/vector simulator behavior deferred instead of relabeling owned algebra tests as simulator parity.
