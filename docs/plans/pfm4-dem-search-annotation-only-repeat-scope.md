...
For selected repeat bodies containing only those instructions, Stab should avoid repeat-count-scaled search-graph edge work and SAT/WCNF error-variable work for the affected consumers.
...
The test should prove graphlike, hypergraph, unweighted SAT, and weighted WCNF parity against the compact model, and should include a nested annotation-only repeat plus a coordinate-only zero-detector shift.
