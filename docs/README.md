...
- Use `just qualification::correctness-list` and `just qualification::correctness-check` for the finite benchmark correctness-prerequisite bridge. Use `just qualification::correctness-regenerate` only after an active runtime group changes its exact prerequisite set, then update the frozen digest and run the canonical check. `oracle/qualification-cases.json` contains only independently selectable Cargo tests required by active runtime groups; it must not regain upstream-test, public-API, fixture, or feature-status ownership. The runtime ledger and bridge must cover exactly the same prerequisite IDs.
...
- The blocker closure ledger is a read-only historical migration input. Use `just oracle::blockers` only when auditing that history; current feature and evidence status comes exclusively from the parity ledger and generated parity view.
