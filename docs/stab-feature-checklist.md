...
| Circuit coordinate queries | Partial with Rust final-coordinate subset | Coordinates are parsed and used in analyzer and detection workflows. Rust `Circuit` now exposes folded `final_coordinate_shift` and `final_qubit_coordinates`. Full detector-coordinate map parity and Python-style API shape are not complete. |
