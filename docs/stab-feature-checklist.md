...
| Flows | Partial | `Flow`, unitary circuit flow generators, scoped measurement/reset/pair-measurement/feedback/MPAD flow generators, tableau-backed `Gate` flow metadata, and unsigned `check_if_circuit_has_unsigned_stabilizer_flows` measurement-record and observable dependency cases are implemented for the scoped Rust semantics. Broader composed measurement-rich `flow_generators`, MPP generator synthesis, `solve_for_flow_measurements`, transform integration, variable-target gate flows, and Python API parity remain deferred or active follow-up work. See [plans/rpf5-flow-progress-report.md](plans/rpf5-flow-progress-report.md). |
