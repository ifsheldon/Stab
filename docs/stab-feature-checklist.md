...
| Programmatic mutation | Partial | `append_instruction` and `append_repeat_block` exist. Python-style `append_from_stim_program_text`, `insert`, `pop`, `clear`, concatenation operators, file constructors, and complete ergonomic mutation parity are not implemented as public Rust APIs. |
| Core introspection | Partial | `count_qubits`, `measurement_record_count`, `detection_record_width`, and related helpers exist. Full Python property parity for operation count, detectors, observables, sweep bits, ticks, coordinates, and all instruction-range views is not complete. |
| Circuit coordinate queries | Partial | Coordinates are parsed and used in analyzer and detection workflows. Full `get_final_qubit_coordinates` and `get_detector_coordinates` style public API parity is not complete. |
