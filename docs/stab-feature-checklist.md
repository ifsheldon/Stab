...
| Flows | Partial | `Flow`, circuit flow-generator subsets, and tableau-backed `Gate` flow metadata are implemented for deterministic unitary and scoped M6/PF1 semantics. Full measurement-rich flow solving, variable-target gate flows, and Python API parity remain deferred. |
