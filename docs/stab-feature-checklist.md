...
| Sparse reverse detector-frame tracking | Partial | A staged internal subset is implemented for M10, including generic reverse propagation for the full single-qubit Clifford gate set, `CX`/`CY`/`CZ` reverse propagation, and the PF6 supported-Clifford unitary-repeat folding subset covered by `pf6-sparse-rev-unitary-repeat-rust` plus the report-only `pf6-sparse-rev-frame-loop` benchmark. Broader all-unitary fuzzing, analyzer/search consumption beyond the promoted unsigned-flow path, active matched-error hardening, and full ErrorMatcher provenance remain future work. |
