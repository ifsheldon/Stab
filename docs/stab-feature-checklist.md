...
| `stim analyze_errors` | Partial | `stab analyze_errors` implements the staged M10 flag surface and current analyzer subset, with PF7 path-IO evidence for `--in` and `--out` success, missing input paths, output-open precedence, stdout behavior, stderr class, and exit status. |
