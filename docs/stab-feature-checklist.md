...
| Circuit transforms | Partial | `without_tags`, `to_tableau`, `inverse_unitary`, `inverse_qec`, `simplified`, and a scoped `circuit_with_inlined_feedback` helper for `m2d --ran_without_feedback` are implemented. Full `flattened`, full `decomposed`, `without_noise`, full Python-style `with_inlined_feedback`, `time_reversed_for_flows`, and complete measurement-rich flow transforms are not implemented. |
...
| Full feedback-inlining transform parity | Partial | `m2d --ran_without_feedback` now supports the command-level feedback-inlining subset covered by M9 tests. Full `Circuit.with_inlined_feedback` parity, exact loop refolding, and MPP transform parity remain broader transform work. |
