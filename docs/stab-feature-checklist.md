...
| Target kinds | Partial | Qubit, inverted qubit, measurement record, sweep bit, Pauli, inverted Pauli, and combiner parsing are implemented in [../crates/stab-core/src/target.rs](../crates/stab-core/src/target.rs). Sweep-conditioned conversion is intentionally rejected in detection workflows until sweep inputs are designed. |
...
| Circuit transforms | Partial | `without_tags`, `to_tableau`, `inverse_unitary`, `inverse_qec`, and `simplified` are implemented for scoped M6 subsets. Full `flattened`, full `decomposed`, `without_noise`, `with_inlined_feedback`, `time_reversed_for_flows`, and complete measurement-rich flow transforms are not implemented. |
...
| Measurement-to-detection conversion | Partial | `CompiledDetectionConverter`, `convert_measurements_to_detection_events`, streaming visitor conversion, and `stab m2d` are implemented. Sweep-conditioned conversion, `m2d --sweep`, `m2d --sweep_format`, and `m2d --ran_without_feedback` are explicitly absent or rejected until future work. |
...
| `m2d --ran_without_feedback` | Deferred | Explicitly rejected until feedback-removal conversion is promoted into an implementation milestone. |
