...
| Broader sweep-conditioned simulator and analysis parity | Partial | Public `m2d --sweep` and non-frame `detect` omitted-sweep default-false sampling are implemented for current execution subsets, but frame-path sweep-conditioned detector sampling, analyzer behavior, Python APIs, and every sweep target shape remain broader work. |
