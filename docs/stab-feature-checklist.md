...
| Broader sweep-conditioned simulator and analysis parity | Partial | Public `m2d --sweep`, non-frame and selected frame-path `detect` omitted-sweep default-false sampling, and analyzer sweep-control no-op behavior are implemented for current subsets, but broader analyzer sweep behavior, typed `detect` sweep input, Python APIs, and every sweep target shape remain broader work. |
