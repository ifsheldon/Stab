...
| Detector-analysis utility APIs | Partial | `circuit_detecting_regions` supports the simple H/CX/MXX detecting-region case with typed detector and tick inputs plus bounded repeat-tick traversal, `missing_detectors` supports basic reset/measure suggestions plus Gaussian row reduction for multi-record detector rows, repeated MPP and pair-measurement stabilizer-product cases, record-only observable rows, ignored Pauli observable rows, and the toric global-stabilizer generated-code suffix case, and `circuit_with_inlined_feedback` covers the supported MPP feedback-transform case. Broader detecting-region gate and target-shape support, detector filtering, multi-detector regions, gauge handling, honeycomb and broader toric generated-code missing-detector suffix analysis, flow generator solving, transform integration, and exact loop refolding remain future work. |
