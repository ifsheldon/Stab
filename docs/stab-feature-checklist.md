...
| Programmatic mutation | Partial with Rust mutation-helper subset | `append_instruction`, `append_repeat_block`, `append_from_stim_text`, `append_from_stim_program_text`, `append_circuit`, `concatenated`, `repeated`, `repeat_in_place`, `insert_item`, `insert_instruction`, `insert_repeat_block`, `insert_circuit`, `pop_item`, `pop_last_item`, derived `Clone`, and `clear` exist. File constructors or writers, Python operator ergonomics, and complete ergonomic mutation parity are not implemented as public Rust APIs. |
| Core introspection | Partial with Rust count subset | `count_qubits`, top-level `len`/`is_empty`, `count_measurements`, `count_detectors`, `count_observables`, `count_ticks`, `count_sweep_bits`, `measurement_record_count`, `detection_record_width`, and related helpers exist. Full Python property parity for all instruction-range views and binding-style indexing is not complete. |
...
| Reference samples and determined measurements | Partial | Reference-sample logic and determined-measurement subsets support current sampling and detection paths. Full Python API parity is not exposed. |
...
| `stim.Circuit` class | Deferred with Rust subset | `Circuit` exists as a Rust type, but Python class operators, mutation API, file helpers, transforms, diagram/export methods, and properties are not bound. |
