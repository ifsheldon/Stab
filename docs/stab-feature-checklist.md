...
| Gate validation flags and categories | Partial with Rust accessor subset | Stab has typed validation for argument counts, target categories, grouping, probabilities, and selected metadata. Rust `Gate` now exposes aliases, argument rules, target rules, target grouping, fusing, noisy/reset/measurement/unitary/single-qubit/two-qubit/target-capability/symmetry flags, unitary inverse, generalized inverse, local Clifford tableau metadata, tableau-backed unitary flow metadata, and fixed-shape one- or two-qubit `GateUnitaryMatrix` metadata for the owned PF1 subset. Full Python `GateData` parity, measurement-rich or variable-target flow metadata, and decomposition metadata accessors are not exposed. |
...
| Repeat handling | Partial | Repeat blocks parse, print, sample, analyze, and convert in supported paths, with documented expansion or planning limits for some analysis operations. Fully folded traversal across every transform remains partial. |
| Circuit transforms | Partial | `without_tags`, `to_tableau`, `inverse_unitary`, `inverse_qec`, `simplified`, and a scoped `circuit_with_inlined_feedback` helper for `m2d --ran_without_feedback` are implemented, including the supported MPP feedback-transform case. The feedback helper rejects repeat blocks and unsupported classical controlled gates until full transform parity and exact loop refolding are planned. Full `flattened`, full `decomposed`, `without_noise`, full Python-style `with_inlined_feedback`, `time_reversed_for_flows`, and complete measurement-rich flow transforms are not implemented. |
...
| Detector sampling | Done for current CLI surface; partial public API parity | `sample_detection_events`, `try_for_each_sampled_detection_event`, and `stab detect` are implemented in [../crates/stab-core/src/detection.rs](../crates/stab-core/src/detection.rs) and [../crates/stab-cli/src/detection.rs](../crates/stab-cli/src/detection.rs). Full Python `CompiledDetectorSampler` API parity is deferred. |
