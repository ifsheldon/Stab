...
| Gate validation flags and categories | Partial with Rust accessor subset | Stab has typed validation for argument counts, target categories, grouping, probabilities, and selected metadata. Rust `Gate` now exposes aliases, argument rules, target rules, target grouping, fusing, noisy/reset/measurement/unitary/single-qubit/two-qubit/target-capability/symmetry flags, unitary inverse, generalized inverse, local Clifford tableau metadata, and tableau-backed unitary flow metadata for the owned PF1 subset. Full Python `GateData` parity, measurement-rich or variable-target flow metadata, arbitrary unitary matrix metadata, and decomposition metadata accessors are not exposed. |
