...
| DEM detector shifts, observables, coordinates, and counts | Partial with Rust count and coordinate subset | `count_detectors`, `count_observables`, `count_errors`, `total_detector_shift`, folded `final_coordinate_shift`, all-detector coordinate maps, selected-detector coordinate maps, and single-detector coordinate lookup are implemented with `DemDetectorId`. Exact Python API shape and full folded traversal for every large-repeat coordinate-map case remain broader work. |
...
| DEM transforms | Partial with Rust tag-stripping subset | Scoped flattening and repeat traversal exist for search, SAT, analyzer, matcher, and sampler paths, `DetectorErrorModel::without_tags` is available, and `iter_flattened_instructions` exposes a lazy adjusted traversal for Rust callers. Public materialized `flattened`, `rounded`, and full transform API parity is not complete. |
