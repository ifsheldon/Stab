...
| Measurement-to-detection conversion | Partial | `CompiledDetectionConverter`, `convert_measurements_to_detection_events`, additive sweep-aware conversion, streaming visitor conversion, and `stab m2d` are implemented for the current detector-conversion subset. `m2d --sweep`, `--sweep_format`, and scoped `--ran_without_feedback` are supported, including selected bounded feedback loop refolding and selected nested bounded-repeat feedback detector-parity evidence, while full transform API parity, broader repeat-contained feedback parity, and broader sweep-conditioned simulator surfaces remain partial. |
...
| Broader sweep-conditioned simulator and analysis parity | Partial | Public `m2d --sweep`, non-frame and selected frame-path `detect` omitted-sweep default-false sampling, and selected analyzer sweep-control no-op plus invalid target-position rejection behavior are implemented for current subsets, but broader analyzer sweep behavior, Python detector-sampler sweep APIs, and every sweep target shape remain broader work. Pinned Stim v1.16.0 does not expose `stim detect --sweep`, so that flag is not a current CLI parity gap. |
