...
| Flows | Partial | `Flow`, unitary circuit flow generators, scoped measurement, reset, pair-measurement, nonconstant and constant `MPP`, feedback, `MPAD`, and promoted heralded-noise MPP flow generators, scoped `solve_for_flow_measurements` examples, scoped unitary `time_reversed_for_flows`, Stim v1.16.0 `GateData.flows` metadata for implemented gate-table metadata shapes, and unsigned `check_if_circuit_has_unsigned_stabilizer_flows` measurement-record and observable dependency cases are implemented for the scoped Rust semantics. Broader composed measurement-rich `flow_generators`, broader heralded-noise generator synthesis, full generator-table measurement solving, measurement-rich transform integration, and Python API parity remain deferred or active follow-up work. See [plans/rpf5-flow-progress-report.md](plans/rpf5-flow-progress-report.md). |
