...
| Detector-analysis utility APIs | Partial | `circuit_detecting_regions` supports the simple H/CX/MXX detecting-region case with typed detector and tick inputs plus bounded repeat-tick traversal, `missing_detectors` supports the basic reset/measure detector-suggestion subset for single-record detector coverage, and `circuit_with_inlined_feedback` covers the supported MPP feedback-transform case. Broader detecting-region gate and target-shape support, detector filtering, multi-detector regions, gauge handling, missing-detector row reduction for multi-record detectors, repeated MPP stabilizer-product missing-detector analysis, observable-interaction missing-detector analysis, honeycomb suffix analysis, toric suffix analysis, and exact loop refolding remain future work. |
