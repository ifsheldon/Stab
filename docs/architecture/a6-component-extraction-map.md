...
The engine foundation, circuit-sampling, measurement-to-detection, circuit detection-sampling, and DEM-sampling slices are physically extracted. `stab-engine` owns `CompilationOperation`, `CompilationRequestFingerprint`, `biased_randomize_bits`, sampling capability descriptors, the sampling, detection, and DEM compilers, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, conversion reference and sweep state, direct detector-frame execution, fused sample-and-convert execution, lowered folded DEM execution, detector-only and sampled-error DEM sampling, incremental replay, cancellation, progress, poisoning, and typed measurement, detection, and DEM-sample delivery. The crate root is the sole canonical public execution namespace. `stab-core` retains compatibility reexports plus `CompiledSampler`, `CompiledDetectionConverter`, `CompiledDemSampler`, callback streaming, materialized records, byte encoding, and output routing as facade adapters. Direct benchmark callers use the canonical package, qualification records facade paths as aliases, and source-current M8, M9, and M11 evidence preserves every existing comparator and gate. Reference-sample trees, sampled-flow execution, and portable SIMD remain the active extraction work.
