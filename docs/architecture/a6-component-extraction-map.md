...
The engine foundation, circuit-sampling, measurement-to-detection, and circuit detection-sampling slices are physically extracted. `stab-engine` owns `CompilationOperation`, `CompilationRequestFingerprint`, `biased_randomize_bits`, sampling capability descriptors, the sampling and detection compilers, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, conversion reference and sweep state, direct detector-frame execution, fused sample-and-convert execution, cancellation, progress, poisoning, and typed measurement and detection delivery. The crate root is the sole canonical public execution namespace. `stab-core` retains compatibility reexports plus `CompiledSampler`, `CompiledDetectionConverter`, callback streaming, materialized records, byte encoding, and detector or observable output routing as facade adapters. Direct benchmark callers use the canonical package, qualification records facade paths as aliases, and source-current M8 and M9 evidence preserves every existing comparator and gate. DEM sampling, reference-sample trees, sampled-flow execution, and portable SIMD remain the active extraction work.
