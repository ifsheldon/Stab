...
The scalar engine is physically extracted. `stab-engine` owns `CompilationOperation`, `CompilationRequestFingerprint`, `biased_randomize_bits`, sampling capability descriptors, the sampling, detection, and DEM compilers, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, conversion reference and sweep state, direct detector-frame execution, fused sample-and-convert execution, lowered folded DEM execution, detector-only and sampled-error DEM sampling, incremental replay, reference-sample trees, sampled-flow execution, cancellation, progress, poisoning, and typed measurement, detection, and DEM-sample delivery. The crate root is the sole canonical public execution namespace. `stab-core` retains compatibility reexports plus `CompiledSampler`, `CompiledDetectionConverter`, `CompiledDemSampler`, callback streaming, materialized records, byte encoding, output routing, and aggregate-error conversion as facade adapters. Direct benchmark callers use the canonical package and qualification records facade paths as aliases. Source-current M8, M9, and M11 evidence preserves every valid comparator and gate; the non-equivalent M8 flat-tree diagnostic is explicitly report-only and cannot pair with Stim's folded-tree measurements. Portable SIMD remains the active physical extraction work.
