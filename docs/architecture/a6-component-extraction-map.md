...
The scalar engine is physically extracted. `stab-engine` owns `CompilationOperation`, `CompilationRequestFingerprint`, `biased_randomize_bits`, sampling capability descriptors, the sampling, detection, and DEM compilers, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, conversion reference and sweep state, direct detector-frame execution, fused sample-and-convert execution, lowered folded DEM execution, detector-only and sampled-error DEM sampling, incremental replay, reference-sample trees, sampled-flow execution, cancellation, progress, poisoning, and typed measurement, detection, and DEM-sample delivery. Reference trees have private checked structure, exact logical-size and nesting admission, iterative random access and decompression, and fallible materialization. The crate root is the sole canonical public execution namespace. `stab-core` retains common plan and session reexports, while `CompiledSampler`, `CompiledDetectionConverter`, `CompiledDemSampler`, callback streaming, materialized records, byte encoding, and output routing are explicit `advanced::compat` adapters. Additive fallible sampler adapters preserve allocation and execution failures, while source-compatible infallible methods document their panic contract. Direct benchmark callers use canonical component APIs or qualification-tracked advanced facade paths. Source-current M8, M9, and M11 evidence preserves every valid comparator and gate; the non-equivalent M8 flat-tree diagnostic is explicitly report-only and cannot pair with Stim's folded-tree measurements.
...
Every threshold-owned row and the selected equal-width `m6-clifford-string` row must execute an exact semantic preflight before timing. Within that gated set, the legacy CLI generation row uses the exact pinned-Stim output digest; `m2d` rows use fixed outputs derived from their circuit, measurement, sweep, routing, and format semantics; detection-sampling rows require exact ordered record content or source-owned statistical behavior; and analyzer rows use pinned-Stim semantic digests that retain instruction and target identity while normalizing only insignificant probability-printing differences. Each exact preflight runs outside the timed region and has a same-shape wrong-content rejection test. Compile-and-release rows keep source-owned plan dimensions or fingerprints outside the timed region. The remaining report-only rows establish workload continuity only and make no correctness claim.

The final A6 ownership-continuity matrix is the complete 166-row pre-M12 manifest: M4 through M11 and PF1 through PF7. It contains 165 executable timing rows and the intentional `m7-perf-harness` `baseline-metadata` anchor, which validates pinned baseline provenance and is not a runtime workload. This deliberately includes adjacent unchanged rows so the source-current report cannot hide a moved path behind a hand-selected subset. `benchmarks/a6-measurement-contract.json` freezes the 166-row order, all 309 ordered Stab measurement identities, and the exact 66-row executable-preflight set derived from threshold policy plus the selected M6 row. The final comparison uses one fresh schema-version-3 pinned-Stim baseline and one schema-version-4 warmup plus three recorded Stab runs, both with create-new output semantics; the A6 compare command explicitly selects the checked contract with `--measurement-contract benchmarks/a6-measurement-contract.json`. The compare report binds the baseline digest, scalar Cargo feature set, row-native timing boundary, private host fingerprint, exact executing-binary digest, and selected measurement-contract path and digest. Strict comparison requires every non-metadata contract to produce its complete measurement set and accepts the metadata anchor only in its declared class. Existing comparator classes, thresholds, and waivers do not change, and validation reconstructs derived timing and memory fields from raw measurements before replaying tracked threshold, waiver, and profiler-note sources.

Report-only comparisons require identical row, ordered measurement set, workload observations, implementation and executable digest, private host fingerprint and CPU identity, build profile, feature selection, measurement-contract digest, timing boundary, and normalized row-native iteration count. Historical schema-version-1 through schema-version-3 reports cannot satisfy this contract; their predecessors must be regenerated from committed clean schema-version-4 instrumentation backports on the same host before the current matrix. `benchmarks/a6-predecessors.json` binds each historical product commit to a direct-child instrumentation backport, canonical raw-tree-delta digest, deterministic preservation tag, and exact phase set; publication derives ownership from that registry instead of accepting phase assignments. The 19 phases enumerated in the A6 plan have no clean predecessors and become initial seeds without retrospective regression claims. A comparable phase more than 15% slower than its clean extraction report requires one isolated source-owned row, completion of any exact executable preflight required by the checked contract, a warmup, and exactly one outer run retaining the row's source-owned internal timing count of at least eight. The ledger derives a typed resolved or reproduced outcome from the focused ratios and a validated Linux-perf receipt, while bounded free text records only the owner's next action. A clean publisher derives and validates one append-only `benchmarks/a6-focused-evidence-<source-revision>-<sha256>.json` object, and ordinary validation reads its tracked bytes and reopens every bound artifact. The fixed `benchmarks/a6-focused-evidence.json` path remains explicit legacy input only.
