...
All output-producing rows keep their independent semantic witness. The legacy CLI generation row uses the exact pinned-Stim output digest; `m2d` rows use fixed outputs derived from their circuit, measurement, sweep, routing, and format semantics; detection-sampling rows require exact record shape plus deterministic or source-owned statistical behavior; and analyzer rows use pinned-Stim semantic digests that retain instruction and target identity while normalizing only insignificant probability-printing differences. Each witness runs outside the timed region and has a same-width wrong-content rejection test. Compile-and-release rows keep source-owned plan dimensions or fingerprints outside the timed region.

The final A6 ownership-continuity matrix is the complete 166-row pre-M12 manifest: M4 through M11 and PF1 through PF7. It contains 165 executable timing rows and the intentional `m7-perf-harness` `baseline-metadata` anchor, which validates pinned baseline provenance and is not a runtime workload. This deliberately includes adjacent unchanged rows so the source-current report cannot hide a moved path behind a hand-selected subset. `benchmarks/a6-measurement-contract.json` freezes the 166-row order, all 309 ordered Stab measurement identities, and the exact Rust semantic-witness source for each executable row. The final comparison uses one fresh schema-version-2 pinned-Stim baseline and one schema-version-3 warmup plus three recorded Stab runs, both with create-new output semantics; the A6 compare command explicitly selects the checked contract with `--measurement-contract benchmarks/a6-measurement-contract.json`. The compare report binds the baseline digest, scalar Cargo feature set, row-native timing boundary, and selected measurement-contract path and digest. Strict comparison requires every non-metadata contract to produce its complete measurement set and accepts the metadata anchor only in its declared class. Existing comparator classes, thresholds, and waivers do not change, and validation replays current thresholds, waivers, and source-owned profiler notes instead of trusting recorded statuses.

Report-only comparisons require identical row, ordered measurement set, workload observations, implementation, host and CPU identity, build profile, feature selection, measurement-contract digest, timing boundary, and normalized row-native iteration count. Historical schema-version-1 and schema-version-2 reports cannot satisfy this contract; their predecessors must be regenerated from committed clean instrumentation backports on the same host before the current matrix. The 19 phases enumerated in the A6 plan have no clean predecessors and become initial seeds without retrospective regression claims. A comparable phase more than 15% slower than its clean extraction report requires one isolated source-owned row, its contract-owned semantic witness, a warmup, and exactly one outer run retaining the row's source-owned internal timing count of at least eight. The ledger derives a typed resolved or reproduced outcome from the focused ratios and profile availability, while free text records only the owner's next action. A clean publisher derives and validates the ledger before atomically creating its checked path, and ordinary validation always reopens every bound artifact.
