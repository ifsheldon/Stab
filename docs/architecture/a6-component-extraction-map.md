...
The shared model-diagnostic slice is also physically complete. `stab-model` owns `ByteSpan`, parse codes and contexts, dialect identity, parser limits and their typed resource failures, `Estimate`, and `ResourceEstimate`; `stab-core` preserves public aliases and aggregate-error conversion. The model resource context is intentionally closed so adding a cause forces exhaustive facade conversion, while its public operation and dimension enums remain non-exhaustive for callers. Narrow `stab_model::advanced` constructors are temporary cross-crate implementation seams for the still-unmoved parser, fingerprint, and sampling-estimate code. They preserve private fields and checked construction instead of exposing model storage or creating a facade dependency. `stab_records::EncodedSizeEstimate` remains independent: the former facade-local `From` implementation cannot survive Rust's orphan rules after both types become foreign, so internal callers convert explicitly rather than introducing a forbidden model-to-records edge.
