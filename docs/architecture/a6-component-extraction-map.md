...
The engine foundation and circuit-sampling slices are physically extracted. `stab-engine` owns `CompilationOperation`, `CompilationRequestFingerprint`, `biased_randomize_bits`, sampling capability descriptors, the sampling compiler, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, cancellation, progress, poisoning, and typed measurement-sink delivery. The crate root is the sole canonical public sampling namespace. `stab-core` retains compatibility reexports plus `CompiledSampler`, callback streaming, materialized records, and byte encoding as facade adapters. Direct benchmark callers use the canonical package, qualification records facade paths as aliases, and source-current M8 evidence preserves every existing comparator and gate. Detection conversion, circuit detection sampling, DEM sampling, reference-sample trees, sampled-flow execution, and portable SIMD remain the active extraction work.
