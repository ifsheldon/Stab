...
| Dimension | Default | Exact test selector | Real default max executed? | Reduced-boundary justification or remaining gap |
| --- | ---: | --- | --- | --- |
| Record width | 1,000,000 bits | `cargo test -p stab-engine --test detection_resource_limits default_record_width_accepts_the_boundary_and_rejects_the_next_bit -- --exact` | Yes | A single wide measurement executes the real default and rejects 1,000,001 before compiled-plan construction; `a2_detection_record_policy_admission` isolates a smaller custom boundary and typed policy context. |
| Repeat unroll | 100,000 | `cargo test -p stab-engine --test detection_resource_limits default_traversal_limits_accept_the_boundary_and_reject_the_next_unit -- --exact` | Yes | A compact repeat reaches exactly 100,000 under defaults and rejects 100,001; `a2_detection_repeat_policy_admission` isolates reduced first-excess behavior. |
| Expanded instructions | 1,000,000 | `cargo test -p stab-engine --test detection_resource_limits default_traversal_limits_accept_the_boundary_and_reject_the_next_unit -- --exact` | Yes | A no-term `TICK` fixture executes exactly one million instructions through admission and construction and rejects 1,000,001 with typed context; `a2_detection_work_policy_admission` isolates the configured dimension. |
| Aggregate repeat iterations | 1,000,000 | `cargo test -p stab-engine --test detection_resource_limits default_traversal_limits_accept_the_boundary_and_reject_the_next_unit -- --exact` | Yes | A compact nested empty-body fixture executes exactly one million aggregate repeat iterations and rejects 1,000,001 without retaining terms; `a2_detection_work_policy_admission` isolates the configured dimension. |
| Retained compiled terms | 16,000,000 | `cargo test -p stab-engine --test canonical_owner_evidence a2_detection_compiled_plan_policy -- --exact` | No | Closed by substitution. The selector accepts six terms and rejects the first excess at five in the dry admission pass. Sixteen million `usize` terms alone require at least 128 MiB on supported 64-bit hosts before per-detector vectors and allocator overhead. |
| Conservative compiled-plan bytes | 256 MiB | `cargo test -p stab-engine --test canonical_owner_evidence a2_detection_compiled_plan_policy -- --exact` | No | Closed by substitution. The selector accepts the exact conservative byte count for three detector vectors and six terms and rejects one byte below it before term retention. Intentionally allocating 256 MiB exceeds the ordinary single-test budget. |
