...
| `crates/stab-engine/src/fingerprint.rs`, `probability.rs`, `sampling/mod.rs`, `sampling/**` | Engine | Backend-neutral request identity, execution-side biased randomization, sampling capability descriptors, compilation, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, cancellation, progress, poisoning, and typed measurement-sink delivery are physically engine-owned. The engine imports model, records, algebra, and analysis but no facade, codec, filesystem, CLI, or ops API. Its crate root is the sole canonical public sampling namespace. Temporary doc-hidden sweep/reference bridges exist only for still-core detection and must disappear with that extraction. |
| `crates/stab-core/src/compilation_fingerprint.rs`, `crates/stab-core/src/probability_util.rs`, `capabilities.rs`, `sampling.rs`, `sampling/stream.rs`, `sampling_output_compat.rs` | Facade compatibility | Fingerprint, probability, compiler, plan, session, and descriptor paths are thin reexports or aggregation over canonical engine owners. `CompiledSampler`, callback streaming, materialized records, and byte-oriented encoding remain explicit compatibility adapters over engine plans, sessions, and records-owned sinks. |
...
| `execution/reference_sample_tree.rs`, `execution/sampled_flow.rs`, `detection.rs`, `detection/**`, `dem_sampler.rs`, `dem_sampler/**` | Engine, not yet physically moved | Measurement-to-detection conversion, circuit detection sampling, DEM sampling, reference-sample trees, and sampled-flow execution expose distinct compiler, immutable-plan, mutable-session, cancellation, progress, and typed-sink families through `stab_core::execution`. Mutable sessions own RNG where applicable, reference and conversion scratch, detector frames, bounded 64-shot batches, counters, and poison state. Incremental conversion binds one session to one sink lifecycle. Direct-detector-frame, fused sample-convert, detector-only DEM, sampled-error DEM, and replay implementations remain private operation-specific variants. Direct detector-frame compilation admits the complete retained conversion and executable representation before fallible materialization, and DEM replay admits poison state and total work before caller-record traversal. `CompiledDetectionConverter` remains the public per-record compatibility kernel used by measurement-to-detection sessions, and the unknown-length DEM replay iterator retains direct folded traversal because it cannot declare a shot count before iteration; neither exception is used by the CLI. These paths emit separate detector, observable, and optional sampled-error planes. Their physical extraction into `stab-engine` remains A6 work. |
