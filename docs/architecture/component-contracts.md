...
| `crates/stab-core/src/compilation_fingerprint.rs`, `crates/stab-core/src/probability_util.rs`, `capabilities.rs`, `detection.rs`, `detection/**`, `dem_sampler.rs`, `dem_sampler/**` | Facade compatibility | Fingerprint, probability, compiler, plan, session, converter, and descriptor paths are direct canonical reexports or aggregation over engine owners. The facade must not create a second public execution type identity. P2 has deleted the sampling compatibility layer; `CompiledDetectionConverter`, `CompiledDemSampler`, their materialized or callback routes, aggregate error conversion, and output routing remain pending deletion. |
