...
- Resource behavior: `ParseLimits` owns inclusive source-byte, physical-line, compact-declaration, retained-target, and caller-tightenable 256-level repeat admission. One parser admission object counts declarations before repeat expansion and circuit fusion and admits each decoded target before model retention; byte admission precedes byte-metadata preparation, which is bounded by source-byte and admitted-line policy and intentionally precedes semantic declaration and target admission. Top-level model storage grows only after successful declarations, so rejected suffixes and blank or comment-only source cannot drive model-item allocation. Programmatic models may exceed the parser envelope only for APIs with an established deeper contract; those consumers must be iterative or reject before recursive work. `DemErrorMechanismTraversalLimits` independently admits represented mechanisms and represented instruction visits before or during callback traversal, skips error-free repeat subtrees, and never allocates a target vector per mechanism. `DemFlattenLimits` belongs to the analysis adapter rather than the model.
...
- Purpose: own Stable packed bit storage, checked views, scalar kernels, and bit-matrix layouts.
- Inputs and outputs: typed bit vectors, slices, blocks, matrices, and `BitError`.
...
- Resource behavior: owned storage is proportional to declared logical width or matrix shape; in-place kernels allocate nothing after construction.
...
- Conformance tests: scalar references, tails, dirty padding, empty and block boundaries, unequal-width rejection without mutation, transpose semantics, allocation counts, and default-versus-portable feature runs.
- Benchmarks: dense XOR, not-zero, popcount, and transpose workloads with pinned Stim comparators where faithful; scalar-versus-SIMD diagnostics apply only to kernels whose executed path changes.
...
- Resource behavior: `DetectionConversionLimits` and `DemSamplerLimits` own the caller-selectable conversion and DEM execution budgets. Circuit sampling, measurement-to-detection conversion, circuit detection sampling, and DEM sampling each have operation-specific compiler, immutable-plan, owned mutable-session, and typed-sink boundaries. Sampling, detection conversion, and direct detector-frame execution retain folded bodies once in validated flat operation tapes compiled and executed with fixed-depth stacks. Measurement-bearing sampling plans admit at most one million expanded operation dispatches per shot, while zero-width plans validate but never traverse represented work; represented counts above `u64::MAX` are reported as explicit lower bounds. Detection conversion admits record shape, expanded work, aggregate iterations, compact terms, and compact bytes independently instead of imposing a repeat-count syntax cap. Direct-frame SPP lowering emits and charges one lowered instruction at a time during compilation, stops at the first admission failure, and checks actual retained vector capacities before a compiled plan escapes. Sweep-conditioned conversion applies a static all-zero-sweep reference plus a reusable frame-derived correction for each supplied sweep record, including record and Pauli observables in either reference mode. Reference sampling uses a bounded inverse-tableau path for H/CX circuits with Z-basis measurements and resets, and falls back to the general stabilizer simulator for other Clifford or measurement operations and large expanded loops. The general path reuses a repeat's measured output only after exact stabilizer-state recurrence; `ReferenceSampleLoopPolicy::Iterate` always iterates, while `Fold` also iterates when estimated saved stabilizer work cannot pay for snapshot capture and comparison or when the optional snapshot would exceed the existing session-storage ceiling. Session construction uses fallible reservation and rejects more than 256 MiB of reusable buffer storage before allocation. Direct detection and sweep correction share the frame-owned estimate for 64-shot `u64` planes, and direct detection charges both its pending and delivery packed batches. Each packed batch rounds detector and observable rows independently to 64-bit words; measurement-to-detection conversion uses the same batch estimate. These session limits bound executable storage rather than the model qubit-ID range, so a valid maximum-ID circuit can compile while detection session construction rejects its retained state. DEM replay work and active retained bytes remain distinct limits. Public sampling delivery and DEM replay reuse at most one 64-shot batch. Generated DEM samples use a separately admitted internal window of at most 1024 shots, counter-keyed Bernoulli streams, and word-major detector, observable, and optional sampled-error planes; each 64-shot slice is copied into the existing delivery batch so cancellation and sink-failure progress remain defined at the public batch boundary. Sessions retain no caller-owned model or sink borrow, and replay reset reuses only a completed non-poisoned lifecycle. Conversion may accept smaller caller batches or one record at a time, and cancellation is checked only at documented batch boundaries.
- Extension points: measurement, detection, and DEM-sample sinks.
- Conformance tests: seeded reproducibility, pinned-Stim statistical semantics, compilation and plan fingerprints, fixed implementation identity, legal all-false sweep controls, session reuse, cancellation and poisoning, resource admission, sink lifecycle, reference samples, sampled flows, and composed sampling-to-detection behavior.
...
| `crates/stab-engine/src/fingerprint.rs`, `probability.rs`, `bernoulli.rs`, `sampling/mod.rs`, `sampling/**`, `sampling_estimate.rs`, `detection/mod.rs`, `detection/**`, `dem_sampling/mod.rs`, `dem_sampling/**`, `reference_sample_tree.rs`, `sampled_flow.rs` | Engine | Backend-neutral request identity, execution-side biased randomization, sampling request estimates, sampling, detection, and DEM capability descriptors, compilation, immutable plans, owned mutable sessions, direct-Z, reusable bit-plane Pauli-frame, general-frame reference and fallback execution, deterministic reference samples, measurement-to-detection conversion, direct detector-frame sampling, lowered folded DEM execution, detector-only and sampled-error DEM sampling, owned reusable replay state, bounded reference-sample trees, sampled-flow execution, cancellation, progress, poisoning, and typed measurement, detection, or DEM-sample delivery are physically engine-owned. Sampling and detection share one generic bit-plane Pauli-frame execution owner: detection and sweep paths retain 64-bit lanes, while circuit sampling uses the wider packed block owned by `stab-bits`. DEM sampling similarly applies deterministic counter-keyed Bernoulli masks directly to bounded word-major output planes, while replay keeps the scalar error-record path because its records are caller-owned. Incremental conversion and replay use short-lived transactions that bind exactly one sink across writes and finalization; reusable plans and sessions retain no caller borrow. The engine imports bits, model, records, algebra, and analysis but no facade, codec, filesystem, CLI, or ops API. Its crate root is the sole canonical public execution namespace. |
...
| `crates/stab-bits/src/**` | Bits | Stable Rust 1.97.1 packed storage, checked views, scalar kernels, and transpose implementation are physically extracted. The optional `portable-simd` feature reaches only the dependency-free raw kernel crate and replaces complete four-word XOR blocks while retaining scalar tails and logical tail masking locally. |
