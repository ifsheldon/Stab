...
| `error.rs` | Facade compatibility | Losslessly converts model-owned `ModelError`, including complete parse, parse-resource, and structural-validation payloads, plus records and operation resource failures into `CircuitError` without changing established variants or human display. |
