...
| `crates/stab-core/src/compilation_fingerprint.rs`, `crates/stab-core/src/probability_util.rs`, `capabilities.rs`, `sampling.rs`, `sampling/stream.rs`, `sampling_output_compat.rs`, `detection.rs`, `detection/**`, `dem_sampler.rs`, `dem_sampler/**` | Facade compatibility | Fingerprint, probability, compiler, plan, session, converter, and descriptor paths are direct canonical reexports or aggregation over engine owners. The facade must not create a second public execution type identity. `CompiledSampler`, `CompiledDetectionConverter`, `CompiledDemSampler`, callback streaming, materialized records, byte-oriented encoding, aggregate error conversion, and output routing remain explicit compatibility adapters over canonical engine plans, sessions, and records-owned sinks. |
