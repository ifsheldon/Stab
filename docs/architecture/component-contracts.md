...
| --- | --- | --- |
| `circuit.rs`, `circuit/**`, `model_bytes.rs`, `model_parse.rs`, `model_tag.rs`, `source_text.rs` | Model, awaiting physical move | The model keeps syntax, values, byte-aware parsing, canonical text and byte printing, iteration, structural counts, and operation-owned parse admission. The shared byte preparation path preserves source-order failures and opaque Stim metadata without applying lossy whole-input UTF-8 conversion. Named line and repeat limits now come from `stab-model`; the remaining parser implementation still consumes them through narrow advanced construction hooks until the complete circuit move. Programmatic depth beyond the parser envelope remains consumer-specific rather than being silently accepted by recursive algorithms. A6 removed the algorithmic inherent adapters; analysis and execution behavior is now reached through named owner functions. |
...
| `crates/stab-model/src/diagnostics.rs`, `dialect.rs`, `parse_limits.rs`, `resource_limit.rs`, `resources.rs`, and `error.rs` | Model | Stable byte spans, parser diagnostics, model dialect identity, parser limits and their four real admission failures, and the shared honest-estimate vocabulary are physically model-owned. Attacker-controlled diagnostic text remains UTF-8-safe and bounded, while `ModelError` carries complete typed parser and parse-resource failures. The closed `ResourceLimitContext` makes every future model cause a compile-time facade-conversion obligation. Temporary functions under `stab_model::advanced` let the still-unmoved core parsers construct these values without exposing fields or making the model depend on the facade. |
...
| `crates/stab-core/src/resources.rs` | Facade compatibility and operation-specific resource errors | Reexports model-owned estimate values, converts every closed model parse-resource context into the established aggregate `ResourceLimitError`, and retains the operation identities and resource dimensions needed by analysis and execution. Operation-specific policies remain beside their model, analysis, or engine operations. |
...
| `error.rs` | Facade compatibility | Losslessly converts model-owned `ModelError`, including complete `ParseError` payloads, plus records and operation resource failures into `CircuitError` without changing established human display. |
