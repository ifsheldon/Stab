...
| `crates/stab-core/src/analysis/**`, `circuit_simplify.rs`, `circuit_tableau.rs`, `circuit_transforms.rs`, `circuit_generation.rs`, `circuit_flow.rs`, `circuit_inverse.rs`, `circuit_feedback.rs`, `circuit_detecting_regions*`, `circuit_missing_detectors*`, `dem/analyze.rs`, `dem/flatten.rs`, `dem/sat.rs`, `dem.rs`, `mbqc_decomposition.rs` | Analysis facade | Gate projections, circuit and DEM tag stripping, tableau conversion, simplification, decomposition, circuit and DEM flattening, DEM probability rounding, SAT/WCNF materialization, graphlike and hypergraph logical-error search, noise removal, generation, flow utilities, inversion, feedback lowering, detecting regions, missing-detector analysis, circuit-to-DEM lowering, error-probability conversion, sparse reverse tracking, and MBQC decomposition delegate to `stab-analysis` and preserve established `CircuitError`, generated-value, facade-owned option, and aggregate resource signatures. The analyzer, DEM-transform, SAT, and logical-search wrappers map `AnalysisResult` losslessly while reexporting canonical option, probability, and resource-policy identities where appropriate. Matcher adapters still delegate to core implementations awaiting extraction. |
