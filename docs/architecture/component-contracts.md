...
| `crates/stab-model/src/diagnostics.rs`, `dialect.rs`, `parse_limits.rs`, `resources.rs`, and `error.rs` | Model | Stable byte spans, parser diagnostics, model dialect identity, parser limits, and the shared honest-estimate vocabulary are physically model-owned. Attacker-controlled diagnostic text remains UTF-8-safe and bounded, while `ModelError::Parse` carries the complete typed parser failure. Temporary functions under `stab_model::advanced` let the still-unmoved core parsers construct these values without exposing fields or making the model depend on the facade. |
...
| `crates/stab-core/src/resources.rs` | Facade compatibility and operation-specific resource errors | Reexports model-owned estimate values while retaining the aggregate `ResourceLimitError`, operation identities, and resource dimensions required to preserve the established facade error contract. Operation-specific policies remain beside their model, analysis, or engine operations. |
