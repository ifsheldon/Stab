...
| `crates/stab-core/src/compilation_fingerprint.rs`, `crates/stab-core/src/probability_util.rs`, `capabilities.rs` | Facade composition | Fingerprint, probability, and descriptor paths are direct canonical reexports or aggregation over engine owners. The facade must not create a second public execution type identity. P2 deleted the sampling, detection, and DEM compatibility layers; execution compilers, plans, sessions, and errors are direct engine reexports. |
