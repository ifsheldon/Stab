...
| `crates/stab-core/src/analysis/**`, `circuit_simplify.rs`, `circuit_tableau.rs`, `circuit_transforms.rs`, `circuit_generation.rs`, `circuit_flow.rs`, `circuit_inverse.rs`, `circuit_feedback.rs`, `circuit_detecting_regions*`, `circuit_missing_detectors*`, `dem/analyze.rs`, `dem/flatten.rs`, `mbqc_decomposition.rs` | Analysis facade | Gate projections, circuit and DEM tag stripping, tableau conversion, simplification, decomposition, circuit and DEM flattening, DEM probability rounding, noise removal, generation, flow utilities, inversion, feedback lowering, detecting regions, missing-detector analysis, circuit-to-DEM lowering, error-probability conversion, sparse reverse tracking, and MBQC decomposition delegate to `stab-analysis` and preserve established `CircuitError`, generated-value, facade-owned option, and aggregate resource signatures. The analyzer and DEM-transform wrappers map `AnalysisResult` losslessly while reexporting canonical option, probability, and flatten-policy identities where appropriate. Search, SAT, and matcher adapters still delegate to core implementations awaiting extraction. |
| `dem/arena_index.rs`, `dem/error_traversal.rs`, `dem/graphlike*`, `dem/hyper*`, `dem/sat*`, `dem/search_budget.rs` | Analysis | These consume the model-owned folded traversal boundary and own the remaining indexing, search, SAT, and operation-specific resource policies. |
