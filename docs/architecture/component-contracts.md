...
- Resource behavior: `DetectionConversionLimits` and `DemSamplerLimits` own the A2 caller-selectable execution budgets. Ordinary circuit sampling now has compiler, immutable-plan, mutable-session, and sink boundaries but retains fixed representability checks because no honest caller-selectable sampling budget exists yet. Session construction conservatively rejects more than 256 MiB of frame, span-solving, reference, record, and bit-plane storage before allocation. Reusable scratch and cancellation checks remain bounded at batch or record boundaries, but one expensive shot has no wall-clock cancellation deadline.
...
| `sampling.rs`, `sampling/**`, `execution/**`, `detection.rs`, `detection/**`, `dem_sampler.rs`, `probability_util.rs` | Engine | The sampling compiler lowers into immutable backend-bearing plans; mutable non-`Sync` sessions own RNG, simulator frames, reference state, bounded 64-shot batches, cancellation, counters, and poison state. Direct-Z, small-frame, and general-frame variants remain private. Execution emits typed batches and imports no codec or filesystem API. Detection conversion and DEM sampling remain execution concerns for A5. |
