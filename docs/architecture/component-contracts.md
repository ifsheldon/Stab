...
- Files changed together: decoder source and tests, records prediction views, model error-mechanism traversal, facade reexports, architecture policy, Stable consumers, API migration inventory, correctness ownership, selected runtime contracts and profiler notes, and architecture documentation.

### Facade

- Purpose: provide the curated ergonomic `stab-core` API over canonical component owners while P2 removes obsolete pre-0.2 adapters.
- Inputs and outputs: common model, algebra, plan, session, batch, diagnostic, capability, resource-policy, and filesystem values.
- Invariants: root exports stay curated, lower-level storage remains visibly advanced until P2 removes that namespace, aggregate diagnostic conversion is lossless, and the facade does not become a second implementation owner.
- Dependencies: bits, records, algebra, model, analysis, engine, and decoder.
...
| `crates/stab-engine/src/fingerprint.rs`, `probability.rs`, `sampling/mod.rs`, `sampling/**`, `detection/mod.rs`, `detection/**`, `dem_sampling/mod.rs`, `dem_sampling/**`, `reference_sample_tree.rs`, `sampled_flow.rs` | Engine | Backend-neutral request identity, execution-side biased randomization, sampling, detection, and DEM capability descriptors, compilation, immutable plans, mutable sessions, direct-Z, small-frame, general-frame, deterministic reference-sample, measurement-to-detection conversion, direct detector-frame and fused detection sampling, lowered folded DEM execution, detector-only and sampled-error DEM sampling, incremental replay, bounded reference-sample trees, sampled-flow execution, cancellation, progress, poisoning, and typed measurement, detection, or DEM-sample delivery are physically engine-owned. The engine imports model, records, algebra, and analysis but no facade, codec, filesystem, CLI, or ops API. Its crate root is the sole canonical public execution namespace. |
...
| `crates/stab-core/src/compilation_fingerprint.rs`, `crates/stab-core/src/probability_util.rs`, `capabilities.rs`, `dem_sampler.rs`, `dem_sampler/**` | Facade compatibility | Fingerprint, probability, compiler, plan, session, and descriptor paths are direct canonical reexports or aggregation over engine owners. The facade must not create a second public execution type identity. P2 has deleted the sampling and detection compatibility layers; `CompiledDemSampler`, its materialized or callback routes, aggregate error conversion, and output routing remain pending deletion. |
...
| `crates/stab-records/src/**` | Records | Stable Rust 1.97.1 strict codecs, typed semantic widths, shot-major and 64-shot bit-plane batches, correction-typed mutable prediction prefixes, typed DETS layouts, bounded visitors, and measurement, detection, and DEM-sample sinks are physically extracted. Mutable prediction views borrow existing storage and cannot expose reusable suffix records. Shared text and packed decoders own grammar and length diagnostics so materialized and streaming consumers cannot drift; representation-preserving B8 consumers borrow one validated packed frame at a time and remain responsible for canonicalizing unused tail bits before output. The codec capability registry lives beside these implementations and is consumed by the facade and CLI instead of being copied into a status manifest. |
| `crates/stab-core/src/result_formats.rs`, `crates/stab-core/src/result_streaming.rs` | Private facade compatibility adapters exported through `advanced::records` | These wrappers convert structured format failures losslessly into `CircuitError` and preserve established materialized and callback signatures while callers migrate. `MeasureRecordWriter` and `MeasureRecordBatchWriter` remain byte-oriented compatibility adapters for CLI and migration paths; new component code uses typed sink traits and typed DETS namespace selection. The wrappers must not reimplement codec grammar, layout, or buffering policy. |
| `crates/stab-core/src/execution/mod.rs`, `crates/stab-core/src/execution/circuit_adapters.rs`, `crates/stab-core/src/execution/sampled_flow.rs`, `crates/stab-core/src/error.rs` | Facade compatibility | Reference-tree and sampled-flow entry points delegate to `stab-engine`, preserving the established `CircuitResult`, integer shot count, optional seed, root reexports, and error conversion. They own no simulator frame, tree construction, batching, or sampled-flow algorithm. |
| `sampling_estimate.rs` | Facade planning | Sampling estimates may inspect codec size formulas without making result encoding part of the executable engine contract. The old sampler materialization and byte-returning compatibility module has been deleted. |
| `crates/stab-core/src/analysis/**`, `circuit_simplify.rs`, `circuit_tableau.rs`, `circuit_transforms.rs`, `circuit_generation.rs`, `circuit_flow.rs`, `circuit_inverse.rs`, `circuit_feedback.rs`, `circuit_detecting_regions*`, `circuit_missing_detectors*`, `dem/analyze.rs`, `dem/flatten.rs`, `dem/sat.rs`, `dem.rs`, `error_matcher.rs`, `matched_error.rs`, `mbqc_decomposition.rs` | Analysis facade | Pure transforms and analyses delegate to `stab-analysis` while preserving established `CircuitError`, generated-value, facade-owned option, aggregate resource, matcher-entry, and matched-error DTO signatures. The three matched-error wrapper types whose public mutating methods historically return `CircuitResult` retain explicit conversion at that error boundary; read-only formatting and comparison borrow their storage through analysis-owned views, and simple provenance values are direct reexports. |
