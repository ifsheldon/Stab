...
The current A6 product graph after complete model and analysis extraction plus circuit-sampling, detection-execution, and DEM-sampling extraction is:
...
Rust 1.97.1 is the minimum supported Stable compiler for model, bits, records, scalar algebra, pure analysis, and the current scalar engine. The extracted `stab-bits`, `stab-records`, scalar-default `stab-algebra`, `stab-model`, `stab-analysis`, and sampling, detection, and DEM `stab-engine` packages build on that compiler. Completed portable-backend integration retains the pinned Nightly target.
