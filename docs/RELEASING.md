...
The P9 procedure permits a narrow evidence descendant after the measured source revision. That descendant may add the immutable full and soak bundles, the current AArch64 evidence pointer, the first reviewed self-regression baseline, generated suite documentation, and named status prose; it may not change product or benchmark-runner code. Review `benchmarks/current-aarch64-evidence.toml`, replay both named bundles, run `just bench::e2e-release-check`, set `RELEASE_COMMIT` to the descendant's full commit identity, and verify that the worktree is clean. The generated [Stim parity ledger](stim-parity.md) owns current product status. The human operator must then verify that every required GitHub CI check for `RELEASE_COMMIT` has passed. Local Rust checks do not query GitHub CI and do not replace that exact-revision check.
