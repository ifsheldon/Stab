...
use stab_analysis::{
    ErrorAnalyzerOptions, circuit_to_detector_error_model, decomposed_circuit, gate_has_tableau,
    gate_tableau,
};
use stab_engine::{
    DetectionCompileError, MeasurementToDetectionCompiler, ReferenceSampleMode,
...
fn anti_hermitian_spp_execution_is_rejected_by_sampler_and_detection_conversion() {
    for gate_name in ["SPP", "SPP_DAG"] {
        ...
        assert!(
            matches!(sampler_error, SamplingCompileError::InvalidCircuit { .. }),
