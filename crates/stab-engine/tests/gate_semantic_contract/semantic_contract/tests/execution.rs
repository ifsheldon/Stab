...
use crate::{
    Circuit, Gate, PauliBasis, PauliSign, PauliString, Probability,
    analysis::{
        ErrorAnalyzerOptions, circuit_flow_generators, circuit_to_detector_error_model,
        decomposed_circuit, gate_tableau,
    },
    execution::{ReferenceSampleMode, circuit_reference_sample},
};
...
fn run_surface(text: &str, surface: GateSurface) -> TestResult<SurfaceFingerprint> {
    ...
    match surface {
        ...
        GateSurface::DetectionConverter => {
            let sweep_width = usize::try_from(circuit.count_sweep_bits()?)?;
            ...
            let record =
                output.records.into_iter().next().ok_or_else(|| {
                    std::io::Error::other("conversion fixture produced no record")
                })?;
...
fn test_reference_sample(circuit: &Circuit) -> TestResult<Vec<bool>> {
    Ok(circuit_reference_sample(circuit)?)
}
