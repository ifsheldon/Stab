...
fn validate_materialized_instruction_capacity(count: u64) -> CircuitResult<usize> {
...
fn validate_flattening_budget(
    circuit: &Circuit,
    limits: CircuitFlattenLimits,
) -> CircuitResult<CircuitFlattenEstimate> {
...
impl CircuitFlattenEstimate {
    fn checked_add(self, other: Self) -> CircuitResult<Self> {
        Ok(Self {
            operations: self
                .operations
    ...
    fn materialized_bytes(self) -> CircuitResult<u64> {
        let operation_bytes =
...
fn checked_flatten_sum(left: u64, right: u64, resource: &'static str) -> CircuitResult<u64> {
...
fn checked_flatten_product(left: u64, right: u64, resource: &'static str) -> CircuitResult<u64> {
...
fn usize_to_u64(value: usize) -> CircuitResult<u64> {
    u64::try_from(value)
        .map_err(|_| CircuitError::invalid_domain_value("flattened circuit count", "exceeds u64"))
}
...
fn validate_flattened_repeat_nesting(circuit: &Circuit) -> CircuitResult<()> {
...
fn try_clone_slice<T: Clone>(values: &[T], context: &'static str) -> CircuitResult<Vec<T>> {
