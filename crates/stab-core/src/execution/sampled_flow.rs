...
/// Unlike [`crate::check_if_circuit_has_unsigned_stabilizer_flows`], signs are meaningful and each
/// queried flow is checked by appending an ancilla witness measurement to a noiseless copy of the
/// circuit. Each false flow has a 50 percent chance of surviving an individual sample, so callers
/// should use enough samples for their desired confidence. The effective sample count is rounded up
/// to 256 to match Stim's `MAX_BITWORD_WIDTH` confidence behavior on the public Python path.
