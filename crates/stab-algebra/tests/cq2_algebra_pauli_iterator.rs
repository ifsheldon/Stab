...
const RANGE_SPECS: [IteratorSpec; 3] = [
    ...
    IteratorSpec::xz_range(16),
];
const SINGLETON_WIDTHS: [usize; 7] = [1, 63, 64, 65, 255, 256, 4_096];
...
fn cq2_algebra_pauli_iterator_runtime_contract_matches_independent_reference() -> TestResult<()> {
    ...
    for spec in RANGE_SPECS {
        ...
        assert_range_summary(&actual, spec)?;
    }
    ...
    for width in SINGLETON_WIDTHS {
        ...
        let reference = summarize_reference(spec)?;
        assert_eq!(actual, reference, "{spec:?}");
        assert_singleton_summary(&actual, spec)?;
    }

    let restart_spec = IteratorSpec::xyz_singleton(257);
...
fn assert_range_summary(summary: &SequenceSummary, spec: IteratorSpec) -> TestResult<()> {
    assert!(summary.output_count > 0, "{spec:?}");
    assert_eq!(
...
fn assert_singleton_summary(summary: &SequenceSummary, spec: IteratorSpec) -> TestResult<()> {
