...
use stab_model::{
    Circuit, CircuitInstruction, CircuitItem, Gate, GateCategory, MeasureRecordOffset, Pauli,
    QubitId, RepeatBlock, Target,
};
...
impl MissingDetectorFinder {
    fn new(qubit_count: usize, options: MissingDetectorOptions) -> AnalysisResult<Self> {
        Ok(Self {
    ...
    fn process_circuit(&mut self, circuit: &Circuit) -> AnalysisResult<()> {
    ...
    fn process_repeat_block(&mut self, repeat: &RepeatBlock) -> AnalysisResult<()> {
    ...
    fn process_instruction(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        if crate::gate_has_tableau(instruction.gate()) {
    ...
    fn process_unitary_tableau(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let gate_name = instruction.gate().canonical_name();
        let tableau = crate::gate_tableau(instruction.gate()).map_err(|error| {
            AnalysisError::invalid_detector_error_model(format!(
    ...
    fn process_mpp(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        for group in instruction.target_groups() {
            let mut raw_terms = Vec::new();
            for target in group {
    ...
    fn process_mpad(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn process_detector(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn absolute_record_index(&self, offset: MeasureRecordOffset) -> AnalysisResult<usize> {
...
fn validate_repeat_budget(circuit: &Circuit) -> AnalysisResult<()> {
...
fn direct_instruction_work_units(instruction: &CircuitInstruction) -> AnalysisResult<u64> {
...
fn decomposed_instruction_work_units(instruction: &CircuitInstruction) -> AnalysisResult<u64> {
    let decomposed =
...
fn expanded_circuit_work_units(circuit: &Circuit) -> AnalysisResult<u64> {
    let mut total = 0_u64;
    for item in circuit.items() {
...
impl InvariantTracker {
    ...
    fn reset(&mut self, qubit: usize, basis: PauliBasis) -> AnalysisResult<()> {
    ...
    ) -> AnalysisResult<Option<MeasurementRow>> {
    ...
    fn require_qubit(&self, qubit: usize, role: &str) -> AnalysisResult<()> {
...
fn plain_qubit_index(instruction: &CircuitInstruction, target: &Target) -> AnalysisResult<usize> {
    match target {
        Target::Qubit {
            inverted: false,
            id,
...
fn instruction_qubits(instruction: &CircuitInstruction) -> AnalysisResult<Vec<(usize, bool)>> {
    instruction
        .targets()
        .iter()
        .map(|target| {
...
fn qubit_index(qubit: QubitId) -> AnalysisResult<usize> {
...
fn normalize_pauli_product_terms(
...
) -> AnalysisResult<(Vec<(usize, PauliBasis)>, bool)> {
    ...
    match phase {
        0 => Ok((terms, inverted)),
        2 => Ok((terms, !inverted)),
        _ => Err(AnalysisError::invalid_detector_error_model(
