...
use stab_model::{
    Circuit, CircuitInstruction, CircuitItem, DemInstructionKind, DemTarget, DetectorErrorModel,
    MeasureRecordOffset, Probability, QubitId, RepeatBlock,
};

use crate::{AnalysisError, AnalysisResult, single_qubit_clifford_for_gate};
...
impl Analyzer {
    ...
    fn analyze(mut self, circuit: &Circuit) -> AnalysisResult<DetectorErrorModel> {
    ...
    fn visit_circuit(&mut self, circuit: &Circuit) -> AnalysisResult<()> {
    ...
    fn visit_repeat(&mut self, repeat: &RepeatBlock) -> AnalysisResult<()> {
    ...
    fn visit_instruction(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        ...
        match gate_name {
            ...
            name => {
                if let Ok(clifford) = single_qubit_clifford_for_gate(instruction.gate()) {
    ...
    fn record_pauli_channel1(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
            ...
            try_disjoint_to_independent_xyz_errors(*x_probability, *y_probability, *z_probability)?
        {
            for target in instruction.targets() {
                ...
                let tag = owned_tag(instruction.tag_bytes());
                self.push_single_qubit_pauli_error(
                    independent.x,
                    qubit,
                    AnalyzerPauli::X,
    ...
    fn record_pauli_channel2(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        if instruction.targets().is_empty() {
            return Ok(());
        }
        let Some(threshold) = self.options.approximate_disjoint_errors_threshold else {
    ...
    fn allocate_disjoint_group_id(&mut self) -> AnalysisResult<u64> {
        let group_id = self.next_disjoint_group_id;
        self.next_disjoint_group_id =
            self.next_disjoint_group_id.checked_add(1).ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("disjoint error group count overflowed")
            })?;
        Ok(group_id)
    }

    fn record_depolarize2(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let Some(probability) = instruction.probability_argument()? else {
            return Ok(());
        };
        if probability.get() == 0.0 {
    ...
    fn record_heralded_pauli_components(
        ...
        for target in instruction.targets() {
            ...
            self.measurement_count = self.measurement_count.checked_add(1).ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("measurement count overflowed")
            })?;
    ...
    fn record_pair_measurements(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let basis =
            pair_measurement_basis(instruction.gate().canonical_name()).ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("unknown pair measurement basis")
            })?;
        for group in instruction.target_groups() {
            let [left, right] = group else {
                return Err(AnalysisError::invalid_detector_error_model(format!(
    ...
    fn record_measurement_pads(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn record_resets(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        for target in instruction.targets() {
            let Some(qubit) = target.qubit_id() else {
                return Err(AnalysisError::invalid_detector_error_model(format!(
                    "{} target {target} is not a qubit",
    ...
    fn apply_swap(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let gate_name = instruction.gate().canonical_name();
        for group in instruction.target_groups() {
            let [left, right] = group else {
                return Err(AnalysisError::invalid_detector_error_model(format!(
    ...
    fn record_detector(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn record_observable(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let observable = instruction.observable_id_argument()?.ok_or_else(|| {
            AnalysisError::invalid_detector_error_model("OBSERVABLE_INCLUDE missing observable id")
        })?;
    ...
    fn shift_coordinates(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn measurement_index_from_offset(&self, offset: MeasureRecordOffset) -> AnalysisResult<usize> {
        let measurement_count = i64::try_from(self.measurement_count).map_err(|_| {
            AnalysisError::invalid_detector_error_model("measurement count does not fit i64")
        })?;
        let index = measurement_count
            .checked_add(i64::from(offset.get()))
            .ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("measurement offset overflowed")
            })?;
        ...
        usize::try_from(index).map_err(|_| {
            AnalysisError::invalid_detector_error_model("measurement index does not fit usize")
        })
    }

    fn into_dem(self) -> AnalysisResult<DetectorErrorModel> {
        ...
        for ((targets, tag), probability) in merged_error_probabilities {
            ...
            dem.push_instruction(stab_model::advanced::dem_instruction_with_tag_bytes(
