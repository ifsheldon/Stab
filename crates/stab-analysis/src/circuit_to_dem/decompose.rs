...
fn symmetric_difference(left: &[DemTarget], right: &[DemTarget]) -> AnalysisResult<Vec<DemTarget>> {
...
fn toggle_targets(targets: &mut BTreeSet<DemTarget>, values: &[DemTarget]) -> AnalysisResult<()> {
    for value in values {
        match value {
