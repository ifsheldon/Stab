use stab_model::{CircuitInstruction, MeasureRecordOffset, QubitId, Target};
...
pub(super) fn controlled_pauli_action(
    ...
    first: &Target,
    second: &Target,
) -> AnalysisResult<ControlledPauliAction> {
...
fn controlled_pauli_bases(gate_name: &str) -> AnalysisResult<(AnalyzerPauli, AnalyzerPauli)> {
    match gate_name {
...
fn require_qubit(gate_name: &str, role: &str, target: &Target) -> AnalysisResult<QubitId> {
    target
        .qubit_id()
        .ok_or_else(|| non_qubit_target_error(gate_name, role, target))
}

fn non_qubit_target_error(gate_name: &str, role: &str, target: &Target) -> AnalysisError {
