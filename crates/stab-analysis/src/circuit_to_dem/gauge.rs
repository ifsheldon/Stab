...
use stab_algebra::{PauliBasis, PauliString, SingleQubitClifford, Tableau};
use stab_model::{
    Circuit, CircuitInstruction, CircuitItem, DemTarget, MeasureRecordOffset, QubitId,
};
...
use super::{AnalyzerBasis, AnalyzerPauli};
...
impl GaugeTracker {
    ...
    fn undo_swap(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn undo_measurement_pads(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        for _ in instruction.targets().iter().rev() {
            self.pop_record_sensitivity()?;
        }
        Ok(())
    ...
    fn undo_observable_include(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn check_reset_gauge(&mut self, qubit: QubitId, basis: AnalyzerBasis) -> AnalysisResult<()> {
        match basis {
            AnalyzerBasis::X => self.check_gauge(self.zs_for(qubit)?.clone()),
            AnalyzerBasis::Y => {
                self.check_gauge(xor_sets(self.xs_for(qubit)?, self.zs_for(qubit)?))
    ...
    fn check_initial_resets(&mut self) -> AnalysisResult<()> {
    ...
    fn check_gauge(&mut self, gauge: BTreeSet<DemTarget>) -> AnalysisResult<()> {
        if gauge.is_empty() {
            return Ok(());
        }
        ...
        Err(AnalysisError::invalid_detector_error_model(message))
    }
    ...
    fn toggle_clifford_basis_sensitivity(
        ...
        match basis {
            PauliBasis::I => Err(AnalysisError::invalid_detector_error_model(
    ...
    fn undo_two_qubit_clifford(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let gate_name = instruction.gate().canonical_name();
        let tableau = gate_tableau(instruction.gate()).map_err(|error| {
            AnalysisError::invalid_detector_error_model(format!(
    ...
    fn undo_spp(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
        let gate_name = instruction.gate().canonical_name();
        for group in instruction.target_groups().into_iter().rev() {
    ...
    fn clear_qubit(&mut self, qubit: QubitId) -> AnalysisResult<()> {
    ...
    fn xs_for(&self, qubit: QubitId) -> AnalysisResult<&BTreeSet<DemTarget>> {
    ...
    fn zs_for(&self, qubit: QubitId) -> AnalysisResult<&BTreeSet<DemTarget>> {
    ...
    fn toggle_xs(&mut self, qubit: QubitId, targets: &BTreeSet<DemTarget>) -> AnalysisResult<()> {
    ...
    fn toggle_zs(&mut self, qubit: QubitId, targets: &BTreeSet<DemTarget>) -> AnalysisResult<()> {
