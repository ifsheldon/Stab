...
use stab_model::{CircuitInstruction, Pauli, QubitId, Target};
...
impl Analyzer {
    pub(super) fn apply_spp(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
...
pub(super) fn reduced_pauli_product_terms(
    gate_name: &str,
    group: &[Target],
) -> AnalysisResult<Vec<(QubitId, AnalyzerBasis)>> {
    let mut phase = 0_u8;
