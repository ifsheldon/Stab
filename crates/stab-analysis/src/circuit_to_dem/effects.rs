...
use stab_algebra::{PauliBasis, PauliSign, PauliString, SingleQubitClifford, Tableau};
use stab_model::{Pauli, Probability, QubitId};
...
impl PendingSingleQubitPauliChannel {
    ...
    pub(super) fn flip_probability(&self, basis: AnalyzerBasis) -> AnalysisResult<Probability> {
...
fn pauli_basis_from_mask(mask: u8, gate_name: &str) -> AnalysisResult<PauliBasis> {
    match mask {
...
fn two_qubit_tableau_output_length_error(gate_name: &str, index: usize) -> AnalysisError {
    AnalysisError::invalid_detector_error_model(format!(
...
fn missing_channel_basis(clifford: SingleQubitClifford, basis: &str) -> AnalysisError {
    AnalysisError::invalid_detector_error_model(format!(
...
fn non_identity_mapped_to_identity(clifford: SingleQubitClifford) -> AnalysisError {
    AnalysisError::invalid_detector_error_model(format!(
