...
use stab_model::{DemInstruction, DemInstructionKind, DemItem, DemRepeatBlock, DetectorErrorModel};

use super::flatten::{DemFlattenLimits, validate_flattening_budget_with_limits};
...
pub fn detector_error_model_without_tags(model: &DetectorErrorModel) -> DetectorErrorModel {
    let transformed: Result<DetectorErrorModel, Infallible> =
        transform_compact_dem(model, false, |instruction| {
            ...
            stab_model::advanced::dem_instruction_clear_tag(&mut instruction);
            Ok(instruction)
        });
    match transformed {
...
pub fn flattened_detector_error_model_with_limits(
    ...
    stab_model::advanced::dem_try_reserve_items_exact(
        &mut flattened,
        budget.materialized_capacity()?,
    )?;
...
fn rounded_instruction(instruction: &DemInstruction, digits: u8) -> AnalysisResult<DemInstruction> {
    ...
    let args = instruction
        ...
        .collect::<Vec<_>>();
    Ok(stab_model::advanced::dem_instruction_with_tag_bytes(
        instruction.kind(),
        args,
...
fn transform_compact_dem<E>(
    ...
    loop {
        ...
        let transformed_repeat = stab_model::advanced::dem_repeat_block_with_tag_bytes(
            repeat.repeat_count(),
            completed.output,
            tag,
        );
