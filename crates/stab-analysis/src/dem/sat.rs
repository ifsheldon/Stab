...
mod tests {
    ...
    use stab_model::{DemTarget, DetectorErrorModel};
...
";
    ...
    #[test]
    fn sat_problem_shortest_no_error_is_unsatisfiable() -> AnalysisResult<()> {
        assert_eq!(
    ...
    #[test]
    fn sat_problem_shortest_single_observable_without_detectors_matches_stim() -> AnalysisResult<()>
    {
        assert_eq!(
    ...
    #[test]
    fn sat_problem_shortest_ignores_error_probabilities() -> AnalysisResult<()> {
        for dem_text in [
            "error(0.1) D0 L0\nerror(0.1) D0\n",
            "error(1.0) D0 L0\nerror(0) D0\n",
            "error(0.5) D0 L0\nerror(0.999) D0\n",
    ...
    #[test]
    fn sat_problem_likeliest_no_error_is_unsatisfiable() -> AnalysisResult<()> {
        assert_eq!(
    ...
    #[test]
    fn sat_problem_likeliest_omits_zero_probability_error_variables() -> AnalysisResult<()> {
    ...
    #[test]
    fn sat_problem_likeliest_skips_zero_probability_repeats() -> AnalysisResult<()> {
        let model = dem("\
repeat 100001 {
...
")?;
    ...
    #[test]
    fn sat_problem_likeliest_compresses_sparse_observable_ids() -> AnalysisResult<()> {
        assert_eq!(
    ...
    #[test]
    fn sat_problem_likeliest_balanced_probabilities_match_stim() -> AnalysisResult<()> {
        assert_eq!(
...
"
    ...
    #[test]
    fn sat_problem_likeliest_large_probability_flips_soft_clause_sign() -> AnalysisResult<()> {
        assert_eq!(
            likeliest_error_sat_problem(&dem("error(0.1) D0 L0\nerror(0.9) D0\n")?, 10)?,
            "\
p wcnf 3 8 81
...
"
    ...
    #[test]
    fn sat_problem_likeliest_half_probability_skips_soft_clause() -> AnalysisResult<()> {
        assert_eq!(
            likeliest_error_sat_problem(&dem("error(0.1) D0 L0\nerror(0.5) D0\n")?, 10)?,
            "\
p wcnf 3 7 71
...
"
    ...
    #[test]
    fn sat_problem_likeliest_header_counts_stored_clauses_like_stim() -> AnalysisResult<()> {
    ...
    #[test]
    fn sat_error_visitor_rejects_target_occurrences_before_materialization() -> AnalysisResult<()> {
        let mut errors = Vec::new();
    ...
    fn wcnf_clause_count(wcnf: &str) -> AnalysisResult<usize> {
        let header = wcnf.lines().next().ok_or_else(|| {
            AnalysisError::invalid_detector_error_model("test WCNF output is missing a header")
        })?;
        let clause_count = header.split_whitespace().nth(3).ok_or_else(|| {
            AnalysisError::invalid_detector_error_model(
                "test WCNF header is missing a clause count",
            )
        })?;
    ...
    #[test]
    fn sat_problem_flattens_repeat_detector_offsets() -> AnalysisResult<()> {
        let model = dem("\
repeat 2 {
    error(0.1) D0
    shift_detectors 1
...
"
    ...
    #[test]
    fn sat_problem_rejects_excessive_repeat_expansion() -> AnalysisResult<()> {
        let model = dem("\
repeat 100001 {
    error(0.1) D0
    shift_detectors 1
...
")?;
        ...
        let error = match shortest_error_sat_problem(&model) {
            Ok(output) => {
                return Err(crate::AnalysisError::invalid_detector_error_model(format!(
    ...
    #[test]
    fn sat_problem_weighted_quantization_must_be_positive() -> AnalysisResult<()> {
        assert!(likeliest_error_sat_problem(&dem("error(0.1) L0")?, 0).is_err());
        Ok(())
    }

    #[test]
    fn sat_problem_likeliest_treats_deterministic_error_as_hard() -> AnalysisResult<()> {
        assert_eq!(
