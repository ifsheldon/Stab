...
use stab_model::{
    Circuit, CircuitInstruction, CircuitItem, Gate, MeasureRecordOffset, Probability, QubitId,
    RepeatBlock, RepeatCount, Target,
};
...
impl CodeDistance {
    pub fn try_new(value: u32) -> AnalysisResult<Self> {
        if !(2..=2047).contains(&value) {
            return Err(AnalysisError::invalid_domain_value("code distance", value));
        }
        Ok(Self(value))
    }
...
impl RoundCount {
    pub fn try_new(value: u64) -> AnalysisResult<Self> {
        if value == 0 {
            return Err(AnalysisError::invalid_domain_value("round count", value));
        }
        Ok(Self(value))
    }
...
impl CircuitGenParams {
    fn new(rounds: RoundCount, distance: CodeDistance) -> AnalysisResult<Self> {
        Ok(Self {
            rounds,
            distance,
...
pub fn generate_repetition_code_circuit(
    ...
    let qubit_count = measurement_count
        ...
        .ok_or_else(|| {
            AnalysisError::invalid_domain_value("code distance", common.distance.get())
        })?;
...
fn validate_repetition_generation_size(distance: u32) -> AnalysisResult<()> {
...
pub(super) fn validate_surface_generation_size(distance: u32, rotated: bool) -> AnalysisResult<()> {
...
pub(super) fn validate_color_generation_size(distance: u32) -> AnalysisResult<()> {
    let distance = u64::from(distance);
    let width = distance
        .checked_add(
            distance
...
fn reset_gate(basis: char) -> AnalysisResult<&'static str> {
    match basis {
        'X' => Ok("RX"),
        'Y' => Ok("RY"),
        'Z' => Ok("R"),
        _ => Err(AnalysisError::invalid_domain_value("reset basis", basis)),
    }
}

fn measure_gate(basis: char) -> AnalysisResult<&'static str> {
    match basis {
        'X' => Ok("MX"),
        'Y' => Ok("MY"),
        'Z' => Ok("M"),
        _ => Err(AnalysisError::invalid_domain_value("measure basis", basis)),
    }
}

fn measure_reset_gate(basis: char) -> AnalysisResult<&'static str> {
    match basis {
        'X' => Ok("MRX"),
        'Y' => Ok("MRY"),
        'Z' => Ok("MR"),
        _ => Err(AnalysisError::invalid_domain_value(
            "measure-reset basis",
            basis,
        )),
    }
...
fn append_repeat_detectors(circuit: &mut Circuit, measurement_count: u32) -> AnalysisResult<()> {
    for detector in 0..measurement_count {
        let rec = measurement_count - detector;
        append_instruction(
            circuit,
...
fn append_tail_detectors(circuit: &mut Circuit, measurement_count: u32) -> AnalysisResult<()> {
    for detector in 0..measurement_count {
        let rec = measurement_count - detector;
        append_instruction(
            circuit,
...
fn rec_targets(offsets: &[u32]) -> AnalysisResult<Vec<Target>> {
...
fn qubit_targets(targets: &[u32]) -> AnalysisResult<Vec<Target>> {
    targets
        .iter()
        .copied()
        .map(|target| {
...
fn rec_target(offset: u32) -> AnalysisResult<Target> {
    let offset = i32::try_from(offset)
        .map_err(|_| AnalysisError::invalid_domain_value("measurement record offset", offset))?;
