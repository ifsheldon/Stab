...
use stab_model::{
    Circuit, CircuitInstruction, CircuitItem, DetectorErrorModel, Gate, Pauli, Probability,
    QubitId, RepeatBlock, Target,
};

use crate::{
    AnalysisError, AnalysisResult, CircuitErrorLocation, CircuitErrorLocationStackFrame,
    CircuitTargetsInsideInstruction, ErrorAnalyzerOptions, ExplainedError, FlippedMeasurement,
    GateTargetWithCoords, circuit_to_detector_error_model,
};
...
) -> AnalysisResult<Vec<ExplainedError>> {
...
impl<'a> CandidateCollector<'a> {
    fn new(circuit: &'a Circuit) -> AnalysisResult<Self> {
        Ok(Self {
            circuit,
    ...
    fn collect(mut self) -> AnalysisResult<Vec<ErrorCandidate>> {
    ...
    fn collect_depolarize2_errors(
        ...
        for (group_index, group) in groups.by_ref().enumerate() {
            ...
            let target_range_start = group_index.checked_mul(2).ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("target range overflowed")
            })?;
    ...
    fn collect_single_measurement_errors(
        ...
        let basis = measurement_basis(instruction.gate().canonical_name()).ok_or_else(|| {
            AnalysisError::invalid_detector_error_model("unknown measurement basis")
        })?;
        for (target_index, target) in instruction.targets().iter().enumerate() {
            let Some(qubit) = target.qubit_id() else {
                return Err(AnalysisError::invalid_detector_error_model(format!(
            ...
            let measurement_record_index = self
                ...
                .ok_or_else(|| {
                AnalysisError::invalid_detector_error_model("measurement index overflowed")
            })?;
            candidates.push(ErrorCandidate {
                instruction_offset,
                target_range_start: target_index,
    ...
    ) -> AnalysisResult<CircuitErrorLocation> {
    ...
    fn pauli_with_coords(
        ...
        pauli: Pauli,
        qubit: QubitId,
    ) -> AnalysisResult<GateTargetWithCoords> {
    ...
    ) -> AnalysisResult<Vec<GateTargetWithCoords>> {
        let mut result = Vec::new();
...
impl ScanState {
    fn apply_repeat(&mut self, repeat: &RepeatBlock) -> AnalysisResult<()> {
    ...
    fn apply_instruction(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn apply_qubit_coords(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
    ...
    fn apply_detector(&mut self, instruction: &CircuitInstruction) -> AnalysisResult<()> {
...
fn detector_coordinates(circuit: &Circuit) -> AnalysisResult<BTreeMap<u64, Vec<f64>>> {
...
fn isolate_candidate(circuit: &Circuit, candidate: &ErrorCandidate) -> AnalysisResult<Circuit> {
