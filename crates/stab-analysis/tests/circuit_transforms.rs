...
use stab_model::{Circuit, CircuitItem, DetectorErrorModel};

trait CircuitTransformTestExt {
    fn flattened(&self) -> AnalysisResult<Circuit>;
    fn without_noise(&self) -> AnalysisResult<Circuit>;
    fn decomposed(&self) -> AnalysisResult<Circuit>;
    fn with_inlined_feedback(&self) -> AnalysisResult<Circuit>;
}
...
trait DemTransformTestExt {
    fn flattened(&self) -> AnalysisResult<DetectorErrorModel>;
}
...
fn with_inlined_feedback_rejects_synthesized_negative_zero_detector_target() {
    ...
    assert!(matches!(
        error,
        AnalysisError::InvalidDetectorErrorModel { .. }
    ));
