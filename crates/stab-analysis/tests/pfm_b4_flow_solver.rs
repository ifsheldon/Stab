...
use stab_analysis::{
    AnalysisError, check_if_circuit_has_unsigned_stabilizer_flows, circuit_flow_generators,
...
fn pfm_b4_flow_solver_rejects_invalid_circuits_without_exhaustive_fallback() {
    for circuit_text in [
        ...
        assert!(
            matches!(
                error,
                AnalysisError::InvalidTableauConversion { .. }
                    | AnalysisError::InvalidCircuitSimplification { .. }
            ),
            "{error:?}"
        );
