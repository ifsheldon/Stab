...
use stab_analysis::{
    AnalysisError, CodeDistance, ErrorAnalyzerOptions, RepetitionCodeParams, RepetitionCodeTask,
...
use stab_model::{
    Circuit, DemInstructionKind, DemItem, DemTarget, DetectorErrorModel, Probability,
};
...
fn assert_no_logical_error(
    result: Result<DetectorErrorModel, AnalysisError>,
    expected_prefix: &str,
) {
    let error = result.expect_err("model should not contain an undetectable logical error");
    let AnalysisError::InvalidDetectorErrorModel { message } = error else {
