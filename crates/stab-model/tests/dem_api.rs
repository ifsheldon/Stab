...
use stab_model::{
    DemDetectorId, DemInstruction, DemInstructionKind, DemItem, DemRepeatBlock, DemRepeatCount,
    DemTarget, DetectorErrorModel, ModelResult, Probability,
};

#[test]
fn pf1_dem_basic_mutation_and_append() {
    ...
    assert!(!dem.is_empty());
    assert_eq!(dem.len(), 2);
    assert!(dem.to_dem_string().contains("error[first]"));
...
#[test]
fn pf4_dem_introspection_queries_cover_final_counts_and_shifts() {
