...
- PQ1 must recompute the pinned-Stim adapter build fingerprint from the recorded source, library, tools, and canonical compiler arguments; derive expected work in the parent; bind every post-preflight worker digest to the untimed semantic preflight; and audit tracked, staged, and untracked repository state through a private Git index reconstructed from `HEAD`.
